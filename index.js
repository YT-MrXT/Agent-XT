...
function initDatabase() {
  ...
  // Config de sugestões (uma linha por categoria — "geral", "construções", etc.)
  // A categoria 'geral' é a usada pelo Dashboard e é a categoria padrão de /sugerir.
  db.exec(`
  ...
  `);
  // Migração: bases de dados antigas tinham guild_id como PK única (sem categoria).
  // Se a coluna 'category' não existir, recria a tabela preservando os dados como categoria 'geral'.
  try {
    const scCols = db.prepare("PRAGMA table_info(suggestion_config)").all().map(c => c.name);
    if (!scCols.includes('category')) {
      db.exec(`ALTER TABLE suggestion_config RENAME TO suggestion_config_old`);
      db.exec(`
      ...
      `);
      db.exec(`
        INSERT INTO suggestion_config (guild_id, category, channel_id, log_channel, enabled, ping_role)
        SELECT guild_id, 'geral', channel_id, log_channel, enabled, ping_role FROM suggestion_config_old
      `);
      db.exec(`DROP TABLE suggestion_config_old`);
    }
  } catch (e) { console.error('❌ Erro na migração de suggestion_config (category):', e.message); }

  // Migração: coluna 'category' em suggestions (categoria da sugestão em si)
  try {
    const sCols = db.prepare("PRAGMA table_info(suggestions)").all().map(c => c.name);
    if (!sCols.includes('category')) db.exec(`ALTER TABLE suggestions ADD COLUMN category TEXT NOT NULL DEFAULT 'geral'`);
  } catch (e) { console.error('❌ Erro na migração de suggestions (category):', e.message); }
...
const commands = [
  ...
  // ── Sugestões ──
  // Suporta várias "categorias" de sugestões no mesmo servidor (ex: "geral", "construções", etc.),
  // cada uma com o seu próprio canal/log/ping configurável separadamente.
  new SlashCommandBuilder()
    .setName('sugestao-setup')
    .setDescription('Configura um tipo/categoria de sugestões')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal de sugestões').setRequired(true))
    .addStringOption(o => o.setName('categoria').setDescription('Nome da categoria (padrão: geral). Ex: construções').setRequired(false).setMaxLength(50))
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('categoria').setDescription('Categoria da sugestão (padrão: geral). Ex: construções').setRequired(false).setMaxLength(50)),

  new SlashCommandBuilder()
    .setName('sugestao-categorias')
    .setDescription('Lista as categorias de sugestões configuradas neste servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  ...
  new SlashCommandBuilder()
    .setName('lock')
    .setDescription('Tranca um canal, impedindo @everyone de enviar mensagens')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal a trancar (padrão: este canal)').setRequired(false))
    .addStringOption(o => o.setName('motivo').setDescription('Motivo do lock').setRequired(false)),

  new SlashCommandBuilder()
    .setName('unlock')
    .setDescription('Destranca um canal, permitindo @everyone de enviar mensagens novamente')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
