...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tk-panel-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; margin-top: 4px; align-self: flex-start; }
  .tk-panel-row-info { min-width: 0; width: 100%; }
  .tk-panel-row-name { font-weight: 700; font-size: 0.95rem; color: var(--text); display: flex; flex-direction: column; align-items: flex-start; gap: 8px; }
  ...
  .tk-panel-row-actions {
    display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px;
    width: 100%; border-top: 1px solid var(--border); padding-top: 10px; margin-top: 2px;
  }
  .tk-icon-btn {
    display: inline-flex; align-items: center; justify-content: center; gap: 5px;
    background: var(--bg2); border: 1px solid var(--border);
    color: var(--text); padding: 7px 8px; border-radius: 8px; font-size: 0.74rem; font-weight: 600;
      ...
      return \`
        <div class="tk-panel-row \${p.active ? 'active' : ''}">
          ...
          <div class="tk-panel-row-actions">
            ...
            <button type="button" class="tk-icon-btn danger" onclick="apagarPainelTicket('\${guildId}', \${p.id}, '\${(p.name||'').replace(/'/g,"\\\\'")}')" \${__tkPanels.length <= 1 ? 'disabled title="Precisas de ter pelo menos 1 painel"' : ''}>🗑️ Apagar</button>
          </div>
        </div>
      \`;
    }).join('');
