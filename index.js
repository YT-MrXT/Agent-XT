...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'sugerir') {
    ...
    // A thread começa com uma mensagem simples (só autor + número + texto, sem
    // votos/estado/botões), sem afixar nada — os votos ficam só na mensagem
    // original no canal de sugestões. Se falhar, não bloqueia o resto.
    try {
      const thread = await msg.startThread({
      ...
      const embedSimples = new EmbedBuilder().setDescription(conteudo).setColor(CONFIG.COR_AVISO).setAuthor({
        name: user.tag,
        iconURL: user.displayAvatarURL()
      }).setTitle(`${tipo.emoji || '💡'} Sugestão #${guildSeq}`);
      await thread.send({
        embeds: [embedSimples]
      }).catch(() => {});
    } catch (e) {
