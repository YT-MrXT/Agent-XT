  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: `Configura tudo de forma visual em: **[Abrir Dashboard](${DASHBOARD_URL})**\n${DASHBOARD_URL}`,
      })
...
const dashboardCSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ticket-type-row {
    display: flex; align-items: center; gap: 10px; background: var(--bg3); border: 1px solid var(--border);
    border-radius: 9px; padding: 8px 10px; margin-bottom: 8px;
  }
  .ticket-type-swatch { width: 10px; height: 32px; border-radius: 4px; flex-shrink: 0; }
  .btn-icon {
    background: var(--bg4); border: 1px solid var(--border); color: var(--text); border-radius: 6px;
    width: 28px; height: 28px; font-size: 0.75rem; cursor: pointer; display: flex; align-items: center; justify-content: center;
    transition: all 0.15s ease; flex-shrink: 0;
  }
  .btn-icon:hover:not(:disabled) { background: var(--bg2); border-color: var(--accent); }
  .btn-icon:disabled { opacity: 0.3; cursor: not-allowed; }
  .btn-icon-danger:hover:not(:disabled) { border-color: var(--danger); color: var(--danger); }
...
const dashboardJS = `
  ...
  async function addTicketType(guildId) {
    ...
    const data = new FormData(form);
    const body = new URLSearchParams(data).toString();
    const totalAtual = document.querySelectorAll('.ticket-type-row').length;
  ...
  async function updateTicketTypeColor(guildId, typeId, color) {
    ...
    try {
      ...
      if (json.ok && row) {
        ...
        const swatch = row.querySelector('.ticket-type-swatch');
  ...
  async function moveTicketType(guildId, typeId, direction) {
    const rows = Array.from(document.querySelectorAll('.ticket-type-row'));
    const ids = rows.map(r => parseInt(r.id.replace('tt-row-', '')));
    const idx = ids.indexOf(typeId);
    const newIdx = idx + direction;
    if (newIdx < 0 || newIdx >= ids.length) return;
    [ids[idx], ids[newIdx]] = [ids[newIdx], ids[idx]];
    try {
      const res = await fetch('/api/' + guildId + '/ticket-types/reorder', {
        method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ order: ids })
      });
      const json = await res.json();
      if (json.ok) location.reload();
      else toast('❌ Erro ao reordenar', 'error');
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  ...
  function setPanelMode(mode) {
    ...
    const countEl = document.getElementById('ticket-types-count');
    const countLabel = document.getElementById('ticket-types-count-label');
    if (countLabel) countLabel.textContent = mode === 'buttons' ? '/10' : '';
    renderTicketPreview();
  }
  function onPanelColorChange(color) {
    document.getElementById('panel_color_hidden').value = color;
    renderTicketPreview();
  }
  ...
  function renderTicketPreview() {
    ...
    // Inclui o tipo que está a ser preenchido no formulário de "novo tipo", se tiver nome
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    renderTicketPreview();
    const ti = document.getElementById('panel_titulo_input');
    const de = document.getElementById('panel_descricao_input');
    if (ti) renderTicketPreview();
    if (de) renderTicketPreview();
  });
  ...
  async function enviarPainelTicket(guildId) {
    const channel_id = document.getElementById('panel_channel_id')?.value;
    const titulo = document.getElementById('panel_titulo_input')?.value || '🎫 Suporte';
    const descricao = document.getElementById('panel_descricao_input')?.value || '';
    if (!channel_id) return toast('❌ Escolhe um canal para o painel', 'error');

    // 1. Guarda o modo e a cor do painel na configuração de tickets
    try {
    ...
    // 2. Envia o painel
    try {
      const body = new URLSearchParams({ channel_id, titulo, descricao }).toString();
...
function renderGuildDashboard(user, guild, data) {
  ...
  /** Renderiza uma linha da lista de tipos de ticket no editor do dashboard */
  const renderTicketTypeRow = (t, idx, total) => `
    <div class="ticket-type-row" id="tt-row-${t.id}" data-color="${t.color || '#5865F2'}" data-emoji="${escHtml(t.emoji || '🎫')}" data-label="${escHtml(t.label)}">
      <div class="ticket-type-swatch" style="background:${t.color || '#5865F2'}"></div>
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${t.emoji || '🎫'} ${escHtml(t.label)}</div>
        <div style="color:var(--text2);font-size:0.76rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(t.description) || 'Sem descrição'} ${t.has_form ? '· 📋 c/ formulário' : ''}</div>
      </div>
      <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
        <button type="button" class="btn-icon" title="Mover para cima" ${idx === 0 ? 'disabled' : ''} onclick="moveTicketType('${guild.id}', ${t.id}, -1)">▲</button>
        <button type="button" class="btn-icon" title="Mover para baixo" ${idx === total - 1 ? 'disabled' : ''} onclick="moveTicketType('${guild.id}', ${t.id}, 1)">▼</button>
        <input type="color" value="${t.color || '#5865F2'}" title="Cor do botão" style="width:28px;height:28px;padding:0;border:none;cursor:pointer;background:none" onchange="updateTicketTypeColor(${JSON.stringify(guild.id)}, ${t.id}, this.value)">
        ${t.has_form ? `<button type="button" class="btn-icon" title="Perguntas" onclick="toggleQuestionsPanel('${guild.id}', ${t.id})">📋</button>` : ''}
        <button type="button" class="btn-icon btn-icon-danger" title="Remover" onclick="removeTicketType('${guild.id}', ${t.id})">🗑️</button>
      </div>
    </div>`;
...
<body>
  ...
  <div class="main-content" id="main">
    ...
    <div id="tickets" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        <h2>🎨 Editor do Painel de Tickets</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Escolhe como os membros vão abrir tickets, personaliza a mensagem, e vê a pré-visualização em tempo real — igual ao que vai aparecer no Discord.
        </p>
        ...
        <div class="grid-2">
          <div class="form-group">
            ...
            ${makeSelect('panel_channel_id', channels, ticketConfig?.panel_channel_id, 'Canal')}
          </div>
          <div class="form-group">
            <label>Cor do Embed</label>
            <input type="color" id="panel_color_input" value="${ticketConfig?.panel_color || '#5865F2'}" style="height:42px;padding:4px;cursor:pointer" oninput="onPanelColorChange(this.value)">
          </div>
        </div>
        <div class="form-group">
          <label>Título</label>
          <input type="text" id="panel_titulo_input" value="🎫 Suporte" maxlength="256" oninput="renderTicketPreview()">
        </div>
        <div class="form-group">
          <label>Descrição</label>
          <textarea id="panel_descricao_input" rows="3" oninput="renderTicketPreview()">Clica abaixo para abrir um ticket de suporte.
A nossa equipa irá responder o mais brevemente possível!</textarea>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start;margin-top:8px">
          <div>
            <label style="display:block;font-size:0.875rem;font-weight:600;margin-bottom:8px;color:var(--text2)">🏷️ Tipos de Ticket / Botões (<span id="ticket-types-count">${ticketTypes.length}</span><span id="ticket-types-count-label">${ticketConfig?.panel_mode === 'buttons' ? '/10' : ''}</span>)</label>
            <div id="ticket-types-list">
              ${ticketTypes.map((t, i) => renderTicketTypeRow(t, i, ticketTypes.length)).join('') || `<p style="color:var(--text2);font-size:0.85rem">Nenhum tipo criado ainda — o painel usará um botão simples "Abrir Ticket".</p>`}
            </div>

            <div style="background:var(--bg3);border:1px dashed var(--border);border-radius:9px;padding:14px;margin-top:14px">
              <div style="font-weight:600;font-size:0.85rem;margin-bottom:10px">➕ Adicionar novo tipo</div>
              <form id="form-ticket-type">
                <div class="grid-2">
                  ...
                  <div class="form-group">
                    <label>Emoji</label>
                    <input type="text" name="emoji" id="tt-emoji" placeholder="🎫" maxlength="10" oninput="renderTicketPreview()">
                  </div>
                </div>
                <div class="grid-2">
                  <div class="form-group">
                ...
                <div class="grid-2">
                  <div class="form-group">
                    <label>Cor do Botão (modo botões)</label>
                    <input type="color" name="color" id="tt-color" value="#5865F2" style="height:42px;padding:4px;cursor:pointer" oninput="renderTicketPreview()">
                  </div>
          ...
          <div>
            <label style="display:block;font-size:0.875rem;font-weight:600;margin-bottom:8px;color:var(--text2)">👁️ Pré-visualização (Discord)</label>
            ...
            <p style="color:var(--text2);font-size:0.75rem;margin-top:10px">A pré-visualização atualiza automaticamente à medida que editas o título, descrição, cor e tipos de ticket.</p>
          </div>
        </div>

        <div style="margin-top:20px;border-top:1px solid var(--border);padding-top:16px">
          <button type="button" class="btn btn-primary" onclick="enviarPainelTicket('${guild.id}')">📤 Guardar e Enviar Painel</button>
        </div>
      </div>

      <!-- Formulários dos tipos de ticket (aberto por tipo) -->
      <div id="ticket-questions-panels">
        ${ticketTypes.filter(t => t.has_form).map(t => `
          <div class="card" id="questions-row-${t.id}" style="margin-top:20px;display:none">
            <h2>📋 Formulário — ${t.emoji || '🎫'} ${t.label}</h2>
