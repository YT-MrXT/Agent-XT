...
- Aplica este mesmo raciocínio a outras palavras que tenham um uso comum como elogio/gíria positiva em português informal, mesmo que a mesma palavra também tenha um sentido vulgar — não bloqueies o uso positivo.
...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  app.post('/api/:guildId/ai-moderation-config', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const { enabled, log_channel, max_warnings, timeout_minutes, min_severity } = req.body;
    ...
    await db.prepare(`
      ...
      SET enabled = ?, log_channel = ?, max_warnings = ?, timeout_minutes = ?, min_severity = ?, updated_at = CURRENT_TIMESTAMP
      WHERE guild_id = ?
    `).run(enabled ? 1 : 0, log_channel || null, maxW, timeoutMin, severidadeValida, guildId);
