    ...
    <div id="tickets" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <form id="form-ticket-type">
          ...
          <div class="form-group">
            <label>Cor do Botão (usada no modo Botões Diretos)</label>
        ...
        <div id="ticket-types-table" style="margin-top:16px">
          ${ticketTypes.length ? `
            <table class="data-table">
              <thead><tr><th>Cor</th><th>Emoji</th><th>Nome</th><th>Descrição</th><th>Formulário</th><th></th></tr></thead>
              <tbody>
                ${ticketTypes.map(t => `
                  <tr id="tt-row-${t.id}" data-color="${t.color || '#5865F2'}" data-emoji="${escHtml(t.emoji || '🎫')}" data-label="${escHtml(t.label)}">
                    <td><input type="color" value="${t.color || '#5865F2'}" title="Cor do botão" style="width:32px;height:26px;padding:0;border:none;cursor:pointer;background:none" onchange="updateTicketTypeColor('${guild.id}', ${t.id}, this.value)"></td>
                    <td>${t.emoji || '🎫'}</td>
                    <td>${t.label}</td>
