...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Adiciona um utilizador à blacklist pelo username: se ele entrar no servidor, é banido automaticamente')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
...
app.post('/api/:guildId/role-exclusivity', requireGuildAdminApi, async (req, res) => {
  ...
  try {
    ...
    // Aplica retroativamente: todos os membros que JÁ têm o cargo ganho perdem já os cargos escolhidos
    let afetados = 0;
    await guild.members.fetch();
    ...
    res.json({ ok: true, message: `Regra(s) adicionada(s)! ${afetados > 0 ? `${afetados} cargo(s) já removido(s) de membros que se aplicavam.` : 'Nenhum membro atual foi afetado.'}` });
  } catch (e) {
    res.status(500).json({ ok: false, message: `Erro: ${e.message}` });
  }
});
...
const dashboardJS = `
  ...
  async function addExclusividade(guildId) {
    ...
    const loseSelect = document.getElementById('lose_role_ids');
    const gain = gainSelect.value;
    const loseIds = Array.from(loseSelect.selectedOptions).map(o => o.value);
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <div id="cargos_tab" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Configura: quando alguém <strong>recebe</strong> um cargo, o bot remove automaticamente o(s) cargo(s) escolhido(s) (se a pessoa os tiver) — mesmo quem já os tinha antes de a regra existir. Podes escolher vários cargos a remover (Ctrl/Cmd + clique). Funciona em tempo real, seja qual for o motivo do cargo ter sido atribuído.
        </p>
        <form id="form-exclusividade">
          <div class="grid-2">
            ...
            <div class="form-group">
              ...
              ${makeMultiSelect('lose_role_ids', roles, [])}
            </div>
          </div>
