...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('duracao').setDescription('Duração (ex: 30s, 10m, 2h, 1d)').setRequired(true))
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'giveaway-criar') {
    ...
    if (!duracaoMs || duracaoMs < 5000) {
      return interaction.reply({ content: '❌ Duração inválida. Usa um formato como `30s`, `10m`, `2h` ou `1d` (mínimo 5 segundos).', ephemeral: true });
    }
    ...
    agendarEncerramentoGiveaway({ ...gw, message_id: msg.id });
  ...
  if (commandName === 'giveaway-cancelar') {
    ...
    const timeoutPendente = giveawayTimeouts.get(gw.id);
    if (timeoutPendente) { clearTimeout(timeoutPendente); giveawayTimeouts.delete(gw.id); }
...
async function encerrarGiveaway(guild, gw) {
  // Cancela qualquer timeout agendado para este giveaway (evita encerrar duas vezes)
  const timeoutPendente = giveawayTimeouts.get(gw.id);
  if (timeoutPendente) { clearTimeout(timeoutPendente); giveawayTimeouts.delete(gw.id); }
...
/** Guarda os timeouts agendados por giveaway, para poder cancelar/reagendar (ex: ao reroll/cancelar) */
const giveawayTimeouts = new Map();

/** Agenda o encerramento exato de um giveaway no instante em que termina, em vez de esperar pelo próximo tick do cron */
function agendarEncerramentoGiveaway(gw) {
  // Cancela um timeout anterior para este giveaway, se existir (evita duplicados)
  const anterior = giveawayTimeouts.get(gw.id);
  if (anterior) clearTimeout(anterior);

  const endsAt = new Date(gw.ends_at + 'Z');
  const delay = endsAt.getTime() - Date.now();

  // setTimeout tem um limite máximo (~24.8 dias); para giveaways mais longos, o cron de segurança trata disso mais tarde
  if (delay > 2147483000) return;

  const timeout = setTimeout(async () => {
    giveawayTimeouts.delete(gw.id);
    const atual = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(gw.id);
    if (!atual || atual.ended) return; // já foi encerrado (ex: por /giveaway-terminar manual)
    const guild = client.guilds.cache.get(atual.guild_id);
    if (!guild) return; // o cron de segurança apanha isto no próximo minuto
    await encerrarGiveaway(guild, atual).catch(err => console.error('❌ Erro ao encerrar giveaway (timeout exato):', err.message));
  }, Math.max(0, delay));

  giveawayTimeouts.set(gw.id, timeout);
}

/** Verifica giveaways ativos e encerra os que já passaram da hora (rede de segurança, corre a cada 10s) */
...
/** Converte string de duração (ex: "10m", "2h", "1d", "30s") em milissegundos */
...
app.post('/api/:guildId/giveaways', requireGuildAdminApi, async (req, res) => {
  ...
  if (!duracaoMs || duracaoMs < 5000) {
    return res.status(400).json({ ok: false, message: '❌ Duração inválida. Usa um formato como 30s, 10m, 2h ou 1d (mínimo 5 segundos).' });
  }
      ...
      <div class="card">
        ...
        <form id="form-giveaway-criar">
          ...
          <div class="grid-2">
            <div class="form-group">
              <label>Duração</label>
              <input type="text" name="duracao" placeholder="Ex: 30s, 10m, 2h, 1d" required>
            </div>
            <div class="form-group">
