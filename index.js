...
async function isEquipaAdminTicket(member, guild, ticket) {
  ...
  const cargoId = await obterCargoSuporteTicket(ticket, ticketConfig);
...
function construirPainelTempVoice() {
  const embed = new EmbedBuilder()
    ...
    .setDescription('Entra em **➕│Criar-Call** para criares a tua própria call. Depois, usa os botões abaixo para a geres — a call é apagada automaticamente quando ficar vazia.')
    .addFields(
      {
        name: '⚙️ Configurar',
        value: '✏️ **Nome** — muda o nome da call\n👥 **Limite** — nº máximo de pessoas\n🔒 **Trancar** — impede novas entradas',
        inline: true
      },
      {
        name: '👤 Pessoas',
        value: '💬 **Convidar** — chama alguém\n⛔ **Expulsar** — tira alguém da call\n🚫 **Bloquear** — impede reentrada\n✅ **Desbloquear** — permite voltar a entrar',
        inline: true
      },
      {
        name: '👑 Posse',
        value: '👑 **Reclamar** — se o dono saiu\n🔁 **Transferir** — passa a call a outra pessoa\n🗑️ **Apagar** — apaga a call já',
        inline: true
      }
    )
    .setFooter({ text: 'Só o dono da call pode usar a maioria destas opções' });

  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_nome').setLabel('Nome').setEmoji('✏️').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_limite').setLabel('Limite').setEmoji('👥').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_trancar').setLabel('Trancar / Destrancar').setEmoji('🔒').setStyle(ButtonStyle.Secondary)
  );
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_convidar').setLabel('Convidar').setEmoji('💬').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('tv_expulsar').setLabel('Expulsar').setEmoji('⛔').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_bloquear').setLabel('Bloquear').setEmoji('🚫').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('tv_desbloquear').setLabel('Desbloquear').setEmoji('✅').setStyle(ButtonStyle.Success)
  );
  const row3 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_reclamar').setLabel('Reclamar').setEmoji('👑').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_transferir').setLabel('Transferir').setEmoji('🔁').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_apagar').setLabel('Apagar Call').setEmoji('🗑️').setStyle(ButtonStyle.Danger)
  );
