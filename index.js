 ...
 */
async function obterExecutorAuditLog(guild, tipo, targetId, janelaMs = 10000) {
  try {
    const audit = await guild.fetchAuditLogs({ type: tipo, limit: 5 });
    ...
    return null; // sem permissão "Ver Registo de Auditoria", ou outro erro
  }
}
...
function embedLogModeracao(opts) {
  const {
    ...
    alvo, alvoIdManual, moderador, motivo, duracao,
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  try {
    ...
    if (cargosGanhos.size > 0) {
      const executor = await obterExecutorAuditLog(newMember.guild, 25 /* AuditLogEvent.MemberRoleUpdate */, newMember.id);
      const embed = embedLogModeracao({
    ...
    if (oldMember.nickname !== newMember.nickname) {
      const executor = await obterExecutorAuditLog(newMember.guild, 24 /* AuditLogEvent.MemberUpdate */, newMember.id);
      const embed = embedLogModeracao({
...
client.on(Events.GuildRoleCreate, async role => {
  const executor = await obterExecutorAuditLog(role.guild, 30 /* AuditLogEvent.RoleCreate */, role.id); // 30
  const embed = embedLogModeracao({
    titulo: '🆕 Cargo Criado',
    cor: CONFIG.COR_SUCESSO,
    moderador: executor,
...
client.on(Events.GuildRoleDelete, async role => {
  const executor = await obterExecutorAuditLog(role.guild, 32 /* AuditLogEvent.RoleDelete */, role.id);
  const embed = embedLogModeracao({
...
client.on(Events.GuildRoleUpdate, async (oldRole, newRole) => {
  ...
  const executor = await obterExecutorAuditLog(newRole.guild, 31 /* AuditLogEvent.RoleUpdate */, newRole.id);
  const embed = embedLogModeracao({
...
client.on(Events.ChannelCreate, async channel => {
  ...
  const executor = await obterExecutorAuditLog(channel.guild, 10 /* AuditLogEvent.ChannelCreate */, channel.id);
  const embed = embedLogModeracao({
    titulo: '🆕 Canal Criado',
    cor: CONFIG.COR_SUCESSO,
    moderador: executor,
...
client.on(Events.ChannelDelete, async channel => {
  if (!channel.guild) return;
  const executor = await obterExecutorAuditLog(channel.guild, 12 /* AuditLogEvent.ChannelDelete */, channel.id);
  const embed = embedLogModeracao({
...
client.on(Events.ChannelUpdate, async (oldChannel, newChannel) => {
  ...
  const executor = await obterExecutorAuditLog(newChannel.guild, 11 /* AuditLogEvent.ChannelUpdate */, newChannel.id);
  const embed = embedLogModeracao({
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <div id="suggestions_tab" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Cria vários tipos, por exemplo "Sugestão" e "Sugestão de Construção", cada um com o seu canal.
          Quem usar <code>/sugerir</code> escolhe o tipo antes de escrever.
        </p>
