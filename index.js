...
function requireGuildAdminPage(req, res, next) {
...
function requireGuildAdminApi(req, res, next) {
...
app.get('/dashboard/:guildId', requireGuildAdminPage, async (req, res) => {
  const { guildId } = req.params;
...
app.post('/api/:guildId/bot-identity', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { bot_nickname, bot_avatar_url, bot_webhook_name } = req.body;

  const nickname = (bot_nickname || '').trim().slice(0, 32) || null;
  const avatarUrl = (bot_avatar_url || '').trim() || null;
  const webhookName = (bot_webhook_name || '').trim().slice(0, 80) || null;

  if (avatarUrl && !/^https?:\/\/.+\.(png|jpe?g|gif|webp)(\?.*)?$/i.test(avatarUrl)) {
    return res.status(400).json({ ok: false, message: 'URL de imagem inválido. Usa um link direto para .png, .jpg, .gif ou .webp' });
  }

  db.prepare(`
    INSERT INTO guild_config (guild_id, bot_nickname, bot_avatar_url, bot_webhook_name)
    VALUES (?,?,?,?)
    ON CONFLICT(guild_id) DO UPDATE SET
      bot_nickname=excluded.bot_nickname,
      bot_avatar_url=excluded.bot_avatar_url,
      bot_webhook_name=excluded.bot_webhook_name
  `).run(guildId, nickname, avatarUrl, webhookName);
  ...
  res.json({
    ...
    message: nickResult.ok
      ? '✅ Identidade do bot guardada! O nome foi atualizado neste servidor. A foto será usada nas próximas mensagens enviadas via webhook (não afeta o avatar global do bot).'
...
app.get('/api/:guildId/bot-identity', requireGuildAdminApi, (req, res) => {
  ...
  res.json({
    ...
    bot_webhook_name: config?.bot_webhook_name || '',
  });
});
    ...
    <div id="overview" class="section active">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Podes definir um <strong>nome (apelido)</strong> diferente para o bot só neste servidor — isto funciona sempre e aparece em todo o lado (lista de membros, menções, etc).<br>
          Quanto à <strong>foto</strong>: o Discord não permite avatares diferentes por servidor para bots. Como alternativa, define uma foto aqui e o bot passa a usá-la nas mensagens que enviar através de um Webhook criado automaticamente neste servidor — só nas mensagens, não no perfil/lista de membros.
        </p>
        <form id="form-bot-identity">
          <div class="grid-2">
            ...
            <div class="form-group">
              <label>Nome usado nas mensagens via Webhook</label>
              <input type="text" name="bot_webhook_name" id="bot_webhook_name" placeholder="Se vazio, usa o apelido/nome normal" maxlength="80" value="${guildConfig?.bot_webhook_name || ''}">
            </div>
          </div>
          <div class="form-group">
            <label>URL da foto (link direto .png / .jpg / .gif / .webp)</label>
            <input type="text" name="bot_avatar_url" id="bot_avatar_url" placeholder="https://exemplo.com/imagem.png" value="${guildConfig?.bot_avatar_url || ''}">
          </div>
          ${guildConfig?.bot_avatar_url ? `<div style="margin:8px 0 16px"><img src="${guildConfig.bot_avatar_url}" alt="Pré-visualização" style="width:64px;height:64px;border-radius:50%;object-fit:cover"></div>` : ''}
          <div style="margin:8px 0 16px;padding:12px;background:var(--bg3);border-radius:8px;font-size:0.85rem;color:var(--text2)">
            ⚠️ O bot precisa da permissão <strong>Gerir Webhooks</strong> e <strong>Gerir Apelidos</strong> neste servidor para isto funcionar.
          </div>
          <button type="button" class="btn btn-primary" onclick="saveBotIdentity('${guild.id}')">💾 Guardar Identidade</button>
        </form>
      </div>
    </div>
  ...
  <script>
    ...
    async function saveBotIdentity(guildId) {
      const body = {
        ...
        bot_webhook_name: document.getElementById('bot_webhook_name').value,
        bot_avatar_url: document.getElementById('bot_avatar_url').value,
      };
      try {
