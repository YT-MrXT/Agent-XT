...
<body>
  ...
  <div class="main-content" id="main">
    ...
    <div id="tickets" class="section" style="display:none">
      ...
      <div class="card">
        <h2>📑 Painéis de Tickets (${ticketPanels.length}/10)</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">Cada painel tem a sua própria configuração completa (categoria, cargos, tipos de ticket, etc). Usa "Editar" para trocares de painel nesta página.</p>
        <table class="data-table">
          <thead><tr><th>Painel</th><th>Categoria</th><th>Enviado em</th><th></th></tr></thead>
          <tbody>
            ${ticketPanels.map(p => `
              <tr ${p.id === ticketConfig?.id ? 'style="background:rgba(88,101,242,0.1)"' : ''}>
                <td><strong>${escHtml(p.panel_name || `Painel ${p.panel_num}/10`)}</strong> ${p.id === ticketConfig?.id ? '<span class="badge badge-green">A editar</span>' : ''}</td>
                <td style="color:var(--text2)">${categories.find(c => c.id === p.category_id)?.name || 'Não definida'}</td>
                <td style="color:var(--text2)">${p.panel_channel_id ? '#' + (channels.find(c => c.id === p.panel_channel_id)?.name || '?') : '—'}</td>
                <td style="white-space:nowrap">
                  ${p.id !== ticketConfig?.id ? `<a href="/dashboard/${guild.id}?painel=${p.id}#tickets" class="btn btn-secondary" style="padding:4px 10px;font-size:0.8rem">✏️ Editar</a>` : ''}
                  ${ticketPanels.length > 1 ? `<button type="button" class="btn btn-danger" style="padding:4px 10px;font-size:0.8rem" onclick="apagarPainelTicket('${guild.id}', ${p.id}, '${escHtml(p.panel_name || '')}')">🗑️</button>` : ''}
                </td>
              </tr>
            `).join('')}
          </tbody>
