...
async function initDatabase() {
  ...
  // Configuração do sistema de avaliações de staff (liga/desliga por servidor + cargos que podem avaliar)
  await db.exec(`
    CREATE TABLE IF NOT EXISTS staff_ratings_config (
      guild_id            TEXT PRIMARY KEY,
      enabled             INTEGER NOT NULL DEFAULT 1,
      allowed_rater_roles TEXT DEFAULT '[]'
    );
  `);
  ...
  // Migração: guarda o ID da thread de discussão criada para cada sugestão,
  // para poder ser eliminada quando um administrador responder à sugestão.
  try {
    await db.exec(`ALTER TABLE suggestions ADD COLUMN thread_id TEXT`);
  } catch (_) {}
  ...
  // NOTA: o sistema de AutoMod IA foi removido (dashboard, comandos e chamadas
  // à API externa). As tabelas "ai_moderation_config" e "ai_warnings" deixaram
  // de ser criadas/lidas por este ficheiro, mas NÃO são apagadas — se já
  // existiam no Turso, os dados guardados continuam lá intactos, só deixam
  // de ser usados. Se o sistema for reativado no futuro, os dados reaparecem.
...
// Cache simples da config de avaliações por guild (evita 1 query a cada uso do comando/botão)
const _staffRatingsConfigCache = new Map();

/** Obtém a config do sistema de avaliações (enabled + cargos permitidos a avaliar) para um servidor */
async function getStaffRatingsConfig(guildId) {
  if (_staffRatingsConfigCache.has(guildId)) return _staffRatingsConfigCache.get(guildId);
  let row = await db.prepare('SELECT * FROM staff_ratings_config WHERE guild_id = ?').get(guildId);
  if (!row) row = { guild_id: guildId, enabled: 1, allowed_rater_roles: '[]' };
  let allowedRoles = [];
  try {
    const arr = JSON.parse(row.allowed_rater_roles || '[]');
    if (Array.isArray(arr)) allowedRoles = arr.filter(Boolean);
  } catch (_) {}
  const config = { enabled: !!row.enabled, allowedRoles };
  _staffRatingsConfigCache.set(guildId, config);
  return config;
}

/** Invalida a cache da config de avaliações (chamar sempre que o dashboard guardar alterações) */
function invalidarStaffRatingsConfigCache(guildId) {
  _staffRatingsConfigCache.delete(guildId);
}

/**
 * Verifica se um membro pode avaliar staff, de acordo com a config do servidor.
 * Retorna { ok: boolean, motivo?: string }.
 * Se o sistema estiver desativado → ok:false.
 * Se não houver cargos configurados → qualquer pessoa pode avaliar (comportamento antigo).
 * Se houver cargos configurados → só quem tiver pelo menos 1 desses cargos pode avaliar.
 */
async function podeAvaliarStaff(guildId, member) {
  const config = await getStaffRatingsConfig(guildId);
  if (!config.enabled) return { ok: false, motivo: 'O sistema de avaliações de staff está desativado neste servidor.' };
  if (config.allowedRoles.length > 0) {
    const cargosDoMembro = member?.roles?.cache ? [...member.roles.cache.keys()] : (Array.isArray(member?.roles) ? member.roles : []);
    const temCargoPermitido = config.allowedRoles.some(r => cargosDoMembro.includes(r));
    if (!temCargoPermitido) return { ok: false, motivo: 'Não tens um cargo autorizado a avaliar staff neste servidor.' };
  }
  return { ok: true };
}
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'avaliar-staff') {
    const permissao = await podeAvaliarStaff(guild.id, interaction.member);
    if (!permissao.ok) return interaction.reply({
      content: `❌ ${permissao.motivo}`,
      ephemeral: true
    });
  ...
  if (commandName === 'sugerir') {
    ...
    // Cria uma thread na mensagem da sugestão, para as pessoas poderem discuti-la.
    // Não envia nenhuma mensagem extra — a própria mensagem original (com votos e
    // estado) já aparece automaticamente como primeira mensagem da thread.
    // Se falhar (ex: sem permissões), não bloqueia o resto.
    try {
      const thread = await msg.startThread({
        name: `${tipo.emoji || '💡'} Sugestão #${guildSeq}`.slice(0, 100),
        autoArchiveDuration: 10080 // 7 dias
      });
      await db.prepare('UPDATE suggestions SET thread_id = ? WHERE id = ?').run(thread.id, sugId);
    } catch (e) {
      console.error('❌ Erro ao criar thread da sugestão:', e.message);
    }
  ...
  if (commandName === 'sugestao-responder') {
    ...
    // Elimina a thread de discussão da sugestão, já que foi respondida
    if (sug.thread_id) {
      try {
        const thread = await guild.channels.fetch(sug.thread_id).catch(() => null);
        if (thread) await thread.delete(`Sugestão #${id} respondida por ${user.tag}`);
      } catch (e) {
        console.error('❌ Erro ao eliminar thread da sugestão:', e.message);
      }
      await db.prepare('UPDATE suggestions SET thread_id = NULL WHERE id = ?').run(sug.id);
    }
...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  app.get('/api/:guildId/tickets', requireGuildAdminApi, async (req, res) => {
    ...
    const tickets = await db.prepare("SELECT * FROM tickets WHERE guild_id = ? ORDER BY (status='open') DESC, created_at DESC LIMIT 50").all(guildId);
    res.json(tickets);
  });
  app.post('/api/:guildId/tickets/:channelId/close', requireGuildAdminApi, async (req, res) => {
    const {
      guildId,
      channelId
    } = req.params;
    ...
    try {
      const ticket = await db.prepare("SELECT * FROM tickets WHERE guild_id = ? AND channel_id = ?").get(guildId, channelId);
      if (!ticket) return res.status(404).json({
        ok: false,
        message: 'Ticket não encontrado.'
      });
      if (ticket.status === 'closed') return res.status(400).json({
        ok: false,
      ...
      if (!channel) {
        await db.prepare("UPDATE tickets SET status='closed', closed_at=CURRENT_TIMESTAMP WHERE channel_id=?").run(channelId);
        return res.json({
          ok: true,
          message: '✅ Ticket marcado como fechado (o canal já não existia no Discord).'
        });
      }
      await fecharTicket(channel, req.session?.user?.id || 'dashboard', guild, reason || null);
      res.json({
        ok: true,
        message: '✅ Ticket fechado com sucesso!'
      });
    } catch (err) {
      console.error('❌ Erro ao fechar ticket via dashboard:', err);
      res.status(500).json({
        ok: false,
        message: 'Erro ao fechar o ticket.'
      });
    }
  });
  ...
  // ── Staff (Dashboard) ──
  app.get('/api/:guildId/staff/ratings-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const config = await getStaffRatingsConfig(guildId);
  ...
  app.post('/api/:guildId/staff/ratings-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const {
    ...
    // Checkboxes: array (vários cargos), string única (só 1 marcado) ou undefined (nenhum)
    const rolesRaw = req.body.allowed_rater_roles;
    const allowedRoles = rolesRaw === undefined ? [] : (Array.isArray(rolesRaw) ? rolesRaw : [rolesRaw]).filter(Boolean);
    await db.prepare(`
      INSERT INTO staff_ratings_config (guild_id, enabled, allowed_rater_roles)
      VALUES (?, ?, ?)
      ON CONFLICT(guild_id) DO UPDATE SET enabled=excluded.enabled, allowed_rater_roles=excluded.allowed_rater_roles
    `).run(guildId, enabled === '1' ? 1 : 0, JSON.stringify(allowedRoles));
    invalidarStaffRatingsConfigCache(guildId);
    res.json({
      ok: true,
      message: '✅ Configuração de avaliações guardada!'
    });
  });
  app.get('/api/:guildId/staff/ranking', requireGuildAdminApi, async (req, res) => {
    const {
  ...
  app.post('/api/:guildId/staff/avaliar', requireGuildAdminApi, async (req, res) => {
    ...
    const memberSessao = await guild.members.fetch(req.session.user.id).catch(() => null);
    const permissao = await podeAvaliarStaff(guildId, memberSessao);
    if (!permissao.ok) return res.status(403).json({
      ok: false,
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tk-panel-row {
    ...
    justify-content: flex-start; gap: 10px; transition: all 0.15s ease;
  ...
  .tk-panel-row-left { display: flex; align-items: center; gap: 10px; min-width: 0; width: 100%; }
  .tk-panel-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
  .tk-panel-row-info { min-width: 0; width: 100%; }
  .tk-panel-row-name { font-weight: 700; font-size: 0.95rem; color: var(--text); display: flex; flex-direction: column; align-items: flex-start; gap: 6px; }
  .tk-panel-row-sub { font-size: 0.8rem; color: var(--text2); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 100%; }
  ...
  .tk-panel-row-actions { display: flex; gap: 6px; flex-wrap: wrap; width: 100%; border-top: 1px solid var(--border); padding-top: 10px; margin-top: 2px; }
  .tk-icon-btn {
...
<head>
  ...
  <style>
    ...
    @media(max-width:768px){
      ...
      .tk-panel-list{grid-template-columns:1fr}
    }
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <!-- MODAL: FECHAR TICKET COM MOTIVO -->
    <div id="modal-close-ticket" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:1000;align-items:center;justify-content:center">
      <div class="card" style="max-width:440px;width:92%">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <h3 style="margin:0">🔒 Fechar Ticket</h3>
          <button type="button" class="btn" onclick="fecharModalCloseTicket()" style="padding:4px 10px">✖</button>
        </div>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:14px">
          Tens a certeza que queres fechar este ticket? Podes indicar um motivo (opcional) que será enviado ao utilizador.
        </p>
        <input type="hidden" id="close_ticket_channel_id" value="">
        <div class="form-group">
          <label>Motivo do fecho (opcional)</label>
          <textarea id="close_ticket_reason" rows="3" placeholder="Ex: Pedido resolvido"></textarea>
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px">
          <button type="button" class="btn" onclick="fecharModalCloseTicket()">Cancelar</button>
          <button type="button" class="btn btn-danger" onclick="confirmarFecharTicket()">🔒 Fechar Ticket</button>
        </div>
      </div>
    </div>
    ...
    <div id="ratings" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <form id="form-ratings-config">
          <label class="toggle" style="margin-bottom:14px">
            <input type="checkbox" name="enabled" value="1" ${staffRatingsConfig?.enabled ? 'checked' : ''}>
            <span>Sistema de avaliações ativado neste servidor</span>
          </label>
          <div class="form-group">
            <label>Cargos que podem avaliar staff</label>
            <div style="color:var(--text2);font-size:0.8rem;margin-bottom:6px">
              Deixa tudo desmarcado para permitir que qualquer pessoa avalie. Marca 1 ou mais cargos para restringir só a esses cargos.
            </div>
            ${makeRoleCheckboxes('allowed_rater_roles', roles, staffRatingsConfig?.allowedRoles || [])}
          </div>
          <button type="button" class="btn btn-primary" style="margin-top:12px" onclick="saveConfig('${guild.id}','staff/ratings-config','form-ratings-config')">💾 Guardar</button>
        </form>
      </div>
  ...
  <script>
    ...
    async function refreshSection(sectionId) {
      try {
        ...
        // IMPORTANTE: definir innerHTML NÃO executa as tags <script> que vêm
        // dentro do HTML novo (é uma limitação do próprio browser). Isto
        // fazia com que, por exemplo, window.__ticketPanelsData nunca fosse
        // atualizado após guardar/editar um painel — o painel de tickets
        // ficava "vazio"/desaparecido até se recarregar a página inteira.
        // Aqui recriamos manualmente cada <script> encontrado para o browser
        // o correr de verdade.
        ...
        // Repõe a lista de painéis de tickets (o script acima já recarregou
        // window.__ticketPanelsData com os dados frescos vindos do servidor).
        if (sectionId === 'tickets' && typeof tkInitPanels === 'function') {
          tkInitPanels();
        }
        return true;
      } catch (e) {
    ...
    function closeTicketDashboard(channelId) {
      document.getElementById('close_ticket_channel_id').value = channelId;
      document.getElementById('close_ticket_reason').value = '';
      document.getElementById('modal-close-ticket').style.display = 'flex';
    }

    function fecharModalCloseTicket() {
      document.getElementById('modal-close-ticket').style.display = 'none';
    }

    async function confirmarFecharTicket() {
      const channelId = document.getElementById('close_ticket_channel_id').value;
      const reason = document.getElementById('close_ticket_reason').value.trim();
      fecharModalCloseTicket();
      try {
        const r = await fetch('/api/'+GUILD_ID+'/tickets/'+channelId+'/close', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ reason: reason || null })
        });
        const data = await r.json();
        toast(data.message || (data.ok ? 'Ticket fechado!' : 'Erro ao fechar ticket.'), data.ok ? 'success' : 'error');
        if (data.ok) loadOverviewData();
      } catch (e) {
        toast('❌ Erro de ligação ao fechar o ticket.', 'error');
      }
    }
    ...
    async function loadOverviewData() {
      ...
      try {
        ...
        const allTickets = await r.json();
        const tickets = allTickets.filter(t => t.status === 'open');
        const html = tickets.length ? '<table class="table"><thead><tr><th>#</th><th>Utilizador</th><th>Estado</th><th>Data</th><th>Ações</th></tr></thead><tbody>' +
          tickets.slice(0,8).map(t => '<tr><td>#'+String(t.ticket_number).padStart(4,'0')+'</td><td>'+t.user_id+'</td><td><span class="badge badge-green">'+t.status+'</span></td><td>'+new Date(t.created_at).toLocaleDateString('pt-PT')+'</td><td><button type="button" class="btn btn-danger" style="padding:4px 10px;font-size:0.78rem" onclick="closeTicketDashboard(\\''+t.channel_id+'\\')">🔒 Fechar</button></td></tr>').join('') +
          '</tbody></table>' : '<p style="color:var(--text2)">Nenhum ticket aberto.</p>';
