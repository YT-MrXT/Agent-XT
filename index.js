...
function construirPainelTempVoice() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      'Esta interface serve para geres a **tua call de voz temporária**.\n' +
      'Entra em **➕│Criar Call** para criares a tua, e depois usa os botões abaixo para a configurares.\n\n' +
      '✏️ **Nome** · Muda o nome da call\n' +
      '👥 **Limite** · Define o número máximo de pessoas\n' +
      '🔒 **Trancar / 🔓 Destrancar** · Só quem tens permitido entra\n' +
      '💬 **Convidar** · Convida alguém para a tua call\n' +
      '⛔ **Expulsar** · Expulsa alguém da tua call\n' +
      '🚫 **Bloquear / ✅ Desbloquear** · Impede alguém de voltar a entrar\n' +
      '👑 **Reclamar** · Torna-te dono se o dono anterior saiu\n' +
      '🔁 **Transferir** · Passa a call a outra pessoa\n' +
      '🗑️ **Apagar** · Apaga a tua call imediatamente\n\n' +
      '*A tua call é apagada automaticamente quando fica vazia.*'
    );
  ...
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_convidar').setLabel('Convidar').setEmoji('💬').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_expulsar').setLabel('Expulsar').setEmoji('⛔').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_bloquear').setLabel('Bloquear').setEmoji('🚫').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_desbloquear').setLabel('Desbloquear').setEmoji('✅').setStyle(ButtonStyle.Secondary)
  );
  const row3 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('tv_apagar').setLabel('Apagar').setEmoji('🗑️').setStyle(ButtonStyle.Danger)
  );
...
async function handleTempVoiceButton(interaction, customId) {
  ...
  if (customId === 'tv_convidar') {
    const modal = new ModalBuilder().setCustomId(`tv_modal_convidar_${channel.id}`).setTitle('Convidar para a call');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('user_id').setLabel('ID do utilizador a convidar').setStyle(TextInputStyle.Short).setMaxLength(25).setRequired(true).setPlaceholder('Ex: 123456789012345678')
    ));
    return interaction.showModal(modal);
  }
  ...
  if (customId === 'tv_expulsar') {
    ...
    const modal = new ModalBuilder().setCustomId(`tv_modal_expulsar_${channel.id}`).setTitle('Expulsar da call');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('user_id').setLabel('ID do utilizador a expulsar').setStyle(TextInputStyle.Short).setMaxLength(25).setRequired(true).setPlaceholder('Ex: 123456789012345678')
    ));
    return interaction.showModal(modal);
  }
  ...
  if (customId === 'tv_bloquear') {
    ...
    const modal = new ModalBuilder().setCustomId(`tv_modal_bloquear_${channel.id}`).setTitle('Bloquear utilizador');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('user_id').setLabel('ID do utilizador a bloquear').setStyle(TextInputStyle.Short).setMaxLength(25).setRequired(true).setPlaceholder('Ex: 123456789012345678')
    ));
    return interaction.showModal(modal);
  }
  ...
  if (customId === 'tv_desbloquear') {
    ...
    const modal = new ModalBuilder().setCustomId(`tv_modal_desbloquear_${channel.id}`).setTitle('Desbloquear utilizador');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('user_id').setLabel('ID do utilizador a desbloquear').setStyle(TextInputStyle.Short).setMaxLength(25).setRequired(true).setPlaceholder('Ex: 123456789012345678')
    ));
    return interaction.showModal(modal);
  }
  ...
  if (customId === 'tv_transferir') {
    ...
    const modal = new ModalBuilder().setCustomId(`tv_modal_transferir_${channel.id}`).setTitle('Transferir dono da call');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('user_id').setLabel('ID do novo dono').setStyle(TextInputStyle.Short).setMaxLength(25).setRequired(true).setPlaceholder('Ex: 123456789012345678')
    ));
    return interaction.showModal(modal);
  }
...
/** Trata a submissão de todos os modais da interface de Calls Temporárias (customId "tv_modal_ação_channelId") */
async function handleTempVoiceModal(interaction, customId) {
  const match = customId.match(/^tv_modal_(nome|limite|convidar|expulsar|bloquear|desbloquear|transferir)_(\d+)$/);
  if (!match) return;
  ...
  if (acao === 'convidar') {
    ...
    if (!alvo) return interaction.reply({ content: '❌ Não encontrei nenhum utilizador com esse ID.', ephemeral: true });
    ...
    return interaction.reply({ content: `✅ <@${userId}> foi convidado para a tua call.`, ephemeral: true });
  }
  ...
  if (acao === 'expulsar') {
    ...
    if (!alvoMember) return interaction.reply({ content: '❌ Não encontrei nenhum membro com esse ID.', ephemeral: true });
    if (alvoMember.voice.channelId !== channel.id) {
      return interaction.reply({ content: '❌ Essa pessoa não está na tua call.', ephemeral: true });
    }
    await alvoMember.voice.disconnect().catch(() => {});
    return interaction.reply({ content: `✅ <@${userId}> foi expulso da tua call.`, ephemeral: true });
  }
  ...
  if (acao === 'bloquear') {
    ...
    return interaction.reply({ content: `🚫 <@${userId}> foi bloqueado e já não pode entrar nesta call.`, ephemeral: true });
  }
  ...
  if (acao === 'desbloquear') {
    ...
    return interaction.reply({ content: `✅ <@${userId}> foi desbloqueado e já pode voltar a entrar.`, ephemeral: true });
  }
  ...
  if (acao === 'transferir') {
    ...
    if (!alvoMember) return interaction.reply({ content: '❌ Não encontrei nenhum membro com esse ID.', ephemeral: true });
    if (alvoMember.voice.channelId !== channel.id) {
      return interaction.reply({ content: '❌ Só podes transferir a call para alguém que esteja dentro dela.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: `🔁 A call foi transferida! <@${userId}> é agora o dono.`, ephemeral: false });
  }
}
...
new SlashCommandBuilder().setName('calls-remover').setDescription('Remove o sistema de Calls Temporárias deste servidor (não apaga os canais)').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
// ── Help ──
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'calls-remover') {
    ...
    await db.prepare('UPDATE tempvoice_config SET enabled = 0 WHERE guild_id = ?').run(guild.id);
    return interaction.reply({
      content: '✅ Sistema de Calls Temporárias desativado. Os canais não foram apagados — podes removê-los manualmente ou reativar com `/setup-calls`.',
      ephemeral: true
    });
  }
  ...
  app.post('/api/:guildId/tempvoice-config', requireGuildAdminApi, async (req, res) => {
    ...
    try {
      ...
      if (enabled) {
        ...
        await db.prepare('UPDATE tempvoice_config SET enabled = 0 WHERE guild_id = ?').run(guildId);
        return res.json({ ok: true, message: '✅ Sistema de Calls Temporárias desativado. Os canais não foram apagados.' });
      }
    } catch (e) {
    ...
    <div id="tempvoice_tab" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <div class="grid-2">
          ...
          ${tvConfig?.enabled ? `<button type="button" class="btn btn-danger" onclick="saveTempVoiceConfig('${guild.id}', false)">⛔ Desativar (não apaga os canais)</button>` : ''}
        </div>
      </div>
    </div>
