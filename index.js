...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  app.post('/api/:guildId/tickets/:channelId/close', requireGuildAdminApi, async (req, res) => {
    ...
    try {
      ...
      if (!channel) {
        // Canal já não existe no Discord — apenas atualiza o estado na BD
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
  ...
  <script>
    ...
    async function closeTicketDashboard(channelId) {
      const reason = prompt('Motivo do fecho (opcional — deixa vazio para fechar sem motivo):');
      if (reason === null) return; // utilizador cancelou
      if (!confirm('Tens a certeza que queres fechar este ticket?' + (reason ? ('\nMotivo: ' + reason) : ''))) return;
      try {
