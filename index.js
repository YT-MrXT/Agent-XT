...
function initDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS giveaways (
      ...
      imagem        TEXT,
      vencedores_n  INTEGER NOT NULL DEFAULT 1,
      ping_texto    TEXT,
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Cria e publica um giveaway (sorteio)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal onde publicar o giveaway').setRequired(true)
      .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
    .addStringOption(o => o.setName('titulo').setDescription('Título do giveaway').setRequired(true).setMaxLength(200))
    .addStringOption(o => o.setName('premio').setDescription('O que vai ser sorteado').setRequired(true).setMaxLength(200))
    .addStringOption(o => o.setName('duracao').setDescription('Duração (ex: 10m, 2h, 1d)').setRequired(true))
    .addStringOption(o => o.setName('descricao').setDescription('Descrição extra dentro do embed').setRequired(false).setMaxLength(1000))
    .addStringOption(o => o.setName('imagem').setDescription('URL da imagem do embed').setRequired(false))
    .addIntegerOption(o => o.setName('vencedores').setDescription('Número de vencedores (padrão: 1)').setRequired(false).setMinValue(1).setMaxValue(20))
    .addStringOption(o => o.setName('mensagem').setDescription('Mensagem fora do embed, ex: @everyone ou @cargo').setRequired(false))
    .addStringOption(o => o.setName('cor').setDescription('Cor hexadecimal do embed (ex: #5865F2)').setRequired(false)),

  new SlashCommandBuilder()
    .setName('giveaway-terminar')
    .setDescription('Termina um giveaway antecipadamente e sorteia o(s) vencedor(es)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway').setRequired(true)),
  ...
  new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway').setRequired(true))
    .addIntegerOption(o => o.setName('vencedores').setDescription('Quantos vencedores sortear (padrão: igual ao original)').setRequired(false).setMinValue(1).setMaxValue(20)),

  new SlashCommandBuilder()
    .setName('giveaway-lista')
    .setDescription('Lista os giveaways ativos neste servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

  new SlashCommandBuilder()
    .setName('giveaway-cancelar')
    .setDescription('Cancela um giveaway sem sortear vencedor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway').setRequired(true)),

  // ── Trancar/Destrancar Canal ──
  new SlashCommandBuilder()
    .setName('lock')
    .setDescription('Tranca o canal para @everyone (impede enviar mensagens)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
  ...
  new SlashCommandBuilder()
    .setName('unlock')
    .setDescription('Destranca o canal para @everyone (permite enviar mensagens de novo)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'sugerir') {
    ...
    const stmt = db.prepare(`
      INSERT INTO suggestions (guild_id, channel_id, user_id, content, guild_seq) VALUES (?, ?, ?, ?, ?)
    `);
    const info = stmt.run(guild.id, canal.id, user.id, conteudo, guildSeq);
    const sugId = info.lastInsertRowid;

    const embed = new EmbedBuilder()
      .setTitle(`💡 Sugestão #${guildSeq}`)
    ...
    return interaction.reply({ content: `✅ Sugestão #${guildSeq} submetida com sucesso!`, ephemeral: true });
  }
  ...
  if (commandName === 'giveaway-criar') {
    ...
    const duracaoRaw = options.getString('duracao');
    const descricao  = options.getString('descricao');
    const imagem     = options.getString('imagem');
    const vencedoresN = options.getInteger('vencedores') || 1;
    const mensagem   = options.getString('mensagem');
    const cor        = options.getString('cor');

    const duracaoMs = parseDuracao(duracaoRaw);
    if (!duracaoMs) {
      return interaction.reply({ content: '❌ Duração inválida. Usa por exemplo: `10m`, `2h` ou `1d`.', ephemeral: true });
    }
    if (imagem && !/^https?:\/\/.+/i.test(imagem)) {
      return interaction.reply({ content: '❌ A URL da imagem tem de começar com http:// ou https://', ephemeral: true });
    }
    if (cor && !/^#?[0-9A-Fa-f]{6}$/.test(cor)) {
      return interaction.reply({ content: '❌ Cor inválida. Usa um hexadecimal, ex: `#5865F2`.', ephemeral: true });
    }

    await interaction.deferReply({ ephemeral: true });

    try {
      const giveawayId = await criarGiveaway(guild, {
        channelId: canal.id,
        titulo,
        descricao,
        premio,
        ...
        pingTexto: mensagem,
        duracaoMs,
        cor: cor ? (cor.startsWith('#') ? cor : `#${cor}`) : null,
        createdBy: user.id,
      });

      return interaction.editReply({ content: `🎉 Giveaway **#${giveawayId}** criado e publicado em ${canal}!` });
    } catch (e) {
      return interaction.editReply({ content: `❌ Erro ao criar o giveaway: ${e.message}` });
    }
  }

  if (commandName === 'giveaway-terminar') {
    const id = options.getInteger('id');
    const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!g) return interaction.reply({ content: '❌ Giveaway não encontrado.', ephemeral: true });
    if (g.ended) return interaction.reply({ content: '❌ Este giveaway já terminou.', ephemeral: true });

    await interaction.deferReply({ ephemeral: true });
    const resultado = await terminarGiveaway(guild, id);
    if (!resultado) return interaction.editReply({ content: '❌ Não foi possível terminar o giveaway.' });

    return interaction.editReply({
      content: resultado.vencedores.length
        ? `✅ Giveaway **#${id}** terminado! Vencedor(es): ${resultado.vencedores.map(u => `<@${u}>`).join(', ')}`
        : `✅ Giveaway **#${id}** terminado sem participantes.`
    });
  }

  if (commandName === 'giveaway-reroll') {
    const id = options.getInteger('id');
    const n  = options.getInteger('vencedores');
    const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!g) return interaction.reply({ content: '❌ Giveaway não encontrado.', ephemeral: true });
    if (!g.ended) return interaction.reply({ content: '❌ Este giveaway ainda não terminou. Usa `/giveaway-terminar` primeiro.', ephemeral: true });

    await interaction.deferReply({ ephemeral: true });
    const resultado = await rerollGiveaway(guild, id, n);
    if (!resultado) return interaction.editReply({ content: '❌ Não foi possível fazer reroll.' });

    return interaction.editReply({
      content: resultado.vencedores.length
        ? `🔄 Reroll feito! Novo(s) vencedor(es): ${resultado.vencedores.map(u => `<@${u}>`).join(', ')}`
        : '❌ Não há participantes suficientes para reroll.'
    });
  }

  if (commandName === 'giveaway-lista') {
    const ativos = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? AND ended = 0 ORDER BY ends_at ASC').all(guild.id);
    if (!ativos.length) return interaction.reply({ content: 'ℹ️ Não há giveaways ativos neste momento.', ephemeral: true });

    const linhas = ativos.map(g => {
      const n = db.prepare('SELECT COUNT(*) as c FROM giveaway_entries WHERE giveaway_id = ?').get(g.id).c;
      return `**#${g.id}** — ${g.titulo} (${g.premio})\n👥 ${n} participante(s) · ⏰ Termina <t:${Math.floor(g.ends_at/1000)}:R> · <#${g.channel_id}>`;
    }).join('\n\n');

    const embed = embedPadrao('🎉 Giveaways Ativos', linhas, CONFIG.COR_PRINCIPAL);
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
  ...
  if (commandName === 'giveaway-cancelar') {
    ...
    const canal = guild.channels.cache.get(g.channel_id);
    if (canal && g.message_id) {
      try {
        const msg = await canal.messages.fetch(g.message_id);
        const embedCancelado = new EmbedBuilder()
          .setTitle(`🎉 ${g.titulo}`)
          .setDescription(`~~${g.premio}~~\n\n❌ **Este giveaway foi cancelado.**`)
          .setColor(CONFIG.COR_ERRO)
          .setFooter({ text: `Giveaway cancelado · ID: ${g.id}` });
        await msg.edit({ embeds: [embedCancelado], components: [botaoGiveaway(id, true)] });
      } catch (_) {}
    }

    return interaction.editReply({ content: `🗑️ Giveaway **#${id}** cancelado.` });
  }
  ...
  if (commandName === 'lock') {
    ...
    try {
      ...
      const embed = embedPadrao(
        '🔒 Canal Trancado',
        `Este canal foi trancado. Ninguém pode enviar mensagens (exceto a staff).\n\n**Moderador:** <@${user.id}>\n**Motivo:** ${motivo}`,
        CONFIG.COR_ERRO
      );

      await canal.send({ embeds: [embed] }).catch(() => {});
      logMod(guild.id, 'LOCK', canal.id, user.id, motivo);
      await sendLogTyped(guild, 'lock', embed);

      if (canal.id === interaction.channel.id) {
        return interaction.editReply({ content: `🔒 Canal ${canal} trancado com sucesso.` });
      }
      return interaction.editReply({ content: `🔒 Canal ${canal} trancado com sucesso.` });
    } catch (e) {
      return interaction.editReply({ content: `❌ Não foi possível trancar o canal: ${e.message}` });
    }
  }
  ...
  if (commandName === 'unlock') {
    ...
    try {
      ...
      const embed = embedPadrao(
        '🔓 Canal Destrancado',
        `Este canal foi destrancado. Já podes enviar mensagens novamente.\n\n**Moderador:** <@${user.id}>\n**Motivo:** ${motivo}`,
        CONFIG.COR_SUCESSO
      );

      await canal.send({ embeds: [embed] }).catch(() => {});
      logMod(guild.id, 'UNLOCK', canal.id, user.id, motivo);
      await sendLogTyped(guild, 'unlock', embed);

      return interaction.editReply({ content: `🔓 Canal ${canal} destrancado com sucesso.` });
    } catch (e) {
      return interaction.editReply({ content: `❌ Não foi possível destrancar o canal: ${e.message}` });
    }
  }
  ...
  if (commandName === 'help') {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '🎉 Giveaways',
          value: '`/giveaway-criar` · Cria e publica um giveaway\n`/giveaway-terminar` · Termina um giveaway antecipadamente\n`/giveaway-reroll` · Sorteia novo(s) vencedor(es)\n`/giveaway-lista` · Lista os giveaways ativos\n`/giveaway-cancelar` · Cancela um giveaway sem sortear',
          inline: false,
        },
        {
          name: 'ℹ️ Informação',
...
async function handleButton(interaction) {
  ...
  if (customId.startsWith('giveaway_join_')) {
    const giveawayId = parseInt(customId.slice('giveaway_join_'.length), 10);
    const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(giveawayId, guild.id);

    if (!g || g.ended) {
      return interaction.reply({ content: '❌ Este giveaway já não está ativo.', ephemeral: true });
    }
    if (g.ends_at <= Date.now()) {
      return interaction.reply({ content: '❌ Este giveaway já terminou.', ephemeral: true });
    }

    const jaParticipa = db.prepare('SELECT 1 FROM giveaway_entries WHERE giveaway_id = ? AND user_id = ?').get(giveawayId, user.id);
    if (jaParticipa) {
      ...
      try {
        const embed = embedGiveaway(g, total, false);
        await interaction.message.edit({ embeds: [embed] });
      } catch (_) {}
      return interaction.reply({ content: '❌ Deixaste de participar neste giveaway.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: `✅ Estás a participar em **${g.premio}**! Boa sorte 🍀`, ephemeral: true });
  }
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  try {
    ...
    if (cargosGanhos.size > 0) {
      const embed = embedPadrao(
        '➕ Cargo(s) Atribuído(s)',
        `**Utilizador:** ${newMember.user.tag} (\`${newMember.id}\`)\n**Cargo(s):** ${cargosGanhos.map(r => `<@&${r.id}>`).join(', ')}`,
        CONFIG.COR_SUCESSO
      );
    ...
    if (cargosPerdidos.size > 0) {
      const embed = embedPadrao(
        '➖ Cargo(s) Removido(s)',
        `**Utilizador:** ${newMember.user.tag} (\`${newMember.id}\`)\n**Cargo(s):** ${cargosPerdidos.map(r => `<@&${r.id}>`).join(', ')}`,
        CONFIG.COR_ERRO
      );
      await sendLogTyped(newMember.guild, 'member_role', embed);
    }
    ...
    if (oldMember.nickname !== newMember.nickname) {
      const embed = embedPadrao(
        '✏️ Apelido Alterado',
        `**Utilizador:** ${newMember.user.tag} (\`${newMember.id}\`)\n**Antes:** ${oldMember.nickname || '*Nenhum*'}\n**Depois:** ${newMember.nickname || '*Nenhum*'}`,
        CONFIG.COR_AVISO
      );
...
/** Constrói o embed de um giveaway (ativo ou já terminado) */
function embedGiveaway(g, participantes, terminado = false, vencedoresIds = []) {
  const embed = new EmbedBuilder()
    .setTitle(`🎉 ${g.titulo}`)
    .setColor(terminado ? CONFIG.COR_AVISO : (g.cor || CONFIG.COR_PRINCIPAL))
    .setTimestamp(g.ends_at);

  let desc = '';
  if (g.descricao) desc += `${g.descricao}\n\n`;
  desc += `🎁 **Prémio:** ${g.premio}\n`;
  desc += `🏆 **Vencedores:** ${g.vencedores_n}\n`;
  desc += `👥 **Participantes:** ${participantes}\n`;

  if (terminado) {
    desc += vencedoresIds.length
      ? `\n🎊 **Vencedor(es):** ${vencedoresIds.map(id => `<@${id}>`).join(', ')}`
      : '\n😕 Ninguém participou, sem vencedor.';
  } else {
    desc += `⏰ **Termina:** <t:${Math.floor(g.ends_at / 1000)}:R> (<t:${Math.floor(g.ends_at / 1000)}:f>)\n\nClica no botão 🎉 abaixo para participares!`;
  }

  embed.setDescription(desc);
  if (g.imagem) embed.setImage(g.imagem);
  embed.setFooter({ text: terminado ? 'Giveaway terminado' : `ID do Giveaway: ${g.id}` });
  return embed;
}

function botaoGiveaway(giveawayId, disabled = false) {
...
/** Cria e publica um novo giveaway no canal escolhido */
async function criarGiveaway(guild, { channelId, titulo, descricao, premio, imagem, vencedoresN, pingTexto, duracaoMs, cor, createdBy }) {
  ...
  const info = db.prepare(`
    INSERT INTO giveaways (guild_id, channel_id, titulo, descricao, premio, imagem, vencedores_n, ping_texto, ends_at, cor, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(guild.id, channelId, titulo, descricao || null, premio, imagem || null, vencedoresN, pingTexto || null, endsAt, cor || CONFIG.COR_PRINCIPAL, createdBy);

  const giveawayId = info.lastInsertRowid;
  const g = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(giveawayId);

  const embed = embedGiveaway(g, 0, false);
  const row = botaoGiveaway(giveawayId);

  const msg = await canal.send({
    content: pingTexto ? pingTexto : undefined,
    embeds: [embed],
    components: [row],
    allowedMentions: { parse: ['everyone', 'here', 'roles', 'users'] }
  });
...
/** Sorteia N vencedores entre os participantes de um giveaway */
function sortearVencedores(giveawayId, n) {
  const participantes = db.prepare('SELECT user_id FROM giveaway_entries WHERE giveaway_id = ?').all(giveawayId).map(r => r.user_id);
  const pool = [...participantes];
  const vencedores = [];
  while (vencedores.length < n && pool.length > 0) {
    const idx = Math.floor(Math.random() * pool.length);
    vencedores.push(pool.splice(idx, 1)[0]);
  }
  return { vencedores, totalParticipantes: participantes.length };
}

/** Termina um giveaway: desativa o botão, sorteia vencedores e anuncia */
async function terminarGiveaway(guild, giveawayId) {
  const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(giveawayId, guild.id);
  if (!g || g.ended) return null;

  const { vencedores, totalParticipantes } = sortearVencedores(giveawayId, g.vencedores_n);
  ...
  if (canal && g.message_id) {
    try {
      const msg = await canal.messages.fetch(g.message_id);
      const embedFinal = embedGiveaway(g, totalParticipantes, true, vencedores);
      await msg.edit({ embeds: [embedFinal], components: [botaoGiveaway(giveawayId, true)] });

      if (vencedores.length) {
        await canal.send({
          content: `🎉 Parabéns ${vencedores.map(id => `<@${id}>`).join(', ')}! Ganhaste(s) **${g.premio}**!`,
          allowedMentions: { users: vencedores }
        }).catch(() => {});
      } else {
        await canal.send({ content: `😕 O giveaway de **${g.premio}** terminou sem participantes.` }).catch(() => {});
      }
    } catch (_) {}
  }

  return { vencedores, totalParticipantes };
}

/** Faz reroll: sorteia novo(s) vencedor(es) para um giveaway já terminado */
async function rerollGiveaway(guild, giveawayId, n = null) {
  const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(giveawayId, guild.id);
  if (!g) return null;

  const { vencedores } = sortearVencedores(giveawayId, n || g.vencedores_n);
  const canal = guild.channels.cache.get(g.channel_id);

  if (canal) {
    if (vencedores.length) {
      await canal.send({
        content: `🔄 **Reroll!** Novo(s) vencedor(es) de **${g.premio}**: ${vencedores.map(id => `<@${id}>`).join(', ')} 🎉`,
        allowedMentions: { users: vencedores }
      }).catch(() => {});
    } else {
      await canal.send({ content: `😕 Não há participantes suficientes para fazer reroll de **${g.premio}**.` }).catch(() => {});
    }
  }
...
/** Verifica giveaways ativos cujo tempo já terminou (chamado pelo cron a cada minuto) */
async function verificarGiveaways() {
  const agora = Date.now();
  const pendentes = db.prepare('SELECT id, guild_id FROM giveaways WHERE ended = 0 AND ends_at <= ?').all(agora);

  for (const g of pendentes) {
    const guild = client.guilds.cache.get(g.guild_id);
    if (!guild) continue;
    await terminarGiveaway(guild, g.id).catch(err => console.error(`❌ Erro ao terminar giveaway ${g.id}:`, err.message));
  }
}

/** Converte string tipo "10m", "2h", "1d" em milissegundos. Retorna null se inválido. */
function parseDuracao(str) {
  const match = /^(\d+)\s*(m|min|minuto|minutos|h|hora|horas|d|dia|dias)$/i.exec(str.trim());
  if (!match) return null;
  const valor = parseInt(match[1], 10);
  const unidade = match[2].toLowerCase();
  if (unidade.startsWith('m')) return valor * 60 * 1000;
  if (unidade.startsWith('h')) return valor * 60 * 60 * 1000;
  if (unidade.startsWith('d')) return valor * 24 * 60 * 60 * 1000;
  return null;
}
...
function iniciarCrons() {
  ...
  // Verifica giveaways a terminar (a cada minuto)
...
app.get('/dashboard/:guildId', requireAuth, requireGuildAdminPage, async (req, res) => {
  ...
  const giveaways = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50').all(guildId)
    .map(g => ({ ...g, participantes: db.prepare('SELECT COUNT(*) as c FROM giveaway_entries WHERE giveaway_id = ?').get(g.id).c }));
  ...
  res.send(renderGuildDashboard(req.session.user, guild, {
    ...
    ticketTypes, savedEmbeds, perguntas, staffRanking, members, giveaways,
...
// ── Giveaways ──
app.post('/api/:guildId/giveaway-criar', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { channel_id, titulo, premio, duracao, descricao, imagem, vencedores, cor, mensagem } = req.body;

  if (!channel_id || !titulo || !premio || !duracao) {
    return res.status(400).json({ ok: false, message: 'Preenche todos os campos obrigatórios.' });
  }

  const duracaoMs = parseDuracao(duracao);
  if (!duracaoMs) return res.status(400).json({ ok: false, message: 'Duração inválida. Usa por exemplo: 10m, 2h ou 1d.' });

  if (imagem && !/^https?:\/\/.+/i.test(imagem)) {
    return res.status(400).json({ ok: false, message: 'A URL da imagem tem de começar com http:// ou https://' });
  }
  if (cor && !/^#?[0-9A-Fa-f]{6}$/.test(cor)) {
    return res.status(400).json({ ok: false, message: 'Cor inválida. Usa um hexadecimal, ex: #5865F2.' });
  }

  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });

  const vencedoresN = Math.max(1, Math.min(20, parseInt(vencedores, 10) || 1));
  ...
  try {
    const giveawayId = await criarGiveaway(guild, {
      ...
      imagem: imagem || null,
      vencedoresN,
      pingTexto: mensagem || null,
      duracaoMs,
      cor: cor ? (cor.startsWith('#') ? cor : `#${cor}`) : null,
      createdBy: req.session.user.id,
    });
    res.json({ ok: true, message: `✅ Giveaway #${giveawayId} criado e publicado!` });
  } catch (e) {
    res.status(500).json({ ok: false, message: `Erro ao criar giveaway: ${e.message}` });
  }
});

app.post('/api/:guildId/giveaway-terminar/:id', requireGuildAdminApi, async (req, res) => {
  const { guildId, id } = req.params;
  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });

  const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!g) return res.status(404).json({ ok: false, message: 'Giveaway não encontrado.' });
  if (g.ended) return res.status(400).json({ ok: false, message: 'Este giveaway já terminou.' });

  const resultado = await terminarGiveaway(guild, id).catch(() => null);
  if (!resultado) return res.status(500).json({ ok: false, message: 'Erro ao terminar giveaway.' });

  res.json({ ok: true, message: resultado.vencedores.length ? `✅ Giveaway terminado! ${resultado.vencedores.length} vencedor(es) sorteado(s).` : '✅ Giveaway terminado sem participantes.' });
});

app.post('/api/:guildId/giveaway-reroll/:id', requireGuildAdminApi, async (req, res) => {
  const { guildId, id } = req.params;
  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });
  ...
  const g = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!g) return res.status(404).json({ ok: false, message: 'Giveaway não encontrado.' });
  if (!g.ended) return res.status(400).json({ ok: false, message: 'Este giveaway ainda não terminou.' });

  const resultado = await rerollGiveaway(guild, id).catch(() => null);
  if (!resultado) return res.status(500).json({ ok: false, message: 'Erro ao fazer reroll.' });

  res.json({ ok: true, message: resultado.vencedores.length ? '🔄 Reroll feito com sucesso!' : '❌ Não há participantes suficientes.' });
});

app.post('/api/:guildId/giveaway-cancelar/:id', requireGuildAdminApi, async (req, res) => {
  const { guildId, id } = req.params;
  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });
  ...
  res.json({ ok: true, message: '🗑️ Giveaway cancelado.' });
});
...
const dashboardJS = `
  ...
  async function criarGiveaway(guildId) {
    const form = document.getElementById('form-giveaway');
    const data = new FormData(form);
    const body = new URLSearchParams(data).toString();
    try {
      const res = await fetch('/api/' + guildId + '/giveaway-criar', {
        method: 'POST', headers: {'Content-Type':'application/x-www-form-urlencoded'}, body
      });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
  ...
  async function terminarGiveawayDash(guildId, id) {
    if (!confirm('Terminar este giveaway agora e sortear vencedor(es)?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaway-terminar/' + id, { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 1000);
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  async function cancelarGiveawayDash(guildId, id) {
    if (!confirm('Cancelar este giveaway sem sortear vencedor?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaway-cancelar/' + id, { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 1000);
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  async function rerollGiveawayDash(guildId, id) {
    if (!confirm('Sortear novo(s) vencedor(es) para este giveaway?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaway-reroll/' + id, { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
...
function renderGuildDashboard(user, guild, data) {
  const { ticketConfig, guildConfig, antispam, statsConfig, votacaoConfig, sugestaoConfig, reactionRoles, ticketTypes, savedEmbeds, perguntas, staffRanking, members, giveaways, totalTickets, openTickets, totalWarns, totalSugs, channels, roles, categories, autoroleHumanos, autoroleBots, roleExclusivity, blacklist, immuneRoles } = data;
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <div id="giveaways_tab" class="section" style="display:none">
      ...
      <div class="card">
        <h2>➕ Criar Giveaway</h2>
        <form id="form-giveaway">
          <div class="grid-2">
            <div class="form-group">
            ...
            <div class="form-group">
              <label>Duração</label>
              <input type="text" name="duracao" placeholder="Ex: 10m, 2h, 1d">
            </div>
          </div>
          <div class="form-group">
            <label>Título</label>
            <input type="text" name="titulo" placeholder="Ex: Giveaway de Nitro!" maxlength="200">
          </div>
          <div class="form-group">
            <label>Prémio</label>
            <input type="text" name="premio" placeholder="Ex: 1x Discord Nitro (1 mês)" maxlength="200">
          </div>
          <div class="form-group">
            <label>Descrição (opcional, dentro do embed)</label>
            <textarea name="descricao" rows="2" placeholder="Detalhes extra do giveaway..."></textarea>
          </div>
          <div class="grid-2">
            <div class="form-group">
              <label>URL da Imagem (opcional)</label>
          ...
          <div class="grid-2">
            <div class="form-group">
              ...
              <input type="text" name="cor" placeholder="#5865F2">
            </div>
            <div class="form-group">
              <label>Mensagem fora do embed (opcional, para marcar pessoal)</label>
              <input type="text" name="mensagem" placeholder="Ex: @everyone ou @Cargo, novo giveaway!">
            </div>
          </div>
          <div style="margin:8px 0 16px;padding:12px;background:var(--bg3);border-radius:8px;font-size:0.85rem;color:var(--text2)">
            ⚠️ Para mencionar @everyone, @here ou um cargo na "Mensagem fora do embed", escreve a menção normalmente (ex: @everyone). O bot precisa de permissão para mencionar esse cargo.
          </div>
          <button type="button" class="btn btn-primary" onclick="criarGiveaway('${guild.id}')">🎉 Criar e Publicar Giveaway</button>
        </form>
      </div>
      ...
      <div class="card">
        ...
        ${giveaways.length ? `
          <table class="table">
            <thead><tr><th>#</th><th>Título</th><th>Prémio</th><th>Canal</th><th>Participantes</th><th>Estado</th><th>Ações</th></tr></thead>
            <tbody>
              ${giveaways.map(g => `
                <tr>
                  <td>#${g.id}</td>
                  <td>${g.titulo}</td>
                  <td>${g.premio}</td>
                  <td>#${(channels.find(c=>c.id===g.channel_id)||{}).name || g.channel_id}</td>
                  <td>${g.participantes}</td>
                  <td>${g.ended ? '⚪ Terminado' : (g.ends_at <= Date.now() ? '🟡 A terminar' : '🟢 Ativo')}</td>
                  <td style="white-space:nowrap">
                    ${!g.ended ? `
                      <button class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick="terminarGiveawayDash('${guild.id}', ${g.id})">🏁 Terminar</button>
                      <button class="btn btn-danger" style="padding:4px 10px;font-size:0.8rem" onclick="cancelarGiveawayDash('${guild.id}', ${g.id})">🗑️ Cancelar</button>
                    ` : `
                      <button class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick="rerollGiveawayDash('${guild.id}', ${g.id})">🔄 Reroll</button>
                    `}
                  </td>
                </tr>
              `).join('')}
        ...
        ` : '<p style="color:var(--text2)">Ainda não criaste nenhum giveaway.</p>'}
      </div>
    </div>
    ...
    async function loadSuggestions() {
      try {
        ...
        const html = sugs.length ? '<table class="table"><thead><tr><th>#</th><th>Conteúdo</th><th>Utilizador</th><th>Estado</th><th>Votos</th></tr></thead><tbody>' +
          sugs.slice(0,15).map(s => '<tr><td>'+(s.guild_seq ?? s.id)+'</td><td style="max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+s.content+'</td><td>'+s.user_id+'</td><td>'+statusMap[s.status]+'</td><td>👍 '+s.votes_up+' / 👎 '+s.votes_down+'</td></tr>').join('') +
