...
async function fecharTicket(channel, closedBy, guild, reason = null) {
  ...
  (async () => {
    try {
      ...
      // Nome do painel (se o ticket tiver sido criado a partir de um painel guardado na BD)
      const ticketConfigParaMsg = ticket?.panel_id ? await db.prepare('SELECT panel_name FROM ticket_config WHERE id = ?').get(ticket.panel_id) : null;
      const nomePainel = ticketConfigParaMsg?.panel_name || guild?.name || 'Ticket';

      const embedUser = new EmbedBuilder()
        .setTitle('🎫 Ticket Fechado')
        .setDescription(`O teu ticket foi fechado em **${nomePainel}**!`)
        .setColor(CONFIG.COR_AVISO)
        .addFields(
          {
            name: '📋 Informação do Ticket',
            value: `**Data de Abertura:** ${formatarDataPT(ticket.created_at)}\n**Nome do Painel:** ${nomePainel}\n**Nome do Ticket:** ${channel.name}`
          },
          {
