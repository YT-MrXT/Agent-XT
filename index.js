...
function initDatabase() {
  ...
  } catch (e) { console.error('❌ Erro na migração de giveaways (titulo/descricao/imagem_url):', e.message); }
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'giveaway-criar') {
    ...
    const info = db.prepare(`
      INSERT INTO giveaways (guild_id, channel_id, premio, vencedores, ends_at, host_id, titulo, descricao, imagem_url)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(guild.id, canal.id, premio, vencedores, endsAtISO, user.id, titulo, descricao, imagem);
...
/** Verifica giveaways ativos e encerra os que já passaram da hora */
...
app.post('/api/:guildId/giveaways', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { canal_id, titulo, descricao, premio, imagem_url, duracao, vencedores } = req.body || {};
  ...
  `).run(guildId, canal.id, String(premio).trim(), nVencedores, endsAtISO, req.session?.user?.id || null,
         (titulo || '').trim() || null, (descricao || '').trim() || null, imagemUrl);
