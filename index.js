...
async function sendLogTyped(guild, tipo, embed) {
  try {
    ...
    // Evita enviar o mesmo embed 2x quando "Logs Gerais" e "Mod Log" apontam
    // para o MESMO canal (causa comum de "logs duplicados" na dashboard).
    const mesmoCanal = allowedLog && allowedModLog && config?.log_channel && config?.log_channel === config?.mod_log;

    const envios = [];
    if (allowedLog && config?.log_channel) {
      const ch = guild.channels.cache.get(config.log_channel);
      if (ch) envios.push(ch.send({ embeds: [embed] }).catch(() => {}));
    }
    if (!mesmoCanal && allowedModLog && config?.mod_log) {
      const ch = guild.channels.cache.get(config.mod_log);
      if (ch) envios.push(ch.send({ embeds: [embed] }).catch(() => {}));
    }
    // Envia para os dois canais em paralelo (em vez de um a seguir ao outro) — resposta mais rápida.
    if (envios.length) await Promise.all(envios);
  } catch (e) {
...
// ── Canal eliminado ──
// (Este handler também limpa os tickets ligados ao canal apagado — antes havia
// um SEGUNDO "client.on(Events.ChannelDelete...)" mais abaixo no ficheiro só
// para isso, o que fazia este evento correr duas vezes por cada canal apagado
// e era uma das causas de logs/mensagens duplicadas.)
client.on(Events.ChannelDelete, async channel => {
  db.prepare("UPDATE tickets SET status='deleted' WHERE channel_id = ?").run(channel.id).catch(() => {});
  if (!channel.guild) return;
  const {
    executor,
    motivo
