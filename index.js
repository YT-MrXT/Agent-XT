...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Bane automaticamente pelo username se a conta entrar no servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('username').setDescription('Username do utilizador (ex: mrxt.wls)').setRequired(true))
    .addStringOption(o => o.setName('motivo').setDescription('Motivo (ex: raid anterior)').setRequired(false)),

  new SlashCommandBuilder()
    .setName('blacklist-remove')
    .setDescription('Remove um utilizador da blacklist')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('id').setDescription('ID do utilizador a remover').setRequired(true)),
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'blacklist-add') {
    ...
    // Tenta encontrar a conta pelo username: primeiro nos membros atuais do servidor,
    // depois na lista de utilizadores banidos (útil se já expulsaste/baniste manualmente o raider antes)
    let foundUser = null;

    try {
      const results = await guild.members.fetch({ query: usernameInput, limit: 5 });
      foundUser = results.find(m => m.user.username.toLowerCase() === usernameInput)?.user
        || results.first()?.user
        || null;
    } catch (_) {}
    ...
    if (!foundUser) {
      try {
        ...
        const match = bans.find(b => b.user.username.toLowerCase() === usernameInput);
    ...
    if (!foundUser) {
      return interaction.editReply({
        content: `❌ Não encontrei nenhuma conta com o username \`${usernameInput}\` (nem nos membros atuais, nem nos já banidos deste servidor).\n\nIsto só funciona se a conta já passou por este servidor em algum momento. Se souberes o ID do Discord dele, posso adicionar um comando alternativo para esse caso.`,
      });
    }

    const targetId = foundUser.id;
    if (targetId === user.id) return interaction.editReply({ content: '❌ Não te podes adicionar a ti próprio à blacklist.' });

    try {
      db.prepare('INSERT INTO blacklist (guild_id, user_id, username, reason, added_by) VALUES (?, ?, ?, ?, ?)')
        .run(guild.id, targetId, foundUser.tag, motivo, user.id);
    } catch (e) {
      if (String(e.message).includes('UNIQUE')) {
        return interaction.editReply({ content: `⚠️ **${foundUser.tag}** já está na blacklist deste servidor.` });
      }
      return interaction.editReply({ content: `❌ Erro ao guardar na blacklist: ${e.message}` });
    }

    // Se já estiver no servidor, bane imediatamente
    let jaBanido = false;
    const memberJaNoServidor = await guild.members.fetch(targetId).catch(() => null);
    if (memberJaNoServidor && memberJaNoServidor.bannable) {
      await memberJaNoServidor.ban({ reason: `Blacklist: ${motivo}` }).catch(() => {});
      jaBanido = true;
    }

    logMod(guild.id, 'BLACKLIST-ADD', targetId, user.id, motivo);

    const embed = embedPadrao(
      '🚫 Utilizador Adicionado à Blacklist',
      `**Utilizador:** ${foundUser.tag} (\`${targetId}\`)\n**Moderador:** <@${user.id}>\n**Motivo:** ${motivo}\n\n${jaBanido ? '⚠️ Este utilizador já estava no servidor e foi banido agora.' : '✅ Se esta conta entrar no servidor, será banida automaticamente.'}`,
      CONFIG.COR_ERRO
    );
    await sendLog(guild, embed);
    return interaction.editReply({ embeds: [embed] });
  ...
  if (commandName === 'blacklist-remove') {
    ...
    const res = db.prepare('DELETE FROM blacklist WHERE guild_id = ? AND user_id = ?').run(guild.id, targetId);

    if (res.changes === 0) return interaction.reply({ content: `⚠️ O utilizador \`${targetId}\` não estava na blacklist.`, ephemeral: true });

    logMod(guild.id, 'BLACKLIST-REMOVE', targetId, user.id, 'Removido da blacklist');
    return interaction.reply({ content: `✅ Utilizador \`${targetId}\` removido da blacklist.`, ephemeral: true });
  }
  ...
  if (commandName === 'blacklist-lista') {
    ...
    const embed = embedPadrao(
      ...
      lista.map(b => `**${b.username || 'Desconhecido'}** (\`${b.user_id}\`)\n↳ Motivo: ${b.reason} — por <@${b.added_by}>`).join('\n\n')
    );
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
...
client.on(Events.GuildMemberAdd, async member => {
  // Verifica blacklist: se o ID deste utilizador estiver bloqueado neste servidor, bane de imediato
  const blEntry = db.prepare('SELECT * FROM blacklist WHERE guild_id = ? AND user_id = ?').get(member.guild.id, member.id);
  if (blEntry) {
    await member.ban({ reason: `Blacklist: ${blEntry.reason || 'Conta bloqueada'}` }).catch(() => {});
    const embed = embedPadrao(
...
async function processarEmbedsAgendadas() {
  ...
  for (const saved of pendentes) {
    try {
      ...
      const nextSend = new Date(Date.now() + saved.schedule_interval_minutes * 60 * 1000).toISOString();
      db.prepare('UPDATE saved_embeds SET schedule_next_send = ? WHERE id = ?').run(nextSend, saved.id);
    } catch (err) {
...
app.post('/api/:guildId/antispam-config', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { enabled, max_messages, action, anti_links, anti_invites, anti_raid, log_channel, trap_channel, anti_bot_add } = req.body;
...
app.post('/api/:guildId/role-exclusivity', requireGuildAdminApi, async (req, res) => {
  ...
  loseRoleIds = loseRoleIds.filter(id => id && id !== gain_role_id);

  if (!gain_role_id || !loseRoleIds.length) {
    return res.status(400).json({ ok: false, message: 'Escolhe o cargo a ganhar e pelo menos um cargo a perder.' });
  }
  ...
  try {
    ...
    `);
    const tx = db.transaction((ids) => {
      for (const loseId of ids) ins.run(guildId, gain_role_id, loseId);
    });
    tx(loseRoleIds);
    ...
    // Aplica retroativamente em background: todos os membros que JÁ têm o cargo
    // ganho perdem já os cargos escolhidos. Pode demorar em servidores grandes,
    // por isso corre depois de já termos respondido ao pedido.
    (async () => {
      try {
        await guild.members.fetch();
        const membrosComCargo = guild.members.cache.filter(m => m.roles.cache.has(gain_role_id));
        for (const m of membrosComCargo.values()) {
          for (const loseId of loseRoleIds) {
...
app.post('/api/:guildId/embeds/agendar', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { id, channel_id, interval_minutes } = req.body;
  ...
  const nextSend = new Date(Date.now() + minutos * 60 * 1000).toISOString();
  db.prepare(`
    UPDATE saved_embeds
    SET schedule_channel = ?, schedule_interval_minutes = ?, schedule_active = 1, schedule_next_send = ?
    WHERE id = ? AND guild_id = ?
  `).run(channel_id, minutos, nextSend, id, guildId);

  res.json({ ok: true, message: `✅ Envio automático ativado — a cada ${minutos} minuto(s) em #${canal.name}.` });
});
...
const dashboardJS = `
  ...
  async function saveAutoRole(guildId, target) {
    const selectId = target === 'bot' ? 'autorole_bot_roles' : 'autorole_human_roles';
  ...
  async function addExclusividade(guildId) {
    const gainSelect = document.getElementById('gain_role_id');
    const loseContainer = document.getElementById('lose_role_ids');
    const gain = gainSelect.value;
    const loseIds = Array.from(loseContainer.querySelectorAll('input[type="checkbox"]:checked')).map(c => c.value);
    if (!gain || !loseIds.length) { toast('❌ Escolhe o cargo a ganhar e pelo menos um cargo a perder.', 'error'); return; }
    if (loseIds.includes(gain)) { toast('❌ O cargo a ganhar não pode estar também nos cargos a perder.', 'error'); return; }
    try {
      const res = await fetch('/api/' + guildId + '/role-exclusivity', {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ gain_role_id: gain, lose_role_ids: loseIds })
      });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 800);
  ...
  async function agendarEmbed(guildId, id) {
    ...
    try {
      const res = await fetch('/api/' + guildId + '/embeds/agendar', {
        method: 'POST', headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: 'id=' + id + '&channel_id=' + channel_id + '&interval_minutes=' + interval_minutes
      });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 1000);
...
function renderGuildDashboard(user, guild, data) {
  const { ticketConfig, guildConfig, antispam, statsConfig, votacaoConfig, sugestaoConfig, reactionRoles, ticketTypes, savedEmbeds, staffRanking, members, totalTickets, openTickets, totalWarns, totalSugs, channels, roles, categories, autoroleHumanos, autoroleBots, roleExclusivity } = data;
  ...
  const makeMultiSelect = (name, options, currentList=[]) =>
    `<select name="${name}" id="${name}" multiple size="8" style="height:auto">
      ${options.map(o => `<option value="${o.id}" ${currentList.includes(o.id) ? 'selected' : ''}>${o.name}</option>`).join('')}
    </select>`;

  const makeCheckboxGroup = (name, options, currentList=[]) =>
    `<div id="${name}" class="checkbox-group" style="max-height:220px;overflow-y:auto;border:1px solid var(--border,#444);border-radius:8px;padding:8px">
      ${options.map(o => `
        <label style="display:flex;align-items:center;gap:8px;padding:6px 8px;cursor:pointer;border-radius:6px" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
          <input type="checkbox" value="${o.id}" ${currentList.includes(o.id) ? 'checked' : ''} style="cursor:pointer">
          <span>${o.name}</span>
        </label>
      `).join('')}
    </div>`;
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <div id="embeds_tab" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div id="embeds-table">
          ${savedEmbeds.length ? `
            <table class="data-table">
              <thead><tr><th>Nome</th><th>Título</th><th>Enviar uma vez</th><th>Envio automático</th><th></th></tr></thead>
              <tbody>
                ${savedEmbeds.map(e => {
                  let titulo = '—';
                  ...
                  const statusAuto = e.schedule_active
                    ? `<span style="color:var(--success, #3ba55c)">🟢 A cada ${e.schedule_interval_minutes} min em #${canalAtual ? canalAtual.name : '?'}</span>`
    ...
    <div id="cargos_tab" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Cargos atribuídos automaticamente a novos membros humanos que entrarem no servidor. Podes escolher vários (Ctrl/Cmd + clique).
        </p>
        <div class="form-group">
          ${makeMultiSelect('autorole_human_roles', roles, autoroleHumanos)}
        </div>
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div class="form-group">
          ${makeMultiSelect('autorole_bot_roles', roles, autoroleBots)}
        </div>
      ...
      <div class="card" style="margin-top:20px">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Configura: quando alguém <strong>recebe</strong> um cargo, o bot remove automaticamente o(s) cargo(s) escolhido(s) (se a pessoa os tiver) — mesmo quem já os tinha antes de a regra existir. Marca as caixas dos cargos que queres remover (um clique cada). Funciona em tempo real, seja qual for o motivo do cargo ter sido atribuído.
        </p>
        <form id="form-exclusividade">
          <div class="grid-2">
            <div class="form-group">
              <label>Ao receber este cargo…</label>
              ${makeSelect('gain_role_id', roles, '', 'Cargo ganho')}
            </div>
            <div class="form-group">
              <label>…perde estes cargos</label>
              ${makeCheckboxGroup('lose_role_ids', roles, [])}
            </div>
          </div>
