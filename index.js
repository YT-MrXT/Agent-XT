...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Cria um painel de informação (ex: Bem-vindo ao Servidor) publicado neste canal')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'painel-criar') {
    ...
    const panel = db.prepare('SELECT * FROM info_panels WHERE id = ?').get(info.lastInsertRowid);
    ...
    db.prepare('UPDATE info_panels SET message_id = ?, channel_id = ? WHERE id = ?').run(msg.id, canal.id, panel.id);

    return interaction.reply({
      content: `✅ Painel **${nome}** criado em <#${canal.id}>! Usa \`/painel-botao-add painel:${nome}\` para adicionares botões (as respostas só aparecem para quem clicar).`,
      ephemeral: true,
    });
  }
  ...
  if (commandName === 'painel-botao-add') {
    ...
    // Atualiza a mensagem publicada com o novo botão
  ...
  if (commandName === 'painel-lista') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(paineis.map(p => {
        ...
        return `**${p.name}** — ${p.channel_id ? `<#${p.channel_id}>` : '*sem canal*'} — ${nBotoes} botão(ões)`;
      }).join('\n'));
...
app.post('/api/:guildId/infopanels', requireGuildAdminApi, async (req, res) => {
  ...
  res.json({ ok: true, message: `✅ Painel "${nome}" criado em #${canal.name}!` });
});
...
app.post('/api/:guildId/infopanels/:id/buttons', requireGuildAdminApi, async (req, res) => {
  ...
  // Atualiza a mensagem publicada no Discord com o novo botão
  if (guild && panel.message_id && panel.channel_id) {
    const canalMsg = guild.channels.cache.get(panel.channel_id);
    const msg = canalMsg ? await canalMsg.messages.fetch(panel.message_id).catch(() => null) : null;
    if (msg) {
    ...
    <div id="infopanels_tab" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          ...
          Depois de criado, adiciona botões abaixo — cada botão mostra um texto que só a pessoa que clicou consegue ver.
        </p>
      ...
      <div class="card" id="infopanel-buttons-card" style="display:none">
        <h2>🔘 Botões do painel: <span id="infopanel-buttons-panel-name"></span></h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Cada botão que adicionares aparece na mensagem do Discord. Quando alguém clica, só essa pessoa vê a "Resposta" (mensagem ephemeral).
        </p>
  ...
  <script>
    ...
    async function loadInfoPanels() {
      try {
        ...
        const html = paineis.length ? '<table class="table"><thead><tr><th>Nome</th><th>Título</th><th>Canal</th><th>Botões</th><th>Ações</th></tr></thead><tbody>' +
          paineis.map(p => '<tr>' +
            '<td>'+p.name+'</td>' +
            ...
            '<td>' +
              '<button class="btn btn-secondary" style="padding:4px 10px;font-size:0.8rem" onclick="abrirBotoesInfoPanel('+p.id+',\\''+p.name.replace(/'/g,"\\\\'")+'\\')">🔘 Botões</button> ' +
    ...
    async function abrirBotoesInfoPanel(panelId, panelName) {
      ...
      document.getElementById('infopanel-buttons-card').style.display = 'block';
      document.getElementById('infopanel-buttons-card').scrollIntoView({ behavior: 'smooth' });
