...
async function initDatabase() {
  // Garante que a réplica local (embedded replica) já sincronizou com a
  // base primária remota antes de sequer verificar/criar tabelas — evita
  // ler uma cópia local vazia/desatualizada logo no arranque.
  await db.pronto;
...
// Cache em memória: guildId -> Set de trigger_commands configurados nesse servidor.
// Evita uma query à BD em CADA slash command só para verificar se é um comando de
// embed guardada (a grande maioria dos comandos não é). É preenchida no arranque
// (sincronizarComandosEmbedTodosServidores) e atualizada sempre que um comando de
// embed é criado/editado/apagado (sincronizarComandosEmbed).
const triggerCommandsCache = new Map(); // guildId -> Set<string>
...
async function sincronizarComandosEmbed(guildId) {
  try {
    ...
    // Atualiza a cache em memória com os trigger_commands atuais deste servidor.
    triggerCommandsCache.set(guildId, new Set(embedsComComando.map(e => e.trigger_command)));
...
async function sincronizarComandosEmbedTodosServidores() {
  try {
    // Inicializa a cache com um Set vazio para TODOS os servidores onde o bot
    // está, mesmo os que não têm nenhum comando de embed configurado. Assim, a
    // verificação rápida em tratarComandoSlashEmbed funciona logo desde o arranque
    // para todos os servidores, e não só para os que já tinham comandos guardados.
    for (const [guildId] of client.guilds.cache) {
      triggerCommandsCache.set(guildId, new Set());
    }
...
async function tratarComandoSlashEmbed(interaction) {
  ...
  // Verificação rápida em memória primeiro: se este servidor não tem NENHUM
  // trigger_command igual a este commandName, sabemos de imediato que não é um
  // comando de embed guardada, sem precisar de ir à base de dados (que é remota).
  const cacheServidor = triggerCommandsCache.get(guild.id);
  if (cacheServidor && !cacheServidor.has(commandName)) return false;
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'blacklist-add') {
    ...
    try {
      console.time('membersFetch_' + guild.id); await guild.members.fetch(); console.timeEnd('membersFetch_' + guild.id); console.log('🔍 [membersFetch] Guild:', guild.id, '- membros em cache:', guild.members.cache.size);
...
client.on(Events.GuildCreate, async guild => {
  try {
    ...
    // Inicializa a cache de trigger_commands para este servidor novo, para que a
    // verificação rápida em tratarComandoSlashEmbed funcione desde já, sem cair
    // na query à base de dados a cada slash command.
    triggerCommandsCache.set(guild.id, new Set());
...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  app.get('/auth/discord', (req, res) => {
    ...
    // 🔍 DEBUG: confirma exatamente o que está a ser enviado ao Discord
    console.log('🔍 [OAuth] A iniciar login. CLIENT_ID:', CONFIG.CLIENT_ID);
    console.log('🔍 [OAuth] REDIRECT_URI enviado:', CONFIG.REDIRECT_URI);
    res.redirect(`https://discord.com/api/oauth2/authorize?${params}`);
  });
  app.get('/auth/callback', async (req, res) => {
    const code = req.query.code;
    console.log('🔍 [OAuth Callback] code recebido:', code ? '(presente)' : '(ausente)');
    if (!code) return res.redirect('/login?error=no_code');
    try {
      // Troca o code por token
      console.log('🔍 [OAuth Callback] Passo 1/3 - a trocar code por token...');
      console.log('🔍 [OAuth Callback] REDIRECT_URI usado na troca:', CONFIG.REDIRECT_URI);
      ...
      console.log('✅ [OAuth Callback] Passo 1/3 OK - token obtido. Headers rate limit:', {
        remaining: tokenRes.headers['x-ratelimit-remaining'],
        limit: tokenRes.headers['x-ratelimit-limit'],
        resetAfter: tokenRes.headers['x-ratelimit-reset-after']
      });
      const {
        access_token
      } = tokenRes.data;
      ...
      console.log('🔍 [OAuth Callback] Passo 2/3 - a obter dados do utilizador...');
      const userRes = await axios.get('https://discord.com/api/users/@me', {
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
      console.log('✅ [OAuth Callback] Passo 2/3 OK - utilizador:', userRes.data.username, userRes.data.id);
      const discordUser = userRes.data;

      // Obtém servidores
      console.log('🔍 [OAuth Callback] Passo 3/3 - a obter lista de servidores...');
      const guildsRes = await axios.get('https://discord.com/api/users/@me/guilds', {
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
      console.log('✅ [OAuth Callback] Passo 3/3 OK -', guildsRes.data.length, 'servidores recebidos');
      ...
      console.log('✅ [OAuth Callback] Login completo para', discordUser.username);
      res.redirect('/dashboard');
    } catch (e) {
      // 🔍 DEBUG DETALHADO: mostra exatamente QUAL pedido falhou e PORQUÊ
      console.error('❌ [OAuth Callback] Auth error:', e.message);
      if (e.response) {
        console.error('❌ [OAuth Callback] Status:', e.response.status);
        console.error('❌ [OAuth Callback] URL que falhou:', e.config?.url);
        console.error('❌ [OAuth Callback] Resposta do Discord:', JSON.stringify(e.response.data));
        if (e.response.status === 429) {
          console.error('❌ [OAuth Callback] 🚨 RATE LIMIT! Retry-After:', e.response.headers['retry-after'], 'segundos');
          console.error('❌ [OAuth Callback] 🚨 Rate limit global?', e.response.headers['x-ratelimit-global']);
          console.error('❌ [OAuth Callback] 🚨 Scope do rate limit:', e.response.headers['x-ratelimit-scope']);
        }
      } else {
        console.error('❌ [OAuth Callback] Sem resposta do Discord (erro de rede/timeout):', e.code);
      }
  ...
  async function buildDashboardData(guild, selectedPanelId = null) {
    ...
    // Lista de todos os painéis de tickets do servidor (até 10) + o painel selecionado para edição
    const ticketPanels = await db.prepare('SELECT * FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id').all(guildId);
    ...
    const tvConfig = await db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ?').get(guildId);
    const tvChannelsCount = (await db.prepare('SELECT COUNT(*) as c FROM tempvoice_channels WHERE guild_id = ?').get(guildId))?.c || 0;
    ...
    const reactionRoles = await Promise.all(rrPaineis.map(async p => ({
      ...p,
      itens: await db.prepare('SELECT * FROM reaction_roles WHERE guild_id = ? AND message_id = ?').all(guildId, p.message_id)
    })));
    const ticketTypes = ticketConfig ? await db.prepare('SELECT * FROM ticket_types WHERE guild_id = ? AND (panel_id = ? OR panel_id IS NULL) ORDER BY order_num, id').all(guildId, ticketConfig.id) : [];
    ...
    const staffRanking = await getRankingStaff(guildId);
