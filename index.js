...
function initDatabase() {
  ...
  `);
  // Migração: garante a coluna de apelido do bot em bases de dados já existentes
  try {
  ...
  } catch (e) { console.error('❌ Erro na migração de guild_config (bot_nickname):', e.message); }
 ...
 * Se nickname vier vazio/nulo, remove o apelido e o bot volta a mostrar o nome original.
 */
...
app.post('/api/:guildId/bot-identity', requireGuildAdminApi, async (req, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?,?)
    ON CONFLICT(guild_id) DO UPDATE SET bot_nickname=excluded.bot_nickname
  ...
  res.json({
    ...
    message: nickResult.ok
      ? (nickname ? `✅ Nome do bot atualizado para "${nickname}" neste servidor!` : '✅ Nome do bot repпосto para o original neste servidor!')
      : `❌ Não foi possível mudar o nome: ${nickResult.error}`,
  });
});
...
app.get('/api/:guildId/bot-identity', requireGuildAdminApi, (req, res) => {
  ...
  res.json({ bot_nickname: config?.bot_nickname || '' });
});
