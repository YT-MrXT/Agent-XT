...
function normalizarTexto(texto) {
  ...
  // Remove separadores usados para "disfarçar" (pontos, espaços, hífens, underscores, etc.)
  t = t.replace(/[\s._\-*~^]/g, '');
...
const AI_MOD_SYSTEM_PROMPT = `Analisas mensagens de um servidor de Discord em português para deteção de conteúdo impróprio (insultos, discurso de ódio, assédio, ameaças, conteúdo sexual/NSFW em texto, etc). Considera variações disfarçadas (leetspeak, espaços, símbolos).
...
async function verificarSpam(message) {
  ...
  try {
    ...
    const temBudget = podeChamarIA(message.guild.id);
    console.log('[AutoMod IA][DEBUG]', {
      guild: message.guild.id,
      aiConfigEncontrado: !!aiConfig,
      temApiKey: !!CONFIG.AI_API_KEY,
      tamanhoMsg: conteudoTrim.length,
      minChars: AI_MOD_LIMITS.MIN_CHARS,
      temBudget
    });
    if (
      aiConfig &&
      ...
      console.log('[AutoMod IA][DEBUG] Classificação recebida:', JSON.stringify(classificacao));
      if (classificacao && classificacao.violacao) {
        ...
        console.log('[AutoMod IA][DEBUG] Severidades:', { minSeveridade, severidadeMsg, vaiAgir: severidadeMsg >= minSeveridade });
      ...
      console.log('[AutoMod IA][DEBUG] Condição inicial falhou, não chamou a IA.');
    }
  } catch (e) {
