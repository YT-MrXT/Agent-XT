...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tk-panel-nav { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 14px; }
  .tk-panel-navbtn { background: var(--bg3); border: 1px solid var(--border); color: var(--text); width: 36px; height: 36px; border-radius: 9px; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s ease; }
  .tk-panel-navbtn:hover:not(:disabled) { background: var(--bg4); border-color: var(--accent); }
  .tk-panel-navbtn:disabled { opacity: 0.35; cursor: not-allowed; }
  .tk-panel-counter { font-weight: 700; color: var(--text2); font-size: 0.9rem; white-space: nowrap; }
  .tk-panel-counter b { color: var(--text); }
  .tk-actions-row { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 16px; }
  .tk-action-btn { display: inline-flex; align-items: center; gap: 7px; background: var(--bg3); border: 1px solid var(--border); color: var(--text); padding: 9px 16px; border-radius: 10px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.15s ease; }
  .tk-action-btn:hover { background: var(--bg4); border-color: #3a4066; transform: translateY(-1px); }
  .tk-action-btn.danger:hover { border-color: var(--danger); color: var(--danger); }
  .tk-panel-body { background: var(--bg3); border: 1px solid var(--border); border-radius: 12px; padding: 18px 20px; }
  .tk-panel-meta { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin-bottom: 14px; }
  .tk-panel-meta-left { color: var(--text2); font-size: 0.85rem; }
  .tk-panel-meta-left b { color: var(--text); }
  .tk-panel-badges { display: flex; gap: 8px; flex-wrap: wrap; }
  .tk-badge { padding: 4px 12px; border-radius: 999px; font-size: 0.78rem; font-weight: 700; border: 1px solid transparent; }
  ...
  .tk-panel-preview { background: #1a1c22; border-left: 4px solid var(--accent); border-radius: 6px; padding: 14px 16px; margin-top: 4px; }
  .tk-panel-preview .tk-pp-title { color: #f2f3f5; font-weight: 700; font-size: 1rem; margin-bottom: 4px; }
  .tk-panel-preview .tk-pp-desc { color: #dbdee1; font-size: 0.85rem; line-height: 1.5; white-space: pre-wrap; }
...
`;
  ...
  // ── Painéis de tickets (viewer estilo "Ticket King") ──
  let __tkPanels = [];
  let __tkFiltered = [];
  let __tkIndex = 0;
  ...
  function tkInitPanels() {
    ...
    // Abre sempre no painel ativo (o que está a ser editado / veio do ?painel=)
    const activeIdx = __tkFiltered.findIndex(p => p.active);
    __tkIndex = activeIdx >= 0 ? activeIdx : 0;
    tkRenderViewer();
  }
  ...
  function tkGoPrev() { if (__tkIndex > 0) { __tkIndex--; tkRenderViewer(); } }
  function tkGoNext() { if (__tkIndex < __tkFiltered.length - 1) { __tkIndex++; tkRenderViewer(); } }

  function tkRenderViewer() {
    const empty = document.getElementById('tk-panel-empty');
    const viewer = document.getElementById('tk-panel-viewer');
    if (!viewer) return;
    ...
    const p = __tkFiltered[__tkIndex];
    const guildId = window.__ticketGuildId;
    const total = __tkFiltered.length;
    const modeLabel = p.mode === 'buttons' ? '🔘 Botões' : '📋 Menu de Seleção';

    viewer.innerHTML = \`
      <div class="tk-panel-nav">
        <button type="button" class="tk-panel-navbtn" onclick="tkGoPrev()" \${__tkIndex === 0 ? 'disabled' : ''} title="Painel anterior">◀</button>
        <div class="tk-panel-counter"><b>\${__tkIndex + 1}</b> / \${total} Painéis</div>
        <button type="button" class="tk-panel-navbtn" onclick="tkGoNext()" \${__tkIndex === total - 1 ? 'disabled' : ''} title="Próximo painel">▶</button>
      </div>

      <div class="tk-actions-row">
        <a class="tk-action-btn" href="/dashboard/\${guildId}?painel=\${p.id}#tickets">✏️ Editar</a>
        <button type="button" class="tk-action-btn" onclick="tkMovePanel('\${guildId}', \${p.id})">🖐️ Mover</button>
        <button type="button" class="tk-action-btn" onclick="tkResendPanel('\${guildId}', \${p.id})" \${p.sent ? '' : 'disabled title="Este painel ainda não foi enviado"'}>📨 Reenviar</button>
        <a class="tk-action-btn" href="/dashboard/\${guildId}?painel=\${p.id}#tickets">📑 Template</a>
        <button type="button" class="tk-action-btn danger" onclick="apagarPainelTicket('\${guildId}', \${p.id}, '\${(p.name||'').replace(/'/g,"\\\\'")}')" \${__tkPanels.length <= 1 ? 'disabled title="Precisas de ter pelo menos 1 painel"' : ''}>🗑️ Apagar</button>
      </div>

      <div class="tk-panel-body">
        <div class="tk-panel-meta">
          <div class="tk-panel-meta-left">Última atualização: <b>\${p.sent ? 'Painel enviado' : 'Ainda por enviar'}</b></div>
          <div class="tk-panel-badges">
            <span class="tk-badge tk-badge-type">\${modeLabel}</span>
            \${p.channelName ? \`<span class="tk-badge tk-badge-channel"># \${p.channelName}</span>\` : '<span class="tk-badge tk-badge-draft">Sem canal definido</span>'}
          </div>
        </div>
        <div class="tk-panel-preview" style="border-left-color:\${p.color || '#5865F2'}">
          <div class="tk-pp-title">\${p.name}</div>
          <div class="tk-pp-desc">\${p.categoryName ? ('Categoria: ' + p.categoryName) : 'Categoria por definir'} — clica em ✏️ Editar para configurar este painel por completo.</div>
        </div>
      </div>
    \`;
  }
