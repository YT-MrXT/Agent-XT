...
function initDatabase() {
  ...
  try {
    ...
    if (!gcCols.some(c => c.name === 'welcome_title'))    db.exec('ALTER TABLE guild_config ADD COLUMN welcome_title TEXT');
    if (!gcCols.some(c => c.name === 'welcome_image'))    db.exec('ALTER TABLE guild_config ADD COLUMN welcome_image TEXT');
    // Novos campos do editor avançado de embeds de boas-vindas (estilo Sapphire)
    if (!gcCols.some(c => c.name === 'welcome_color'))       db.exec("ALTER TABLE guild_config ADD COLUMN welcome_color TEXT DEFAULT '#5865F2'");
    if (!gcCols.some(c => c.name === 'welcome_author_name')) db.exec('ALTER TABLE guild_config ADD COLUMN welcome_author_name TEXT');
    if (!gcCols.some(c => c.name === 'welcome_author_icon')) db.exec('ALTER TABLE guild_config ADD COLUMN welcome_author_icon TEXT');
    if (!gcCols.some(c => c.name === 'welcome_thumbnail'))   db.exec('ALTER TABLE guild_config ADD COLUMN welcome_thumbnail TEXT');
    if (!gcCols.some(c => c.name === 'welcome_footer'))      db.exec('ALTER TABLE guild_config ADD COLUMN welcome_footer TEXT');
    if (!gcCols.some(c => c.name === 'welcome_image_pos'))   db.exec("ALTER TABLE guild_config ADD COLUMN welcome_image_pos TEXT DEFAULT 'bottom'"); // bottom | thumbnail(direita) | none
    if (!gcCols.some(c => c.name === 'welcome_content'))     db.exec('ALTER TABLE guild_config ADD COLUMN welcome_content TEXT'); // texto fora da embed (ex: ${usermention} bem-vindo a ${guildname})
    if (!gcCols.some(c => c.name === 'welcome_url'))         db.exec('ALTER TABLE guild_config ADD COLUMN welcome_url TEXT'); // link do título
    if (!gcCols.some(c => c.name === 'log_types'))        db.exec('ALTER TABLE guild_config ADD COLUMN log_types TEXT');
    if (!gcCols.some(c => c.name === 'mod_log_types'))    db.exec('ALTER TABLE guild_config ADD COLUMN mod_log_types TEXT');
  ...
  try {
    const tcCols = db.prepare("PRAGMA table_info(ticket_config)").all().map(c => c.name);
    if (!tcCols.includes('panel_mode'))  db.exec("ALTER TABLE ticket_config ADD COLUMN panel_mode TEXT DEFAULT 'select'");
    if (!tcCols.includes('panel_color')) db.exec("ALTER TABLE ticket_config ADD COLUMN panel_color TEXT DEFAULT '#5865F2'");
  } catch (e) { console.error('❌ Erro na migração de ticket_config (panel_mode/panel_color):', e.message); }
  ...
  try { db.exec(`ALTER TABLE suggestions ADD COLUMN type_id INTEGER`); } catch (_) {}
  ...
  try {
    const configsAntigas = db.prepare('SELECT * FROM suggestion_config').all();
    for (const cfg of configsAntigas) {
      const jaTem = db.prepare('SELECT id FROM suggestion_types WHERE guild_id = ?').get(cfg.guild_id);
      if (!jaTem && cfg.channel_id) {
        ...
        db.prepare('UPDATE suggestions SET type_id = ? WHERE guild_id = ? AND type_id IS NULL')
          .run(info.lastInsertRowid, cfg.guild_id);
      }
    }
  } catch (e) { console.error('❌ Erro na migração de suggestion_config → suggestion_types:', e.message); }
  ...
  try { db.exec(`ALTER TABLE antispam_config ADD COLUMN link_invite_channels TEXT DEFAULT '[]'`); } catch (_) {}
  ...
  try { db.exec(`ALTER TABLE antispam_config ADD COLUMN link_invite_excluded_channels TEXT DEFAULT '[]'`); } catch (_) {}

  // Migração suave: número da sugestão específico de cada servidor (para não "saltarem" IDs entre servidores)
  try { db.exec(`ALTER TABLE suggestions ADD COLUMN guild_seq INTEGER`); } catch (_) {}
  // Preenche guild_seq para sugestões antigas que ainda não o têm, servidor a servidor
  try {
    const guildsComSugestoes = db.prepare(`SELECT DISTINCT guild_id FROM suggestions WHERE guild_seq IS NULL`).all();
    for (const { guild_id } of guildsComSugestoes) {
      const antigas = db.prepare(`SELECT id FROM suggestions WHERE guild_id = ? ORDER BY id ASC`).all(guild_id);
      antigas.forEach((row, idx) => {
        db.prepare(`UPDATE suggestions SET guild_seq = ? WHERE id = ?`).run(idx + 1, row.id);
      });
    }
  } catch (_) {}
  ...
  try { db.exec(`ALTER TABLE saved_embeds ADD COLUMN trigger_command TEXT`); } catch (_) {}
  try { db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_saved_embeds_trigger ON saved_embeds(guild_id, trigger_command)`); } catch (_) {}
  ...
  try {
    const ipCols = db.prepare("PRAGMA table_info(info_panels)").all().map(c => c.name);
    if (!ipCols.includes('published')) {
      db.exec('ALTER TABLE info_panels ADD COLUMN published INTEGER DEFAULT 0');
      // Painéis já publicados anteriormente (têm message_id) ficam marcados como publicados
      db.exec("UPDATE info_panels SET published = 1 WHERE message_id IS NOT NULL AND message_id != ''");
    }
  } catch (e) { console.error('❌ Erro na migração de info_panels (published):', e.message); }
  ...
  try {
    const ipbCols = db.prepare("PRAGMA table_info(info_panel_buttons)").all().map(c => c.name);
    if (!ipbCols.includes('response_title'))     db.exec('ALTER TABLE info_panel_buttons ADD COLUMN response_title TEXT');
    if (!ipbCols.includes('response_image'))     db.exec('ALTER TABLE info_panel_buttons ADD COLUMN response_image TEXT');
    if (!ipbCols.includes('response_thumbnail')) db.exec('ALTER TABLE info_panel_buttons ADD COLUMN response_thumbnail TEXT');
    if (!ipbCols.includes('response_color'))     db.exec('ALTER TABLE info_panel_buttons ADD COLUMN response_color TEXT');
  } catch (e) { console.error('❌ Erro na migração de info_panel_buttons:', e.message); }
  ...
  try {
    const pCols = db.prepare("PRAGMA table_info(perguntas)").all().map(c => c.name);
    if (!pCols.includes('mensagem_extra')) db.exec('ALTER TABLE perguntas ADD COLUMN mensagem_extra TEXT');
  } catch (e) { console.error('❌ Erro na migração de perguntas.mensagem_extra:', e.message); }
  ...
  try {
    const gwCols = db.prepare("PRAGMA table_info(giveaways)").all().map(c => c.name);
    if (!gwCols.includes('titulo'))     db.exec('ALTER TABLE giveaways ADD COLUMN titulo TEXT');
    if (!gwCols.includes('descricao'))  db.exec('ALTER TABLE giveaways ADD COLUMN descricao TEXT');
    if (!gwCols.includes('imagem_url')) db.exec('ALTER TABLE giveaways ADD COLUMN imagem_url TEXT');
    if (!gwCols.includes('mensagem_extra')) db.exec('ALTER TABLE giveaways ADD COLUMN mensagem_extra TEXT');
  } catch (e) { console.error('❌ Erro na migração de giveaways (titulo/descricao/imagem_url/mensagem_extra):', e.message); }
...
async function obterExecutorAuditLog(guild, tipo, targetId, janelaMs = 15000) {
  let audit;
  try {
    audit = await guild.fetchAuditLogs({ type: tipo, limit: 5 });
  } catch (e) {
    ...
    return { executor: null, motivo: semPermissao ? 'sem_permissao' : 'erro' };
  }

  const entry = audit.entries.find(e =>
    (e.target?.id === targetId || e.targetId === targetId) &&
    (Date.now() - e.createdTimestamp) < janelaMs
  );

  if (!entry?.executor) return { executor: null, motivo: 'nao_encontrado' };
  return { executor: entry.executor, motivo: null };
}
...
function embedLogModeracao(opts) {
  const {
    titulo, cor = CONFIG.COR_PRINCIPAL,
    alvo, alvoIdManual, moderador, motivoDesconhecido, motivo, duracao,
    canal, linkMensagem, camposExtra, descricaoExtra,
  } = opts;
  ...
  const campos = [];
  if (alvoId) {
    campos.push({ name: '👤 Utilizador', value: alvoTag ? `<@${alvoId}> (\`${alvoTag}\`)` : `<@${alvoId}> (\`${alvoId}\`)`, inline: false });
  }
  if (modUser?.id) {
    campos.push({ name: '🛡️ Moderador Responsável', value: `<@${modUser.id}> (\`${modUser.tag || modUser.id}\`)`, inline: false });
  } else {
    ...
    campos.push({ name: '🛡️ Moderador Responsável', value: motivoExecutorDesconhecidoTexto(motivoDesconhecido), inline: false });
  }
  if (canal?.id) {
    campos.push({ name: '📍 Canal', value: `<#${canal.id}> (\`#${canal.name || canal.id}\`)`, inline: false });
  }
  if (duracao) {
    campos.push({ name: '⏱️ Duração', value: duracao, inline: true });
  }
  if (motivo) {
    campos.push({ name: '📝 Motivo', value: motivo, inline: false });
  }
  if (linkMensagem) {
    campos.push({ name: '🔗 Mensagem', value: `[Ir para a mensagem](${linkMensagem})`, inline: false });
  }
  ...
  embed.setFooter({ text: footerParts.length ? footerParts.join(' • ') : 'Discord Bot PT' });
...
const LOG_TYPES = {
  // Moderação
  ban:            { label: '🔨 Bans',                 categoria: 'Moderação' },
  unban:          { label: '✅ Unbans',                categoria: 'Moderação' },
  kick:           { label: '👢 Expulsões (Kick)',      categoria: 'Moderação' },
  timeout:        { label: '🔇 Timeouts / Silêncios',  categoria: 'Moderação' },
  warn:           { label: '⚠️ Avisos (Warn)',         categoria: 'Moderação' },
  blacklist:      { label: '🚫 Blacklist',             categoria: 'Moderação' },
  clear:          { label: '🗑️ Limpeza de Mensagens',  categoria: 'Moderação' },
  // Membros
  member_join:    { label: '📥 Membro Entrou',         categoria: 'Membros' },
  member_leave:   { label: '📤 Membro Saiu',           categoria: 'Membros' },
  member_role:    { label: '🎭 Cargos de Membro',      categoria: 'Membros' },
  member_nick:    { label: '✏️ Apelido Alterado',      categoria: 'Membros' },
  // Servidor
  role_update:    { label: '🎨 Cargos (Criar/Editar/Apagar)', categoria: 'Servidor' },
  channel_update: { label: '📁 Canais (Criar/Editar/Apagar)', categoria: 'Servidor' },
  // Mensagens
  message_sent:   { label: '💬 Mensagens Enviadas',    categoria: 'Mensagens' },
  message_delete: { label: '🗑️ Mensagens Apagadas',    categoria: 'Mensagens' },
  message_edit:   { label: '✏️ Mensagens Editadas',    categoria: 'Mensagens' },
  // Voz
  voice_update:   { label: '🔊 Atividade de Voz',      categoria: 'Voz' },
};
...
function corHexParaButtonStyle(hex) {
  const map = {
    '#5865F2': ButtonStyle.Primary,   // Blurple
    '#4E5058': ButtonStyle.Secondary, // Cinzento
    '#57F287': ButtonStyle.Success,   // Verde
  ...
  const palette = [
    { style: ButtonStyle.Primary,   rgb: [0x58, 0x65, 0xF2] },
    { style: ButtonStyle.Secondary, rgb: [0x4E, 0x50, 0x58] },
    { style: ButtonStyle.Success,   rgb: [0x57, 0xF2, 0x87] },
    { style: ButtonStyle.Danger,    rgb: [0xED, 0x42, 0x45] },
  ];
  let melhor = palette[0], melhorDist = Infinity;
...
function montarComponentesPainelTicket(guildId, ticketConfig) {
  ...
  if (modo === 'buttons') {
    ...
    for (let i = 0; i < tiposLimitados.length; i += 5) {
      ...
      const row = new ActionRowBuilder().addComponents(
        grupo.map(t =>
          new ButtonBuilder()
            .setCustomId(`ticket_create_btn_${t.id}`)
            .setLabel((t.label || 'Ticket').substring(0, 80))
            .setEmoji(t.emoji || '🎫')
            .setStyle(corHexParaButtonStyle(t.color))
        )
      );
      linhas.push(row);
    }
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .addOptions(tipos.slice(0, 25).map(t => ({
      label: t.label,
...
async function criarCanaisDeLogs(guild) {
  ...
  return { logChannelId: canalLogs.id, modLogChannelId: canalModLogs.id };
}
...
function construirEmbedWelcome(config, member) {
  ...
  const autorNome = substituirVariaveisWelcome(config.welcome_author_name || '', member);
  const footer    = substituirVariaveisWelcome(config.welcome_footer || '', member);

  const embed = new EmbedBuilder()
    .setDescription(descricao)
    .setColor(config.welcome_color || CONFIG.COR_SUCESSO)
    .setTimestamp();

  if (titulo) {
    embed.setTitle(titulo);
    if (config.welcome_url) embed.setURL(config.welcome_url);
  }

  if (autorNome) {
    embed.setAuthor({
      name: autorNome,
      iconURL: config.welcome_author_icon || undefined,
    });
  }

  if (footer) embed.setFooter({ text: footer });

  // Posição da imagem: "thumbnail" = imagem pequena à direita do texto,
  // "bottom" = imagem grande em baixo (banner), "none" = sem imagem.
  const pos = config.welcome_image_pos || 'bottom';
  if (pos === 'thumbnail' && (config.welcome_thumbnail || config.welcome_image)) {
    embed.setThumbnail(config.welcome_thumbnail || config.welcome_image);
  } else if (pos === 'bottom' && config.welcome_image) {
    embed.setImage(config.welcome_image);
  }
  // Permite ter as duas ao mesmo tempo se ambos os campos estiverem preenchidos
  if (config.welcome_thumbnail && pos !== 'thumbnail') embed.setThumbnail(config.welcome_thumbnail);
  if (config.welcome_image && pos === 'thumbnail' && config.welcome_thumbnail) embed.setImage(config.welcome_image);
...
async function sendWelcome(member) {
  ...
  if (config.welcome_embed) {
    ...
    await channel.send({ content: conteudoFora || undefined, embeds: [embed] });
  } else {
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('titulo').setDescription('Título da embed ({user}, {server}, {count})').setRequired(false))
    .addStringOption(o => o.setName('imagem').setDescription('URL da imagem/banner da embed').setRequired(false))
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('mensagem').setDescription('Mensagem extra enviada fora da embed (ex: @everyone)').setRequired(false).setMaxLength(1000)),

  // ── Sugestões ──
  new SlashCommandBuilder()
    .setName('sugestao-tipo-criar')
    .setDescription('Cria um novo tipo de sugestão (ex: Sugestão, Sugestão de Construção)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('nome').setDescription('Nome do tipo, ex: Sugestão de Construção').setRequired(true).setMaxLength(80))
    .addChannelOption(o => o.setName('canal').setDescription('Canal onde as sugestões deste tipo são publicadas').setRequired(true))
    .addStringOption(o => o.setName('emoji').setDescription('Emoji do tipo, ex: 🏗️').setRequired(false))
    .addChannelOption(o => o.setName('log').setDescription('Canal de log deste tipo de sugestão').setRequired(false))
    .addRoleOption(o => o.setName('ping').setDescription('Cargo a mencionar em novas sugestões deste tipo').setRequired(false)),

  new SlashCommandBuilder()
    .setName('sugestao-tipo-apagar')
    .setDescription('Apaga um tipo de sugestão (as sugestões já submetidas não são apagadas)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('nome').setDescription('Nome do tipo a apagar').setRequired(true).setAutocomplete(true)),

  new SlashCommandBuilder()
    .setName('sugestao-tipo-lista')
    .setDescription('Lista os tipos de sugestão configurados neste servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

  new SlashCommandBuilder()
    .setName('sugerir')
    .setDescription('Submete uma sugestão')
    .addStringOption(o => o.setName('tipo').setDescription('Tipo de sugestão').setRequired(true).setAutocomplete(true))
  ...
  new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
    .addIntegerOption(o => o.setName('id').setDescription('Número da sugestão (o # mostrado no embed, ex: #3)').setRequired(true))
  ...
  new SlashCommandBuilder()
    .setName('lock')
    .setDescription('Tranca um canal (impede @everyone de enviar mensagens)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal a trancar (padrão: este canal)').setRequired(false)),

  new SlashCommandBuilder()
    .setName('unlock')
    .setDescription('Destranca um canal (volta a permitir @everyone enviar mensagens)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal a destrancar (padrão: este canal)').setRequired(false)),
  ...
  new SlashCommandBuilder()
    .setName('logs-setup')
    .setDescription('Configura o canal de logs (se não escolheres nenhum, cria uma categoria "Logs" automaticamente)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(o => o.setName('canal').setDescription('Canal de logs (deixa vazio para criar automaticamente)').setRequired(false))
    .addChannelOption(o => o.setName('mod-log').setDescription('Canal de logs de moderação (deixa vazio para criar automaticamente)').setRequired(false)),
  ...
  // ── Giveaways ──
  new SlashCommandBuilder()
    .setName('giveaway-criar')
    .setDescription('Cria um sorteio (giveaway)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('premio').setDescription('O que vai ser sorteado').setRequired(true))
    .addStringOption(o => o.setName('duracao').setDescription('Duração (ex: 1m, 10m, 2h, 1d)').setRequired(true))
    .addIntegerOption(o => o.setName('vencedores').setDescription('Número de vencedores').setRequired(false).setMinValue(1).setMaxValue(20))
    .addChannelOption(o => o.setName('canal').setDescription('Canal onde publicar (padrão: este canal)').setRequired(false)
      .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
    .addStringOption(o => o.setName('titulo').setDescription('Título da embed (padrão: 🎉 SORTEIO 🎉)').setRequired(false))
    .addStringOption(o => o.setName('descricao').setDescription('Descrição/texto extra da embed').setRequired(false))
    .addStringOption(o => o.setName('imagem').setDescription('URL de uma imagem para a embed').setRequired(false))
    .addStringOption(o => o.setName('mensagem').setDescription('Mensagem fora da embed (ex: @everyone, @cargo) para marcar pessoas').setRequired(false)),

  new SlashCommandBuilder()
    .setName('giveaway-terminar')
    .setDescription('Termina um sorteio imediatamente e sorteia o(s) vencedor(es)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),

  new SlashCommandBuilder()
    .setName('giveaway-reroll')
    .setDescription('Sorteia novo(s) vencedor(es) para um giveaway já terminado')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),

  new SlashCommandBuilder()
    .setName('giveaway-cancelar')
    .setDescription('Cancela um sorteio sem sortear vencedores')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),

  new SlashCommandBuilder()
    .setName('giveaway-lista')
    .setDescription('Lista os sorteios ativos e recentes do servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

  // ── Painéis de Informação ──
  new SlashCommandBuilder()
    .setName('painel-criar')
    .setDescription('Cria um painel de informação em rascunho (usa /painel-publicar depois de adicionares os botões)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('nome').setDescription('Nome interno para identificar o painel (não aparece no Discord)').setRequired(true))
    .addStringOption(o => o.setName('titulo').setDescription('Título do embed').setRequired(false))
    .addStringOption(o => o.setName('descricao').setDescription('Descrição/texto principal').setRequired(false))
    .addStringOption(o => o.setName('banner').setDescription('URL da imagem grande (banner)').setRequired(false))
    .addStringOption(o => o.setName('thumbnail').setDescription('URL da imagem pequena (thumbnail)').setRequired(false))
    .addStringOption(o => o.setName('cor').setDescription('Cor em hex, ex: #5865F2').setRequired(false))
    .addStringOption(o => o.setName('dono').setDescription('Texto do campo Dono, ex: @Léo').setRequired(false))
    .addStringOption(o => o.setName('fundado').setDescription('Texto do campo Fundado, ex: há 7 anos').setRequired(false))
    .addStringOption(o => o.setName('rodape').setDescription('Texto do rodapé (footer)').setRequired(false))
    .addChannelOption(o => o.setName('canal').setDescription('Canal onde publicar (padrão: este canal)').setRequired(false)),

  new SlashCommandBuilder()
    .setName('painel-botao-add')
    .setDescription('Adiciona um botão a um painel de informação já criado')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('painel').setDescription('Nome do painel (usa /painel-lista para veres os nomes)').setRequired(true))
    .addStringOption(o => o.setName('label').setDescription('Texto do botão, ex: Regras').setRequired(true))
    .addStringOption(o => o.setName('resposta').setDescription('Texto mostrado só a quem clicar no botão').setRequired(true))
    .addStringOption(o => o.setName('emoji').setDescription('Emoji do botão, ex: 📜').setRequired(false))
    .addStringOption(o => o.setName('estilo').setDescription('Estilo/cor do botão').setRequired(false)
      .addChoices(
    ...
    .addStringOption(o => o.setName('resposta-titulo').setDescription('Título opcional para a resposta (se quiseres embed)').setRequired(false))
    .addStringOption(o => o.setName('resposta-imagem').setDescription('URL de imagem grande na resposta').setRequired(false))
    .addStringOption(o => o.setName('resposta-thumbnail').setDescription('URL de imagem pequena na resposta').setRequired(false))
    .addStringOption(o => o.setName('resposta-cor').setDescription('Cor hex da resposta, ex: #5865F2').setRequired(false)),

  new SlashCommandBuilder()
    .setName('painel-publicar')
    .setDescription('Publica (ou republica) um painel de informação já criado, com todos os botões de uma vez')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('painel').setDescription('Nome do painel a publicar').setRequired(true)),

  new SlashCommandBuilder()
    .setName('painel-lista')
    .setDescription('Lista os painéis de informação configurados neste servidor')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

  new SlashCommandBuilder()
    .setName('painel-apagar')
    .setDescription('Apaga um painel de informação (e os seus botões)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('painel').setDescription('Nome do painel a apagar').setRequired(true)),
...
async function sincronizarComandosEmbed(guildId) {
  try {
    const embedsComComando = db.prepare(
      'SELECT id, name, trigger_command FROM saved_embeds WHERE guild_id = ? AND trigger_command IS NOT NULL'
    ).all(guildId);

    const comandosEmbed = embedsComComando.map(e =>
      new SlashCommandBuilder()
        .setName(e.trigger_command)
        .setDescription(`Publica a embed guardada "${e.name}"`)
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .addChannelOption(o => o.setName('canal').setDescription('Canal onde publicar (padrão: este canal)').setRequired(false))
        .toJSON()
    );
...
async function sincronizarComandosEmbedTodosServidores() {
  try {
    const guildIds = db.prepare(
      'SELECT DISTINCT guild_id FROM saved_embeds WHERE trigger_command IS NOT NULL'
...
async function tratarComandoSlashEmbed(interaction) {
  ...
  const saved = db.prepare(
    'SELECT * FROM saved_embeds WHERE guild_id = ? AND trigger_command = ?'
  ...
  if (!interaction.member?.permissions?.has(PermissionFlagsBits.Administrator)) {
    await interaction.reply({ content: '❌ Só administradores podem usar este comando.', ephemeral: true });
    return true;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(data.color || CONFIG.COR_PRINCIPAL)
    .setTimestamp();
  ...
  return interaction.editReply({ content: `✅ Embed enviada em ${canal}!` }), true;
}
...
async function handleAutocomplete(interaction) {
  ...
  if (commandName === 'sugerir' && focused.name === 'tipo') {
    const tipos = db.prepare('SELECT name FROM suggestion_types WHERE guild_id = ? AND enabled = 1 ORDER BY order_num, id').all(guild.id);
  ...
  if (commandName === 'sugestao-tipo-apagar' && focused.name === 'nome') {
    const tipos = db.prepare('SELECT name FROM suggestion_types WHERE guild_id = ? ORDER BY order_num, id').all(guild.id);
    const filtrados = tipos
      .filter(t => t.name.toLowerCase().includes((focused.value || '').toLowerCase()))
      .slice(0, 25)
      .map(t => ({ name: t.name, value: t.name }));
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'ticket-painel') {
    ...
    const embed = new EmbedBuilder()
      .setTitle(titulo)
      .setDescription(descricao)
      .setColor(ticketConfig.panel_color || CONFIG.COR_PRINCIPAL)
      .setTimestamp();
  ...
  if (commandName === 'sugestao-tipo-criar') {
    ...
    const existente = db.prepare('SELECT id FROM suggestion_types WHERE guild_id = ? AND name = ?').get(guild.id, nome);
    if (existente) return interaction.reply({ content: `❌ Já existe um tipo de sugestão chamado **${nome}**.`, ephemeral: true });

    const { c: total } = db.prepare('SELECT COUNT(*) AS c FROM suggestion_types WHERE guild_id = ?').get(guild.id);
    db.prepare(`
  ...
  if (commandName === 'sugestao-tipo-apagar') {
    const nome = options.getString('nome').trim();
    const tipo = db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? AND name = ?').get(guild.id, nome);
    if (!tipo) return interaction.reply({ content: `❌ Não encontrei nenhum tipo de sugestão chamado **${nome}**.`, ephemeral: true });

    db.prepare('DELETE FROM suggestion_types WHERE id = ?').run(tipo.id);
    // As sugestões já submetidas ficam com type_id "órfão" mas continuam intactas no histórico
    return interaction.reply({ content: `✅ Tipo de sugestão **${nome}** apagado.`, ephemeral: true });
  }

  if (commandName === 'sugestao-tipo-lista') {
    const tipos = db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? ORDER BY order_num, id').all(guild.id);
    if (!tipos.length) return interaction.reply({ content: 'ℹ️ Ainda não há nenhum tipo de sugestão configurado. Usa `/sugestao-tipo-criar` para criares o primeiro.', ephemeral: true });

    const embed = new EmbedBuilder()
      .setTitle('📋 Tipos de Sugestão')
      .setColor(CONFIG.COR_PRINCIPAL)
      .setDescription(tipos.map(t =>
        `${t.emoji || '💡'} **${t.name}** ${t.enabled ? '' : '*(desativado)*'} — <#${t.channel_id}>` +
        (t.log_channel ? ` · log: <#${t.log_channel}>` : '') +
        (t.ping_role ? ` · ping: <@&${t.ping_role}>` : '')
      ).join('\n'));
  ...
  if (commandName === 'sugerir') {
    ...
    const tipo = db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? AND name = ? AND enabled = 1').get(guild.id, nomeTipo);
    if (!tipo) return interaction.reply({ content: `❌ Tipo de sugestão **${nomeTipo}** não encontrado ou desativado. Usa \`/sugestao-tipo-lista\` para veres os tipos disponíveis.`, ephemeral: true });

    const conteudo = options.getString('sugestao');
    const canal    = guild.channels.cache.get(tipo.channel_id);
    if (!canal) return interaction.reply({ content: '❌ Canal deste tipo de sugestão não encontrado.', ephemeral: true });

    // Número da sugestão específico deste servidor (não é partilhado com outros servidores)
    const { c: totalNesteServidor } = db.prepare('SELECT COUNT(*) as c FROM suggestions WHERE guild_id = ?').get(guild.id);
    ...
    `);
    const info = stmt.run(guild.id, canal.id, user.id, conteudo, guildSeq, tipo.id);
    const sugId = info.lastInsertRowid;

    const embed = new EmbedBuilder()
      .setTitle(`${tipo.emoji || '💡'} ${tipo.name} #${guildSeq}`)
    ...
    const msg = await canal.send({ content, embeds: [embed], components: [row], allowedMentions: { roles: tipo.ping_role ? [tipo.ping_role] : [] } });

    db.prepare('UPDATE suggestions SET message_id = ? WHERE id = ?').run(msg.id, sugId);

    return interaction.reply({ content: `✅ ${tipo.emoji || '💡'} ${tipo.name} #${guildSeq} submetida com sucesso!`, ephemeral: true });
  }
  ...
  if (commandName === 'sugestao-responder') {
    ...
    const sug = db.prepare('SELECT * FROM suggestions WHERE guild_seq = ? AND guild_id = ?').get(id, guild.id);
    ...
    db.prepare('UPDATE suggestions SET status = ?, mod_response = ? WHERE id = ?').run(acao, resposta, sug.id);
    ...
    if (sug.type_id) {
      const tipo = db.prepare('SELECT * FROM suggestion_types WHERE id = ?').get(sug.type_id);
      if (tipo?.log_channel) {
        const canalLog = guild.channels.cache.get(tipo.log_channel);
        if (canalLog) {
          const logEmbed = new EmbedBuilder()
            .setTitle(`${tipo.emoji || '💡'} ${tipo.name} #${id} — ${s.label}`)
            .setDescription(sug.content)
            .addFields({ name: '💬 Resposta', value: resposta })
            .setColor(s.color)
            .setFooter({ text: `Respondido por ${user.tag}` })
            .setTimestamp();
  ...
  if (commandName === 'ban') {
    ...
    const embed = embedLogModeracao({
      ...
      camposExtra: [{ name: '🗑️ Mensagens Apagadas', value: `Últimos ${dias} dia(s)`, inline: true }],
    });
  ...
  if (commandName === 'blacklist-add') {
    ...
    const embed = embedLogModeracao({
      ...
      motivo,
      canal: interaction.channel,
      camposExtra: [{ name: 'ℹ️ Username na Blacklist', value: `\`${usernameInput}\``, inline: false }],
  ...
  if (commandName === 'warn') {
    ...
    const embed = embedLogModeracao({
      ...
      motivo,
      canal: interaction.channel,
      camposExtra: [{ name: '📊 Total de Avisos', value: `${total}`, inline: true }],
    });
  ...
  if (commandName === 'limpar') {
    ...
    const embed = embedLogModeracao({
      ...
      moderador: user,
      canal: interaction.channel,
      camposExtra: [{ name: '🔢 Quantidade', value: `${apagadas.size} mensagem(ns)`, inline: true }],
    });
  ...
  if (commandName === 'painel-criar') {
    ...
    if (cor && !/^#([0-9A-Fa-f]{6})$/.test(cor)) {
      return interaction.reply({ content: '❌ Cor inválida. Usa o formato hex, ex: `#5865F2`.', ephemeral: true });
    }

    const existente = db.prepare('SELECT id FROM info_panels WHERE guild_id = ? AND name = ?').get(guild.id, nome);
    if (existente) {
      return interaction.reply({ content: `❌ Já existe um painel chamado **${nome}** neste servidor. Escolhe outro nome ou apaga-o primeiro com /painel-apagar.`, ephemeral: true });
    }
    ...
    `).run(
      guild.id, nome,
      options.getString('titulo') || null,
      options.getString('descricao') || null,
      cor,
      options.getString('banner') || null,
      options.getString('thumbnail') || null,
      options.getString('rodape') || null,
      options.getString('dono') || null,
      options.getString('fundado') || null,
      user.id,
    );
    ...
    db.prepare('UPDATE info_panels SET channel_id = ? WHERE id = ?').run(canal.id, info.lastInsertRowid);
  ...
  if (commandName === 'painel-publicar') {
    ...
    const panel = db.prepare('SELECT * FROM info_panels WHERE guild_id = ? AND name = ?').get(guild.id, nome);
    if (!panel) return interaction.reply({ content: `❌ Não encontrei nenhum painel chamado **${nome}**. Usa \`/painel-lista\` para veres os nomes disponíveis.`, ephemeral: true });
    if (!panel.channel_id) return interaction.reply({ content: '❌ Este painel não tem canal definido.', ephemeral: true });

    const canal = guild.channels.cache.get(panel.channel_id);
    if (!canal) return interaction.reply({ content: '❌ O canal deste painel já não existe.', ephemeral: true });

    const embed = embedInfoPanel(panel);
    const botoes = db.prepare('SELECT * FROM info_panel_buttons WHERE panel_id = ? ORDER BY order_num, id').all(panel.id);
    ...
    const msg = await canal.send({ embeds: [embed], components: rows }).catch(() => null);
    if (!msg) {
      return interaction.reply({ content: '❌ Não foi possível publicar o painel nesse canal (verifica as permissões do bot).', ephemeral: true });
    }
    db.prepare('UPDATE info_panels SET message_id = ?, published = 1 WHERE id = ?').run(msg.id, panel.id);

    return interaction.reply({ content: `✅ Painel **${nome}** publicado em <#${canal.id}>!`, ephemeral: true });
  }
  ...
  if (commandName === 'painel-botao-add') {
    ...
    const contagem = db.prepare('SELECT COUNT(*) AS c FROM info_panel_buttons WHERE panel_id = ?').get(panel.id).c;
    if (contagem >= 25) {
      return interaction.reply({ content: '❌ Este painel já tem o máximo de 25 botões.', ephemeral: true });
    }
    ...
    if (respCor && !/^#([0-9A-Fa-f]{6})$/.test(respCor)) {
      return interaction.reply({ content: '❌ Cor da resposta inválida. Usa o formato hex, ex: `#5865F2`.', ephemeral: true });
    }
    ...
    if (panel.published && panel.message_id && panel.channel_id) {
      ...
      if (msg) {
        const botoes = db.prepare('SELECT * FROM info_panel_buttons WHERE panel_id = ? ORDER BY order_num, id').all(panel.id);
        const rows = botoesInfoPanel(panel.id, botoes);
        await msg.edit({ embeds: [embedInfoPanel(panel)], components: rows }).catch(() => {});
      }
    }

    return interaction.reply({ content: `✅ Botão **${label}** adicionado ao painel **${nome}**! (id do botão: ${info.lastInsertRowid})`, ephemeral: true });
  }

  if (commandName === 'painel-lista') {
    const paineis = db.prepare('SELECT * FROM info_panels WHERE guild_id = ? ORDER BY created_at DESC').all(guild.id);
    if (!paineis.length) return interaction.reply({ content: 'ℹ️ Ainda não há nenhum painel de informação configurado. Usa `/painel-criar` para criares o primeiro.', ephemeral: true });

    const embed = new EmbedBuilder()
      .setTitle('📋 Painéis de Informação')
      .setColor(CONFIG.COR_PRINCIPAL)
      .setDescription(paineis.map(p => {
        const nBotoes = db.prepare('SELECT COUNT(*) AS c FROM info_panel_buttons WHERE panel_id = ?').get(p.id).c;
  ...
  if (commandName === 'painel-apagar') {
    const nome = options.getString('painel').trim();
    const panel = db.prepare('SELECT * FROM info_panels WHERE guild_id = ? AND name = ?').get(guild.id, nome);
    if (!panel) return interaction.reply({ content: `❌ Não encontrei nenhum painel chamado **${nome}**.`, ephemeral: true });
    ...
    db.prepare('DELETE FROM info_panel_buttons WHERE panel_id = ?').run(panel.id);
    db.prepare('DELETE FROM info_panels WHERE id = ?').run(panel.id);

    return interaction.reply({ content: `✅ Painel **${nome}** apagado.`, ephemeral: true });
  }
  ...
  if (commandName === 'giveaway-criar') {
    ...
    if (imagem && !/^https?:\/\/.+\.(png|jpe?g|gif|webp)(\?.*)?$/i.test(imagem)) {
      return interaction.reply({ content: '❌ URL de imagem inválido. Tem de ser um link direto (png, jpg, gif ou webp).', ephemeral: true });
    }

    const duracaoMs = parseDuracao(duracaoStr);
    if (!duracaoMs || duracaoMs < 60000) {
      return interaction.reply({ content: '❌ Duração inválida. Usa um formato como `1m`, `10m`, `2h` ou `1d` (mínimo 1 minuto).', ephemeral: true });
    }
    ...
    const gw = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(info.lastInsertRowid);
    ...
    if (!msg) {
      db.prepare('DELETE FROM giveaways WHERE id = ?').run(gw.id);
      return interaction.reply({ content: '❌ Não foi possível publicar o giveaway nesse canal.', ephemeral: true });
    }

    db.prepare('UPDATE giveaways SET message_id = ? WHERE id = ?').run(msg.id, gw.id);
    agendarEncerramentoGiveaway(gw);

    return interaction.reply({ content: `✅ Giveaway **#${gw.id}** criado em <#${canal.id}>! Termina <t:${Math.floor(endsAt.getTime()/1000)}:R>.`, ephemeral: true });
  }

  if (commandName === 'giveaway-terminar') {
    const id = parseInt(options.getString('id'));
    const gw = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!gw) return interaction.reply({ content: '❌ Giveaway não encontrado.', ephemeral: true });
    if (gw.ended) return interaction.reply({ content: '❌ Este giveaway já terminou. Usa `/giveaway-reroll` para sortear de novo.', ephemeral: true });

    const vencedores = await encerrarGiveaway(guild, gw);
    if (vencedores === null) {
      return interaction.reply({ content: '❌ Não foi possível encerrar o giveaway (canal ou mensagem indisponível). Tenta novamente em instantes.', ephemeral: true });
    }
    cancelarTimerGiveaway(gw.id);
    return interaction.reply({
      content: vencedores.length
        ? `✅ Giveaway **#${gw.id}** encerrado! Vencedor(es): ${vencedores.map(v => `<@${v}>`).join(', ')}`
        : `✅ Giveaway **#${gw.id}** encerrado sem participantes.`,
      ephemeral: true,
    });
  }
  ...
  if (commandName === 'giveaway-reroll') {
    ...
    const gw = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!gw) return interaction.reply({ content: '❌ Giveaway não encontrado.', ephemeral: true });
    if (!gw.ended) return interaction.reply({ content: '❌ Este giveaway ainda está ativo. Usa `/giveaway-terminar` primeiro.', ephemeral: true });
    ...
    if (!vencedores.length) {
      return interaction.reply({ content: '❌ Não há participantes para sortear.', ephemeral: true });
    }

    if (canal) {
      await canal.send({ content: `🔁 **Reroll do giveaway #${gw.id}** (${gw.premio})\n🎉 Novo(s) vencedor(es): ${vencedores.map(v => `<@${v}>`).join(', ')}` }).catch(() => {});
    }

    return interaction.reply({ content: `✅ Reroll feito! Novo(s) vencedor(es): ${vencedores.map(v => `<@${v}>`).join(', ')}`, ephemeral: true });
  }
  ...
  if (commandName === 'giveaway-cancelar') {
    ...
    if (gw.message_id) {
      ...
      if (msg) {
        const total = db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(gw.id).c;
        await msg.edit({
    ...
    db.prepare('DELETE FROM giveaways WHERE id = ?').run(gw.id);
    db.prepare('DELETE FROM giveaway_entries WHERE giveaway_id = ?').run(gw.id);
    cancelarTimerGiveaway(gw.id);

    return interaction.reply({ content: `✅ Giveaway **#${gw.id}** cancelado.`, ephemeral: true });
  }

  if (commandName === 'giveaway-lista') {
    const giveaways = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? ORDER BY ended ASC, created_at DESC LIMIT 15').all(guild.id);
    if (!giveaways.length) {
      return interaction.reply({ content: '📭 Ainda não há giveaways neste servidor.', ephemeral: true });
    }

    const linhas = giveaways.map(gw => {
      const total = db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(gw.id).c;
    ...
    const embed = new EmbedBuilder()
      .setTitle('🎉 Giveaways do Servidor')
      .setDescription(linhas.join('\n'))
      .setColor(CONFIG.COR_PRINCIPAL)
      .setFooter({ text: 'Usa o ID para /giveaway-terminar, /giveaway-reroll ou /giveaway-cancelar' });
  ...
  if (commandName === 'lock') {
    ...
    if (!canal || !canal.permissionOverwrites) {
      return interaction.reply({ content: '❌ Não foi possível trancar este canal.', ephemeral: true });
    }
    ...
    try {
      ...
      return interaction.reply({ content: `❌ Não foi possível trancar o canal: ${e.message}`, ephemeral: true });
    }
  }
  ...
  if (commandName === 'unlock') {
    ...
    if (!canal || !canal.permissionOverwrites) {
      return interaction.reply({ content: '❌ Não foi possível destrancar este canal.', ephemeral: true });
    }
    ...
    try {
      ...
      return interaction.reply({ content: `❌ Não foi possível destrancar o canal: ${e.message}`, ephemeral: true });
    }
  }
  ...
  if (commandName === 'logs-setup') {
    ...
    if (!canal && !modLog) {
      ...
      try {
        ...
        return interaction.editReply({ content: `❌ Não foi possível criar os canais de logs: ${e.message}` });
      }
    }

    if (!canal) {
      return interaction.reply({ content: '❌ Escolhe pelo menos o canal de logs, ou não escolhas nenhum para eu criar automaticamente.', ephemeral: true });
    }
...
async function handleButton(interaction) {
  ...
  if (customId.startsWith('infopanel_btn_')) {
    ...
    const botao = db.prepare('SELECT * FROM info_panel_buttons WHERE id = ?').get(btnId);
    if (!botao) {
      return interaction.reply({ content: '❌ Este botão já não está configurado.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: botao.response_text, ephemeral: true });
  }
  ...
  if (customId.startsWith('giveaway_join_')) {
    ...
    const gw = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(giveawayId, guild.id);

    if (!gw || gw.ended) {
      return interaction.reply({ content: '❌ Este sorteio já não está ativo.', ephemeral: true });
    }

    const jaEntrou = db.prepare('SELECT 1 FROM giveaway_entries WHERE giveaway_id = ? AND user_id = ?').get(giveawayId, user.id);
    if (jaEntrou) {
      db.prepare('DELETE FROM giveaway_entries WHERE giveaway_id = ? AND user_id = ?').run(giveawayId, user.id);
      const totalAtual = db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(giveawayId).c;
      await atualizarBotaoGiveaway(guild, gw, totalAtual).catch(() => {});
      return interaction.reply({ content: '🚫 Saíste do sorteio.', ephemeral: true });
    }

    db.prepare('INSERT INTO giveaway_entries (giveaway_id, user_id) VALUES (?, ?)').run(giveawayId, user.id);
    const total = db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(giveawayId).c;
    await atualizarBotaoGiveaway(guild, gw, total).catch(() => {});
    return interaction.reply({ content: `🎉 Estás a participar no sorteio de **${gw.premio}**! Boa sorte!`, ephemeral: true });
  }
...
async function aplicarIdentidadeGlobalDoBot() {
  try {
    db.exec(`CREATE TABLE IF NOT EXISTS bot_identity (
      chave TEXT PRIMARY KEY,
      valor TEXT
    )`);
    const ultimoNome   = db.prepare('SELECT valor FROM bot_identity WHERE chave = ?').get('bot_name')?.valor;
    const ultimoAvatar = db.prepare('SELECT valor FROM bot_identity WHERE chave = ?').get('bot_avatar')?.valor;
    ...
    if (CONFIG.BOT_NAME && client.user.username !== CONFIG.BOT_NAME && ultimoNome !== CONFIG.BOT_NAME) {
      try {
        ...
        db.prepare('INSERT INTO bot_identity (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO UPDATE SET valor=excluded.valor').run('bot_name', CONFIG.BOT_NAME);
    ...
    if (CONFIG.BOT_AVATAR_URL && ultimoAvatar !== CONFIG.BOT_AVATAR_URL) {
      try {
        ...
        db.prepare('INSERT INTO bot_identity (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO UPDATE SET valor=excluded.valor').run('bot_avatar', CONFIG.BOT_AVATAR_URL);
...
client.on(Events.GuildCreate, async guild => {
  try {
    ...
    const linkRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setLabel('🌐 Abrir Dashboard').setStyle(ButtonStyle.Link).setURL(DASHBOARD_URL)
    );

    await canal.send({ embeds: [embed], components: [linkRow] });
  } catch (err) {
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  try {
    ...
    if (cargosGanhos.size > 0) {
      const { executor, motivo } = await obterExecutorAuditLog(newMember.guild, 25 /* AuditLogEvent.MemberRoleUpdate */, newMember.id);
      const embed = embedLogModeracao({
        titulo: '➕ Cargo(s) Atribuído(s)',
        cor: CONFIG.COR_SUCESSO,
        alvo: newMember,
        moderador: executor, motivoDesconhecido: motivo,
        camposExtra: [{ name: '🎭 Cargo(s)', value: cargosGanhos.map(r => `<@&${r.id}>`).join(', '), inline: false }],
      });
    ...
    if (cargosPerdidos.size > 0) {
      ...
      const embed = embedLogModeracao({
        ...
        alvo: newMember,
        moderador: executor, motivoDesconhecido: motivo,
        camposExtra: [{ name: '🎭 Cargo(s)', value: cargosPerdidos.map(r => `<@&${r.id}>`).join(', '), inline: false }],
      });
      await sendLogTyped(newMember.guild, 'member_role', embed);
    }
    ...
    if (oldMember.nickname !== newMember.nickname) {
      const { executor, motivo } = await obterExecutorAuditLog(newMember.guild, 24 /* AuditLogEvent.MemberUpdate */, newMember.id);
      const embed = embedLogModeracao({
...
client.on(Events.GuildRoleCreate, async role => {
  const { executor, motivo } = await obterExecutorAuditLog(role.guild, 30 /* AuditLogEvent.RoleCreate */, role.id); // 30
  const embed = embedLogModeracao({
    titulo: '🆕 Cargo Criado',
    cor: CONFIG.COR_SUCESSO,
    moderador: executor, motivoDesconhecido: motivo,
    camposExtra: [
      { name: '🎭 Cargo', value: `<@&${role.id}> (\`${role.name}\`)`, inline: false },
...
client.on(Events.GuildRoleDelete, async role => {
  const { executor, motivo } = await obterExecutorAuditLog(role.guild, 32 /* AuditLogEvent.RoleDelete */, role.id);
  const embed = embedLogModeracao({
...
client.on(Events.GuildRoleUpdate, async (oldRole, newRole) => {
  if (oldRole.name === newRole.name && oldRole.hexColor === newRole.hexColor &&
      oldRole.permissions.bitfield === newRole.permissions.bitfield) return;
  const { executor, motivo } = await obterExecutorAuditLog(newRole.guild, 31 /* AuditLogEvent.RoleUpdate */, newRole.id);
  const embed = embedLogModeracao({
    titulo: '✏️ Cargo Editado',
    cor: CONFIG.COR_AVISO,
    moderador: executor, motivoDesconhecido: motivo,
    camposExtra: [
      { name: '🎭 Cargo', value: `<@&${newRole.id}>`, inline: false },
      { name: '📛 Nome', value: `${oldRole.name} → ${newRole.name}`, inline: false },
      { name: '🎨 Cor', value: `${oldRole.hexColor} → ${newRole.hexColor}`, inline: true },
      { name: '🆔 ID do Cargo', value: `\`${newRole.id}\``, inline: true },
    ],
  });
...
client.on(Events.ChannelCreate, async channel => {
  ...
  const { executor, motivo } = await obterExecutorAuditLog(channel.guild, 10 /* AuditLogEvent.ChannelCreate */, channel.id);
  const embed = embedLogModeracao({
    titulo: '🆕 Canal Criado',
    cor: CONFIG.COR_SUCESSO,
    moderador: executor, motivoDesconhecido: motivo,
    canal: channel,
    camposExtra: [{ name: '🗂️ Tipo', value: `\`${channel.type}\``, inline: true }],
  });
...
client.on(Events.ChannelDelete, async channel => {
  if (!channel.guild) return;
  const { executor, motivo } = await obterExecutorAuditLog(channel.guild, 12 /* AuditLogEvent.ChannelDelete */, channel.id);
  const embed = embedLogModeracao({
    titulo: '🗑️ Canal Eliminado',
    cor: CONFIG.COR_ERRO,
    moderador: executor, motivoDesconhecido: motivo,
    camposExtra: [
      { name: '📛 Nome do Canal', value: `\`#${channel.name}\``, inline: false },
...
client.on(Events.ChannelUpdate, async (oldChannel, newChannel) => {
  ...
  const { executor, motivo } = await obterExecutorAuditLog(newChannel.guild, 11 /* AuditLogEvent.ChannelUpdate */, newChannel.id);
  const embed = embedLogModeracao({
    titulo: '✏️ Canal Editado',
    cor: CONFIG.COR_AVISO,
    moderador: executor, motivoDesconhecido: motivo,
    canal: newChannel,
    camposExtra: [{ name: '📛 Nome', value: `${oldChannel.name} → ${newChannel.name}`, inline: false }],
  });
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
  ...
  if (oldState.serverMute !== newState.serverMute) {
    const embed = embedPadrao(
      newState.serverMute ? '🔇 Silenciado no Servidor (Voz)' : '🔊 Dessilenciado no Servidor (Voz)',
      `**Utilizador:** ${member.user.tag} (\`${member.id}\`)`,
      newState.serverMute ? CONFIG.COR_ERRO : CONFIG.COR_SUCESSO
    );
    await sendLogTyped(guild, 'voice_update', embed);
  }
  if (oldState.serverDeaf !== newState.serverDeaf) {
    const embed = embedPadrao(
      newState.serverDeaf ? '🔇 Ensurdecido no Servidor (Voz)' : '🔊 Voz Restaurada no Servidor',
      `**Utilizador:** ${member.user.tag} (\`${member.id}\`)`,
      newState.serverDeaf ? CONFIG.COR_ERRO : CONFIG.COR_SUCESSO
    );
    await sendLogTyped(guild, 'voice_update', embed);
  }
});
...
client.on(Events.GuildMemberRemove, async member => {
  ...
  const embed = embedLogModeracao({
    ...
    alvo: member,
    camposExtra: [
      { name: '📅 Estava no Servidor Desde', value: member.joinedTimestamp ? `<t:${Math.floor(member.joinedTimestamp/1000)}:R>` : '*Desconhecido*', inline: false },
...
client.on(Events.MessageCreate, async message => {
  ...
  const embedMsg = embedPadrao(
    '💬 Mensagem Enviada',
    `**Autor:** ${message.author.tag} (\`${message.author.id}\`)\n**Canal:** <#${message.channel.id}>\n**Conteúdo:**\n${message.content?.substring(0, 1000) || '*Sem conteúdo de texto (anexo/embed)*'}`,
    CONFIG.COR_PRINCIPAL
  ).addFields({ name: '🔗 Link', value: `[Ver mensagem](${message.url})` });
  ...
  if (message.content.startsWith(CONFIG.PREFIX)) {
    ...
    if (cmd === 'lock' || cmd === 'unlock') {
      if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) {
        return message.reply({ content: '❌ Não tens permissão para usar este comando (é necessário ser Administrador).' }).catch(() => {});
      }

      const canal = message.mentions.channels.first() || message.channel;
      if (!canal || !canal.permissionOverwrites) {
        return message.reply({ content: '❌ Não foi possível alterar este canal.' }).catch(() => {});
      }

      try {
        if (cmd === 'lock') {
          await canal.permissionOverwrites.edit(message.guild.roles.everyone, { SendMessages: false });
          const embed = embedPadrao(
            '🔒 Canal Trancado',
          ...
          await canal.permissionOverwrites.edit(message.guild.roles.everyone, { SendMessages: null });
          const embed = embedPadrao(
            '🔓 Canal Destrancado',
            `**Canal:** <#${canal.id}>\n**Moderador:** <@${message.author.id}>\n\nO envio de mensagens para @everyone foi restaurado.\n\nPara bloquear o canal use \`/lock\` ou \`!lock\`.`,
            CONFIG.COR_SUCESSO
        ...
        await message.reply({ content: `❌ Erro: ${e.message}` }).catch(() => {});
      }
      return;
    }
  }
...
function embedInfoPanel(panel) {
  ...
  const campos = [];
  if (panel.founded_text) campos.push({ name: '📅 Fundado', value: panel.founded_text, inline: true });
  if (panel.owner_text) campos.push({ name: '👑 Dono', value: panel.owner_text, inline: true });

  let extra = [];
  try { extra = JSON.parse(panel.extra_fields || '[]'); } catch (_) {}
  for (const f of extra) {
    if (f && f.name && f.value) campos.push({ name: f.name, value: f.value, inline: !!f.inline });
  }
  if (campos.length) embed.addFields(campos);
...
function botoesInfoPanel(panelId, botoes) {
  ...
  botoes.forEach((b, i) => {
    ...
    const btn = new ButtonBuilder()
      .setCustomId(`infopanel_btn_${b.id}`)
...
function embedGiveaway(gw, totalEntradas, encerrado, vencedoresTexto) {
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: encerrado ? '🔴 Sorteio Encerrado' : '🟢 Sorteio Ativo' })
    .setTitle(`🎉 ${tituloBase} 🎉`)
    // Cor muda consoante o estado: dourado enquanto ativo, vermelho quando termina
    .setColor(encerrado ? CONFIG.COR_ERRO : 0xFFD700)
    .setDescription(
      (gw.descricao ? `*${gw.descricao}*\n\n` : '') +
      `${'▬'.repeat(20)}\n` +
      (encerrado
        ? (vencedoresTexto
            ? `## 🏆 Vencedor(es)\n${vencedoresTexto}\n\n*Parabéns! Contacta o organizador para receberes o teu prémio.*`
            : `## 😔 Sem vencedor\nNinguém participou neste sorteio.`)
        : `Clica no botão 🎉 abaixo para participares!\n\nBoa sorte a todos! 🍀`)
    )
    .addFields(
      { name: '🎁 Prémio', value: `**${gw.premio}**`, inline: true },
      { name: '👥 Vencedores', value: `**${gw.vencedores}**`, inline: true },
      { name: '🙋 Participantes', value: `**${totalEntradas}**`, inline: true },
      {
        name: '⏰ Termina',
        value: encerrado
          ? '`Encerrado`'
          : `<t:${Math.floor(new Date(gw.ends_at + 'Z').getTime() / 1000)}:R> (<t:${Math.floor(new Date(gw.ends_at + 'Z').getTime() / 1000)}:f>)`,
        inline: false,
      },
    )
    .setFooter({ text: `Giveaway #${gw.id}` })
    .setTimestamp();
...
function botaoGiveaway(gw, totalEntradas, desativado = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`giveaway_join_${gw.id}`)
      .setLabel(desativado ? 'Sorteio Encerrado' : `🎉 Participar (${totalEntradas})`)
...
function sortearVencedores(giveawayId, quantidade) {
  const entradas = db.prepare('SELECT user_id FROM giveaway_entries WHERE giveaway_id = ?').all(giveawayId).map(e => e.user_id);
...
async function encerrarGiveaway(guild, gw) {
  ...
  const canal = guild.channels.cache.get(gw.channel_id)
    || await guild.channels.fetch(gw.channel_id).catch(() => null);
  ...
  if (vencedores.length) {
    ...
    }).catch(() => { anuncioEnviado = false; });
  } else {
    await canal.send({ content: `😔 O sorteio de **${gw.premio}** terminou sem participantes.` }).catch(() => { anuncioEnviado = false; });
  }
  ...
  if (mensagemEditada && anuncioEnviado) {
    db.prepare('UPDATE giveaways SET ended = 1 WHERE id = ?').run(gw.id);
  } else {
...
async function verificarGiveaways() {
  const agora = new Date();
  const ativos = db.prepare('SELECT * FROM giveaways WHERE ended = 0').all();
...
function agendarEncerramentoGiveaway(gw) {
  ...
  const executar = async () => {
    ...
    const atual = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(gw.id);
...
function agendarGiveawaysExistentes() {
  const ativos = db.prepare('SELECT * FROM giveaways WHERE ended = 0').all();
  for (const gw of ativos) {
    agendarEncerramentoGiveaway(gw);
  }
...
app.get('/api/:guildId/section-html/:sectionId', requireGuildAdminApi, async (req, res) => {
  const { guildId, sectionId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(sectionId)) {
    return res.status(400).json({ ok: false, message: 'Id de secção inválido.' });
  }
  ...
  try {
    ...
    if (!match) return res.status(404).json({ ok: false, message: 'Secção não encontrada.' });
    ...
    if (innerEnd === -1) return res.status(500).json({ ok: false, message: 'Erro ao extrair a secção.' });
    ...
    res.status(500).json({ ok: false, message: 'Erro ao atualizar a secção.' });
  }
});
...
app.post('/api/:guildId/ticket-config', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { category_id, log_channel, support_role, transcript_channel, max_tickets, welcome_msg, panel_mode, panel_color } = req.body;
...
app.post('/api/:guildId/welcome-config', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const {
    welcome_channel, welcome_title, welcome_msg, welcome_embed, welcome_image, autorole,
  ...
  `).run(
    guildId, welcome_channel||null, welcome_title||null, welcome_msg||'Bem-vindo ${usermention}!', welcome_embed==='1'?1:0, welcome_image||null, autorole||null,
    corValida, welcome_author_name||null, welcome_author_icon||null, welcome_thumbnail||null,
    welcome_footer||null, posValida, welcome_content||null, welcome_url||null
  );
  ...
  try {
    ...
    const fakeMember = {
      ...
      user: {
        ...
        displayAvatarURL: () => previewUser?.avatar
          ? `https://cdn.discordapp.com/avatars/${previewUser.id}/${previewUser.avatar}.png`
...
app.post('/api/:guildId/antispam-link-channels', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { channels, mode } = req.body; // mode: 'all' ou 'specific'
  if (!Array.isArray(channels)) return res.status(400).json({ ok: false, message: 'Lista inválida.' });
  ...
  db.prepare('UPDATE antispam_config SET link_invite_channels = ? WHERE guild_id = ?')
    .run(JSON.stringify(lista), guildId);

  res.json({ ok: true, message: mode === 'all'
    ? '✅ Anti-links/Anti-convites vão aplicar-se a TODOS os canais!'
    : `✅ Anti-links/Anti-convites vão aplicar-se apenas a ${lista.length} canal(is) escolhido(s)!` });
});

// ── Canais onde o Anti-Links/Anti-Convites NUNCA se aplica (exceções) ──
app.post('/api/:guildId/antispam-link-excluded-channels', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  ...
  db.prepare('INSERT OR IGNORE INTO antispam_config (guild_id) VALUES (?)').run(guildId);
  db.prepare('UPDATE antispam_config SET link_invite_excluded_channels = ? WHERE guild_id = ?')
    .run(JSON.stringify(lista), guildId);

  res.json({ ok: true, message: lista.length
    ? `✅ Anti-links/Anti-convites vão ficar desligados em ${lista.length} canal(is) escolhido(s)!`
    : '✅ Nenhum canal excluído — o anti-links/anti-convites volta a seguir a configuração normal em todos.' });
});
...
app.post('/api/:guildId/logs-criar-canais', requireGuildAdminApi, async (req, res) => {
  ...
  try {
    const { logChannelId, modLogChannelId } = await criarCanaisDeLogs(guild);
    ...
    res.json({ ok: true, message: '✅ Categoria "Logs" criada com os canais 📜│logs e 📜│mod-logs (visíveis só para Administradores)!' });
  } catch (e) {
    res.status(400).json({ ok: false, message: `❌ Não foi possível criar os canais: ${e.message}` });
  }
});
...
app.get('/api/:guildId/suggestions', requireGuildAdminApi, (req, res) => {
  ...
  const comTipo = suggestions.map(s => {
    const tipo = s.type_id ? db.prepare('SELECT name, emoji FROM suggestion_types WHERE id = ?').get(s.type_id) : null;
    return { ...s, type_name: tipo?.name || null, type_emoji: tipo?.emoji || null };
  });
  res.json(comTipo);
});
...
// ── Sugestões: Tipos ──
app.get('/api/:guildId/sugestao-tipos', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  const tipos = db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? ORDER BY order_num, id').all(guildId);
...
app.post('/api/:guildId/sugestao-tipos', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { name, emoji, channel_id, log_channel, ping_role } = req.body || {};

  const nome = (name || '').trim();
  if (!nome) return res.status(400).json({ ok: false, message: '❌ O nome do tipo é obrigatório.' });
  if (!channel_id) return res.status(400).json({ ok: false, message: '❌ Escolhe um canal de sugestões.' });

  const existente = db.prepare('SELECT id FROM suggestion_types WHERE guild_id = ? AND name = ?').get(guildId, nome);
  if (existente) return res.status(400).json({ ok: false, message: `❌ Já existe um tipo chamado "${nome}".` });

  const { c: total } = db.prepare('SELECT COUNT(*) AS c FROM suggestion_types WHERE guild_id = ?').get(guildId);
  db.prepare(`
    INSERT INTO suggestion_types (guild_id, name, emoji, channel_id, log_channel, ping_role, enabled, order_num)
    VALUES (?, ?, ?, ?, ?, ?, 1, ?)
  `).run(guildId, nome, (emoji || '').trim() || '💡', channel_id, log_channel || null, ping_role || null, total);

  res.json({ ok: true, message: `✅ Tipo "${nome}" criado!` });
});

app.post('/api/:guildId/sugestao-tipos/:id/toggle', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const tipo = db.prepare('SELECT * FROM suggestion_types WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!tipo) return res.status(404).json({ ok: false, message: 'Tipo não encontrado.' });
  db.prepare('UPDATE suggestion_types SET enabled = ? WHERE id = ?').run(tipo.enabled ? 0 : 1, tipo.id);
  res.json({ ok: true, message: tipo.enabled ? '✅ Tipo desativado.' : '✅ Tipo ativado.' });
});

app.delete('/api/:guildId/sugestao-tipos/:id', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const tipo = db.prepare('SELECT * FROM suggestion_types WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!tipo) return res.status(404).json({ ok: false, message: 'Tipo não encontrado.' });
  db.prepare('DELETE FROM suggestion_types WHERE id = ?').run(tipo.id);
  res.json({ ok: true, message: `✅ Tipo "${tipo.name}" apagado.` });
});


// ── Perguntas à comunidade (envio + listagem + remoção) ──
app.get('/api/:guildId/perguntas', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  const perguntas = db.prepare('SELECT * FROM perguntas WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50').all(guildId);
  const channels = guild ? guild.channels.cache
    .filter(c => c.type === ChannelType.GuildText)
    .map(c => ({ id: c.id, name: c.name })) : [];
  res.json({ perguntas, html: renderPerguntasHistorico(perguntas, channels, guildId) });
});

app.post('/api/:guildId/perguntas', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { channel_id, pergunta, mensagem_extra } = req.body;
  ...
  res.json({ ok: true, message: `✅ Pergunta enviada em #${canal.name}! Tópico criado para respostas.`, perguntaId: resultado.perguntaId });
});
...
app.post('/api/:guildId/ticket-types/edit', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { id, label, description, emoji, category_id, support_role, color } = req.body;
  ...
  if (!label || !label.trim()) return res.status(400).json({ ok: false, message: 'Indica o nome do tipo de ticket.' });
  ...
  res.json({ ok: true, message: '✅ Tipo de ticket atualizado!' });
});

// ── Reordenar tipos de ticket (define a ordem dos botões/opções no painel) ──
app.post('/api/:guildId/ticket-types/reorder', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { order } = req.body; // array de IDs na nova ordem
  if (!Array.isArray(order)) return res.status(400).json({ ok: false, message: 'Ordem inválida.' });

  const stmt = db.prepare('UPDATE ticket_types SET order_num = ? WHERE id = ? AND guild_id = ?');
  const tx = db.transaction((ids) => {
    ids.forEach((id, idx) => stmt.run(idx, id, guildId));
  });
  tx(order);

  res.json({ ok: true, message: '✅ Ordem atualizada!' });
});
...
app.post('/api/:guildId/embeds/delete', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { id } = req.body;
  const saved = db.prepare('SELECT trigger_command FROM saved_embeds WHERE id = ? AND guild_id = ?').get(id, guildId);
  ...
  if (saved?.trigger_command) {
    try { await sincronizarComandosEmbed(guildId); }
    catch (e) { console.error('❌ Erro ao sincronizar comandos de embed após apagar:', e.message); }
  }
...
app.post('/api/:guildId/embeds/comando', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  let { id, comando } = req.body;

  const saved = db.prepare('SELECT * FROM saved_embeds WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!saved) return res.status(404).json({ ok: false, message: 'Embed não encontrado.' });

  comando = (comando || '').trim().toLowerCase().replace(/^[+!/]+/, ''); // remove prefixo se a pessoa o escrever por engano
  if (!comando) return res.status(400).json({ ok: false, message: 'Indica o nome do comando (ex: abrirservidor).' });
  // Regras de nome de comando slash do Discord: 1-32 caracteres, minúsculas, números, "-" e "_"
  if (!/^[a-z0-9_-]{1,32}$/.test(comando)) {
    return res.status(400).json({ ok: false, message: 'O comando só pode ter letras minúsculas, números, "-" e "_", até 32 caracteres.' });
  }

  // Garante que não existe já outra embed com o mesmo comando NESTE servidor
  const conflito = db.prepare(
    'SELECT id FROM saved_embeds WHERE guild_id = ? AND trigger_command = ? AND id != ?'
  ).get(guildId, comando, id);
  if (conflito) {
    return res.status(400).json({ ok: false, message: `Já existe outra embed guardada neste servidor a usar o comando "${comando}".` });
  }

  db.prepare('UPDATE saved_embeds SET trigger_command = ? WHERE id = ? AND guild_id = ?').run(comando, id, guildId);

  try {
    await sincronizarComandosEmbed(guildId);
  } catch (e) {
    console.error('❌ Erro ao sincronizar comando slash de embed:', e.message);
    return res.json({ ok: true, message: `✅ Comando guardado, mas houve um erro ao registá-lo no Discord. Tenta novamente em instantes.` });
  }

  res.json({ ok: true, message: `✅ Comando "/${comando}" criado! Já podes usá-lo neste servidor (só administradores o veem).` });
});

app.post('/api/:guildId/embeds/comando-remover', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { id } = req.body;
  db.prepare('UPDATE saved_embeds SET trigger_command = NULL WHERE id = ? AND guild_id = ?').run(id, guildId);
  ...
  res.json({ ok: true, message: '✅ Comando removido desta embed.' });
});

// ── Embeds: editar uma embed já guardada (mantém o mesmo nome e comando, se existirem) ──
app.post('/api/:guildId/embeds/editar', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const {
    id, titulo, descricao, cor, imagem, thumbnail, footer, mensagem,
    url, autor_nome, autor_icon, image_pos,
  } = req.body;
  ...
  if (!titulo && !descricao) {
    return res.status(400).json({ ok: false, message: 'Indica pelo menos um título ou descrição.' });
  }
  ...
  db.prepare('UPDATE saved_embeds SET data = ? WHERE id = ? AND guild_id = ?').run(data, id, guildId);
  res.json({ ok: true, message: `✅ Embed "${saved.name}" atualizada!` });
});
...
// ── Giveaways ──
app.get('/api/:guildId/giveaways', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const giveaways = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? ORDER BY ended ASC, created_at DESC LIMIT 50').all(guildId);
  const comContagem = giveaways.map(gw => ({
    ...gw,
    total_entradas: db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(gw.id).c,
  }));
...
app.post('/api/:guildId/giveaways', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { canal_id, titulo, descricao, premio, imagem_url, duracao, vencedores, mensagem_extra } = req.body || {};

  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });

  if (!premio || !String(premio).trim()) return res.status(400).json({ ok: false, message: '❌ O prémio é obrigatório.' });
  if (!canal_id) return res.status(400).json({ ok: false, message: '❌ Escolhe um canal.' });

  const canal = guild.channels.cache.get(canal_id);
  if (!canal) return res.status(400).json({ ok: false, message: '❌ Canal inválido.' });

  const duracaoMs = parseDuracao(String(duracao || '').trim());
  if (!duracaoMs || duracaoMs < 60000) {
    return res.status(400).json({ ok: false, message: '❌ Duração inválida. Usa um formato como 1m, 10m, 2h ou 1d (mínimo 1 minuto).' });
  }
  ...
  if (imagemUrl && !/^https?:\/\/.+\.(png|jpe?g|gif|webp)(\?.*)?$/i.test(imagemUrl)) {
    return res.status(400).json({ ok: false, message: '❌ URL de imagem inválido. Tem de ser um link direto (png, jpg, gif ou webp).' });
  }
  ...
  const info = db.prepare(`
    INSERT INTO giveaways (guild_id, channel_id, premio, vencedores, ends_at, host_id, titulo, descricao, imagem_url, mensagem_extra)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(guildId, canal.id, String(premio).trim(), nVencedores, endsAtISO, req.session?.user?.id || null,
         (titulo || '').trim() || null, (descricao || '').trim() || null, imagemUrl, mensagemExtra);
  ...
  if (!msg) {
    db.prepare('DELETE FROM giveaways WHERE id = ?').run(gw.id);
    return res.status(400).json({ ok: false, message: '❌ Não foi possível publicar o giveaway nesse canal (verifica as permissões do bot).' });
  }
  db.prepare('UPDATE giveaways SET message_id = ? WHERE id = ?').run(msg.id, gw.id);
  agendarEncerramentoGiveaway(gw);

  res.json({ ok: true, message: `✅ Giveaway #${gw.id} criado em #${canal.name}!` });
});
...
app.post('/api/:guildId/giveaways/:id/terminar', requireGuildAdminApi, async (req, res) => {
  ...
  const gw = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!gw) return res.status(404).json({ ok: false, message: 'Giveaway não encontrado.' });
  if (gw.ended) return res.status(400).json({ ok: false, message: 'Este giveaway já terminou.' });

  const vencedores = await encerrarGiveaway(guild, gw);
  cancelarTimerGiveaway(gw.id);
  res.json({ ok: true, message: vencedores.length ? `✅ Encerrado! Vencedor(es): ${vencedores.length}` : '✅ Encerrado sem participantes.' });
});
...
app.post('/api/:guildId/giveaways/:id/reroll', requireGuildAdminApi, async (req, res) => {
  ...
  const gw = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!gw) return res.status(404).json({ ok: false, message: 'Giveaway não encontrado.' });
  if (!gw.ended) return res.status(400).json({ ok: false, message: 'Este giveaway ainda está ativo.' });

  const vencedores = sortearVencedores(gw.id, gw.vencedores);
  if (!vencedores.length) return res.status(400).json({ ok: false, message: 'Não há participantes para sortear.' });
  ...
  res.json({ ok: true, message: '✅ Reroll feito!' });
});
...
app.post('/api/:guildId/giveaways/:id/cancelar', requireGuildAdminApi, async (req, res) => {
  ...
  res.json({ ok: true, message: '✅ Giveaway cancelado.' });
});

// ── Painéis de Informação ──
app.get('/api/:guildId/infopanels', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  const paineis = db.prepare('SELECT * FROM info_panels WHERE guild_id = ? ORDER BY created_at DESC').all(guildId);
  const comContagem = paineis.map(p => ({
    ...p,
    button_count: db.prepare('SELECT COUNT(*) AS c FROM info_panel_buttons WHERE panel_id = ?').get(p.id).c,
...
app.post('/api/:guildId/infopanels', requireGuildAdminApi, async (req, res) => {
  const { guildId } = req.params;
  const { name, title, description, color, banner_url, thumbnail_url, footer_text, owner_text, founded_text, channel_id } = req.body || {};

  const guild = client.guilds.cache.get(guildId);
  if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });

  const nome = (name || '').trim();
  if (!nome) return res.status(400).json({ ok: false, message: '❌ O nome interno é obrigatório.' });

  const existente = db.prepare('SELECT id FROM info_panels WHERE guild_id = ? AND name = ?').get(guildId, nome);
  if (existente) return res.status(400).json({ ok: false, message: `❌ Já existe um painel chamado "${nome}".` });
  ...
  if (!/^#([0-9A-Fa-f]{6})$/.test(cor)) {
    return res.status(400).json({ ok: false, message: '❌ Cor inválida. Usa o formato hex, ex: #5865F2.' });
  }
  ...
  `).run(
    ...
    (footer_text || '').trim() || null,
    (owner_text || '').trim() || null,
    (founded_text || '').trim() || null,
  ...
  db.prepare('UPDATE info_panels SET channel_id = ? WHERE id = ?').run(channel_id, info.lastInsertRowid);

  res.json({ ok: true, message: `✅ Painel "${nome}" criado! Agora adiciona os botões e depois clica em "Publicar".`, id: info.lastInsertRowid });
});
...
app.post('/api/:guildId/infopanels/:id/publish', requireGuildAdminApi, async (req, res) => {
  ...
  const panel = db.prepare('SELECT * FROM info_panels WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!panel) return res.status(404).json({ ok: false, message: 'Painel não encontrado.' });
  if (!panel.channel_id) return res.status(400).json({ ok: false, message: '❌ Este painel não tem canal definido.' });

  const canal = guild.channels.cache.get(panel.channel_id);
  if (!canal) return res.status(400).json({ ok: false, message: '❌ Canal inválido ou já não existe.' });
  ...
  const msg = await canal.send({ embeds: [embed], components: rows }).catch(() => null);
  if (!msg) {
    return res.status(400).json({ ok: false, message: '❌ Não foi possível publicar o painel nesse canal (verifica as permissões do bot).' });
  }
  db.prepare('UPDATE info_panels SET message_id = ?, published = 1 WHERE id = ?').run(msg.id, panel.id);

  res.json({ ok: true, message: `✅ Painel publicado em #${canal.name}!` });
});
...
app.delete('/api/:guildId/infopanels/:id', requireGuildAdminApi, async (req, res) => {
  ...
  res.json({ ok: true, message: '✅ Painel apagado.' });
});

app.get('/api/:guildId/infopanels/:id/buttons', requireGuildAdminApi, (req, res) => {
  const { id, guildId } = req.params;
  const panel = db.prepare('SELECT * FROM info_panels WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!panel) return res.status(404).json({ ok: false, message: 'Painel não encontrado.' });
  const botoes = db.prepare('SELECT * FROM info_panel_buttons WHERE panel_id = ? ORDER BY order_num, id').all(id);
  res.json(botoes);
});

app.post('/api/:guildId/infopanels/:id/buttons', requireGuildAdminApi, async (req, res) => {
  const { id, guildId } = req.params;
  const { label, emoji, style, response_text, response_title, response_image, response_thumbnail, response_color } = req.body || {};
  ...
  if (!lbl) return res.status(400).json({ ok: false, message: '❌ O texto do botão é obrigatório.' });
  if (!resp) return res.status(400).json({ ok: false, message: '❌ A resposta do botão é obrigatória.' });

  const cor = (response_color || '').trim() || null;
  if (cor && !/^#([0-9A-Fa-f]{6})$/.test(cor)) {
    return res.status(400).json({ ok: false, message: '❌ Cor da resposta inválida. Usa o formato hex, ex: #5865F2.' });
  }

  const contagem = db.prepare('SELECT COUNT(*) AS c FROM info_panel_buttons WHERE panel_id = ?').get(panel.id).c;
  if (contagem >= 25) return res.status(400).json({ ok: false, message: '❌ Este painel já tem o máximo de 25 botões.' });
  ...
  const info = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    panel.id, lbl, (emoji || '').trim() || null, estilo, resp,
    (response_title || '').trim() || null,
    (response_image || '').trim() || null,
    (response_thumbnail || '').trim() || null,
    cor,
    contagem,
  );
  ...
  res.json({ ok: true, message: `✅ Botão "${lbl}" adicionado!`, id: info.lastInsertRowid });
});

app.delete('/api/:guildId/infopanels/:id/buttons/:buttonId', requireGuildAdminApi, async (req, res) => {
  const { id, guildId, buttonId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  const panel = db.prepare('SELECT * FROM info_panels WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!panel) return res.status(404).json({ ok: false, message: 'Painel não encontrado.' });

  db.prepare('DELETE FROM info_panel_buttons WHERE id = ? AND panel_id = ?').run(buttonId, panel.id);
  ...
  res.json({ ok: true, message: '✅ Botão apagado.' });
});
...
const dashboardJS = `
  ...
  // ── Editar embed guardada (editor genérico estilo Sapphire) ──
  function abrirEditarEmbed(e) {
    ...
    document.getElementById('editar-embed-titulo').value = e.title || '';
    document.getElementById('editar-embed-url').value = e.url || '';
    document.getElementById('editar-embed-descricao').value = e.description || '';
    ...
    document.getElementById('editar-embed-footer').value = e.footer || '';
    document.getElementById('editar-embed-mensagem').value = e.content || '';
    document.getElementById('editar-embed-autor-nome').value = e.author_name || '';
  ...
  function lerFormularioEmbedGenerico() {
    return {
      welcome_content: document.getElementById('editar-embed-mensagem').value,
      welcome_author_name: document.getElementById('editar-embed-autor-nome').value,
      welcome_author_icon: document.getElementById('editar-embed-autor-icon').value,
      welcome_title: document.getElementById('editar-embed-titulo').value,
      welcome_url: document.getElementById('editar-embed-url').value,
      welcome_msg: document.getElementById('editar-embed-descricao').value,
      welcome_image: document.getElementById('editar-embed-imagem').value,
      welcome_thumbnail: document.getElementById('editar-embed-thumbnail').value,
      welcome_footer: document.getElementById('editar-embed-footer').value,
      welcome_color: document.getElementById('editar-embed-cor').value,
      welcome_image_pos: document.getElementById('editar-embed-image-pos').value,
    };
  }
  ...
  function aplicarRawEmbed() {
    try {
      ...
      document.getElementById('editar-embed-titulo').value = raw.title || '';
      document.getElementById('editar-embed-url').value = raw.url || '';
      document.getElementById('editar-embed-descricao').value = raw.description || '';
      document.getElementById('editar-embed-cor').value = raw.color || '#5865F2';
      document.getElementById('editar-embed-autor-nome').value = raw.author_name || '';
      ...
      document.getElementById('editar-embed-footer').value = raw.footer || '';
      document.getElementById('editar-embed-mensagem').value = raw.content || '';
  ...
  async function guardarEdicaoEmbed(guildId) {
    ...
    const body = {
      id,
      titulo: document.getElementById('editar-embed-titulo').value.trim(),
      descricao: document.getElementById('editar-embed-descricao').value.trim(),
      cor: document.getElementById('editar-embed-cor').value.trim(),
      url: document.getElementById('editar-embed-url').value.trim(),
      autor_nome: document.getElementById('editar-embed-autor-nome').value.trim(),
      autor_icon: document.getElementById('editar-embed-autor-icon').value.trim(),
      imagem: document.getElementById('editar-embed-imagem').value.trim(),
      thumbnail: document.getElementById('editar-embed-thumbnail').value.trim(),
      footer: document.getElementById('editar-embed-footer').value.trim(),
      mensagem: document.getElementById('editar-embed-mensagem').value.trim(),
      image_pos: document.getElementById('editar-embed-image-pos').value,
    };
    try {
...
<body>
  <nav class="navbar">
    <div class="logo">
      <a href="/dashboard" onclick="try{localStorage.removeItem('dashboard_section_${guild.id}')}catch(e){}" style="color:var(--text2);font-size:1rem">← Voltar</a>
      &nbsp;|&nbsp; 🤖 <span>${guild.name}</span> <span class="bot-badge">BOT</span>
    </div>
    <div class="user">
      <img src="${user.avatar}" alt="avatar">
      <span>${user.username}</span>
  ...
  <div class="sidebar">
    ...
    ${[
      ...
      ['🎉','Giveaways','giveaways_tab'],
      ['🪧','Painéis de Info','infopanels_tab'],
          ...
          ${savedEmbeds.length ? `
            <table class="data-table">
              ...
              <tbody>
                ${savedEmbeds.map(e => {
                  let embedData = {};
                  try { embedData = JSON.parse(e.data); } catch(_) {}
                  ...
                  <tr>
                    ...
                    <td>
                      ${e.trigger_command
                        ? `<div style="margin-bottom:4px"><span style="color:var(--success, #3ba55c)">🟢 <code>/${e.trigger_command}</code></span></div>
                           <button type="button" class="btn btn-danger" style="padding:4px 10px;font-size:0.8rem" onclick="removerComandoEmbed('${guild.id}', ${e.id})">🗑️ Remover comando</button>`
                        : `<input type="text" id="embed-cmd-${e.id}" placeholder="ex: abrirservidor" style="width:110px;display:inline-block;padding:4px 8px;font-size:0.8rem">
                    ...
                    <td>
                      <button type="button" class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick='abrirEditarEmbed(${JSON.stringify({ id: e.id, name: e.name, image_pos: "bottom", ...embedData })})' title="Editar esta embed">✏️</button>
          ...
          <div id="sap-panel-edit" class="sap-panel">
            ...
            <div id="sap-sub-visual" class="sap-subpanel" style="display:block">
              <div style="padding:18px 22px;overflow:auto;max-height:60vh">
                <h3 style="margin:0 0 4px;font-size:1rem">✏️ Editar Embed: <span id="editar-embed-nome" style="color:var(--accent)"></span></h3>
                <div class="grid-2">
                  <div class="form-group">
                    <label>Nome do Autor (opcional)</label>
                    <input type="text" id="editar-embed-autor-nome" placeholder="Ex: Nome do servidor" oninput="atualizarPreviewEmbedGenerico()">
                  </div>
                  <div class="form-group">
                    <label>Ícone do Autor (URL, opcional)</label>
                    <input type="text" id="editar-embed-autor-icon" placeholder="https://..." oninput="atualizarPreviewEmbedGenerico()">
                  </div>
                </div>
                <div class="form-group">
                  <label>Título</label>
                  <input type="text" id="editar-embed-titulo" maxlength="256" oninput="atualizarPreviewEmbedGenerico()">
                </div>
                <div class="form-group">
                  <label>Link do Título (opcional)</label>
                  <input type="text" id="editar-embed-url" placeholder="https://..." oninput="atualizarPreviewEmbedGenerico()">
                </div>
                <div class="form-group">
                  <label>Descrição</label>
                  <textarea id="editar-embed-descricao" rows="4" oninput="atualizarPreviewEmbedGenerico()"></textarea>
                </div>
                <div class="grid-2">
                  <div class="form-group">
                    <label>Cor (hex)</label>
                    <input type="color" id="editar-embed-cor" value="#5865F2" style="width:60px;height:38px;padding:2px;cursor:pointer" oninput="atualizarPreviewEmbedGenerico()">
                  </div>
                  <div class="form-group">
                    <label>Posição da Imagem</label>
                    <select id="editar-embed-image-pos" onchange="atualizarPreviewEmbedGenerico()">
                ...
                <div class="grid-2">
                  <div class="form-group">
                    <label>URL da Imagem (topo/banner — consoante posição)</label>
                    <input type="text" id="editar-embed-imagem" placeholder="https://..." oninput="atualizarPreviewEmbedGenerico()">
                  </div>
                  <div class="form-group">
                    <label>URL da Thumbnail (opcional, extra)</label>
                    <input type="text" id="editar-embed-thumbnail" placeholder="https://..." oninput="atualizarPreviewEmbedGenerico()">
                  </div>
                </div>
                <div class="form-group">
                  <label>Rodapé (opcional)</label>
                  <input type="text" id="editar-embed-footer" placeholder="Texto do rodapé" oninput="atualizarPreviewEmbedGenerico()">
                </div>
                <div class="form-group">
                  <label>Mensagem fora do embed (opcional)</label>
                  <input type="text" id="editar-embed-mensagem" placeholder="Ex: @everyone" oninput="atualizarPreviewEmbedGenerico()">
                </div>
              </div>
            </div>
    ...
    <div id="welcome" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <form id="form-welcome" oninput="atualizarPreviewWelcome('${guild.id}')" onchange="atualizarPreviewWelcome('${guild.id}')">
          ...
          <div class="form-group">
            <label class="toggle">
              <input type="checkbox" name="welcome_embed" id="welcome_embed" value="1" ${guildConfig?.welcome_embed ? 'checked' : ''}>
          ...
          <div class="form-group">
            ...
            <input type="text" name="welcome_content" value="${(guildConfig?.welcome_content || '').replace(/"/g,'&quot;')}" placeholder="Bem-Vindo \${usermention} à **\${guildname}**!">
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:10px">
            ...
            <div>
              <div class="form-group">
                ...
                <input type="text" name="welcome_author_name" value="${(guildConfig?.welcome_author_name || '').replace(/"/g,'&quot;')}" placeholder="Ex: ${CONFIG.BOT_NAME}">
              </div>
              <div class="form-group">
                <label>Ícone do Autor (URL, opcional)</label>
                <input type="text" name="welcome_author_icon" value="${(guildConfig?.welcome_author_icon || '').replace(/"/g,'&quot;')}" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Título da Embed</label>
                <input type="text" name="welcome_title" value="${(guildConfig?.welcome_title || '👋 Bem-vindo(a)!').replace(/"/g,'&quot;')}" placeholder="Ex: 🔥 Bem-vindo(a) a \${guildname}!">
              </div>
              <div class="form-group">
                <label>Link do Título (opcional)</label>
                <input type="text" name="welcome_url" value="${(guildConfig?.welcome_url || '').replace(/"/g,'&quot;')}" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Descrição</label>
                <textarea name="welcome_msg" rows="5">${guildConfig?.welcome_msg || 'Bem-vindo(a) \${usermention} ao \${guildname}!'}</textarea>
              </div>
              <div class="form-group">
                <label>Cor da barra lateral</label>
                <input type="color" name="welcome_color" value="${guildConfig?.welcome_color || '#5865F2'}" style="width:60px;height:38px;padding:2px;cursor:pointer">
              </div>
              <div class="form-group">
                <label>Posição da Imagem</label>
                <select name="welcome_image_pos">
                  <option value="bottom" ${(!guildConfig?.welcome_image_pos || guildConfig?.welcome_image_pos==='bottom') ? 'selected' : ''}>Imagem grande em baixo</option>
                  <option value="thumbnail" ${guildConfig?.welcome_image_pos==='thumbnail' ? 'selected' : ''}>Imagem pequena à direita</option>
                  <option value="none" ${guildConfig?.welcome_image_pos==='none' ? 'selected' : ''}>Sem imagem</option>
                </select>
              </div>
              <div class="form-group">
                <label>Imagem (URL — usada consoante a posição escolhida acima)</label>
                <input type="text" name="welcome_image" value="${(guildConfig?.welcome_image || '').replace(/"/g,'&quot;')}" placeholder="https://exemplo.com/imagem.png">
              </div>
              <div class="form-group">
                <label>Miniatura extra (URL, opcional — fica sempre no canto, além da imagem acima)</label>
                <input type="text" name="welcome_thumbnail" value="${(guildConfig?.welcome_thumbnail || '').replace(/"/g,'&quot;')}" placeholder="https://exemplo.com/avatar.png">
              </div>
              <div class="form-group">
                <label>Rodapé (footer, opcional)</label>
                <input type="text" name="welcome_footer" value="${(guildConfig?.welcome_footer || '').replace(/"/g,'&quot;')}" placeholder="Ex: Membro nº \${membercount}">
              </div>
    ...
    <div id="antispam" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div id="link-channels-picker" style="display:${(antispam?.link_invite_channels && JSON.parse(antispam?.link_invite_channels||'[]').length>0) ? 'block' : 'none'};margin-bottom:12px">
          ...
          <div id="link-channels-list" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;max-height:260px;overflow-y:auto;padding:10px;border:1px solid var(--border);border-radius:8px">
            ${(() => { try { return channels || []; } catch(_) { return []; } })().map(ch => {
              const selecionados = (() => { try { return JSON.parse(antispam?.link_invite_channels || '[]'); } catch(_) { return []; } })();
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div id="link-excluded-channels-list" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;max-height:260px;overflow-y:auto;padding:10px;border:1px solid var(--border);border-radius:8px">
          ${(() => { try { return channels || []; } catch(_) { return []; } })().map(ch => {
            const excluidos = (() => { try { return JSON.parse(antispam?.link_invite_excluded_channels || '[]'); } catch(_) { return []; } })();
            return `<label class="toggle" style="font-size:0.85rem">
    ...
    <div id="logs" class="section" style="display:none">
      ...
      <div class="card">
        <form id="form-logs">
          ...
          ${(() => {
            const logTypesAtual    = (() => { try { const p = JSON.parse(guildConfig?.log_types);    return Array.isArray(p) ? p : null; } catch(_) { return null; } })();
            const modLogTypesAtual = (() => { try { const p = JSON.parse(guildConfig?.mod_log_types); return Array.isArray(p) ? p : null; } catch(_) { return null; } })();
            ...
            for (const [key, info] of Object.entries(LOG_TYPES)) {
              ...
              categorias[info.categoria].push({ key, label: info.label });
            }
  ...
  <script>
    ...
    function renderizarPreviewEmbed(body, embed, usarEmbed) {
      ...
      if (body.welcome_content) {
        html += '<div style="color:#dbdee1;margin-bottom:8px;white-space:pre-wrap">'+esc(body.welcome_content)+'</div>';
      }
      if (!usarEmbed) {
        html += '<div style="color:#dbdee1;white-space:pre-wrap">'+esc(body.welcome_msg||'')+'</div>';
        return html;
      }

      const cor = body.welcome_color || '#5865F2';
      const pos = body.welcome_image_pos || 'bottom';
      const temImagemGrande = pos === 'bottom' && body.welcome_image;
      const temThumb = (pos === 'thumbnail' && (body.welcome_thumbnail || body.welcome_image)) || (body.welcome_thumbnail && pos !== 'thumbnail');

      html += '<div style="display:flex;border-left:4px solid '+cor+';border-radius:4px;background:#2b2d31;overflow:hidden">';
      html += '<div style="padding:12px 16px;flex:1;min-width:0">';

      if (body.welcome_author_name) {
        html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;color:#f2f3f5;font-size:0.85rem;font-weight:600">';
        if (body.welcome_author_icon) html += '<img src="'+esc(body.welcome_author_icon)+'" style="width:20px;height:20px;border-radius:50%;object-fit:cover" onerror="this.style.display=\\'none\\'">';
        html += esc(body.welcome_author_name) + '</div>';
      }
      ...
      if (body.welcome_footer) {
        html += '<div style="color:#949ba4;font-size:0.75rem;margin-top:10px">'+esc(body.welcome_footer)+'</div>';
      }
      html += '</div>';
      if (temThumb) {
        const thumbSrc = pos === 'thumbnail' ? (body.welcome_thumbnail || body.welcome_image) : body.welcome_thumbnail;
        html += '<div style="padding:12px 16px 12px 0;flex-shrink:0"><img src="'+esc(thumbSrc)+'" style="width:80px;height:80px;border-radius:6px;object-fit:cover" onerror="this.style.display=\\'none\\'"></div>';
      }
      html += '</div>';
      return html;
    }

    // Gera o preview assim que a secção de boas-vindas é aberta
    const origShowSection = window.showSection;
    if (typeof origShowSection === 'function') {
      window.showSection = function(name) {
        origShowSection(name);
        if (name === 'welcome') setTimeout(() => enviarPreviewWelcome(GUILD_ID), 50);
      };
    }
