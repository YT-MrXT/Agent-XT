...
const zlib = require('zlib'); // nativo do Node — usado para compressão gzip do dashboard, sem gastar RAM extra com o pacote "compression"
const fs = require('fs');
...
const CONFIG = {
  ...
  BOT_AVATAR_URL: 'https://raw.githubusercontent.com/YT-MrXT/Agent-XT/main/xt_logo_v8.png',
  // 🧠 AutoMod IA — chave de API única, definida por ti no Render (não é por cada servidor).
  // AI_PROVIDER: 'openai', 'anthropic' ou 'gemini' (Gemini tem free tier real, sem cartão de crédito)
  AI_PROVIDER: (process.env.AI_PROVIDER || 'openai').toLowerCase(),
  AI_API_KEY: process.env.AI_API_KEY || '',
  // Opcional — deixa vazio para usar o modelo recomendado por defeito
...
async function initDatabase() {
  ...
  // Tabela de tickets (agora suporta múltiplos painéis por servidor)
  // NOTA: guild_id deixou de ser PRIMARY KEY sozinho — cada linha é um "painel".
  // O par (guild_id, panel_num) identifica um painel (Painel 1/10 .. Painel 10/10).
  await db.exec(`
    CREATE TABLE IF NOT EXISTS ticket_config (
      id                INTEGER PRIMARY KEY AUTOINCREMENT,
      guild_id          TEXT NOT NULL,
      panel_num         INTEGER NOT NULL DEFAULT 1,
      panel_name        TEXT,
      category_id       TEXT,
      log_channel       TEXT,
      support_role      TEXT,
  ...
  try {
    ...
    if (!tcCols.includes('support_roles')) await db.exec("ALTER TABLE ticket_config ADD COLUMN support_roles TEXT DEFAULT '[]'");
    if (!tcCols.includes('panel_num')) await db.exec("ALTER TABLE ticket_config ADD COLUMN panel_num INTEGER NOT NULL DEFAULT 1");
    if (!tcCols.includes('panel_name')) await db.exec("ALTER TABLE ticket_config ADD COLUMN panel_name TEXT");
    // Se a tabela ainda tiver guild_id como PRIMARY KEY (versão antiga sem "id"), recria-a
    // preservando todos os dados existentes como "Painel 1/10" de cada servidor.
    if (!tcCols.includes('id')) {
      console.log('🔄 A migrar ticket_config para suportar múltiplos painéis...');
      await db.exec('ALTER TABLE ticket_config RENAME TO ticket_config_old');
      await db.exec(`
      ...
      `);
      const antigos = await db.prepare('SELECT * FROM ticket_config_old').all();
      for (const o of antigos) {
        const rolesJson = JSON.stringify(o.support_role ? [o.support_role] : []);
        await db.prepare(`
          INSERT INTO ticket_config (guild_id, panel_num, panel_name, category_id, log_channel, support_role, support_roles, max_tickets, panel_msg_id, panel_channel_id, transcript_channel, welcome_msg, enabled, panel_mode, panel_color)
          VALUES (?, 1, 'Painel 1/10', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(o.guild_id, o.category_id || null, o.log_channel || null, o.support_role || null, rolesJson, o.max_tickets ?? 3, o.panel_msg_id || null, o.panel_channel_id || null, o.transcript_channel || null, o.welcome_msg || null, o.enabled ?? 1, o.panel_mode || 'select', o.panel_color || '#5865F2');
      }
      await db.exec('DROP TABLE ticket_config_old');
      console.log(`✅ Migração concluída: ${antigos.length} configuração(ões) transformada(s) em "Painel 1/10".`);
    } else {
      // Tabela já tem "id": garante que qualquer linha antiga sem support_roles preenchido
      // é sincronizada a partir de support_role (para não perder cargo já configurado).
      const semRoles = await db.prepare("SELECT id, support_role, support_roles FROM ticket_config WHERE support_role IS NOT NULL AND (support_roles IS NULL OR support_roles = '[]')").all();
      for (const r of semRoles) {
        await db.prepare('UPDATE ticket_config SET support_roles = ? WHERE id = ?').run(JSON.stringify([r.support_role]), r.id);
      }
      // Garante panel_name preenchido para painéis antigos
      await db.exec("UPDATE ticket_config SET panel_name = 'Painel 1/10' WHERE (panel_name IS NULL OR panel_name = '') AND panel_num = 1");
    }
  } catch (e) {
    console.error('❌ Erro na migração de ticket_config (multi-painel):', e.message);
  }
  ...
  `);
  // Migração: garante colunas novas em bases de dados já existentes
  try {
    const cols = await db.prepare("PRAGMA table_info(ticket_types)").all();
    const colNames = cols.map(c => c.name);
    if (!colNames.includes('has_form')) {
      await db.exec('ALTER TABLE ticket_types ADD COLUMN has_form INTEGER DEFAULT 0');
    }
    if (!colNames.includes('support_roles')) {
      await db.exec("ALTER TABLE ticket_types ADD COLUMN support_roles TEXT DEFAULT '[]'");
      // Preserva o cargo único já configurado, agora como lista
      const comCargo = await db.prepare("SELECT id, support_role FROM ticket_types WHERE support_role IS NOT NULL").all();
      for (const t of comCargo) {
        await db.prepare('UPDATE ticket_types SET support_roles = ? WHERE id = ?').run(JSON.stringify([t.support_role]), t.id);
      }
    }
    if (!colNames.includes('panel_id')) {
      await db.exec('ALTER TABLE ticket_types ADD COLUMN panel_id INTEGER');
      // Associa todos os tipos existentes ao Painel 1/10 (id mais baixo) de cada servidor
      const painel1PorGuild = await db.prepare("SELECT guild_id, MIN(id) as pid FROM ticket_config WHERE panel_num = 1 GROUP BY guild_id").all();
      for (const p of painel1PorGuild) {
        await db.prepare('UPDATE ticket_types SET panel_id = ? WHERE guild_id = ? AND panel_id IS NULL').run(p.pid, p.guild_id);
      }
    }
  } catch (e) {
    console.error('❌ Erro na migração de ticket_types (support_roles/panel_id):', e.message);
  }
  ...
  `);
  // Migração: garante a coluna panel_id em bases de dados já existentes
  try {
    const tCols = (await db.prepare("PRAGMA table_info(tickets)").all()).map(c => c.name);
    if (!tCols.includes('panel_id')) {
      await db.exec('ALTER TABLE tickets ADD COLUMN panel_id INTEGER');
    }
  } catch (e) {
    console.error('❌ Erro na migração de tickets.panel_id:', e.message);
  }
  ...
  // Tem de ser criada ANTES da migração abaixo, que lê desta tabela.
  await db.exec(`
  ...
  // Migração: o sistema deixou de suportar múltiplos "tipos" de sugestão por servidor —
  // agora há apenas UM canal de sugestões por servidor (configurado no dashboard).
  // Se um servidor já tinha exatamente 1 tipo guardado, esse tipo passa a ser
  // automaticamente a config única usada por /sugerir (nada a fazer, já está pronto).
  // Se tinha mais do que 1, são todos apagados (as sugestões já submetidas não são
  // apagadas, ficam só com o type_id "órfão") — o servidor tem de reconfigurar no dashboard.
  try {
    const gruposTipos = await db.prepare('SELECT guild_id, COUNT(*) AS c FROM suggestion_types GROUP BY guild_id').all();
    for (const { guild_id, c } of gruposTipos) {
      if (c > 1) {
        await db.prepare('DELETE FROM suggestion_types WHERE guild_id = ?').run(guild_id);
        console.log(`🧹 Servidor ${guild_id} tinha ${c} tipos de sugestão — apagados todos (reconfigura no dashboard).`);
      }
    }
  } catch (e) {
    console.error('❌ Erro na migração de simplificação de suggestion_types:', e.message);
  }
  ...
  // ============================
  // AUTOMOD IA — configuração por servidor
  // ============================
  // A chave de API é ÚNICA e global, definida por ti nas variáveis de ambiente do Render
  // (AI_API_KEY, AI_PROVIDER, AI_MODEL — ver bloco CONFIG). Cada servidor só liga/desliga
  // e ajusta os parâmetros abaixo, sem ver nem poder alterar a key.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS ai_moderation_config (
      ...
      min_severity       TEXT DEFAULT 'MEDIUM',    -- LOW | MEDIUM | HIGH — a partir de que gravidade age
      warning_reset_days INTEGER DEFAULT 0,        -- dias sem nova violação até o contador voltar a 0 (0 = nunca reseta sozinho)
      updated_at         DATETIME DEFAULT CURRENT_TIMESTAMP
    );
  `);

  // Migração: adiciona a coluna em bases de dados já existentes (antes desta versão)
  try {
    await db.exec(`ALTER TABLE ai_moderation_config ADD COLUMN warning_reset_days INTEGER DEFAULT 0;`);
  } catch (e) { /* coluna já existe — ignora */ }

  // Advertências acumuladas por utilizador (reiniciam quando o castigo é aplicado)
  await db.exec(`
  ...
  // Mensagens de DM ao entrar (novo — enviadas por mensagem privada ao membro assim
  // que ele entra no servidor, em paralelo com a mensagem de boas-vindas no canal,
  // se ambas estiverem configuradas). Mesma estrutura multi/nomeada que welcome_messages.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS dm_messages (
      ...
      dm_url             TEXT,
      dm_msg             TEXT,
      dm_color           TEXT DEFAULT '#5865F2',
      dm_author_name     TEXT,
      dm_author_icon     TEXT,
      dm_image_pos       TEXT DEFAULT 'bottom',
      dm_image           TEXT,
      dm_thumbnail       TEXT,
      dm_footer          TEXT,
      created_at         DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  // Mensagens de Saída (novo — anúncio enviado num canal quando um membro sai do
  // servidor). Mesma estrutura multi/nomeada que welcome_messages, mas com canal
  // próprio (leave_channel) em vez de welcome_channel.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS leave_messages (
      ...
      leave_image_pos    TEXT DEFAULT 'bottom',
      leave_image        TEXT,
  ...
  // ── Calls Temporárias ──
  // Configuração por servidor: categoria + canal "Criar Call" + canal "Interface"
  await db.exec(`
    CREATE TABLE IF NOT EXISTS tempvoice_config (
      ...
      interface_msg_id TEXT,
      name_template  TEXT DEFAULT '🔊 Call de {user}',
  ...
  // Calls ativas (uma linha por canal de voz temporário criado)
  await db.exec(`
  ...
  // Utilizadores bloqueados numa call temporária específica (impede reentrada)
  await db.exec(`
    CREATE TABLE IF NOT EXISTS tempvoice_blocked (
      channel_id TEXT NOT NULL,
      user_id    TEXT NOT NULL,
...
// Map para guardar temporariamente o motivo do /fechar até à confirmação
const fecharMotivoMap = new Map();
...
// Formata uma data (Date, string ISO ou "YYYY-MM-DD HH:MM:SS" vinda do SQLite) no
// estilo "31 de julho de 2026 às 15:17", usado nas mensagens de fecho de ticket.
const MESES_PT = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro'];
function formatarDataPT(data) {
  try {
    // SQLite guarda CURRENT_TIMESTAMP como "YYYY-MM-DD HH:MM:SS" em UTC — normaliza para ISO.
    const d = data instanceof Date ? data : new Date(typeof data === 'string' && !data.includes('T') ? data.replace(' ', 'T') + 'Z' : data);
...
/**
 * Cache em memória para getGuildConfig.
 * Objetivo: esta função é chamada em praticamente todos os eventos do bot
 * (ex: cada mensagem enviada, via sendLogTyped), mas a configuração do
 * servidor quase nunca muda entre esses eventos. Sem cache, cada mensagem
 * gerava um SELECT novo à base de dados remota (Turso), o que multiplica
 * muito as "rows read" e adiciona latência de rede a cada resposta do bot.
 * Com um TTL curto, a config só é relida da BD de X em X segundos por
 * servidor — reduzindo drasticamente as leituras sem tornar a config
 * "presa": qualquer alteração feita no dashboard ou por comandos fica
 * visível no máximo GUILD_CONFIG_CACHE_TTL_MS depois.
 */
const GUILD_CONFIG_CACHE_TTL_MS = 15_000; // 15 segundos
const guildConfigCache = new Map(); // guildId -> { config, expiraEm }

/** Obtém a configuração do servidor (com cache em memória para poupar leituras à BD) */
async function getGuildConfig(guildId) {
  ...
  guildConfigCache.set(guildId, {
    config,
    expiraEm: Date.now() + GUILD_CONFIG_CACHE_TTL_MS
  });
  return config;
}

/**
 * Limpa a cache de configuração de um servidor (ou de todos, se guildId omitido).
 * Chamar isto logo após qualquer UPDATE/INSERT em guild_config garante que a
 * próxima leitura reflete a mudança de imediato, em vez de esperar pelo TTL.
 */
function invalidarGuildConfigCache(guildId) {
...
<html lang="pt">
<head>
  <link rel="icon" type="image/png" href="${CONFIG.BOT_AVATAR_URL}">
  <meta charset="UTF-8">
...
/** Lê uma coluna support_roles (JSON) e devolve sempre um array de IDs de cargo, com fallback para a coluna antiga support_role */
function parseSupportRoles(row) {
  if (!row) return [];
  try {
    const arr = JSON.parse(row.support_roles || '[]');
    if (Array.isArray(arr) && arr.length) return arr.filter(Boolean);
  } catch (e) {
    /* ignora JSON inválido, cai no fallback abaixo */
  }
  return row.support_role ? [row.support_role] : [];
}

/** Devolve a lista de IDs de cargo de suporte efetivos de um ticket (cargos do tipo específico, senão os do painel/servidor) */
async function obterCargosSuporteTicket(ticket, ticketConfig) {
  if (!ticket) return parseSupportRoles(ticketConfig);
  const tipo = ticket.type_id ? await db.prepare('SELECT * FROM ticket_types WHERE id = ?').get(ticket.type_id) : null;
  const cargosTipo = parseSupportRoles(tipo);
  if (cargosTipo.length) return cargosTipo;
  return parseSupportRoles(ticketConfig);
}

/** Mantido por compatibilidade: devolve apenas o primeiro cargo de suporte efetivo (usar obterCargosSuporteTicket quando possível) */
async function obterCargoSuporteTicket(ticket, ticketConfig) {
  const cargos = await obterCargosSuporteTicket(ticket, ticketConfig);
  return cargos[0] || null;
}

/** Vai buscar a configuração (painel) de tickets certa para um servidor. Se panelId for indicado, busca esse painel; senão devolve o Painel 1 (ou o único existente / mais antigo). */
async function obterTicketConfig(guildId, panelId = null) {
  if (panelId) {
    const cfg = await db.prepare('SELECT * FROM ticket_config WHERE id = ? AND guild_id = ?').get(panelId, guildId);
    if (cfg) return cfg;
  }
  const porNum = await db.prepare('SELECT * FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id LIMIT 1').get(guildId);
  return porNum || null;
}
...
async function isEquipaAdminTicket(member, guild, ticket) {
  ...
  const ticketConfig = ticket?.panel_id ? await db.prepare('SELECT * FROM ticket_config WHERE id = ?').get(ticket.panel_id) : await obterTicketConfig(guild.id);
  const cargos = await obterCargosSuporteTicket(ticket, ticketConfig);
...
async function criarTicket(guild, user, typeId, interaction, respostas = []) {
  // Tipo de ticket (define também o painel a que este ticket pertence)
  const tipo = typeId ? await db.prepare('SELECT * FROM ticket_types WHERE id = ?').get(typeId) : null;
  const panelId = tipo?.panel_id || null;

  const ticketConfig = await obterTicketConfig(guild.id, panelId);
  ...
  // Múltiplos cargos de suporte: cargos do tipo específico, senão os do painel
  const cargosTipo = parseSupportRoles(tipo);
  const supportRoles = cargosTipo.length ? cargosTipo : parseSupportRoles(ticketConfig);
  for (const cargoId of supportRoles) {
    permOverwrites.push({
      id: cargoId,
  ...
  // Guarda na BD
  const stmt = db.prepare(`
    INSERT INTO tickets (guild_id, panel_id, channel_id, user_id, type_id, ticket_number, status)
    VALUES (?, ?, ?, ?, ?, ?, 'open')
  `);
  const info = await stmt.run(guild.id, ticketConfig.id || null, channel.id, user.id, typeId || null, ticketNum);
  ...
  const embed = new EmbedBuilder().setTitle(`🎫 Ticket #${String(ticketNum).padStart(4, '0')}`).setDescription(welcomeMsg).setColor(CONFIG.COR_PRINCIPAL).addFields({
    ...
    text: 'Para fechar o ticket use /fechar'
  }).setTimestamp();
  ...
  const mencaoCargos = supportRoles.length ? supportRoles.map(id => `<@&${id}>`).join(' ') + ' | ' : '';
  await channel.send({
    content: `${mencaoCargos}<@${user.id}>`,
    embeds: [embed],
...
// ============================
// CALLS TEMPORÁRIAS (TempVoice)
// ============================

/** Constrói o embed + botões da interface de controlo de calls temporárias */
function construirPainelTempVoice() {
  const embed = new EmbedBuilder()
    .setColor(CONFIG.COR_PRINCIPAL)
    .setTitle('🔊 Interface de Calls Temporárias')
    .setDescription(
      'Esta interface serve para configurares a tua call de voz temporária.\n' +
      'Entra em **➕│Criar-Call** para criares a tua, depois usa os botões abaixo.\n\n' +
      '`✏️ Nome`  `👥 Limite`\n' +
      '`🔒 Trancar`  `💬 Convidar`\n' +
      '`⛔ Expulsar`  `🚫 Bloquear`\n' +
      '`✅ Desbloquear`  `👑 Reclamar`\n' +
      '`🔁 Transferir`  `🗑️ Apagar`\n\n' +
      '**Pressiona os botões abaixo para usares a interface**'
    )
    .setFooter({ text: 'A call é apagada automaticamente quando fica vazia' });

  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_nome').setEmoji('✏️').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_limite').setEmoji('👥').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_trancar').setEmoji('🔒').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_convidar').setEmoji('💬').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_expulsar').setEmoji('⛔').setStyle(ButtonStyle.Secondary)
  );
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tv_bloquear').setEmoji('🚫').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_desbloquear').setEmoji('✅').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_reclamar').setEmoji('👑').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_transferir').setEmoji('🔁').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tv_apagar').setEmoji('🗑️').setStyle(ButtonStyle.Secondary)
  );

  return { embeds: [embed], components: [row1, row2] };
}

/** Cria a categoria "Crie sua Call" com os canais Criar Call + Interface. Devolve os IDs criados. */
async function criarSistemaTempVoice(guild, { nomeCategoria = 'Crie sua Call' } = {}) {
  const categoria = await guild.channels.create({
    name: nomeCategoria,
    type: ChannelType.GuildCategory
  });

  const canalCriar = await guild.channels.create({
    name: '➕│Criar-Call',
    type: ChannelType.GuildVoice,
    parent: categoria.id
  });

  const canalInterface = await guild.channels.create({
    name: '✨│interface',
    type: ChannelType.GuildText,
    parent: categoria.id,
    topic: 'Usa os botões abaixo para configurares a tua call de voz temporária.',
  ...
  const painel = construirPainelTempVoice();
  const msg = await canalInterface.send(painel);

  return {
    categoryId: categoria.id,
    creatorChannelId: canalCriar.id,
    interfaceChannelId: canalInterface.id,
    interfaceMsgId: msg.id
  };
}

/** Vai buscar a call temporária ativa (dona ou onde está) do utilizador que está a interagir com o painel */
async function getCallDoUtilizador(interaction) {
  const member = interaction.member;
  const voiceChannelId = member?.voice?.channelId;
  if (!voiceChannelId) return { erro: '❌ Precisas de estar dentro da tua call de voz temporária para usares este botão.' };

  const tv = await db.prepare('SELECT * FROM tempvoice_channels WHERE channel_id = ?').get(voiceChannelId);
  if (!tv) return { erro: '❌ O canal de voz em que estás não é uma call temporária.' };

  const channel = interaction.guild.channels.cache.get(voiceChannelId);
  if (!channel) return { erro: '❌ Não encontrei o canal de voz.' };

  return { tv, channel };
}

/** Verifica se o utilizador é dono da call (ou Admin), devolve true/false + mensagem de erro */
function ehDonoOuAdmin(tv, member) {
  if (tv.owner_id === member.id) return true;
  if (member.permissions.has(PermissionFlagsBits.Administrator)) return true;
  return false;
}

/** Fecha/apaga uma call temporária da BD e limpa os bloqueios associados */
async function limparCallTemporaria(channelId) {
  await db.prepare('DELETE FROM tempvoice_channels WHERE channel_id = ?').run(channelId);
  await db.prepare('DELETE FROM tempvoice_blocked WHERE channel_id = ?').run(channelId);
}

/** Trata todos os botões da interface de Calls Temporárias (customId começado por "tv_") */
async function handleTempVoiceButton(interaction, customId) {
  const { erro, tv, channel } = await getCallDoUtilizador(interaction);
  if (erro) {
    return interaction.reply({ content: erro, ephemeral: true });
  }
  const member = interaction.member;
  const dono = ehDonoOuAdmin(tv, member);

  // ── Ações que abrem um modal (não precisam de verificação de dono aqui, é feita ao submeter) ──
  if (customId === 'tv_nome') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode mudar o nome.', ephemeral: true });
    const modal = new ModalBuilder().setCustomId(`tv_modal_nome_${channel.id}`).setTitle('Mudar nome da call');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('novo_nome').setLabel('Novo nome da call').setStyle(TextInputStyle.Short).setMaxLength(100).setRequired(true).setValue(channel.name.substring(0, 100))
    ));
    return interaction.showModal(modal);
  }

  if (customId === 'tv_limite') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode mudar o limite.', ephemeral: true });
    const modal = new ModalBuilder().setCustomId(`tv_modal_limite_${channel.id}`).setTitle('Limite de utilizadores');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('novo_limite').setLabel('Limite (0 a 99, 0 = sem limite)').setStyle(TextInputStyle.Short).setMaxLength(2).setRequired(true).setValue(String(channel.userLimit || 0))
    ));
    return interaction.showModal(modal);
  }

  if (customId === 'tv_convidar') {
    const select = new UserSelectMenuBuilder().setCustomId(`tv_select_convidar_${channel.id}`).setPlaceholder('Escolhe quem queres convidar').setMinValues(1).setMaxValues(1);
    return interaction.reply({ content: '💬 Escolhe quem queres convidar para a tua call:', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
  }

  if (customId === 'tv_expulsar') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode expulsar alguém.', ephemeral: true });
    const select = new UserSelectMenuBuilder().setCustomId(`tv_select_expulsar_${channel.id}`).setPlaceholder('Escolhe quem queres expulsar').setMinValues(1).setMaxValues(1);
    return interaction.reply({ content: '⛔ Escolhe quem queres expulsar da tua call:', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
  }

  if (customId === 'tv_bloquear') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode bloquear alguém.', ephemeral: true });
    const select = new UserSelectMenuBuilder().setCustomId(`tv_select_bloquear_${channel.id}`).setPlaceholder('Escolhe quem queres bloquear').setMinValues(1).setMaxValues(1);
    return interaction.reply({ content: '🚫 Escolhe quem queres bloquear (não vai conseguir voltar a entrar):', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
  }

  if (customId === 'tv_desbloquear') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode desbloquear alguém.', ephemeral: true });
    const select = new UserSelectMenuBuilder().setCustomId(`tv_select_desbloquear_${channel.id}`).setPlaceholder('Escolhe quem queres desbloquear').setMinValues(1).setMaxValues(1);
    return interaction.reply({ content: '✅ Escolhe quem queres desbloquear:', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
  }

  if (customId === 'tv_transferir') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode transferi-la.', ephemeral: true });
    const select = new UserSelectMenuBuilder().setCustomId(`tv_select_transferir_${channel.id}`).setPlaceholder('Escolhe o novo dono (tem de estar na call)').setMinValues(1).setMaxValues(1);
    return interaction.reply({ content: '🔁 Escolhe para quem queres transferir a call:', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
  }

  // ── Ações diretas ──
  if (customId === 'tv_trancar') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode trancar/destrancar.', ephemeral: true });
    const novoEstado = tv.locked ? 0 : 1;
    await channel.permissionOverwrites.edit(interaction.guild.roles.everyone, {
      Connect: novoEstado ? false : null
    }).catch(() => {});
    await db.prepare('UPDATE tempvoice_channels SET locked = ? WHERE channel_id = ?').run(novoEstado, channel.id);
    return interaction.reply({ content: novoEstado ? '🔒 A tua call foi trancada — só quem já tem acesso pode entrar.' : '🔓 A tua call foi destrancada — qualquer pessoa pode entrar.', ephemeral: true });
  }

  if (customId === 'tv_reclamar') {
    const donoAindaPresente = channel.members.has(tv.owner_id);
    if (donoAindaPresente) {
      return interaction.reply({ content: '❌ Não podes reclamar esta call — o dono ainda está presente.', ephemeral: true });
    }
    if (!channel.members.has(member.id)) {
      return interaction.reply({ content: '❌ Precisas de estar dentro da call para a reclamares.', ephemeral: true });
    }
    await db.prepare('UPDATE tempvoice_channels SET owner_id = ? WHERE channel_id = ?').run(member.id, channel.id);
    await channel.permissionOverwrites.edit(member.id, {
      ViewChannel: true, Connect: true, ManageChannels: true
    }).catch(() => {});
    return interaction.reply({ content: `👑 Reclamaste esta call! Agora és o dono, <@${member.id}>.`, ephemeral: false });
  }

  if (customId === 'tv_apagar') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode apagá-la.', ephemeral: true });
    await interaction.reply({ content: '🗑️ A apagar a tua call...', ephemeral: true });
    await limparCallTemporaria(channel.id);
    await channel.delete().catch(() => {});
    return;
  }
}

/** Trata a submissão dos modais da interface de Calls Temporárias (customId "tv_modal_ação_channelId") */
async function handleTempVoiceModal(interaction, customId) {
  const match = customId.match(/^tv_modal_(nome|limite)_(\d+)$/);
  if (!match) return;
  const [, acao, channelId] = match;

  const tv = await db.prepare('SELECT * FROM tempvoice_channels WHERE channel_id = ?').get(channelId);
  if (!tv) {
    return interaction.reply({ content: '❌ Esta call já não existe.', ephemeral: true });
  }
  const channel = interaction.guild.channels.cache.get(channelId);
  if (!channel) {
    await limparCallTemporaria(channelId);
    return interaction.reply({ content: '❌ Esta call já não existe.', ephemeral: true });
  }
  const member = interaction.member;
  const dono = ehDonoOuAdmin(tv, member);
  ...
  if (acao === 'nome') {
    if (!dono) return interaction.reply({ content: '❌ Só o dono da call pode mudar o nome.', ephemeral: true });
    const novoNome = interaction.fields.getTextInputValue('novo_nome').substring(0, 100);
    await channel.setName(novoNome).catch(e => {});
    return interaction.reply({ content: `✅ Nome da call alterado para **${novoNome}**.`, ephemeral: true });
  }
  ...
  if (acao === 'limite') {
    ...
    if (isNaN(valor) || valor < 0 || valor > 99) {
      return interaction.reply({ content: '❌ Limite inválido. Usa um número entre 0 e 99 (0 = sem limite).', ephemeral: true });
    }
    await channel.setUserLimit(valor).catch(() => {});
    return interaction.reply({ content: valor === 0 ? '✅ Limite removido — a call já não tem número máximo de pessoas.' : `✅ Limite da call definido para **${valor}** pessoas.`, ephemeral: true });
  }
}

/** Trata a seleção de utilizador nos menus da interface de Calls Temporárias (customId "tv_select_ação_channelId") */
async function handleTempVoiceSelectMenu(interaction, customId) {
  const match = customId.match(/^tv_select_(convidar|expulsar|bloquear|desbloquear|transferir)_(\d+)$/);
  if (!match) return;
  const [, acao, channelId] = match;
  ...
  const tv = await db.prepare('SELECT * FROM tempvoice_channels WHERE channel_id = ?').get(channelId);
  if (!tv) {
    return interaction.update({ content: '❌ Esta call já não existe.', components: [] });
  }
  const channel = interaction.guild.channels.cache.get(channelId);
  if (!channel) {
    await limparCallTemporaria(channelId);
  ...
  if (acao === 'convidar') {
    const alvo = await interaction.client.users.fetch(userId).catch(() => null);
    if (!alvo) return interaction.update({ content: '❌ Não encontrei esse utilizador.', components: [] });
    await channel.permissionOverwrites.edit(userId, { ViewChannel: true, Connect: true }).catch(() => {});
    await alvo.send({ content: `🔊 <@${member.id}> convidou-te para a call **${channel.name}** em **${interaction.guild.name}**! Entra em <#${channel.id}>.` }).catch(() => {});
    return interaction.update({ content: `✅ <@${userId}> foi convidado para a tua call.`, components: [] });
  }

  if (acao === 'expulsar') {
    if (!dono) return interaction.update({ content: '❌ Só o dono da call pode expulsar alguém.', components: [] });
    const alvoMember = await interaction.guild.members.fetch(userId).catch(() => null);
    if (!alvoMember) return interaction.update({ content: '❌ Não encontrei esse membro.', components: [] });
    if (alvoMember.voice.channelId !== channel.id) {
      return interaction.update({ content: '❌ Essa pessoa não está na tua call.', components: [] });
    }
    await alvoMember.voice.disconnect().catch(() => {});
    return interaction.update({ content: `✅ <@${userId}> foi expulso da tua call.`, components: [] });
  }

  if (acao === 'bloquear') {
    if (!dono) return interaction.update({ content: '❌ Só o dono da call pode bloquear alguém.', components: [] });
    await db.prepare('INSERT OR IGNORE INTO tempvoice_blocked (channel_id, user_id) VALUES (?, ?)').run(channel.id, userId);
    await channel.permissionOverwrites.edit(userId, { ViewChannel: false, Connect: false }).catch(() => {});
    const alvoMember = await interaction.guild.members.fetch(userId).catch(() => null);
    if (alvoMember?.voice?.channelId === channel.id) await alvoMember.voice.disconnect().catch(() => {});
    return interaction.update({ content: `🚫 <@${userId}> foi bloqueado e já não pode entrar nesta call.`, components: [] });
  }

  if (acao === 'desbloquear') {
    if (!dono) return interaction.update({ content: '❌ Só o dono da call pode desbloquear alguém.', components: [] });
    await db.prepare('DELETE FROM tempvoice_blocked WHERE channel_id = ? AND user_id = ?').run(channel.id, userId);
    await channel.permissionOverwrites.edit(userId, { ViewChannel: null, Connect: null }).catch(() => {});
    return interaction.update({ content: `✅ <@${userId}> foi desbloqueado e já pode voltar a entrar.`, components: [] });
  }

  if (acao === 'transferir') {
    if (!dono) return interaction.update({ content: '❌ Só o dono da call pode transferi-la.', components: [] });
    const alvoMember = await interaction.guild.members.fetch(userId).catch(() => null);
    if (!alvoMember) return interaction.update({ content: '❌ Não encontrei esse membro.', components: [] });
    if (alvoMember.voice.channelId !== channel.id) {
      return interaction.update({ content: '❌ Só podes transferir a call para alguém que esteja dentro dela.', components: [] });
    }
    await db.prepare('UPDATE tempvoice_channels SET owner_id = ? WHERE channel_id = ?').run(userId, channel.id);
    await channel.permissionOverwrites.edit(userId, { ViewChannel: true, Connect: true, ManageChannels: true }).catch(() => {});
    return interaction.update({ content: `🔁 A call foi transferida! <@${userId}> é agora o dono.`, components: [] });
  }
}

/** Apaga por completo o sistema de Calls Temporárias: categoria, canais e calls ativas no momento */
async function apagarSistemaTempVoice(guild, config) {
  if (!config) return;

  // Apaga todas as calls temporárias ainda ativas neste servidor
  const callsAtivas = await db.prepare('SELECT * FROM tempvoice_channels WHERE guild_id = ?').all(guild.id);
  for (const tv of callsAtivas) {
    const canal = guild.channels.cache.get(tv.channel_id);
    if (canal) await canal.delete().catch(() => {});
  }
  await db.prepare('DELETE FROM tempvoice_channels WHERE guild_id = ?').run(guild.id);
  await db.prepare('DELETE FROM tempvoice_blocked WHERE channel_id IN (SELECT channel_id FROM tempvoice_channels WHERE guild_id = ?)').run(guild.id).catch(() => {});

  // Apaga o canal "Criar Call"
  if (config.creator_channel_id) {
    const canalCriar = guild.channels.cache.get(config.creator_channel_id);
    if (canalCriar) await canalCriar.delete().catch(() => {});
  }

  // Apaga o canal "Interface"
  if (config.interface_channel_id) {
    const canalInterface = guild.channels.cache.get(config.interface_channel_id);
    if (canalInterface) await canalInterface.delete().catch(() => {});
  }

  // Apaga a categoria
  if (config.category_id) {
    const categoria = guild.channels.cache.get(config.category_id);
    if (categoria) await categoria.delete().catch(() => {});
  }
}
...
async function fecharTicket(channel, closedBy, guild, reason = null) {
  ...
  // Envia transcript ao utilizador (criador do ticket) e a todos os participantes adicionados
  (async () => {
    try {
      // Nome do tipo de ticket (ex: "Ajuda & Suporte") — vem de ticket_types via type_id.
      // Fallback: nome do painel guardado em ticket_config, e por fim "Ticket".
      const tipoTicketMsg = ticket?.type_id ? await db.prepare('SELECT label FROM ticket_types WHERE id = ?').get(ticket.type_id) : null;
      const painelTicketMsg = ticket?.panel_id ? await db.prepare('SELECT panel_name FROM ticket_config WHERE id = ?').get(ticket.panel_id) : null;
      const nomeTipo = tipoTicketMsg?.label || painelTicketMsg?.panel_name || 'Ticket';

      // Timestamp dinâmico do Discord (<t:unix:f>) — cada pessoa vê a data já convertida
      // para o seu próprio fuso horário, em vez de um texto fixo escrito no fuso do servidor.
      const createdAtISO = ticket.created_at.includes('T') ? ticket.created_at : ticket.created_at.replace(' ', 'T') + 'Z';
      const unixAbertura = Math.floor(new Date(createdAtISO).getTime() / 1000);
      const unixFecho = Math.floor(Date.now() / 1000);

      const embedUser = new EmbedBuilder()
        .setTitle('🎫 Ticket Fechado')
        .setDescription(`O teu ticket foi fechado em **${guild?.name || 'servidor'}**!`)
        .setColor(CONFIG.COR_AVISO)
        .addFields(
          {
            name: '📋 Informação do Ticket',
            value: `**Data de Abertura:** <t:${unixAbertura}:f>\n**Nome do Painel:** ${nomeTipo}\n**Nome do Ticket:** ${channel.name}`
          },
          {
            name: '🔒 Informação de Fecho',
            value: `**Fechado Por:** <@${closedBy}>\n**Data de Fecho:** <t:${unixFecho}:f>${reason ? `\n**Motivo do Fecho:** ${reason}` : ''}`
          }
        )
        .setFooter({ text: 'Se tiveres mais alguma dúvida ou preocupação, sente-te à vontade para abrir um novo ticket.' });

      // Junta o criador do ticket + todos os participantes adicionados (ticket_users),
      // sem duplicados, para todos receberem a mesma mensagem e o mesmo transcript.
      const participantes = await db.prepare('SELECT user_id FROM ticket_users WHERE ticket_id = ?').all(ticket.id);
      const idsParaEnviar = [...new Set([ticket.user_id, ...participantes.map(p => p.user_id)])];

      for (const userId of idsParaEnviar) {
        try {
          const user = await client.users.fetch(userId);
          // Gera um novo AttachmentBuilder por envio — anexos já enviados não podem ser reutilizados noutra mensagem.
          const attachmentUser = new AttachmentBuilder(buffer, {
            name: `transcript-${ticket.ticket_number}.html`
          });
          // Envia primeiro a mensagem de texto, e só depois o ficheiro do transcript,
          // para garantir que a mensagem aparece por cima e o transcript por baixo.
          await user.send({
            embeds: [embedUser]
          }).catch(() => {});
          await user.send({
    ...
    try {
      ...
      if (ticketConfig?.transcript_channel) {
        ...
        if (ch) {
          ...
          // Mensagem de texto/embed primeiro, ficheiro do transcript a seguir, numa mensagem separada.
          await ch.send({
            embeds: [embed]
          });
          await ch.send({
...
// Agrupa (debounce) várias entradas/saídas seguidas num único update por servidor,
// para reagir rápido ao membercount sem violar o limite do Discord de 2 renomeações
// de canal a cada 10 minutos. Espera 15s de "silêncio" antes de atualizar; se
// continuarem a entrar/sair membros, o timer é adiado em vez de disparar várias vezes.
const statsDebounceTimers = new Map(); // guild.id -> Timeout
function agendarAtualizacaoStats(guild) {
  const existente = statsDebounceTimers.get(guild.id);
  if (existente) clearTimeout(existente);
  const timer = setTimeout(() => {
    statsDebounceTimers.delete(guild.id);
    atualizarStats(guild).catch(() => {});
  }, 15000);
  statsDebounceTimers.set(guild.id, timer);
}
...
async function atualizarStats(guild) {
  ...
  for (const c of STATS_CANAIS_DEF) {
    ...
    if (ch && ch.name !== nome) {
      // ⚠️ O Discord só permite 2 renomeações de canal a cada 10 minutos.
      // Se isto disparar 429 é sinal de que o nome está a mudar com demasiada
      // frequência (ex: contador de membros num servidor muito ativo) — nesse
      // caso considera aumentar o intervalo do cron ou remover este canal de stats.
      try {
        await ch.setName(nome);
      } catch (err) {
        if (err?.status === 429 || err?.code === 429) {
          console.warn(`⚠️ Rate limit (429) ao renomear canal de stats "${c.key}" em ${guild.name} (${guild.id}). A saltar para o próximo servidor.`);
        } else {
          console.error(`❌ Erro ao atualizar canal de stats em ${guild.name}:`, err.message);
        }
      }
    }
  }
...
/**
 * Normaliza links de imagem comuns que, tal como colados pelo utilizador, não são
 * um link direto para o ficheiro (e por isso o Discord não os embebe).
 * Não altera links que já são diretos — só corrige casos conhecidos.
 */
function normalizarUrlImagem(u) {
  if (!urlValido(u)) return u;
  let url = u.trim();
  try {
    const parsed = new URL(url);
    const host = parsed.hostname.replace(/^www\./, '');

    // imgur.com/abc123 (página) → i.imgur.com/abc123.png (link direto para o ficheiro)
    if (host === 'imgur.com' && !parsed.pathname.match(/\.\w+$/)) {
      const id = parsed.pathname.split('/').filter(Boolean).pop();
      if (id) url = `https://i.imgur.com/${id}.png`;
    }
  } catch (_) {
    // Se não der para parsear, devolve tal como veio — urlValido já garantiu o mínimo.
  }
  return url;
}
...
function construirEmbedWelcome(config, member) {
  ...
  if (urlValido(config.welcome_thumbnail)) embed.setThumbnail(normalizarUrlImagem(config.welcome_thumbnail));
  if (urlValido(config.welcome_image)) embed.setImage(normalizarUrlImagem(config.welcome_image));
  return embed;
}
...
async function sendWelcome(member) {
  ...
  if (config.autorole) {
    ...
    if (role) await member.roles.add(role).catch(err => avisarFalhaCargo(member.guild, role, err));
  }
}

/** Constrói o embed de DM de entrada a partir da configuração da guild (usado no envio real e no preview do dashboard) */
function construirEmbedDm(config, member) {
  const titulo = substituirVariaveisWelcome(config.dm_title || '', member);
  const descricao = substituirVariaveisWelcome(config.dm_msg || 'Bem-vindo(a) {user} ao {server}!', member);
  const footer = substituirVariaveisWelcome(config.dm_footer || '', member);
  const autorNomeCfg = substituirVariaveisWelcome(config.dm_author_name || '', member);
  const autorNome = autorNomeCfg || CONFIG.BOT_NAME || client.user.username;
  const autorIconCfg = (config.dm_author_icon || '').trim();
  const autorIcon = urlValido(autorIconCfg) ? autorIconCfg : client.user.displayAvatarURL();
  const embed = new EmbedBuilder().setDescription(descricao).setColor(config.dm_color || CONFIG.COR_SUCESSO).setTimestamp();
  if (titulo) {
    embed.setTitle(titulo);
    if (urlValido(config.dm_url)) embed.setURL(config.dm_url.trim());
  }
  embed.setAuthor({ name: autorNome, iconURL: autorIcon });
  if (footer) embed.setFooter({ text: footer });
  if (urlValido(config.dm_thumbnail)) embed.setThumbnail(normalizarUrlImagem(config.dm_thumbnail));
  if (urlValido(config.dm_image)) embed.setImage(normalizarUrlImagem(config.dm_image));
  return embed;
}

/** Envia mensagem privada (DM) ao membro assim que entra no servidor (usa a mensagem marcada como ATIVA em dm_messages) */
async function sendDmJoin(member) {
  const config = await db.prepare('SELECT * FROM dm_messages WHERE guild_id = ? AND is_active = 1').get(member.guild.id);
  if (!config) return;
  const conteudoFora = substituirVariaveisWelcome(config.dm_content || '', member);
  try {
    if (config.dm_embed) {
      const embed = construirEmbedDm(config, member);
      await member.send({ content: conteudoFora || undefined, embeds: [embed] });
    } else {
      const msg = substituirVariaveisWelcome(config.dm_msg || 'Bem-vindo(a) {user} ao {server}!', member);
      await member.send(conteudoFora ? `${conteudoFora}\n${msg}` : msg);
    }
  } catch (_) {
    // Utilizador com DMs fechadas — ignora em silêncio, tal como o resto do bot faz.
  }
}

/** Constrói o embed de mensagem de saída a partir da configuração da guild (usado no envio real e no preview do dashboard) */
function construirEmbedLeave(config, member) {
  const titulo = substituirVariaveisWelcome(config.leave_title || '', member);
  const descricao = substituirVariaveisWelcome(config.leave_msg || '{user} saiu de {server}.', member);
  const footer = substituirVariaveisWelcome(config.leave_footer || '', member);
  const autorNomeCfg = substituirVariaveisWelcome(config.leave_author_name || '', member);
  const autorNome = autorNomeCfg || CONFIG.BOT_NAME || client.user.username;
  const autorIconCfg = (config.leave_author_icon || '').trim();
  const autorIcon = urlValido(autorIconCfg) ? autorIconCfg : client.user.displayAvatarURL();
  const embed = new EmbedBuilder().setDescription(descricao).setColor(config.leave_color || CONFIG.COR_ERRO).setTimestamp();
  if (titulo) {
    embed.setTitle(titulo);
    if (urlValido(config.leave_url)) embed.setURL(config.leave_url.trim());
  }
  embed.setAuthor({ name: autorNome, iconURL: autorIcon });
  if (footer) embed.setFooter({ text: footer });
  if (urlValido(config.leave_thumbnail)) embed.setThumbnail(normalizarUrlImagem(config.leave_thumbnail));
  if (urlValido(config.leave_image)) embed.setImage(normalizarUrlImagem(config.leave_image));
  return embed;
}

/** Envia o anúncio de saída no canal configurado (usa a mensagem marcada como ATIVA em leave_messages) */
async function sendLeaveMessage(member) {
  const config = await db.prepare('SELECT * FROM leave_messages WHERE guild_id = ? AND is_active = 1').get(member.guild.id);
  if (!config?.leave_channel) return;
  const channel = member.guild.channels.cache.get(config.leave_channel);
  if (!channel) return;
  const conteudoFora = substituirVariaveisWelcome(config.leave_content || '', member);
  if (config.leave_embed) {
    const embed = construirEmbedLeave(config, member);
    await channel.send({ content: conteudoFora || undefined, embeds: [embed] }).catch(() => {});
  } else {
    const msg = substituirVariaveisWelcome(config.leave_msg || '{user} saiu de {server}.', member);
    await channel.send(conteudoFora ? `${conteudoFora}\n${msg}` : msg).catch(() => {});
  }
}

/**
 * Avisa no canal de logs (tipo 'member_role', já existente e configurável na dashboard)
 * quando o bot falha a atribuir/remover um cargo — normalmente por hierarquia
 * (o cargo do bot está abaixo do cargo em questão) ou por falta da permissão "Gerir Cargos".
 * Antes isto era silenciado (.catch(() => {})), por isso as falhas passavam despercebidas.
 */
async function avisarFalhaCargo(guild, role, err) {
  try {
    const motivo = err?.code === 50013 || /Missing Permissions/i.test(err?.message || '') ? `O cargo do bot está abaixo de ${role}, ou falta a permissão "Gerir Cargos". Move o cargo do bot para cima na lista de cargos do servidor.` : err?.message || 'Erro desconhecido.';
    const embed = embedPadrao('⚠️ Falha ao atribuir/remover cargo (AutoRole/Exclusividade)', `**Cargo:** ${role}\n**Motivo:** ${motivo}`, CONFIG.COR_AVISO);
    await sendLogTyped(guild, 'member_role', embed);
  } catch (_) {}
}
...
async function aplicarExclusividadeCargos(member) {
  ...
  for (const roleId of paraRemover) {
    ...
    if (role) await member.roles.remove(role).catch(err => avisarFalhaCargo(member.guild, role, err));
  }
}
...
async function verificarTrapChannel(message) {
  ...
  if (await isImune(message.member)) return false;
...
// ============================
// NORMALIZAÇÃO DE TEXTO (deteção de palavras bloqueadas "disfarçadas")
// Ex: "merd4", "m3rda", "m.e.r.d.a", "mérrrda" → tudo normaliza para "merda"
// 100% local, sem chamadas externas — não gasta dinheiro nem afeta o deploy.
// ============================
const LEET_MAP = {
  '4': 'a', '@': 'a',
  '3': 'e',
...
// Caracteres de outros alfabetos/scripts visualmente parecidos com letras latinas,
// usados às vezes para tentar enganar o filtro (ex: "а" cirílico em vez de "a" latino).
const LOOKALIKE_MAP = {
  'а': 'a', 'А': 'a', // cirílico a
  'е': 'e', 'Е': 'e', // cirílico e
  'і': 'i', 'І': 'i', 'ı': 'i', // cirílico/turco i
  'о': 'o', 'О': 'o', // cirílico o
  'р': 'p', 'Р': 'p', // cirílico r/p
  'с': 'c', 'С': 'c', // cirílico c
  'у': 'y', 'У': 'y', // cirílico u/y
  'ѕ': 's', // cirílico s
  'ν': 'v', // grego nu
  'ο': 'o', // grego omicron
  'α': 'a', // grego alfa
...
function normalizarTexto(texto) {
  ...
  // Remove acentos (á→a, é→e, ã→a, etc.)
  t = t.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  // Substitui caracteres de outros alfabetos que se parecem com letras latinas
  t = t.replace(/./g, ch => LOOKALIKE_MAP[ch] ?? ch);
  // Substitui leetspeak comum por letras
  t = t.replace(/[4@31!|056$789]/g, ch => LEET_MAP[ch] ?? ch);
  // Remove qualquer caractere que não seja letra ou número (apanha separadores exóticos:
  // pontos, espaços, hífens, underscores, emojis, zero-width chars, etc.)
  t = t.replace(/[^\p{L}\p{N}]/gu, '');
  // Reduz letras repetidas seguidas (ex: "merrrda" → "merda")
  t = t.replace(/(.)\1+/g, '$1');
  return t;
}

/** Verifica se alguma palavra bloqueada aparece na mensagem, mesmo disfarçada */
function encontrarPalavraBloqueada(conteudo, blockedWords) {
  const conteudoNormalizado = normalizarTexto(conteudo);
  const conteudoLowerOriginal = conteudo.toLowerCase();
  return blockedWords.find(p => {
    const palavraNormalizada = normalizarTexto(p);
    if (!palavraNormalizada) return false;
    // Verifica tanto no texto normalizado (apanha disfarces) como no original (evita falsos positivos estranhos)
    return conteudoNormalizado.includes(palavraNormalizada) || conteudoLowerOriginal.includes(p.toLowerCase());
  });
}

// ============================
// AUTOMOD IA — moderação de conteúdo via API externa
// ============================
// A key é única e global (variáveis de ambiente no Render — ver CONFIG.AI_API_KEY).
// Só é chamada para mensagens que já passaram pelo filtro local (poupa custos).
const AI_SEVERITY_ORDER = { LOW: 1, MEDIUM: 2, HIGH: 3 };

// ── Limites de segurança (protegem o Render free tier e a fatura da API de IA) ──
// Tudo em memória, sem custo nenhum de infraestrutura.
const AI_MOD_LIMITS = {
  MIN_CHARS: 4,              // mensagens mais curtas que isto não vão à IA (raramente compensa)
  MAX_POR_SERVIDOR_MINUTO: 15, // teto de chamadas à IA por servidor, por minuto
  MAX_GLOBAL_MINUTO: 60        // teto de chamadas à IA para o bot inteiro, por minuto (trava a fatura)
};

// Contadores em memória — resetam a cada minuto. Não precisam de base de dados.
const aiModCounters = {
  porServidor: new Map(),  // guildId -> { count, resetAt }
  global: { count: 0, resetAt: 0 }
};

/**
 * Verifica se ainda há "orçamento" para chamar a IA nesta mensagem, sem ultrapassar os limites.
 * Devolve true se pode chamar (e já reserva o slot), false se deve saltar a chamada.
 */
function podeChamarIA(guildId) {
  const agora = Date.now();

  // Reset do contador global se o minuto já passou
  if (agora > aiModCounters.global.resetAt) {
    aiModCounters.global.count = 0;
    aiModCounters.global.resetAt = agora + 60000;
  }
  if (aiModCounters.global.count >= AI_MOD_LIMITS.MAX_GLOBAL_MINUTO) return false;

  // Reset do contador deste servidor se o minuto já passou
  let porServidor = aiModCounters.porServidor.get(guildId);
  if (!porServidor || agora > porServidor.resetAt) {
    porServidor = { count: 0, resetAt: agora + 60000 };
    aiModCounters.porServidor.set(guildId, porServidor);
  }
  if (porServidor.count >= AI_MOD_LIMITS.MAX_POR_SERVIDOR_MINUTO) return false;

  // Reserva o slot em ambos os contadores
  aiModCounters.global.count++;
  porServidor.count++;
  return true;
}

const AI_MOD_SYSTEM_PROMPT = `Analisas mensagens de um servidor de Discord em português (PT-PT) para deteção de conteúdo impróprio. Aplica estas regras de forma consistente e literal, mensagem a mensagem — não suavizes a classificação por a frase "parecer simpática" ou por achares que é brincadeira entre amigos, já que não tens contexto da relação entre os utilizadores.

REGRAS DE CLASSIFICAÇÃO (aplica sempre a mais grave que se enquadre):

1. HIGH — discurso de ódio (insultos por raça, etnia, orientação sexual, religião, deficiência, etc.), ameaças diretas ou veladas, assédio persistente, insultos diretos e agressivos a uma pessoa específica ("és um/a [insulto pesado]", "vai-te foder"). Para classificares como ameaça, a mensagem tem de ser inequívoca e escrita de forma clara — NÃO adivinhes significados de abreviaturas, siglas ou palavras ambíguas/incompletas (ex.: "mrr", "vlw", "tds") para as encaixares numa ameaça. Se só conseguires justificar a ameaça assumindo que uma abreviatura quer dizer algo grave, isso NÃO é uma ameaça — classifica como violacao: false ou, no máximo, LOW.

2. MEDIUM — calão vulgar/sexual EXPLÍCITO usado no sentido literal (não idiomático, ver exceções abaixo): "pila", "cabrão", "puta" (sempre, independentemente do contexto), termos sexuais explícitos, e variações disfarçadas destes. Aplica-se mesmo que a palavra apareça sozinha ou sem intenção agressiva clara. Nunca dês benefício da dúvida a um significado alternativo inofensivo (ex.: "pila" como gíria antiga para dinheiro, ou "puta" como "rapariga") — assume sempre o sentido vulgar para "puta".

EXCEÇÃO IMPORTANTE — apenas para "puto" (masculino): em português (sobretudo PT-PT), "puto" é usado coloquialmente e de forma completamente inofensiva para dizer "rapaz"/"criança" (ex.: "o puto foi à escola", "aquele puto é fixe", "és só um puto"). Este uso NÃO é calão vulgar e deve ser classificado como violacao: false, EXCETO quando usado claramente como insulto sexual dirigido a alguém. Esta exceção aplica-se SÓ à forma masculina "puto" — a forma feminina "puta" continua a ser sempre classificada como MEDIUM (calão vulgar), independentemente do contexto.

ATENÇÃO a palavras com duplo sentido dependendo do contexto — nestes casos o contexto É relevante, ao contrário da regra geral acima:

Em português (PT-PT e PT-BR), muitos palavrões funcionam sobretudo como INTERJEIÇÕES ou intensificadores, não como insultos a sério — é uma característica muito comum da língua. Usa este raciocínio geral, não apenas para os exemplos listados:

- Se o palavrão é usado como DESABAFO/REAÇÃO a algo que correu mal, surpresa, dor, frustração ("caralho, esqueci-me", "porra, doeu", "merda, perdi o autocarro") ou como INTENSIFICADOR/EXPRESSÃO IDIOMÁTICA de quantidade/ênfase ("e o caralho", "para caralho", "que caralho") → severidade LOW. Isto inclui o palavrão usado SOZINHO como mera exclamação sem alvo ("caralho!", "porra!", "merda!", "fuck", "shit").
- Se o palavrão é usado para ELOGIAR ou como gíria positiva ("sou foda", "isto é do caralho" no sentido de "é ótimo") → violacao: false.
- Só sobe a MEDIUM quando o calão é usado no sentido sexual/vulgar explícito e literal (não idiomático nem interjeição solta).
- Só sobe a HIGH quando está claramente dirigido a insultar/atacar uma pessoa específica ("vai para o caralho", "és um caralho", "fuck you", "seu fucking idiota", "vai-te foder").

Resumindo o princípio: o que importa não é a palavra em si, mas SE tem alvo (uma pessoa) e SE é usada no sentido literal/vulgar ou apenas como forma de falar/desabafar. Aplica este raciocínio a qualquer calão que reconheças com este padrão de uso duplo (ex.: caralho, porra, merda, foda, fuck, shit), mesmo que não esteja explicitamente listado.

3. LOW — insultos leves, brincadeira ou linguagem atrevida SEM palavrão explícito: xingamentos brandos como "burro", "banana", "parvo", "idiota", abreviações suavizadas tipo "fdd"/"fds" (sem escrever o palavrão completo), sarcasmo leve, indiretas educadas, expressões mal-educadas mas comuns tipo "cala a boca", "cala-te", "sai daqui", "chateia-me". Isto aplica-se mesmo quando dirigido a alguém ("és burro", "cala a boca") — só sobe a HIGH se for claramente agressivo/persistente/com intenção de ferir a sério, não uma piada, indireta ou brincadeira pontual entre utilizadores.

4. NENHUMA (violacao: false) — mensagens sem qualquer calão, insulto ou conteúdo impróprio, incluindo auto-depreciação leve ("sou burro", "sou banana") ou brincadeiras claramente amigáveis.

Exemplos:
- "o puto foi à escola" / "aquele puto é fixe" / "és só um puto" → violacao: false, categoria: "nenhuma", severidade: "LOW"
- "puta" (sozinha, ou qualquer frase que a contenha) → violacao: true, categoria: "palavrao", severidade: "MEDIUM"
- "sua puta" / "vai-te foder sua puta" (insulto sexual dirigido a alguém) → violacao: true, categoria: "insulto", severidade: "HIGH"
- "pila" (sozinha) → violacao: true, categoria: "palavrao", severidade: "MEDIUM"
- "pika" (variante disfarçada de "pila") → violacao: true, categoria: "palavrao", severidade: "MEDIUM"
- "pila gorda" → violacao: true, categoria: "palavrao", severidade: "MEDIUM"
- "sou foda" / "isto é foda" → violacao: false, categoria: "nenhuma", severidade: "LOW"
- "foda-se" (dito ao ar, sem alvo) → violacao: true, categoria: "palavrao", severidade: "LOW"
- "vai-te foder" (dirigido a alguém) → violacao: true, categoria: "insulto", severidade: "HIGH"
- "caralho!" / "porra!" (exclamação isolada, sem alvo) → violacao: true, categoria: "palavrao", severidade: "LOW"
- "sem contar com as gemas e o caralho" (expressão idiomática, "e tudo o mais") → violacao: true, categoria: "palavrao", severidade: "LOW"
- "vai para o caralho" / "és um caralho" (dirigido a alguém) → violacao: true, categoria: "insulto", severidade: "HIGH"
- "fuck, esqueci-me da chave" / "ah shit" (frustração, não dirigida a ninguém) → violacao: true, categoria: "palavrao", severidade: "LOW"
- "fuck you" / "seu fucking idiota" (dirigido a alguém como insulto) → violacao: true, categoria: "insulto", severidade: "HIGH"
- "sou fdd" / "sou burro" / "és banana" → violacao: true, categoria: "insulto", severidade: "LOW"
- "bom dia pessoal" → violacao: false, categoria: "nenhuma", severidade: "LOW"
- "vais ver e vais mrr" (abreviatura ambígua, não é claramente uma ameaça de morte escrita por extenso) → violacao: false, categoria: "nenhuma", severidade: "LOW"

Deteta também variações disfarçadas (leetspeak, espaços, símbolos, letras repetidas, carateres de outros alfabetos parecidos com letras latinas). Uma palavra proibida disfarçada conta na mesma como se estivesse escrita normalmente. NÃO aplica esta deteção de disfarce a termos comuns de moderação/gíria de Discord como "ban", "banido", "banir", "kick", "kickado", "mute", "mutado", "warn", "advertência" — estas são palavras legítimas e comuns em português informal e em contexto de Discord, não são disfarces de palavrões, e nunca devem gerar violação por si só.

Responde APENAS com um objeto JSON, sem texto à volta, no formato exato:
{"violacao": true|false, "categoria": "discurso_odio|insulto|ameaca|sexual|assedio|palavrao|outro|nenhuma", "severidade": "LOW|MEDIUM|HIGH", "motivo": "explicação curta em português (1 frase)"}
Se a mensagem for normal/inofensiva, devolve {"violacao": false, "categoria": "nenhuma", "severidade": "LOW", "motivo": "Mensagem sem problemas."}`;

/** Chama a API de IA configurada globalmente (CONFIG.AI_API_KEY / AI_PROVIDER / AI_MODEL) e devolve a classificação, ou null se falhar/não configurado */
async function classificarMensagemIA(conteudo) {
  if (!CONFIG.AI_API_KEY) return null;
  if (!conteudo || !conteudo.trim()) return null;

  try {
    if (CONFIG.AI_PROVIDER === 'anthropic') {
      const modelo = CONFIG.AI_MODEL || 'claude-haiku-4-5-20251001';
      const resp = await axios.post('https://api.anthropic.com/v1/messages', {
        model: modelo,
        max_tokens: 200,
        temperature: 0,
        system: AI_MOD_SYSTEM_PROMPT,
      ...
      const texto = resp.data?.content?.find(c => c.type === 'text')?.text || '';
      return JSON.parse(texto.trim().replace(/^```json\s*|\s*```$/g, ''));
    } else if (CONFIG.AI_PROVIDER === 'gemini') {
      // Google Gemini — tem free tier real (sem cartão de crédito), ideal para começar sem custos.
      const modelo = CONFIG.AI_MODEL || 'gemini-3.5-flash-lite';
      const resp = await axios.post(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelo}:generateContent`,
        {
          contents: [{ role: 'user', parts: [{ text: conteudo }] }],
          systemInstruction: { parts: [{ text: AI_MOD_SYSTEM_PROMPT }] },
          generationConfig: {
            maxOutputTokens: 200,
            temperature: 0,
      ...
      // OpenAI (default)
      const modelo = CONFIG.AI_MODEL || 'gpt-4o-mini';
    ...
    console.error('❌ Erro ao chamar API de moderação IA:', e.response?.data || e.message);
    return null;
  }
}

/**
 * Regista uma nova advertência de IA para o utilizador, aplica timeout se atingir o limite,
 * e envia os embeds de log (mensagem removida + castigo aplicado, se aplicável).
 */
async function aplicarModeracaoIA(message, classificacao, aiConfig) {
  ...
  // Reset automático por inatividade: se passou tempo suficiente desde a última
  // advertência (configurável por servidor), zera o contador antes de somar a nova.
  const resetDias = parseInt(aiConfig.warning_reset_days) || 0;
  if (resetDias > 0) {
    const existente = await db.prepare('SELECT count, updated_at FROM ai_warnings WHERE guild_id = ? AND user_id = ?').get(guildId, userId);
    if (existente && existente.count > 0 && existente.updated_at) {
      const ultimaData = new Date(existente.updated_at + 'Z'); // trata como UTC (CURRENT_TIMESTAMP do SQLite é UTC)
      const diasPassados = (Date.now() - ultimaData.getTime()) / (1000 * 60 * 60 * 24);
      if (diasPassados >= resetDias) {
        await db.prepare('UPDATE ai_warnings SET count = 0 WHERE guild_id = ? AND user_id = ?').run(guildId, userId).catch(() => {});
      }
    }
  }

  // Incrementa contagem de advertências
  await db.prepare(`
    INSERT INTO ai_warnings (guild_id, user_id, count, updated_at)
    VALUES (?, ?, 1, CURRENT_TIMESTAMP)
    ON CONFLICT (guild_id, user_id) DO UPDATE SET count = count + 1, updated_at = CURRENT_TIMESTAMP
  `).run(guildId, userId).catch(async () => {
    // fallback caso o driver não suporte ON CONFLICT nesta sintaxe
    const existente = await db.prepare('SELECT count FROM ai_warnings WHERE guild_id = ? AND user_id = ?').get(guildId, userId);
    if (existente) {
      await db.prepare('UPDATE ai_warnings SET count = count + 1, updated_at = CURRENT_TIMESTAMP WHERE guild_id = ? AND user_id = ?').run(guildId, userId);
    } else {
      await db.prepare('INSERT INTO ai_warnings (guild_id, user_id, count) VALUES (?, ?, 1)').run(guildId, userId);
    }
  });

  const row = await db.prepare('SELECT count FROM ai_warnings WHERE guild_id = ? AND user_id = ?').get(guildId, userId);
  const contagemAtual = row?.count || 1;
  const maxWarnings = aiConfig.max_warnings || 3;

  // Log: mensagem removida
  if (aiConfig.log_channel) {
    const ch = message.guild.channels.cache.get(aiConfig.log_channel);
    if (ch) {
      const embed = new EmbedBuilder()
        .setTitle('🛡️ AutoMod IA: Mensagem Removida')
        .addFields(
          { name: 'Autor', value: `<@${userId}> (\`${userId}\`)` },
          { name: 'Canal', value: `<#${message.channel.id}>` },
          { name: 'Categoria', value: `\`${classificacao.categoria || 'outro'}\`` },
          { name: 'Severidade', value: `\`${classificacao.severidade || 'MEDIUM'}\`` },
          { name: 'Advertências Acumuladas', value: `\`${contagemAtual} / ${maxWarnings}\`` },
          { name: 'Motivo', value: `\`${classificacao.motivo || '—'}\`` },
          { name: 'Conteúdo Original', value: '```' + (message.content || '').slice(0, 900) + '```' }
        )
  ...
  // Atinge o limite → timeout automático e reset da contagem
  if (contagemAtual >= maxWarnings) {
    const minutos = aiConfig.timeout_minutes || 10;
    await message.member?.timeout(minutos * 60 * 1000, 'AutoMod IA: Limite de advertências atingido').catch(() => {});
    await db.prepare('UPDATE ai_warnings SET count = 0 WHERE guild_id = ? AND user_id = ?').run(guildId, userId).catch(() => {});
    ...
    if (aiConfig.log_channel) {
      ...
      if (ch) {
        const embed = new EmbedBuilder()
          .setTitle('🚨 AutoMod IA: Castigo Aplicado Automático')
          .setDescription(`O usuário <@${userId}> atingiu o limite de ${maxWarnings} advertências e foi suspenso temporariamente por ${minutos} minutos.`)
          .setColor(CONFIG.COR_ERRO)
          .setTimestamp();
        await ch.send({ embeds: [embed] }).catch(() => {});
      }
    ...
    // Aviso rápido e discreto ao autor, apagado pouco depois (consistente com o resto do bot)
    const conteudoBloqueado = (message.content || '').slice(0, 300).replace(/`/g, "'");
    const warn = await message.channel.send(`<@${userId}> ⚠️ A tua mensagem foi removida pelo AutoMod IA (${contagemAtual}/${maxWarnings} advertências). A mensagem bloqueada foi "${conteudoBloqueado}".`).catch(() => null);
...
async function verificarSpam(message) {
  ...
  if (await isImune(message.member)) return;
  ...
  // Deteta também variações disfarçadas: "merd4", "m3rda", "m.e.r.d.a", "mérrrda", etc.
  const blockedWords = JSON.parse(config.blocked_words || '[]');
  if (blockedWords.length) {
    const palavraEncontrada = encontrarPalavraBloqueada(message.content, blockedWords);
    if (palavraEncontrada) {
      await message.delete().catch(() => {});
      const warn = await message.channel.send(`<@${message.author.id}> ⚠️ A tua mensagem continha uma palavra/expressão bloqueada: \`${palavraEncontrada}\``);
      setTimeout(() => warn.delete().catch(() => {}), 5000);
      return;
    }
  }

  // AutoMod IA — só corre se o filtro local acima não apanhou nada (poupa chamadas/custo).
  // A API key é única e global (definida no Render), cada servidor só liga/desliga e ajusta parâmetros.
  // Limites de segurança (tamanho mínimo + rate limit por servidor + rate limit global) protegem
  // o Render free tier e a fatura da API de IA — se algum limite for atingido, salta-se a IA em
  // silêncio e o filtro local acima continua a proteger o servidor normalmente.
  try {
    const aiConfig = await db.prepare('SELECT * FROM ai_moderation_config WHERE guild_id = ? AND enabled = 1').get(message.guild.id);
    const conteudoTrim = message.content ? message.content.trim() : '';
    if (
      aiConfig &&
      CONFIG.AI_API_KEY &&
      conteudoTrim.length >= AI_MOD_LIMITS.MIN_CHARS &&
      podeChamarIA(message.guild.id)
    ) {
      const classificacao = await classificarMensagemIA(message.content);
      if (classificacao && classificacao.violacao) {
        const minSeveridade = AI_SEVERITY_ORDER[aiConfig.min_severity || 'MEDIUM'] || 2;
        const severidadeMsg = AI_SEVERITY_ORDER[classificacao.severidade] || 1;
        if (severidadeMsg >= minSeveridade) {
          await aplicarModeracaoIA(message, classificacao, aiConfig);
          return;
        }
      }
    }
  } catch (e) {
    console.error('❌ Erro no AutoMod IA:', e.message, e.stack);
  }
...
// ── Sugestões ──
new SlashCommandBuilder().setName('sugerir').setDescription('Submete uma sugestão').addStringOption(o => o.setName('sugestao').setDescription('A tua sugestão').setRequired(true).setMaxLength(1000)), new SlashCommandBuilder().setName('sugestao-responder').setDescription('Responde a uma sugestão (aprovar/rejeitar)').setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages).addIntegerOption(o => o.setName('id').setDescription('Número da sugestão (o # mostrado no embed, ex: #3)').setRequired(true)).addStringOption(o => o.setName('acao').setDescription('Ação').setRequired(true).addChoices({
  name: '✅ Aprovar',
  value: 'approve'
}, {
  name: '❌ Rejeitar',
...
// ── Calls Temporárias ──
new SlashCommandBuilder().setName('setup-calls').setDescription('Cria o sistema de Calls Temporárias (categoria + canal para criar calls)').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addStringOption(o => o.setName('nome-padrao').setDescription('Nome padrão das calls criadas. Usa {user} para o nome de quem criou (padrão: 🔊 Call de {user})').setRequired(false)).addIntegerOption(o => o.setName('limite-padrao').setDescription('Limite de utilizadores padrão nas calls novas (0 = sem limite)').setRequired(false).setMinValue(0).setMaxValue(99)),
new SlashCommandBuilder().setName('calls-remover').setDescription('Remove o sistema de Calls Temporárias deste servidor (apaga a categoria e os canais)').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
// ── Help ──
...
// Ficheiro onde guardamos um "hash" (impressão digital) dos comandos já
// registados. Assim, em arranques/restarts seguidos (ex: Render a reiniciar
// o serviço várias vezes seguidas) só voltamos a fazer PUT à API do Discord
// se os comandos realmente mudaram — evita gastar rate limit à toa e reduz
// bastante o risco de 429 logo no arranque.
const CAMINHO_HASH_COMANDOS = path.join(__dirname, '.comandos-hash.json');

function calcularHashComandos(lista) {
  const crypto = require('crypto');
  return crypto.createHash('sha256').update(JSON.stringify(lista)).digest('hex');
}

async function registarComandos() {
  try {
    const corpo = commands.map(c => c.toJSON());
    const hashAtual = calcularHashComandos(corpo);

    let hashAnterior = null;
    try {
      hashAnterior = JSON.parse(fs.readFileSync(CAMINHO_HASH_COMANDOS, 'utf8')).hash;
    } catch (e) { /* primeiro arranque, ou ficheiro ainda não existe — ignora */ }

    if (hashAnterior === hashAtual) {
      console.log(`✅ Comandos slash globais já estão atualizados (sem alterações) — a saltar registo para poupar rate limit.`);
      return;
    }
    ...
    try {
      fs.writeFileSync(CAMINHO_HASH_COMANDOS, JSON.stringify({ hash: hashAtual }));
    } catch (e) {
      console.error('⚠️ Não foi possível guardar o hash dos comandos (não é crítico):', e.message);
    }
  } catch (err) {
...
async function sincronizarComandosEmbedTodosServidores() {
  try {
    ...
    // Intervalo entre cada servidor para não exceder o rate limit do Discord ao
    // registar comandos (PUT de application commands) — sem isto, com muitos
    // servidores o bot leva 429 logo no arranque.
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    // ── USER SELECT MENUS (adicionar/remover pessoas do ticket) ──
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'ticket-setup') {
    ...
    // Este comando configura sempre o Painel 1/10. Para geres múltiplos painéis usa o dashboard.
    const painel1 = await db.prepare('SELECT id FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id LIMIT 1').get(guild.id);
    const rolesJson = JSON.stringify(suporte?.id ? [suporte.id] : []);
    if (painel1) {
      await db.prepare(`
        UPDATE ticket_config SET category_id=?, log_channel=?, support_role=?, support_roles=?, transcript_channel=?, max_tickets=?, welcome_msg=?, enabled=1
        WHERE id=?
      `).run(categoria.id, log?.id || null, suporte?.id || null, rolesJson, transcripts?.id || null, max, mensagem, painel1.id);
    } else {
      await db.prepare(`
        INSERT INTO ticket_config (guild_id, panel_num, panel_name, category_id, log_channel, support_role, support_roles, transcript_channel, max_tickets, welcome_msg, enabled)
        VALUES (?, 1, 'Painel 1/10', ?, ?, ?, ?, ?, ?, ?, 1)
      `).run(guild.id, categoria.id, log?.id || null, suporte?.id || null, rolesJson, transcripts?.id || null, max, mensagem);
    }
    const embed = embedPadrao('✅ Sistema de Tickets Configurado', [`**Categoria:** ${categoria}`, `**Log:** ${log || 'Não definido'}`, `**Suporte:** ${suporte || 'Não definido'}`, `**Transcripts:** ${transcripts || 'Não definido'}`, `**Máx. Tickets/Utilizador:** ${max}`, '', 'ℹ️ Isto configura o **Painel 1/10**. Para criar mais painéis usa o dashboard web.'].join('\n'), CONFIG.COR_SUCESSO);
    return interaction.editReply({
      embeds: [embed]
    });
  }
  if (commandName === 'ticket-painel') {
    ...
    const ticketConfig = await obterTicketConfig(guild.id);
    ...
    const components = await montarComponentesPainelTicket(guild.id, ticketConfig);
    const msg = await canal.send({
      embeds: [embed],
      components
    });

    // Guarda ID da mensagem do painel (por painel, usando o id interno do ticket_config)
    await db.prepare(`
      UPDATE ticket_config SET panel_msg_id=?, panel_channel_id=? WHERE id=?
    `).run(msg.id, canal.id, ticketConfig.id);
    return interaction.editReply({
      content: `✅ Painel de tickets criado em ${canal}! (${ticketConfig.panel_name || 'Painel 1/10'})`
    });
  }
  if (commandName === 'ticket-tipo') {
    ...
    // Este comando adiciona sempre ao Painel 1/10. Para outros painéis usa o dashboard.
    const painel1 = await obterTicketConfig(guild.id);
    await db.prepare(`
      INSERT INTO ticket_types (guild_id, panel_id, label, description, emoji, order_num)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(guild.id, painel1?.id || null, nome, descricao, emoji, tipos.c);
    return interaction.reply({
      content: `✅ Tipo de ticket **${emoji} ${nome}** adicionado ao ${painel1?.panel_name || 'Painel 1/10'}! Recria o painel com \`/ticket-painel\`.`,
      ephemeral: true
    });
  }
  ...
  if (commandName === 'ticket-help') {
    const embedHelp = embedPadrao(
      '📘 Como configurar o sistema de tickets',
      'Segue estes passos, por ordem, para teres o teu painel de tickets pronto a usar. Todos os comandos abaixo são apenas para administradores.'
    );
    embedHelp.addFields(
      {
        name: '1️⃣ /ticket-setup',
        value: 'Configura a base do sistema: escolhe a **categoria** onde os canais dos tickets vão ser criados, e opcionalmente o **canal de logs**, o **cargo de suporte**, o **canal de transcripts**, o **máximo de tickets por utilizador** e uma **mensagem de boas-vindas** personalizada (podes usar `{user}` e `{ticket}`).'
      },
      {
        name: '2️⃣ /ticket-tipo',
        value: 'Adiciona os tipos de ticket que vão aparecer no menu (ex: Suporte, Denúncias, Compras). Repete este comando uma vez por tipo, definindo **nome**, **descrição** e **emoji**.'
      },
      {
        name: '3️⃣ /ticket-tipos-lista',
        value: 'Mostra a lista de tipos de ticket já configurados, com o respetivo **ID** — útil para depois os removeres se precisares.'
      },
      {
        name: '4️⃣ /ticket-tipo-remover',
        value: 'Remove um tipo de ticket indicando o **ID** obtido em `/ticket-tipos-lista`. Depois de remover, recria o painel para o menu atualizar.'
      },
      {
        name: '5️⃣ /ticket-painel',
        value: 'Cria o painel de tickets no **canal** que escolheres, com **título** e **descrição** opcionais. É aqui que os utilizadores vão clicar para abrir tickets.'
      },
      {
        name: '➕ /ticket-criar',
        value: 'Cria um ticket manualmente, sem passar pelo painel — útil para testes ou casos especiais.'
      },
      {
        name: '➕ /fechar',
        value: 'Fecha o ticket atual (usa-se dentro do canal do próprio ticket), com um **motivo** opcional que é enviado ao utilizador junto com o transcript.'
      }
    );
    embedHelp.setFooter({ text: '💡 Dica: segue a ordem 1 → 2 → 5 para teres o painel completo mais rápido.' });
    return interaction.reply({
      embeds: [embedHelp],
      ephemeral: true
    });
  ...
  if (commandName === 'ranking-staff') {
    ...
    const ranking = await getRankingStaff(guild.id);
  ...
  if (commandName === 'welcome-setup') {
    ...
    invalidarGuildConfigCache(guild.id);
    return interaction.reply({
  ...
  if (commandName === 'sugerir') {
    ...
    const tipo = await db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? AND enabled = 1 ORDER BY id LIMIT 1').get(guild.id);
    if (!tipo) return interaction.editReply({
      content: '❌ O sistema de sugestões ainda não foi configurado neste servidor. Um administrador tem de o configurar no dashboard.'
    });
    const conteudo = options.getString('sugestao');
    const canal = guild.channels.cache.get(tipo.channel_id);
    if (!canal) return interaction.editReply({
  ...
  if (commandName === 'painel-lista') {
    ...
    const linhasPaineis = await Promise.all(paineis.map(async p => {
    ...
    const embed = new EmbedBuilder().setTitle('📋 Painéis de Informação').setColor(CONFIG.COR_PRINCIPAL).setDescription(linhasPaineis.join('\n'));
    return interaction.reply({
      embeds: [embed],
      ephemeral: true
    });
  ...
  if (commandName === 'giveaway-reroll') {
    ...
    const vencedores = await sortearVencedores(gw.id, gw.vencedores);
  ...
  if (commandName === 'giveaway-lista') {
    ...
    const linhas = await Promise.all(giveaways.map(async gw => {
      const total = (await db.prepare('SELECT COUNT(*) AS c FROM giveaway_entries WHERE giveaway_id = ?').get(gw.id)).c;
  ...
  if (commandName === 'fechar') {
    const ticket = await db.prepare('SELECT * FROM tickets WHERE channel_id = ? AND status = ?').get(channel.id, 'open');
    if (!ticket) {
      return interaction.reply({
        content: '❌ Este comando só pode ser usado dentro de um canal de ticket aberto.',
        ephemeral: true
      });
    }
    const motivo = options.getString('motivo') || null;
    // Guarda o motivo temporariamente para usar quando a pessoa confirmar
    fecharMotivoMap.set(`${channel.id}:${user.id}`, motivo);

    const confirmEmbed = embedPadrao('🔒 Confirmar Fecho', 'Tens a certeza que queres fechar este ticket?' + (motivo ? `\n\n**Motivo:** ${motivo}` : ''), CONFIG.COR_AVISO);
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('ticket_close_confirm_cmd').setLabel('✅ Confirmar').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('ticket_close_cancel').setLabel('❌ Cancelar').setStyle(ButtonStyle.Secondary));
    return interaction.reply({
      embeds: [confirmEmbed],
      components: [row],
      ephemeral: true
  ...
  if (commandName === 'lock') {
    ...
    try {
      ...
      const textoLog = embedPadrao('🔒 Canal Trancado', `**Canal:** <#${canal.id}>\n**Moderador:** <@${user.id}>\n\nO envio de mensagens para @everyone foi bloqueado.\n\nPara desbloquear o canal use \`/unlock\` ou \`!unlock\`.`, CONFIG.COR_ERRO);
      logMod(guild.id, 'LOCK', canal.id, user.id, 'Canal trancado');
      await sendLogTyped(guild, 'lock', textoLog);
      return interaction.reply({
        content: `🔒 O canal **${canal.name}** foi trancado para destrancar use \`/unlock\` ou \`!unlock\`.`
      });
    } catch (e) {
      return interaction.reply({
  ...
  if (commandName === 'unlock') {
    ...
    try {
      ...
      const textoLog = embedPadrao('🔓 Canal Destrancado', `**Canal:** <#${canal.id}>\n**Moderador:** <@${user.id}>\n\nO envio de mensagens para @everyone foi restaurado.\n\nPara bloquear o canal use \`/lock\` ou \`!lock\`.`, CONFIG.COR_SUCESSO);
      logMod(guild.id, 'UNLOCK', canal.id, user.id, 'Canal destrancado');
      await sendLogTyped(guild, 'unlock', textoLog);
      return interaction.reply({
        content: `🔓 O canal **${canal.name}** foi destrancado para trancar use \`/lock\` ou \`!lock\`.`
      });
    } catch (e) {
      return interaction.reply({
  ...
  if (commandName === 'setup-calls') {
    await interaction.deferReply({ ephemeral: true });
    const existente = await db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ?').get(guild.id);
    if (existente && existente.enabled && guild.channels.cache.get(existente.creator_channel_id)) {
      return interaction.editReply({
        content: `⚠️ Já existe um sistema de Calls Temporárias configurado neste servidor (<#${existente.creator_channel_id}>). Usa \`/calls-remover\` primeiro se quiseres recriar.`
      });
    }
    const nomeTemplate = options.getString('nome-padrao') || '🔊 Call de {user}';
    const limitePadrao = options.getInteger('limite-padrao') ?? 0;
    try {
      const criados = await criarSistemaTempVoice(guild);
      await db.prepare(`
        INSERT INTO tempvoice_config (guild_id, category_id, creator_channel_id, interface_channel_id, interface_msg_id, name_template, default_limit, enabled)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1)
        ON CONFLICT(guild_id) DO UPDATE SET
          category_id=excluded.category_id, creator_channel_id=excluded.creator_channel_id,
          interface_channel_id=excluded.interface_channel_id, interface_msg_id=excluded.interface_msg_id,
          name_template=excluded.name_template, default_limit=excluded.default_limit, enabled=1
      `).run(guild.id, criados.categoryId, criados.creatorChannelId, criados.interfaceChannelId, criados.interfaceMsgId, nomeTemplate, limitePadrao);
      return interaction.editReply({
        content: `✅ Sistema de Calls Temporárias criado!\n📁 Categoria criada com:\n🔊 <#${criados.creatorChannelId}> · entra aqui para criares a tua call\n✨ <#${criados.interfaceChannelId}> · interface para geres a tua call`
      });
    } catch (e) {
      return interaction.editReply({ content: `❌ Não foi possível criar o sistema de Calls Temporárias: ${e.message}` });
    }
  }

  if (commandName === 'calls-remover') {
    const existente = await db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ?').get(guild.id);
    if (!existente) {
      return interaction.reply({ content: '❌ Não existe nenhum sistema de Calls Temporárias configurado neste servidor.', ephemeral: true });
    }
    await interaction.deferReply({ ephemeral: true });
    await apagarSistemaTempVoice(guild, existente);
    await db.prepare('DELETE FROM tempvoice_config WHERE guild_id = ?').run(guild.id);
    return interaction.editReply({
      content: '✅ Sistema de Calls Temporárias removido — a categoria e os canais foram apagados. Usa `/setup-calls` para criares de novo.'
    });
  }
...
async function handleButton(interaction) {
  ...
  // ── Calls Temporárias: botões da interface (#✨│interface) ──
  if (customId.startsWith('tv_')) {
    return handleTempVoiceButton(interaction, customId);
  }
  ...
  if (customId === 'ticket_claim') {
    ...
    if (!(await isEquipaAdminTicket(member, guild, ticket))) {
      return interaction.reply({
  ...
  // Confirmação de fecho vinda do comando /fechar (mantém o motivo passado no comando)
  if (customId === 'ticket_close_confirm_cmd') {
    await interaction.deferReply({
      ephemeral: true
    });
    const mapKey = `${channel.id}:${user.id}`;
    const motivo = fecharMotivoMap.get(mapKey) || null;
    fecharMotivoMap.delete(mapKey);
  ...
  if (customId === 'ticket_close_cancel') {
    // Limpa qualquer motivo pendente associado a este utilizador/canal
    fecharMotivoMap.delete(`${channel.id}:${user.id}`);
    return interaction.reply({
  ...
  if (customId === 'ticket_transcript') {
    const ticketChk = await db.prepare('SELECT * FROM tickets WHERE channel_id = ?').get(channel.id);
    if (!ticketChk || !(await isEquipaAdminTicket(member, guild, ticketChk))) {
      return interaction.reply({
        content: '❌ Não tens permissão para gerar o transcript deste ticket.',
        ephemeral: true
      });
    }
    await interaction.deferReply({
    ...
    // Envia a mensagem de texto primeiro, e o ficheiro numa mensagem separada logo a seguir.
  ...
  // ── Add User (menu de seleção de membros, sem precisar de ID) ──
  if (customId === 'ticket_adduser') {
    ...
    if (!(await isEquipaAdminTicket(member, guild, ticket))) {
      return interaction.reply({
        content: '❌ Não tens permissão para adicionar pessoas a este ticket.',
        ephemeral: true
      });
    }
    const select = new UserSelectMenuBuilder().setCustomId('ticket_adduser_select').setPlaceholder('Escolhe o membro a adicionar').setMinValues(1).setMaxValues(1);
    const row = new ActionRowBuilder().addComponents(select);
    return interaction.reply({
      content: '➕ Escolhe quem queres adicionar ao ticket:',
      components: [row],
      ephemeral: true
    });
  }

  // ── Remove User (menu de seleção de membros, sem precisar de ID) ──
  if (customId === 'ticket_removeuser') {
    ...
    if (!(await isEquipaAdminTicket(member, guild, ticket))) {
      return interaction.reply({
        content: '❌ Não tens permissão para remover pessoas deste ticket.',
        ephemeral: true
      });
    }
    const select = new UserSelectMenuBuilder().setCustomId('ticket_removeuser_select').setPlaceholder('Escolhe o membro a remover').setMinValues(1).setMaxValues(1);
    const row = new ActionRowBuilder().addComponents(select);
    return interaction.reply({
      content: '➖ Escolhe quem queres remover do ticket:',
      components: [row],
      ephemeral: true
    });
  }
  ...
  if (customId === 'ticket_rename') {
    ...
    if (!(await isEquipaAdminTicket(member, guild, ticket))) {
      return interaction.reply({
        content: '❌ Não tens permissão para renomear este ticket.',
        ephemeral: true
      });
    }
  ...
  if (customId.startsWith('sug_up_') || customId.startsWith('sug_down_')) {
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.update({
        ...
        if (e?.code !== 40060) console.error('❌ Erro ao atualizar voto de sugestão:', e.message);
      });
    } else {
      await interaction.message.edit({
        embeds: [embed],
        components: [row]
      }).catch(e => console.error('❌ Erro ao editar mensagem de sugestão:', e.message));
    }
  }
}

// ============================
// HANDLER DE USER SELECT MENUS (adicionar/remover pessoas do ticket)
// ============================
async function handleUserSelectMenu(interaction) {
  ...
  // ── Calls Temporárias: seleção de utilizador nos botões da interface ──
  if (customId.startsWith('tv_select_')) {
    return handleTempVoiceSelectMenu(interaction, customId);
  }

  // ── Adicionar utilizador ao ticket ──
  if (customId === 'ticket_adduser_select') {
    const ticketChk = await db.prepare('SELECT * FROM tickets WHERE channel_id = ?').get(channel.id);
    if (!ticketChk || !(await isEquipaAdminTicket(member, guild, ticketChk))) {
      return interaction.reply({
        content: '❌ Não tens permissão para adicionar pessoas a este ticket.',
    ...
    const membroId = values[0];
    await interaction.deferUpdate();
    try {
      const membro = await guild.members.fetch(membroId);
      ...
      const ticket = await db.prepare('SELECT * FROM tickets WHERE channel_id = ?').get(channel.id);
      if (ticket) {
        await db.prepare('INSERT OR IGNORE INTO ticket_users (ticket_id, user_id, added_by) VALUES (?,?,?)').run(ticket.id, membroId, user.id);
      }
      return interaction.editReply({
      ...
      return interaction.editReply({
        content: `❌ Não foi possível adicionar: ${e.message}`,
        components: []
      });
    }
  }
  ...
  if (customId === 'ticket_removeuser_select') {
    const ticketChk = await db.prepare('SELECT * FROM tickets WHERE channel_id = ?').get(channel.id);
    if (!ticketChk || !(await isEquipaAdminTicket(member, guild, ticketChk))) {
      return interaction.reply({
        content: '❌ Não tens permissão para remover pessoas deste ticket.',
    ...
    try {
      ...
      await channel.permissionOverwrites.delete(membroId);
      return interaction.editReply({
        content: `✅ ${membro ?? `<@${membroId}>`} removido do ticket!`,
        components: []
      });
    } catch (e) {
      return interaction.editReply({
...
async function handleModal(interaction) {
  ...
  // ── Calls Temporárias: modais da interface ──
  if (customId.startsWith('tv_modal_')) {
    return handleTempVoiceModal(interaction, customId);
  }
...
client.on(Events.GuildMemberAdd, async member => {
  // Atualiza os canais de server-stats (contagem de membros) com debounce,
  // para refletir rápido sem violar o limite de renomeação de canais do Discord.
  agendarAtualizacaoStats(member.guild);
  ...
  await sendWelcome(member);
  await sendDmJoin(member);
  // AutoRole (novo — multi-cargo, aba "Cargos" do dashboard): corre sempre,
  // mesmo que não haja nenhuma mensagem de boas-vindas ativa configurada.
  // Antes estava dentro do sendWelcome e só corria se houvesse welcome + canal definidos.
  await aplicarAutoRole(member);
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
  ...
  // ── Calls Temporárias: entrou no canal "Criar Call" → cria a call dele ──
  if (newState.channelId && newState.channelId !== oldState.channelId) {
    try {
      const tvConfig = await db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ? AND enabled = 1').get(guild.id);
      if (tvConfig && newState.channelId === tvConfig.creator_channel_id) {
        const nome = (tvConfig.name_template || '🔊 Call de {user}').replace('{user}', member.displayName || member.user.username).substring(0, 100);
        const novaCall = await guild.channels.create({
          name: nome,
          type: ChannelType.GuildVoice,
          parent: tvConfig.category_id || undefined,
          userLimit: tvConfig.default_limit || 0,
          permissionOverwrites: [
            {
              id: member.id,
        ...
        await db.prepare('INSERT INTO tempvoice_channels (channel_id, guild_id, owner_id, locked) VALUES (?, ?, ?, 0)').run(novaCall.id, guild.id, member.id);
        await member.voice.setChannel(novaCall.id).catch(() => {});
      }
    } catch (e) {
      console.error('❌ Erro ao criar call temporária:', e.message);
    }
  }

  // ── Calls Temporárias: canal ficou vazio → apaga ──
  if (oldState.channelId && oldState.channelId !== newState.channelId) {
    try {
      const tv = await db.prepare('SELECT * FROM tempvoice_channels WHERE channel_id = ?').get(oldState.channelId);
      if (tv) {
        const canalAntigo = guild.channels.cache.get(oldState.channelId);
        if (canalAntigo && canalAntigo.members.size === 0) {
          await limparCallTemporaria(oldState.channelId);
          await canalAntigo.delete().catch(() => {});
        }
      }
    } catch (e) {
      console.error('❌ Erro ao limpar call temporária vazia:', e.message);
    }
  }
    ...
    if (cmd === 'lock' || cmd === 'unlock') {
      ...
      try {
        if (cmd === 'lock') {
          ...
          await sendLogTyped(message.guild, 'lock', textoLog);
          ...
          await sendLogTyped(message.guild, 'unlock', textoLog);
          await message.channel.send({
            content: `🔓 O canal **${canal.name}** foi destrancado para trancar use \`/lock\` ou \`!lock\`.`
          }).catch(() => {});
        }
...
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  await member.roles.add(role).catch(err => avisarFalhaCargo(guild, role, err));
  await aplicarExclusividadeCargos(member);
});
client.on(Events.MessageReactionRemove, async (reaction, user) => {
  ...
  const role = guild.roles.cache.get(rr.role_id);
  if (!member || !role) return;
  await member.roles.remove(role).catch(err => avisarFalhaCargo(guild, role, err));
});
...
function iniciarCrons() {
  // Atualiza server stats a cada 10 minutos — serve de rede de segurança
  // (ex: boosts/cargos/canais mudam sem passar pelo GuildMemberAdd/Remove).
  // A contagem de membros já é atualizada em tempo real, com debounce, nos
  // eventos GuildMemberAdd/GuildMemberRemove (ver agendarAtualizacaoStats).
  // NOTA: o Discord só permite 2 renomeações por canal a cada 10 min — por isso
  // o intervalo do cron foi alinhado com esse limite. Também há um pequeno delay
  // entre servidores para não rebentar o rate limit global de edição de canais.
...
if (CONFIG.DASHBOARD_ATIVO) {
  ...
  // Rede de segurança extra: se algum BigInt (ex: IDs vindos do Turso) chegar
  // a um res.json() sem passar pelo wrapper db.js, converte para Number em vez
  // de rebentar o pedido (JSON.stringify não sabe serializar BigInt nativamente).
  app.set('json replacer', (key, value) => typeof value === 'bigint' ? Number(value) : value);

  // ── Rede de segurança para rotas assíncronas ──
  // O Express 4 NÃO apanha automaticamente erros/rejeições dentro de handlers
  // async: se uma rota "async (req,res) => {...}" rebentar a meio (ex: erro de
  // rede no Turso, ou uma exceção inesperada), o pedido fica pendurado para
  // sempre e o browser nunca recebe resposta — por isso o botão "Guardar"
  // parecia não fazer nada, e só aparecia depois de dar reload à página.
  // Este wrapper garante que QUALQUER erro numa rota é sempre convertido
  // numa resposta JSON de erro, para o dashboard poder mostrar o toast.
  ['get', 'post', 'put', 'delete', 'patch'].forEach(metodo => {
    const original = app[metodo].bind(app);
    app[metodo] = (path, ...handlers) => {
      const handlersEnvolvidos = handlers.map(h => {
        if (typeof h !== 'function') return h;
        return (req, res, next) => {
          Promise.resolve(h(req, res, next)).catch(err => {
            console.error(`❌ Erro na rota ${metodo.toUpperCase()} ${path}:`, err);
            if (!res.headersSent) {
              res.status(500).json({ ok: false, message: '❌ Erro interno no servidor. Tenta novamente.' });
            }
          });
        };
      });
      return original(path, ...handlersEnvolvidos);
    };
  });

  // ── Compressão gzip leve (sem dependências extra) ──
  // O dashboard envia HTML/CSS/JS grandes em cada pedido; comprimir reduz o
  // tamanho transferido em 70-85%, o que se nota MUITO em dados móveis/4G.
  // Implementado à mão com o "zlib" nativo do Node para não gastar RAM extra
  // com o pacote "compression" (o bot corre num plano gratuito de 100MB).
  app.use((req, res, next) => {
    const aceita = (req.headers['accept-encoding'] || '');
    if (!aceita.includes('gzip')) return next();
    const originalSend = res.send.bind(res);
    res.send = (body) => {
      if (body === undefined || body === null || res.headersSent) return originalSend(body);
      const buf = Buffer.isBuffer(body) ? body : Buffer.from(String(body));
      // Só vale a pena comprimir respostas com algum tamanho
      if (buf.length < 1024) return originalSend(body);
      zlib.gzip(buf, { level: 6 }, (err, comprimido) => {
        if (err || res.headersSent) return originalSend(body);
  ...
  app.get('/', (req, res) => {
    ...
    res.send(renderLoginPage(req.query.error, req.query.retry));
  });
  ...
  app.get('/auth/callback', async (req, res) => {
    ...
    try {
      ...
      // O endpoint OAuth2 do Discord tem o seu próprio rate limit, separado
      // do resto da API — se alguém tentar fazer login várias vezes seguidas
      // (ex: cliques duplos, refresh rápido), pode disparar 429 aqui. Distinguimos
      // esse caso para mostrar uma mensagem clara em vez de um "falhou" genérico.
      if (e.response?.status === 429) {
        const retryAfter = e.response?.data?.retry_after;
        return res.redirect(`/login?error=rate_limited${retryAfter ? `&retry=${Math.ceil(retryAfter)}` : ''}`);
      }
  ...
  // ── Retorno do convite do bot a um servidor ──
  // O Discord reencaminha para cá depois de o utilizador autorizar o bot num servidor.
  // Não há troca de código aqui (o convite já fica autorizado do lado do Discord),
  // só reencaminhamos de volta para o dashboard (ou login, se a sessão tiver expirado).
   ...
   */
  async function buildDashboardData(guild, selectedPanelId = null) {
    const guildId = guild.id;

    // ── Otimização de velocidade ──
    // A base de dados (Turso) é remota: cada await a uma query soma a
    // latência de rede (ida e volta à internet). Antes, ~20 queries corriam
    // uma a seguir à outra (sequencial), o que somava todos esses tempos.
    // Como a maioria destas queries não depende do resultado de nenhuma
    // outra, corremo-las todas ao mesmo tempo com Promise.all — o tempo
    // total passa a ser o da query mais lenta, não a soma de todas.
    // Nenhuma query, tabela ou lógica foi alterada — só a forma de esperar.
    const [
      ticketPanels,
      guildConfig,
      antispam,
      ...
      db.prepare('SELECT * FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id').all(guildId),
      db.prepare('SELECT * FROM guild_config WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM antispam_config WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM server_stats WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT COUNT(*) as c FROM tempvoice_channels WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM votacao_config WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM suggestion_config WHERE guild_id = ?').get(guildId),
      db.prepare('SELECT * FROM reaction_role_panels WHERE guild_id = ? ORDER BY id DESC').all(guildId),
      db.prepare("SELECT role_id FROM autorole_config WHERE guild_id = ? AND target = 'human'").all(guildId),
      db.prepare("SELECT role_id FROM autorole_config WHERE guild_id = ? AND target = 'bot'").all(guildId),
      db.prepare('SELECT * FROM role_exclusivity WHERE guild_id = ? ORDER BY id DESC').all(guildId),
      db.prepare('SELECT * FROM blacklist WHERE guild_id = ? ORDER BY created_at DESC').all(guildId),
      db.prepare('SELECT * FROM saved_embeds WHERE guild_id = ? ORDER BY created_at DESC').all(guildId),
      db.prepare('SELECT * FROM perguntas WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50').all(guildId),
      getRankingStaff(guildId),
      db.prepare("SELECT COUNT(*) as c FROM tickets WHERE guild_id = ?").get(guildId),
      db.prepare("SELECT COUNT(*) as c FROM tickets WHERE guild_id = ? AND status='open'").get(guildId),
      db.prepare("SELECT COUNT(*) as c FROM warns WHERE guild_id = ?").get(guildId),
      db.prepare("SELECT COUNT(*) as c FROM suggestions WHERE guild_id = ?").get(guildId),
      db.prepare('SELECT * FROM ai_moderation_config WHERE guild_id = ?').get(guildId)
    ]);

    let ticketConfig = selectedPanelId ? ticketPanels.find(p => String(p.id) === String(selectedPanelId)) : null;
    if (!ticketConfig) ticketConfig = ticketPanels[0] || null;
    const tvChannelsCount = tvChannelsCountRow?.c || 0;
    const autoroleHumanos = autoroleHumanosRows.map(r => r.role_id);
    const autoroleBots = autoroleBotsRows.map(r => r.role_id);
    let immuneRoles = [];
    try {
      immuneRoles = JSON.parse(guildConfig?.immune_roles || '[]');
    } catch (_) {}

    // Estas duas dependem de resultados acima (reaction roles de cada painel,
    // e tipos de ticket do painel selecionado), mas continuam a correr juntas.
    const [reactionRoles, ticketTypes] = await Promise.all([
      Promise.all(rrPaineis.map(async p => ({
        ...p,
        itens: await db.prepare('SELECT * FROM reaction_roles WHERE guild_id = ? AND message_id = ?').all(guildId, p.message_id)
      }))),
      ticketConfig ? db.prepare('SELECT * FROM ticket_types WHERE guild_id = ? AND (panel_id = ? OR panel_id IS NULL) ORDER BY order_num, id').all(guildId, ticketConfig.id) : []
    ]);

    const totalTickets = totalTicketsRow?.c || 0;
    const openTickets = openTicketsRow?.c || 0;
    const totalWarns = totalWarnsRow?.c || 0;
    const totalSugs = totalSugsRow?.c || 0;
  ...
  app.get('/dashboard/:guildId', requireAuth, requireGuildAdminPage, async (req, res) => {
    ...
    const data = await buildDashboardData(guild, req.query.painel || null);
  ...
  app.post('/api/:guildId/ticket-config', requireGuildAdminApi, async (req, res) => {
    ...
    // support_roles chega como array (checkboxes) ou string única — normaliza para array de strings
    const rolesArr = Array.isArray(support_roles) ? support_roles.filter(Boolean) : support_roles ? [support_roles] : [];
    const rolesJson = JSON.stringify(rolesArr);
    const primeiroCargo = rolesArr[0] || null;

    // Determina qual painel guardar: o indicado no formulário, senão o primeiro do servidor (cria "Painel 1/10" se não existir nenhum)
    let painel = panel_id ? await db.prepare('SELECT * FROM ticket_config WHERE id = ? AND guild_id = ?').get(panel_id, guildId) : null;
    if (!painel) painel = await db.prepare('SELECT * FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id LIMIT 1').get(guildId);

    if (painel) {
      await db.prepare(`
        UPDATE ticket_config SET
          category_id=?, log_channel=?, support_role=?, support_roles=?, transcript_channel=?,
          max_tickets=?, welcome_msg=?, enabled=1, panel_mode=?, panel_color=?
        WHERE id=?
      `).run(category_id || null, log_channel || null, primeiroCargo, rolesJson, transcript_channel || null, parseInt(max_tickets) || 3, welcome_msg || 'Olá {user}!', modoFinal, corFinal, painel.id);
    } else {
      await db.prepare(`
        INSERT INTO ticket_config (guild_id, panel_num, panel_name, category_id, log_channel, support_role, support_roles, transcript_channel, max_tickets, welcome_msg, enabled, panel_mode, panel_color)
        VALUES (?, 1, 'Painel 1/10', ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
      `).run(guildId, category_id || null, log_channel || null, primeiroCargo, rolesJson, transcript_channel || null, parseInt(max_tickets) || 3, welcome_msg || 'Olá {user}!', modoFinal, corFinal);
    }
    res.json({
      ok: true,
  ...
  // Cria um novo painel de tickets (até 10 por servidor). O nome é sempre "Painel N/10".
  app.post('/api/:guildId/ticket-panel/create', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const existentes = await db.prepare('SELECT panel_num FROM ticket_config WHERE guild_id = ? ORDER BY panel_num').all(guildId);
    if (existentes.length >= 10) {
      return res.status(400).json({
        ok: false,
        message: 'Já tens o máximo de 10 painéis de tickets.'
      });
    }
    // Encontra o primeiro número livre entre 1 e 10 (caso algum painel do meio tenha sido apagado)
    const usados = new Set(existentes.map(e => e.panel_num));
    let novoNum = 1;
    while (usados.has(novoNum) && novoNum <= 10) novoNum++;
    await db.prepare(`
      INSERT INTO ticket_config (guild_id, panel_num, panel_name, enabled)
      VALUES (?, ?, ?, 1)
    `).run(guildId, novoNum, `Painel ${novoNum}/10`);
    res.json({
      ok: true,
      message: `✅ Painel ${novoNum}/10 criado!`
    });
  });

  // Apaga um painel de tickets (e os seus tipos de ticket associados). Não apaga os tickets já existentes, só deixa de os associar a um painel ativo.
  app.delete('/api/:guildId/ticket-panel/:panelId', requireGuildAdminApi, async (req, res) => {
    const {
      guildId,
      panelId
    } = req.params;
    const painel = await db.prepare('SELECT * FROM ticket_config WHERE id = ? AND guild_id = ?').get(panelId, guildId);
    if (!painel) return res.status(404).json({
      ok: false,
      message: 'Painel não encontrado.'
    });
    const totalPaineis = (await db.prepare('SELECT COUNT(*) as c FROM ticket_config WHERE guild_id = ?').get(guildId))?.c || 0;
    if (totalPaineis <= 1) {
      return res.status(400).json({
        ok: false,
        message: 'Tens de ter pelo menos 1 painel de tickets.'
      });
    }
    await db.prepare('DELETE FROM ticket_types WHERE panel_id = ?').run(panelId);
    await db.prepare('DELETE FROM ticket_config WHERE id = ? AND guild_id = ?').run(panelId, guildId);
    res.json({
      ok: true,
      message: `🗑️ ${painel.panel_name || 'Painel'} apagado!`
    });
  });
  ...
  app.post('/api/:guildId/ticket-painel', requireGuildAdminApi, async (req, res) => {
    ...
    const ticketConfig = panel_id ? await db.prepare('SELECT * FROM ticket_config WHERE id = ? AND guild_id = ?').get(panel_id, guildId) : await obterTicketConfig(guildId);
    ...
    try {
      ...
      const components = await montarComponentesPainelTicket(guildId, ticketConfig);
      const msg = await canal.send({
        embeds: [embed],
        components
      });
      await db.prepare(`UPDATE ticket_config SET panel_msg_id=?, panel_channel_id=? WHERE id=?`).run(msg.id, canal.id, ticketConfig.id);
      res.json({
        ok: true,
  ...
  `).run(guildId, welcome_channel || null, welcome_title || null, welcome_msg || 'Bem-vindo ${usermention}!', welcome_embed === '1' ? 1 : 0, welcome_image || null, autorole || null, corValida, welcome_author_name || null, welcome_author_icon || null, welcome_thumbnail || null, welcome_footer || null, posValida, welcome_content || null, welcome_url || null);
    invalidarGuildConfigCache(guildId);
    res.json({
      ok: true,
  ...
  // ============================
  // API — MENSAGENS DE DM AO ENTRAR (multi, nomeadas — mesmo padrão de welcome-messages)
  // ============================
  app.get('/api/:guildId/dm-messages', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const linhas = await db.prepare('SELECT * FROM dm_messages WHERE guild_id = ? ORDER BY created_at ASC').all(guildId);
    res.json(linhas);
  });
  app.get('/api/:guildId/dm-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM dm_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de DM não encontrada.' });
    res.json(linha);
  });
  app.post('/api/:guildId/dm-messages', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const nome = (req.body?.name || '').trim();
    if (!nome) return res.status(400).json({ ok: false, message: '❌ Dá um nome a esta mensagem de DM.' });
    const existente = await db.prepare('SELECT id FROM dm_messages WHERE guild_id = ? AND name = ?').get(guildId, nome);
    if (existente) return res.status(400).json({ ok: false, message: `❌ Já existe uma mensagem chamada "${nome}".` });
    const cor = (req.body?.dm_color || '').trim() || '#5865F2';
    if (!/^#([0-9A-Fa-f]{6})$/.test(cor)) return res.status(400).json({ ok: false, message: '❌ Cor inválida. Usa o formato hex, ex: #5865F2.' });
    const posValida = ['bottom', 'thumbnail', 'none'].includes(req.body?.dm_image_pos) ? req.body.dm_image_pos : 'bottom';
    const totalExistentes = (await db.prepare('SELECT COUNT(*) AS c FROM dm_messages WHERE guild_id = ?').get(guildId)).c;
    const ativa = totalExistentes === 0 ? 1 : 0;
    const info = await db.prepare(`
      INSERT INTO dm_messages (
        guild_id, name, is_active, dm_embed, dm_content, dm_title, dm_url, dm_msg,
        dm_color, dm_author_name, dm_author_icon, dm_image_pos, dm_image, dm_thumbnail, dm_footer
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(guildId, nome, ativa, req.body?.dm_embed === '1' || req.body?.dm_embed === 1 ? 1 : 0, (req.body?.dm_content || '').trim() || null, (req.body?.dm_title || '').trim() || null, (req.body?.dm_url || '').trim() || null, (req.body?.dm_msg || '').trim() || null, cor, (req.body?.dm_author_name || '').trim() || null, (req.body?.dm_author_icon || '').trim() || null, posValida, (req.body?.dm_image || '').trim() || null, (req.body?.dm_thumbnail || '').trim() || null, (req.body?.dm_footer || '').trim() || null);
    res.json({ ok: true, message: `✅ Mensagem "${nome}" criada!`, id: info.lastInsertRowid });
  });
  app.put('/api/:guildId/dm-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM dm_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de DM não encontrada.' });
    const nome = (req.body?.name || '').trim();
    if (!nome) return res.status(400).json({ ok: false, message: '❌ Dá um nome a esta mensagem de DM.' });
    const duplicado = await db.prepare('SELECT id FROM dm_messages WHERE guild_id = ? AND name = ? AND id != ?').get(guildId, nome, id);
    if (duplicado) return res.status(400).json({ ok: false, message: `❌ Já existe uma mensagem chamada "${nome}".` });
    const cor = (req.body?.dm_color || '').trim() || '#5865F2';
    if (!/^#([0-9A-Fa-f]{6})$/.test(cor)) return res.status(400).json({ ok: false, message: '❌ Cor inválida. Usa o formato hex, ex: #5865F2.' });
    const posValida = ['bottom', 'thumbnail', 'none'].includes(req.body?.dm_image_pos) ? req.body.dm_image_pos : 'bottom';
    await db.prepare(`
      UPDATE dm_messages SET
        name = ?, dm_embed = ?, dm_content = ?, dm_title = ?, dm_url = ?, dm_msg = ?,
        dm_color = ?, dm_author_name = ?, dm_author_icon = ?, dm_image_pos = ?,
        dm_image = ?, dm_thumbnail = ?, dm_footer = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND guild_id = ?
    `).run(nome, req.body?.dm_embed === '1' || req.body?.dm_embed === 1 ? 1 : 0, (req.body?.dm_content || '').trim() || null, (req.body?.dm_title || '').trim() || null, (req.body?.dm_url || '').trim() || null, (req.body?.dm_msg || '').trim() || null, cor, (req.body?.dm_author_name || '').trim() || null, (req.body?.dm_author_icon || '').trim() || null, posValida, (req.body?.dm_image || '').trim() || null, (req.body?.dm_thumbnail || '').trim() || null, (req.body?.dm_footer || '').trim() || null, id, guildId);
    res.json({ ok: true, message: `✅ Mensagem "${nome}" atualizada!` });
  });
  app.delete('/api/:guildId/dm-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM dm_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de DM não encontrada.' });
    await db.prepare('DELETE FROM dm_messages WHERE id = ? AND guild_id = ?').run(id, guildId);
    if (linha.is_active) {
      const proxima = await db.prepare('SELECT id FROM dm_messages WHERE guild_id = ? ORDER BY created_at ASC LIMIT 1').get(guildId);
      if (proxima) await db.prepare('UPDATE dm_messages SET is_active = 1 WHERE id = ?').run(proxima.id);
    }
    res.json({ ok: true, message: '✅ Mensagem de DM apagada.' });
  });
  app.post('/api/:guildId/dm-messages/:id/activate', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM dm_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de DM não encontrada.' });
    await db.prepare('UPDATE dm_messages SET is_active = 0 WHERE guild_id = ?').run(guildId);
    await db.prepare('UPDATE dm_messages SET is_active = 1 WHERE id = ?').run(id);
    res.json({ ok: true, message: `✅ "${linha.name}" é agora a mensagem de DM ativa.` });
  });
  app.post('/api/:guildId/dm-preview', requireGuildAdminApi, (req, res) => {
    try {
      const { guildId } = req.params;
      const guild = client.guilds.cache.get(guildId);
      ...
      const configPreview = { ...req.body };
      const embed = construirEmbedDm(configPreview, fakeMember);
      const conteudoFora = substituirVariaveisWelcome(configPreview.dm_content || '', fakeMember);
      res.json({ ok: true, content: conteudoFora, embed: embed.toJSON() });
    } catch (e) {
      res.status(500).json({ ok: false, message: 'Erro ao gerar preview: ' + e.message });
    }
  });

  // ============================
  // API — MENSAGENS DE SAÍDA (multi, nomeadas — mesmo padrão de welcome-messages)
  // ============================
  app.get('/api/:guildId/leave-messages', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const guild = client.guilds.cache.get(guildId);
    const linhas = await db.prepare('SELECT * FROM leave_messages WHERE guild_id = ? ORDER BY created_at ASC').all(guildId);
    const comExtras = linhas.map(l => ({ ...l, channel_name: guild?.channels.cache.get(l.leave_channel)?.name || null }));
    res.json(comExtras);
  });
  app.get('/api/:guildId/leave-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM leave_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de saída não encontrada.' });
    res.json(linha);
  });
  app.post('/api/:guildId/leave-messages', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const nome = (req.body?.name || '').trim();
    if (!nome) return res.status(400).json({ ok: false, message: '❌ Dá um nome a esta mensagem de saída.' });
    const existente = await db.prepare('SELECT id FROM leave_messages WHERE guild_id = ? AND name = ?').get(guildId, nome);
    if (existente) return res.status(400).json({ ok: false, message: `❌ Já existe uma mensagem chamada "${nome}".` });
    const cor = (req.body?.leave_color || '').trim() || '#ED4245';
    if (!/^#([0-9A-Fa-f]{6})$/.test(cor)) return res.status(400).json({ ok: false, message: '❌ Cor inválida. Usa o formato hex, ex: #ED4245.' });
    const posValida = ['bottom', 'thumbnail', 'none'].includes(req.body?.leave_image_pos) ? req.body.leave_image_pos : 'bottom';
    const totalExistentes = (await db.prepare('SELECT COUNT(*) AS c FROM leave_messages WHERE guild_id = ?').get(guildId)).c;
    const ativa = totalExistentes === 0 ? 1 : 0;
    const info = await db.prepare(`
      INSERT INTO leave_messages (
        guild_id, name, is_active, leave_channel, leave_embed, leave_content, leave_title,
        leave_url, leave_msg, leave_color, leave_author_name, leave_author_icon,
        leave_image_pos, leave_image, leave_thumbnail, leave_footer
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(guildId, nome, ativa, req.body?.leave_channel || null, req.body?.leave_embed === '1' || req.body?.leave_embed === 1 ? 1 : 0, (req.body?.leave_content || '').trim() || null, (req.body?.leave_title || '').trim() || null, (req.body?.leave_url || '').trim() || null, (req.body?.leave_msg || '').trim() || null, cor, (req.body?.leave_author_name || '').trim() || null, (req.body?.leave_author_icon || '').trim() || null, posValida, (req.body?.leave_image || '').trim() || null, (req.body?.leave_thumbnail || '').trim() || null, (req.body?.leave_footer || '').trim() || null);
    res.json({ ok: true, message: `✅ Mensagem "${nome}" criada!`, id: info.lastInsertRowid });
  });
  app.put('/api/:guildId/leave-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM leave_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de saída não encontrada.' });
    const nome = (req.body?.name || '').trim();
    if (!nome) return res.status(400).json({ ok: false, message: '❌ Dá um nome a esta mensagem de saída.' });
    const duplicado = await db.prepare('SELECT id FROM leave_messages WHERE guild_id = ? AND name = ? AND id != ?').get(guildId, nome, id);
    if (duplicado) return res.status(400).json({ ok: false, message: `❌ Já existe uma mensagem chamada "${nome}".` });
    const cor = (req.body?.leave_color || '').trim() || '#ED4245';
    if (!/^#([0-9A-Fa-f]{6})$/.test(cor)) return res.status(400).json({ ok: false, message: '❌ Cor inválida. Usa o formato hex, ex: #ED4245.' });
    const posValida = ['bottom', 'thumbnail', 'none'].includes(req.body?.leave_image_pos) ? req.body.leave_image_pos : 'bottom';
    await db.prepare(`
      UPDATE leave_messages SET
        name = ?, leave_channel = ?, leave_embed = ?, leave_content = ?, leave_title = ?,
        leave_url = ?, leave_msg = ?, leave_color = ?, leave_author_name = ?,
        leave_author_icon = ?, leave_image_pos = ?, leave_image = ?, leave_thumbnail = ?,
        leave_footer = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND guild_id = ?
    `).run(nome, req.body?.leave_channel || null, req.body?.leave_embed === '1' || req.body?.leave_embed === 1 ? 1 : 0, (req.body?.leave_content || '').trim() || null, (req.body?.leave_title || '').trim() || null, (req.body?.leave_url || '').trim() || null, (req.body?.leave_msg || '').trim() || null, cor, (req.body?.leave_author_name || '').trim() || null, (req.body?.leave_author_icon || '').trim() || null, posValida, (req.body?.leave_image || '').trim() || null, (req.body?.leave_thumbnail || '').trim() || null, (req.body?.leave_footer || '').trim() || null, id, guildId);
    res.json({ ok: true, message: `✅ Mensagem "${nome}" atualizada!` });
  });
  app.delete('/api/:guildId/leave-messages/:id', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM leave_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de saída não encontrada.' });
    await db.prepare('DELETE FROM leave_messages WHERE id = ? AND guild_id = ?').run(id, guildId);
    if (linha.is_active) {
      const proxima = await db.prepare('SELECT id FROM leave_messages WHERE guild_id = ? ORDER BY created_at ASC LIMIT 1').get(guildId);
      if (proxima) await db.prepare('UPDATE leave_messages SET is_active = 1 WHERE id = ?').run(proxima.id);
    }
    res.json({ ok: true, message: '✅ Mensagem de saída apagada.' });
  });
  app.post('/api/:guildId/leave-messages/:id/activate', requireGuildAdminApi, async (req, res) => {
    const { guildId, id } = req.params;
    const linha = await db.prepare('SELECT * FROM leave_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
    if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de saída não encontrada.' });
    await db.prepare('UPDATE leave_messages SET is_active = 0 WHERE guild_id = ?').run(guildId);
    await db.prepare('UPDATE leave_messages SET is_active = 1 WHERE id = ?').run(id);
    res.json({ ok: true, message: `✅ "${linha.name}" é agora a mensagem de saída ativa.` });
  });
  app.post('/api/:guildId/leave-preview', requireGuildAdminApi, (req, res) => {
    try {
      const { guildId } = req.params;
      const guild = client.guilds.cache.get(guildId);
      if (!guild) return res.status(404).json({ ok: false, message: 'Servidor não encontrado.' });
      ...
      const configPreview = { ...req.body };
      const embed = construirEmbedLeave(configPreview, fakeMember);
      const conteudoFora = substituirVariaveisWelcome(configPreview.leave_content || '', fakeMember);
      res.json({ ok: true, content: conteudoFora, embed: embed.toJSON() });
    } catch (e) {
      res.status(500).json({ ok: false, message: 'Erro ao gerar preview: ' + e.message });
    }
  ...
  // ============================
  // AUTOMOD IA — rotas do dashboard
  // ============================
  app.get('/api/:guildId/ai-moderation-config', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const config = await db.prepare('SELECT * FROM ai_moderation_config WHERE guild_id = ?').get(guildId);
    // ai_configured indica se o Render tem AI_API_KEY definida (para o dashboard avisar, se não)
    res.json({ ok: true, config: config || null, ai_configured: !!CONFIG.AI_API_KEY });
  });

  app.post('/api/:guildId/ai-moderation-config', requireGuildAdminApi, async (req, res) => {
    const { guildId } = req.params;
    const { enabled, log_channel, max_warnings, timeout_minutes, min_severity, warning_reset_days } = req.body;

    const severidadeValida = ['LOW', 'MEDIUM', 'HIGH'].includes(min_severity) ? min_severity : 'MEDIUM';
    const maxW = Math.min(Math.max(parseInt(max_warnings) || 3, 1), 20);
    const timeoutMin = Math.min(Math.max(parseInt(timeout_minutes) || 10, 1), 40320); // até 28 dias (limite do Discord)
    const resetDias = Math.min(Math.max(parseInt(warning_reset_days) || 0, 0), 365); // 0 = nunca reseta sozinho, até 365 dias

    await db.prepare('INSERT OR IGNORE INTO ai_moderation_config (guild_id) VALUES (?)').run(guildId);
    await db.prepare(`
      UPDATE ai_moderation_config
      SET enabled = ?, log_channel = ?, max_warnings = ?, timeout_minutes = ?, min_severity = ?, warning_reset_days = ?, updated_at = CURRENT_TIMESTAMP
      WHERE guild_id = ?
    `).run(enabled ? 1 : 0, log_channel || null, maxW, timeoutMin, severidadeValida, resetDias, guildId);

    res.json({ ok: true, message: '✅ Configuração de AutoMod IA guardada!' });
  });
  ...
  app.get('/api/:guildId/bot-identity', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
  ...
  app.get('/api/:guildId/suggestions', requireGuildAdminApi, async (req, res) => {
    ...
    const comTipo = await Promise.all(suggestions.map(async s => {
  ...
  app.get('/api/:guildId/staff-ranking', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const ranking = await getRankingStaff(guildId);
    res.json(ranking);
  });

  // ── Server Stats ──
  app.post('/api/:guildId/tempvoice-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const guild = client.guilds.cache.get(guildId);
    ...
    try {
      let config = await db.prepare('SELECT * FROM tempvoice_config WHERE guild_id = ?').get(guildId);

      if (enabled) {
        const template = (name_template || '🔊 Call de {user}').substring(0, 100);
        let limite = parseInt(default_limit);
        if (isNaN(limite) || limite < 0) limite = 0;
        if (limite > 99) limite = 99;

        // Se ainda não existe sistema criado (ou os canais foram apagados manualmente), cria de novo
        const categoriaValida = config?.category_id && guild.channels.cache.get(config.creator_channel_id) && guild.channels.cache.get(config.interface_channel_id);
        if (!categoriaValida) {
          const criados = await criarSistemaTempVoice(guild);
          await db.prepare(`
            INSERT INTO tempvoice_config (guild_id, category_id, creator_channel_id, interface_channel_id, interface_msg_id, name_template, default_limit, enabled)
            ...
            ON CONFLICT(guild_id) DO UPDATE SET
              ...
              interface_channel_id=excluded.interface_channel_id, interface_msg_id=excluded.interface_msg_id,
              name_template=excluded.name_template, default_limit=excluded.default_limit, enabled=1
          `).run(guildId, criados.categoryId, criados.creatorChannelId, criados.interfaceChannelId, criados.interfaceMsgId, template, limite);
          return res.json({ ok: true, message: `✅ Sistema de Calls Temporárias criado! Vê a categoria "Crie sua Call" no teu servidor.` });
        }

        await db.prepare(`
          UPDATE tempvoice_config SET enabled = 1, name_template = ?, default_limit = ? WHERE guild_id = ?
        `).run(template, limite, guildId);
        return res.json({ ok: true, message: '✅ Configuração das Calls Temporárias guardada!' });
      } else {
        if (!config) {
          return res.json({ ok: true, message: '✅ Sistema já estava desativado.' });
        }
        await apagarSistemaTempVoice(guild, config);
        await db.prepare('DELETE FROM tempvoice_config WHERE guild_id = ?').run(guildId);
        return res.json({ ok: true, message: '✅ Sistema de Calls Temporárias removido — categoria e canais apagados.' });
      }
    } catch (e) {
      console.error('Erro tempvoice-config:', e.message);
      res.status(500).json({
        ok: false,
        message: `Erro: ${e.message}`
      });
  ...
  // ── Sugestões: configuração única do servidor (substitui o antigo sistema de "tipos") ──
  app.get('/api/:guildId/sugestao-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const guild = client.guilds.cache.get(guildId);
    const cfg = await db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? ORDER BY id LIMIT 1').get(guildId);
    if (!cfg) return res.json(null);
    res.json({
      ...cfg,
      channel_name: guild?.channels.cache.get(cfg.channel_id)?.name || null,
      log_channel_name: cfg.log_channel ? guild?.channels.cache.get(cfg.log_channel)?.name || null : null,
      role_name: cfg.ping_role ? guild?.roles.cache.get(cfg.ping_role)?.name || null : null
    });
  });
  app.post('/api/:guildId/sugestao-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    const {
    ...
    const existente = await db.prepare('SELECT id FROM suggestion_types WHERE guild_id = ? ORDER BY id LIMIT 1').get(guildId);
    const emojiFinal = (emoji || '').trim() || '💡';
    const enabledFinal = enabled === false || enabled === '0' || enabled === 0 ? 0 : 1;
    if (existente) {
      await db.prepare(`
        UPDATE suggestion_types SET name = 'Sugestão', emoji = ?, channel_id = ?, log_channel = ?, ping_role = ?, enabled = ?
        WHERE id = ?
      `).run(emojiFinal, channel_id, log_channel || null, ping_role || null, enabledFinal, existente.id);
    } else {
      await db.prepare(`
        INSERT INTO suggestion_types (guild_id, name, emoji, channel_id, log_channel, ping_role, enabled, order_num)
        VALUES (?, 'Sugestão', ?, ?, ?, ?, ?, 0)
      `).run(guildId, emojiFinal, channel_id, log_channel || null, ping_role || null, enabledFinal);
    }
    res.json({
      ok: true,
      message: '✅ Configuração de sugestões guardada!'
    });
  });
  app.delete('/api/:guildId/sugestao-config', requireGuildAdminApi, async (req, res) => {
    const {
      guildId
    } = req.params;
    await db.prepare('DELETE FROM suggestion_types WHERE guild_id = ?').run(guildId);
    res.json({
      ok: true,
      message: '✅ Configuração de sugestões apagada.'
    });
  });
  ...
  app.get('/api/:guildId/reaction-roles', requireGuildAdminApi, async (req, res) => {
    ...
    const paineisComItens = await Promise.all(paineis.map(async p => ({
      ...p,
      itens: await db.prepare('SELECT * FROM reaction_roles WHERE guild_id = ? AND message_id = ?').all(guildId, p.message_id)
    })));
  ...
  app.get('/api/:guildId/ticket-types', requireGuildAdminApi, async (req, res) => {
    ...
    const tipos = panel_id ? await db.prepare('SELECT * FROM ticket_types WHERE guild_id = ? AND (panel_id = ? OR panel_id IS NULL) ORDER BY order_num, id').all(guildId, panel_id) : await db.prepare('SELECT * FROM ticket_types WHERE guild_id = ? ORDER BY order_num, id').all(guildId);
    res.json(tipos);
  });
  app.post('/api/:guildId/ticket-types', requireGuildAdminApi, async (req, res) => {
    ...
    // Se não vier panel_id explícito, associa ao primeiro painel do servidor (comportamento legado)
    let panelIdFinal = panel_id || null;
    if (!panelIdFinal) {
      const painel1 = await db.prepare('SELECT id FROM ticket_config WHERE guild_id = ? ORDER BY panel_num, id LIMIT 1').get(guildId);
      panelIdFinal = painel1?.id || null;
    }
    const rolesArr = Array.isArray(support_roles) ? support_roles.filter(Boolean) : support_roles ? [support_roles] : [];
    const rolesJson = JSON.stringify(rolesArr);
    const maxOrder = (await db.prepare('SELECT MAX(order_num) as m FROM ticket_types WHERE guild_id = ?').get(guildId))?.m || 0;
    await db.prepare(`
    INSERT INTO ticket_types (guild_id, panel_id, label, description, emoji, category_id, support_role, support_roles, color, order_num, has_form)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(guildId, panelIdFinal, label, description || null, emoji || '🎫', category_id || null, rolesArr[0] || null, rolesJson, color || CONFIG.COR_PRINCIPAL, maxOrder + 1, has_form ? 1 : 0);
    res.json({
      ok: true,
  ...
  app.post('/api/:guildId/ticket-types/edit', requireGuildAdminApi, async (req, res) => {
    ...
    const rolesJson = JSON.stringify(rolesArr);
    await db.prepare(`
    UPDATE ticket_types SET label=?, description=?, emoji=?, category_id=?, support_role=?, support_roles=?, color=?
    WHERE id=? AND guild_id=?
  `).run(label.trim(), description || null, emoji || '🎫', category_id || null, rolesArr[0] || null, rolesJson, color || '#5865F2', id, guildId);
    res.json({
      ok: true,
  ...
  app.get('/api/:guildId/staff/ranking', requireGuildAdminApi, async (req, res) => {
    ...
    res.json(await getRankingStaff(guildId));
  });
  ...
  app.get('/api/:guildId/giveaways', requireGuildAdminApi, async (req, res) => {
    ...
    const comContagem = await Promise.all(giveaways.map(async gw => ({
      ...gw,
  ...
  app.get('/api/:guildId/infopanels', requireGuildAdminApi, async (req, res) => {
    ...
    const comContagem = await Promise.all(paineis.map(async p => ({
      ...p,
  ...
  app.put('/api/:guildId/infopanels/:id', requireGuildAdminApi, async (req, res) => {
    const {
      guildId,
      id
    } = req.params;
    ...
    const existente = await db.prepare('SELECT id FROM info_panels WHERE guild_id = ? AND name = ? AND id != ?').get(guildId, nome, id);
    if (existente) return res.status(400).json({
      ok: false,
      message: `❌ Já existe outro painel chamado "${nome}".`
    });
    const canalId = (channel_id || '').trim() || panel.channel_id;
    if (canalId && !guild.channels.cache.get(canalId)) {
      return res.status(400).json({
        ok: false,
        message: '❌ Canal inválido.'
      });
    ...
    await db.prepare(`
    UPDATE info_panels SET name = ?, title = ?, description = ?, color = ?, banner_url = ?, thumbnail_url = ?, footer_text = ?, owner_text = ?, founded_text = ?, channel_id = ?
    WHERE id = ?
  `).run(nome, (title || '').trim() || null, (description || '').trim() || null, cor, (banner_url || '').trim() || null, (thumbnail_url || '').trim() || null, (footer_text || '').trim() || null, (owner_text || '').trim() || null, (founded_text || '').trim() || null, canalId || null, id);

    // Se o painel já estiver publicado, atualiza a mensagem existente no Discord
    // em vez de obrigar a apagar/republicar — é o que permite editar sem perder o histórico.
    let avisoCanal = null;
    if (panel.published && panel.message_id && panel.channel_id) {
      const canal = guild.channels.cache.get(panel.channel_id);
      const msg = canal ? await canal.messages.fetch(panel.message_id).catch(() => null) : null;
      if (msg) {
        const panelAtualizado = await db.prepare('SELECT * FROM info_panels WHERE id = ?').get(id);
        const botoes = await db.prepare('SELECT * FROM info_panel_buttons WHERE panel_id = ? ORDER BY order_num, id').all(id);
        const rows = botoesInfoPanel(id, botoes);
        await msg.edit({
          embeds: [embedInfoPanel(panelAtualizado)],
          components: rows
        }).catch(() => {});
      } else {
        avisoCanal = ' (a mensagem publicada já não foi encontrada no Discord — republica o painel para a recriar)';
      }
    }
    res.json({
      ok: true,
      message: `✅ Painel "${nome}" atualizado!${avisoCanal || ''}`
    });
  });
  ...
  app.put('/api/:guildId/infopanels/:id/buttons/:buttonId', requireGuildAdminApi, async (req, res) => {
    const {
      id,
      guildId,
      buttonId
    ...
    const botao = await db.prepare('SELECT * FROM info_panel_buttons WHERE id = ? AND panel_id = ?').get(buttonId, panel.id);
    if (!botao) return res.status(404).json({
      ok: false,
    ...
    const estilo = estilosValidos.includes(style) ? style : 'Primary';
    await db.prepare(`
    UPDATE info_panel_buttons SET label = ?, emoji = ?, style = ?, response_text = ?, response_title = ?, response_image = ?, response_thumbnail = ?, response_color = ?
    WHERE id = ?
  `).run(lbl, (emoji || '').trim() || null, estilo, resp, (response_title || '').trim() || null, (response_image || '').trim() || null, (response_thumbnail || '').trim() || null, cor, buttonId);

    // Atualiza a mensagem já publicada, se existir, para refletir a alteração do botão.
    if (panel.published && guild && panel.message_id && panel.channel_id) {
      const canalMsg = guild.channels.cache.get(panel.channel_id);
      const msg = canalMsg ? await canalMsg.messages.fetch(panel.message_id).catch(() => null) : null;
      if (msg) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-item { display: flex; align-items: center; gap: 10px; padding: 11px 20px; color: var(--text2); cursor: pointer; transition: all 0.18s ease; border: none; background: none; width: 100%; font-size: 0.9rem; border-right: 2px solid transparent; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  /* ── Painéis de Tickets (estilo "Ticket King") ── */
  .tk-panels-card { padding: 20px 24px 24px; }
  .tk-panels-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 14px; flex-wrap: wrap; margin-bottom: 18px; }
  .tk-search-wrap { position: relative; flex: 1 1 320px; min-width: 220px; }
  .tk-search-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); opacity: 0.6; font-size: 0.9rem; pointer-events: none; }
  .tk-search-input { width: 100%; background: var(--bg3); border: 1px solid var(--border); border-radius: 10px; padding: 10px 14px 10px 38px; color: var(--text); font-size: 0.9rem; outline: none; transition: border-color 0.2s ease; }
  .tk-search-input:focus { border-color: var(--accent); }
  .tk-toolbar-right { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
  .tk-sort-select { background: var(--bg3); border: 1px solid var(--border); border-radius: 10px; padding: 10px 14px; color: var(--text); font-size: 0.85rem; outline: none; cursor: pointer; }

  .tk-panel-list { display: flex; flex-direction: column; gap: 10px; }
  .tk-panel-row {
    background: var(--bg3); border: 1px solid var(--border); border-radius: 12px;
    padding: 14px 18px; display: flex; align-items: center; justify-content: space-between;
    gap: 14px; flex-wrap: wrap; transition: all 0.15s ease;
  }
  .tk-panel-row:hover { border-color: #3a4066; background: var(--bg4); }
  .tk-panel-row.active { border-color: var(--accent); box-shadow: 0 0 0 1px var(--accent) inset; }
  .tk-panel-row-left { display: flex; align-items: center; gap: 12px; min-width: 0; flex: 1 1 260px; }
  .tk-panel-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
  .tk-panel-row-info { min-width: 0; }
  .tk-panel-row-name { font-weight: 700; font-size: 0.95rem; color: var(--text); display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
  .tk-panel-row-sub { font-size: 0.8rem; color: var(--text2); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .tk-panel-row-badges { display: flex; gap: 6px; flex-wrap: wrap; }
  .tk-badge { padding: 3px 10px; border-radius: 999px; font-size: 0.72rem; font-weight: 700; border: 1px solid transparent; white-space: nowrap; }
  .tk-badge-type { background: rgba(254,231,92,.1); border-color: rgba(254,231,92,.4); color: var(--warning); }
  .tk-badge-channel { background: rgba(88,101,242,.18); border-color: rgba(88,101,242,.4); color: #9aa5ff; }
  .tk-badge-draft { background: rgba(139,146,184,.12); border-color: var(--border); color: var(--text2); }
  .tk-panel-row-actions { display: flex; gap: 6px; flex-wrap: wrap; }
  .tk-icon-btn {
    display: inline-flex; align-items: center; gap: 6px; background: var(--bg2); border: 1px solid var(--border);
    color: var(--text); padding: 7px 12px; border-radius: 8px; font-size: 0.78rem; font-weight: 600;
    cursor: pointer; transition: all 0.15s ease; text-decoration: none;
  }
  .tk-icon-btn:hover:not(:disabled) { background: var(--bg4); border-color: var(--accent); transform: translateY(-1px); }
  .tk-icon-btn:disabled { opacity: 0.35; cursor: not-allowed; }
  .tk-icon-btn.danger:hover { border-color: var(--danger); color: var(--danger); }
  .tk-panel-empty-state { text-align: center; color: var(--text2); padding: 24px 0; font-size: 0.9rem; }
  .tk-editor-header { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .tk-editor-title { color: var(--text2); font-size: 0.9rem; }
  .tk-editor-title b { color: var(--text); }
...
`;
  ...
  // ── Calls Temporárias ──
  async function saveTempVoiceConfig(guildId, enabled) {
    try {
      const params = new URLSearchParams();
      params.set('enabled', enabled ? '1' : '');
      if (enabled) {
        const form = document.getElementById('form-tempvoice');
        const data = new FormData(form);
        params.set('name_template', data.get('name_template') || '🔊 Call de {user}');
        params.set('default_limit', data.get('default_limit') || '0');
      }
      const res = await fetch('/api/' + guildId + '/tempvoice-config', {
        method: 'POST', headers: {'Content-Type':'application/x-www-form-urlencoded'}, body: params.toString()
      });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) refreshSection('tempvoice_tab');
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  ...
  async function addTicketType(guildId) {
    ...
    try {
      ...
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) { cancelarEdicaoTipoTicket(); form.reset(); refreshSection('tickets'); }
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }

  // Preenche o formulário "Tipos de Ticket" com os dados de um tipo já existente, para edição
  function editarTipoTicket(typeId) {
    const row = document.getElementById('tt-row-' + typeId);
    if (!row) return;
    document.getElementById('tt-edit-id').value = typeId;
    document.getElementById('tt-label').value = row.dataset.label || '';
    document.getElementById('tt-emoji').value = row.dataset.emoji || '';
    document.getElementById('tt-description').value = row.dataset.description || '';
    const catSelect = document.querySelector('#form-ticket-type select[name="category_id"]');
    if (catSelect) catSelect.value = row.dataset.category || '';
    const colorInput = document.getElementById('tt-color');
    if (colorInput) colorInput.value = row.dataset.color || '#5865F2';
    // Marca os cargos de suporte deste tipo nas checkboxes
    let rolesDoTipo = [];
    try { rolesDoTipo = JSON.parse(row.dataset.roles || '[]'); } catch(_) {}
    document.querySelectorAll('#tt-support-roles input[type="checkbox"]').forEach(cb => {
      cb.checked = rolesDoTipo.includes(cb.value);
    });
    document.getElementById('tt-submit-btn').textContent = '💾 Guardar Alterações';
    document.getElementById('tt-cancel-edit-btn').style.display = 'inline-block';
    document.getElementById('form-ticket-type').scrollIntoView({ behavior: 'smooth', block: 'center' });
    renderTicketPreview();
  }

  function cancelarEdicaoTipoTicket() {
    const form = document.getElementById('form-ticket-type');
    if (form) form.reset();
    document.getElementById('tt-edit-id').value = '';
    document.getElementById('tt-submit-btn').textContent = '➕ Adicionar Tipo';
    document.getElementById('tt-cancel-edit-btn').style.display = 'none';
    document.querySelectorAll('#tt-support-roles input[type="checkbox"]').forEach(cb => cb.checked = false);
    renderTicketPreview();
  }

  // ── Painéis de tickets (lista fechada estilo "Ticket King") ──
  let __tkPanels = [];
  let __tkFiltered = [];

  function tkInitPanels() {
    __tkPanels = Array.isArray(window.__ticketPanelsData) ? window.__ticketPanelsData.slice() : [];
    __tkFiltered = __tkPanels.slice();
    tkRenderList();
  }

  function tkFilterPanels() {
    const q = (document.getElementById('tk-panel-search')?.value || '').toLowerCase().trim();
    __tkFiltered = !q ? __tkPanels.slice() : __tkPanels.filter(p => {
  ...
  function tkSortPanels() {
    const mode = document.getElementById('tk-panel-sort')?.value || 'recent';
    __tkFiltered.sort((a, b) => {
      if (mode === 'name') return (a.name || '').localeCompare(b.name || '');
      if (mode === 'oldest') return a.num - b.num;
      return b.num - a.num; // recent
    });
    tkRenderList();
  }

  function tkOpenEditor() {
    document.getElementById('tk-editor-wrap').style.display = 'block';
    document.getElementById('tk-editor-wrap').scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  function tkCloseEditor() {
    document.getElementById('tk-editor-wrap').style.display = 'none';
    document.getElementById('tk-panel-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  function tkRenderList() {
    const empty = document.getElementById('tk-panel-empty');
    const list = document.getElementById('tk-panel-list');
    if (!list) return;
    if (!__tkFiltered.length) {
      empty.style.display = 'block';
      list.innerHTML = '';
      return;
    }
    empty.style.display = 'none';
    const guildId = window.__ticketGuildId;

    list.innerHTML = __tkFiltered.map(p => {
      const modeLabel = p.mode === 'buttons' ? '🔘 Botões' : '📋 Seleção';
      const channelBadge = p.channelName
        ? '<span class="tk-badge tk-badge-channel"># ' + p.channelName + '</span>'
        : '<span class="tk-badge tk-badge-draft">Sem canal</span>';
      return \`
        <div class="tk-panel-row \${p.active ? 'active' : ''}">
          <div class="tk-panel-row-left">
            <span class="tk-panel-dot" style="background:\${p.color || '#5865F2'}"></span>
            <div class="tk-panel-row-info">
              <div class="tk-panel-row-name">
                \${p.name}
                <div class="tk-panel-row-badges">
                  <span class="tk-badge tk-badge-type">\${modeLabel}</span>
                  \${channelBadge}
                </div>
              </div>
              <div class="tk-panel-row-sub">\${p.categoryName ? ('Categoria: ' + p.categoryName) : 'Categoria por definir'}\${p.sent ? '' : ' · Ainda por enviar'}</div>
            </div>
          </div>
          <div class="tk-panel-row-actions">
            <button type="button" class="tk-icon-btn" onclick="tkEditPanel('\${guildId}', \${p.id}, \${p.active})">✏️ Editar</button>
            <button type="button" class="tk-icon-btn" onclick="tkMovePanel('\${guildId}', \${p.id})">🖐️ Mover</button>
            <button type="button" class="tk-icon-btn" onclick="tkResendPanel('\${guildId}', \${p.id})" \${p.sent ? '' : 'disabled title="Este painel ainda não foi enviado"'}>📨 Reenviar</button>
            <button type="button" class="tk-icon-btn" onclick="tkEditPanel('\${guildId}', \${p.id}, \${p.active})">📑 Template</button>
            <button type="button" class="tk-icon-btn danger" onclick="apagarPainelTicket('\${guildId}', \${p.id}, '\${(p.name||'').replace(/'/g,"\\\\'")}')" \${__tkPanels.length <= 1 ? 'disabled title="Precisas de ter pelo menos 1 painel"' : ''}>🗑️</button>
          </div>
        </div>
      \`;
    }).join('');
  ...
  function tkEditPanel(guildId, panelId, isActive) {
    if (isActive) {
      // Os dados deste painel já estão carregados no formulário nesta página — só abre o editor.
      tkOpenEditor();
    } else {
      // Precisa de recarregar para trazer os dados desse outro painel para o formulário.
      window.location.href = '/dashboard/' + guildId + '?painel=' + panelId + '#tickets-editor';
    }
  }

  async function tkMovePanel(guildId, panelId) {
    toast('ℹ️ Para reordenar, edita cada painel e ajusta as suas definições. A reordenação por arrasto chega em breve.', 'success');
  }

  async function tkResendPanel(guildId, panelId) {
    const p = __tkPanels.find(x => x.id === panelId);
    if (!p || !p.channelId) { toast('❌ Este painel ainda não tem canal definido.', 'error'); return; }
    if (!confirm('Reenviar "' + p.name + '" para #' + (p.channelName || '?') + '?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/ticket-painel', {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ panel_id: panelId, channel_id: p.channelId })
      });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    if (window.__ticketPanelsData) tkInitPanels();
  });

  async function criarPainelTicket(guildId) {
    try {
      const res = await fetch('/api/' + guildId + '/ticket-panel/create', { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
  ...
  async function apagarPainelTicket(guildId, panelId, panelName) {
    if (!confirm('Apagar "' + (panelName || 'este painel') + '"? Os tipos de ticket deste painel também serão removidos. Esta ação não pode ser desfeita.')) return;
    try {
      const res = await fetch('/api/' + guildId + '/ticket-panel/' + panelId, { method: 'DELETE' });
      const json = await res.json();
      toast(json.ok ? json.message : ('❌ ' + json.message), json.ok ? 'success' : 'error');
      if (json.ok) window.location.href = '/dashboard/' + guildId + '#tickets';
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  ...
  async function updateTicketTypeColor(guildId, typeId, color) {
    ...
    const category_id = row?.dataset.category || '';
    let rolesDoTipo = [];
    try { rolesDoTipo = JSON.parse(row?.dataset.roles || '[]'); } catch(_) {}
    try {
      const params = new URLSearchParams({ id: typeId, label, emoji, color, description, category_id });
      rolesDoTipo.forEach(r => params.append('support_roles', r));
  ...
  function renderTicketPreview() {
    ...
    // Inclui o tipo que está a ser preenchido no formulário "Adicionar Tipo", assim que
    // qualquer campo (nome, emoji ou cor) tiver sido alterado — não só o nome — para o
    // preview reagir imediatamente, seja qual for a ordem em que preenches os campos.
    const novoLabel = document.getElementById('tt-label')?.value?.trim();
    const novoEmoji = document.getElementById('tt-emoji')?.value?.trim();
    const novoColorEl = document.getElementById('tt-color');
    const novoColor = novoColorEl?.value;
    const corPorOmissao = '#5865f2';
    const corAlterada = novoColor && novoColor.toLowerCase() !== corPorOmissao;
    if (novoLabel || novoEmoji || corAlterada) {
      tipos = tipos.concat([{
        id: 'novo',
        label: novoLabel || '(sem nome)',
        emoji: novoEmoji || '🎫',
        color: novoColor || '#5865F2',
      }]);
    }
...
`;
  ...
  function renderLoginPage(errorCode, retryAfter) {
    // Mensagens amigáveis para os erros que o /auth/callback pode devolver.
    // O rate_limited é tratado à parte porque é temporário e não é culpa
    // do utilizador — basta esperar uns segundos e tentar de novo.
    const ERROS_LOGIN = {
      rate_limited: retryAfter
        ? `O Discord limitou temporariamente os pedidos de login. Aguarda cerca de ${retryAfter}s e tenta novamente.`
        : 'O Discord limitou temporariamente os pedidos de login. Aguarda uns segundos e tenta novamente.',
      auth_failed: 'Não foi possível concluir o login com o Discord. Tenta novamente.',
      no_code: 'O pedido de login não foi concluído. Tenta novamente a partir do botão abaixo.'
    };
    const mensagemErro = errorCode && ERROS_LOGIN[errorCode] ? ERROS_LOGIN[errorCode] : null;
    return `<!DOCTYPE html>
<html lang="pt">
<head>
  <link rel="icon" type="image/png" href="${CONFIG.BOT_AVATAR_URL}">
  ...
  <style>
    ...
    .login-error { display:flex; align-items:center; gap:10px; background: rgba(237,66,69,0.12); border: 1px solid rgba(237,66,69,0.35); color: #ff8a8d; padding: 12px 18px; border-radius: 12px; font-size: 0.92rem; font-weight: 600; margin-bottom: 22px; max-width: 480px; }
    .login-error::before { content:'⚠️'; font-size: 1rem; }
  </style>
</head>
<body>
  <nav class="sap-home-nav">
    <div class="logo"><img src="${CONFIG.BOT_AVATAR_URL}" alt="logo" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'"> ${CONFIG.BOT_NAME}</div>
    <div class="links">
  ...
  <div class="login-page">
    ...
    <div class="login-hero">
      ...
      ${mensagemErro ? `<div class="login-error">${mensagemErro}</div>` : ''}
  ...
  function renderDashboard(user, selectedGuild, error = null) {
    ...
    const inviteUrl = `https://discord.com/api/oauth2/authorize?client_id=${CONFIG.CLIENT_ID}&permissions=8&scope=bot%20applications.commands&redirect_uri=${encodeURIComponent(CONFIG.REDIRECT_URI.replace('/auth/callback', '/invite/callback'))}&response_type=code`;
    return `<!DOCTYPE html>
<html lang="pt">
<head>
  <link rel="icon" type="image/png" href="${CONFIG.BOT_AVATAR_URL}">
  ...
  <style>
    ...
    @media(max-width:768px){
      .sap-select-header{padding:24px 12px 8px}
      .container{padding:16px 12px}
      .sap-guild-scroll{max-height:60vh}
      .sap-guild-item{padding:12px 14px;gap:10px}
      .sap-guild-item .name{font-size:0.9rem}
    }
  </style>
</head>
<body>
  ...
  <div class="container" style="max-width:640px;padding-top:8px">
    ...
    <div class="card" style="padding:0;overflow:hidden">
      <div style="display:flex;align-items:center;gap:10px;padding:18px 22px;border-bottom:1px solid var(--border);color:var(--text2);font-weight:600">
        ...
        <a href="/logout" class="logout-btn" style="margin-left:auto;background:var(--danger);color:#fff;border:none;padding:7px 16px;border-radius:999px;cursor:pointer;font-size:0.85rem;font-weight:600;text-decoration:none;transition:all 0.2s">Sair</a>
      </div>
      <div class="sap-guild-scroll">
        ${availableGuilds.length === 0 ? `<div class="alert alert-error" style="margin:16px">❌ Não tens servidores em comum com o bot. <a href="${inviteUrl}">Adiciona o bot aqui</a></div>` : availableGuilds.map(g => {
    ...
    <div style="margin-top:24px;text-align:center;color:var(--text2);font-size:0.85rem">
      Bot não está no servidor? <a href="${inviteUrl}">Adiciona aqui</a>
    </div>
    <div style="margin-top:32px;text-align:center;color:var(--text2);font-size:0.78rem">
      ${CONFIG.BOT_NAME} © ${new Date().getFullYear()} · <a href="#">Terms</a> · <a href="#">Privacy</a>
    </div>
  </div>
</body>
</html>`;
  ...
  function renderGuildDashboard(user, guild, data) {
    ...
    // Lê support_roles (JSON) devolvendo sempre um array; usa support_role como fallback (compatibilidade)
    const parseSupportRolesJs = (rolesJson, fallbackSingle) => {
      try {
    ...
    // Lista de cargos com checkboxes (multi-seleção) — usada para "Cargo(s) de Suporte" nos painéis e tipos de ticket
    const makeRoleCheckboxes = (name, options, currentList = []) => `
      <div class="role-checkbox-list" style="max-height:180px;overflow-y:auto;border:1px solid var(--border,rgba(255,255,255,0.1));border-radius:8px;padding:10px;display:flex;flex-direction:column;gap:6px">
        ${options.length ? options.map(o => `
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:400;font-size:0.85rem">
            <input type="checkbox" name="${name}" value="${o.id}" ${currentList.includes(o.id) ? 'checked' : ''} style="width:auto">
            ${escHtml(o.name)}
          </label>
        `).join('') : '<span style="color:var(--text2);font-size:0.85rem">Sem cargos neste servidor.</span>'}
      </div>`;
...
<head>
  ...
  <style>
    ...
    @media(max-width:768px){
      ...
      .navbar .logo{font-size:1.05rem;gap:6px}
      ...
      .mobile-menu-btn{
        ...
        background:var(--bg3);color:var(--text);font-size:1.2rem;cursor:pointer;margin-right:8px;
      }
      .sidebar{
        display:block;transform:translateX(-100%);transition:transform 0.22s ease;
        z-index:200;width:76vw;max-width:280px;box-shadow:4px 0 24px rgba(0,0,0,0.5);
      }
      .sidebar.open{transform:translateX(0)}
      .sidebar-overlay{
        display:none;position:fixed;inset:0;top:60px;background:rgba(0,0,0,0.55);z-index:150;
      }
      ...
      .tk-panels-toolbar{flex-direction:column;align-items:stretch}
      .tk-panel-row{flex-direction:column;align-items:flex-start}
    }
    @media(min-width:769px){.mobile-menu-btn,.sidebar-overlay{display:none}}
  </style>
</head>
<body>
  <nav class="navbar">
    <div class="logo">
      <button class="mobile-menu-btn" onclick="toggleMobileSidebar()" aria-label="Abrir menu">☰</button>
      <img src="${CONFIG.BOT_AVATAR_URL}" alt="logo" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'"> ${CONFIG.BOT_NAME}
  ...
  <div class="sidebar" id="mainSidebar">
    ...
    ${[['📊', 'Visão Geral', 'overview'], ['🎫', 'Tickets', 'tickets'], ['🔨', 'Moderação', 'mod_tab'], ['👋', 'Mensagens', 'welcome'], ['🛡️', 'AntiSpam', 'antispam'], ['📋', 'Logs', 'logs'], ['🎨', 'Embeds', 'embeds_tab'], ['⭐', 'Avaliações Staff', 'ratings'], ['💡', 'Sugestões', 'suggestions_tab'], ['❓', 'Perguntas', 'perguntas_tab'], ['🎭', 'Reaction Roles', 'rr_tab'], ['🎖️', 'Cargos', 'cargos_tab'], ['📈', 'Server Stats', 'stats_tab'], ['🔊', 'Calls Temporárias', 'tempvoice_tab'], ['🗳️', 'Votação', 'votacao_tab'], ['🎉', 'Giveaways', 'giveaways_tab'], ['🪧', 'Painéis de Info', 'infopanels_tab']].map(([ico, lbl, id]) => `<button class="sidebar-item" onclick="showSection('${id}', event)">${ico} ${lbl}</button>`).join('')}
  </div>
  ...
  <div class="main-content" id="main">
    ...
    <div id="tickets" class="section" style="display:none">
      <div class="section-title" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
        <span><span>🎫</span> Sistema de Tickets — ${escHtml(ticketConfig?.panel_name || 'Painel 1/10')}</span>
        <button type="button" class="btn btn-primary" id="btn-criar-painel" ${ticketPanels.length >= 10 ? 'disabled title="Limite de 10 painéis atingido"' : ''} onclick="criarPainelTicket('${guild.id}')">➕ Criar Painel</button>
      </div>

      <div class="card tk-panels-card">
        <div class="tk-panels-toolbar">
          <div class="tk-search-wrap">
            <span class="tk-search-icon">🔎</span>
            <input type="text" id="tk-panel-search" class="tk-search-input" placeholder="Pesquisar por conteúdo ou título do painel..." oninput="tkFilterPanels()">
          </div>
          <div class="tk-toolbar-right">
            <select id="tk-panel-sort" class="tk-sort-select" onchange="tkSortPanels()">
              <option value="recent">Ordenar: Mais Recentes</option>
              <option value="oldest">Ordenar: Mais Antigos</option>
              <option value="name">Ordenar: Nome (A-Z)</option>
            </select>
            <button type="button" class="btn btn-primary" id="btn-criar-painel-2" ${ticketPanels.length >= 10 ? 'disabled title="Limite de 10 painéis atingido"' : ''} onclick="criarPainelTicket('${guild.id}')">➕ Criar Painel</button>
          </div>
        </div>

        <div id="tk-panel-empty" style="display:none;color:var(--text2);text-align:center;padding:30px 0">Nenhum painel encontrado para essa pesquisa.</div>

        <div id="tk-panel-list" class="tk-panel-list"></div>
      </div>

      <script>
        window.__ticketPanelsData = ${JSON.stringify(ticketPanels.map(p => ({
          id: p.id,
          num: p.panel_num,
          name: p.panel_name || ('Painel ' + p.panel_num + '/10'),
          categoryName: categories.find(c => c.id === p.category_id)?.name || null,
          channelName: p.panel_channel_id ? (channels.find(c => c.id === p.panel_channel_id)?.name || null) : null,
          channelId: p.panel_channel_id || null,
          sent: !!p.panel_channel_id,
          active: p.id === (ticketConfig?.id || null),
          color: p.panel_color || '#5865F2',
          mode: p.panel_mode === 'buttons' ? 'buttons' : 'select'
        })))};
        window.__ticketGuildId = '${guild.id}';
        window.__ticketPanelsMax = 10;
      </script>

      <div id="tk-editor-wrap" style="display:none;margin-top:20px">
        <div class="tk-editor-header">
          <button type="button" class="tk-icon-btn" onclick="tkCloseEditor()">← Voltar à lista</button>
          <span class="tk-editor-title">A editar: <b id="tk-editor-panel-name">${escHtml(ticketConfig?.panel_name || 'Painel 1/10')}</b></span>
        </div>

      <div class="card" style="margin-top:14px">
        <h2>⚙️ Configuração de Tickets</h2>
        <form id="form-tickets">
          <input type="hidden" name="panel_id" id="tickets_panel_id" value="${ticketConfig?.id || ''}">
          <div class="grid-2">
            <div class="form-group">
          ...
          <div class="form-group">
            <label>Cargo(s) de Suporte (todos são marcados e têm acesso ao ticket)</label>
            ${makeRoleCheckboxes('support_roles', roles, parseSupportRolesJs(ticketConfig?.support_roles, ticketConfig?.support_role))}
          </div>
          <div class="form-group">
      ...
      <div class="card" style="margin-top:20px">
        <h2>🏷️ Tipos de Ticket (${escHtml(ticketConfig?.panel_name || 'Painel 1/10')})</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">Cria diferentes tipos de ticket (ex: Suporte, Denúncia, Parceria). Se houver pelo menos 1 tipo, o painel de tickets mostra um menu de seleção ou botões (conforme o modo escolhido abaixo) em vez de um botão simples. Clica em ✏️ para editar um tipo já criado (nome, descrição, cargos, etc).</p>
        ...
        <form id="form-ticket-type">
          <input type="hidden" name="id" id="tt-edit-id" value="">
          <input type="hidden" name="panel_id" value="${ticketConfig?.id || ''}">
          <div class="grid-2">
            <div class="form-group">
          ...
          <div class="form-group">
            <label>Cargo(s) de Suporte deste tipo (vazio = usar os cargos padrão do painel)</label>
            <div id="tt-support-roles">${makeRoleCheckboxes('support_roles', roles, [])}</div>
          </div>
          <div class="form-group">
            <label>Descrição (aparece no menu, modo dropdown)</label>
            <input type="text" name="description" id="tt-description" placeholder="Ex: Para problemas técnicos com a tua conta" maxlength="100">
          </div>
          ...
          <div class="form-group">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
              <input type="checkbox" name="has_form" id="tt-has-form" value="1" style="width:auto">
              📋 Com formulário (o utilizador preenche perguntas ao abrir o ticket)
            </label>
          </div>
          <button type="button" class="btn btn-primary" id="tt-submit-btn" onclick="addTicketType('${guild.id}')">➕ Adicionar Tipo</button>
          <button type="button" class="btn btn-secondary" id="tt-cancel-edit-btn" style="display:none" onclick="cancelarEdicaoTipoTicket()">✕ Cancelar edição</button>
        </form>
        <div id="ticket-types-table" style="margin-top:16px">
          ${ticketTypes.length ? `
            <table class="data-table">
              ...
              <tbody>
                ${ticketTypes.map(t => `
                  <tr id="tt-row-${t.id}" data-color="${t.color || '#5865F2'}" data-emoji="${escHtml(t.emoji || '🎫')}" data-label="${escHtml(t.label)}" data-description="${escHtml(t.description || '')}" data-category="${t.category_id || ''}" data-roles="${escHtml(JSON.stringify(parseSupportRolesJs(t.support_roles, t.support_role)))}">
                    ...
                    <td style="white-space:nowrap">
                      <button type="button" class="btn btn-secondary" style="padding:4px 10px;font-size:0.8rem" onclick="editarTipoTicket(${t.id})">✏️</button>
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
          ...
          <div class="form-group">
            ...
            <div class="discord-preview">
              <div class="discord-preview-msgrow">
                <div class="discord-preview-avatar" style="overflow:hidden;padding:0"><img src="${CONFIG.BOT_AVATAR_URL}" alt="logo" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.parentElement.textContent='🤖'"></div>
                <div style="flex:1;min-width:0">
                  <div class="discord-preview-username">${(CONFIG.BOT_NAME || 'Bot').replace(/[<>&]/g, '')} <span class="discord-preview-bottag">BOT</span></div>
    ...
    <div id="welcome" class="section" style="display:none">
      <div class="section-title"><span>👋</span> Mensagens Personalizadas</div>

      <!-- SUB-ABAS: DM ao entrar / Boas-vindas / Mensagem de Saída -->
      <div class="card" style="padding:8px;margin-bottom:16px">
        <div style="display:flex;gap:6px">
          <button type="button" class="btn btn-primary" id="subtab-btn-dm" onclick="showWelcomeSubtab('dm')" style="flex:1">📩 Mensagem de DM</button>
          <button type="button" class="btn" id="subtab-btn-boasvindas" onclick="showWelcomeSubtab('boasvindas')" style="flex:1">👋 Boas-vindas</button>
          <button type="button" class="btn" id="subtab-btn-saida" onclick="showWelcomeSubtab('saida')" style="flex:1">👋 Mensagem de Saída</button>
        </div>
      </div>

      <!-- SUB-ABA: MENSAGEM DE DM AO ENTRAR -->
      <div id="subtab-dm" style="display:none">
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
            <p style="color:var(--text2);font-size:0.85rem;margin:0">
              Mensagem enviada por <b>DM</b> (mensagem privada) a cada novo membro assim que entra no servidor. Funciona em paralelo com a mensagem de boas-vindas no canal, se ambas estiverem configuradas — se o membro tiver as DMs fechadas, é simplesmente ignorada. Só uma pode estar <b>ativa</b> de cada vez.
            </p>
            <button type="button" class="btn btn-primary" onclick="abrirModalDm('${guild.id}')" style="white-space:nowrap;margin-left:14px">➕ Criar nova</button>
          </div>
          <div id="dm-messages-list">
            <p style="color:var(--text2);font-size:0.85rem">A carregar...</p>
          </div>
        </div>
      </div>

      <!-- SUB-ABA: BOAS-VINDAS (inalterado) -->
      <div id="subtab-boasvindas" style="display:none">
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
            <p style="color:var(--text2);font-size:0.85rem;margin:0">
      ...
      <!-- SUB-ABA: MENSAGEM DE SAÍDA -->
      <div id="subtab-saida" style="display:none">
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
            <p style="color:var(--text2);font-size:0.85rem;margin:0">
              Mensagem enviada num <b>canal</b> sempre que um membro sai do servidor (ex: "${'{user}'} saiu do servidor."). Só uma pode estar <b>ativa</b> de cada vez.
            </p>
            <button type="button" class="btn btn-primary" onclick="abrirModalLeave('${guild.id}')" style="white-space:nowrap;margin-left:14px">➕ Criar nova</button>
          </div>
          <div id="leave-messages-list">
            <p style="color:var(--text2);font-size:0.85rem">A carregar...</p>
          </div>
        </div>
      </div>
    ...
    <!-- MODAL: CRIAR/EDITAR MENSAGEM DE DM AO ENTRAR -->
    <div id="modal-dm" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:1000;align-items:center;justify-content:center">
      <div class="card" style="max-width:960px;width:95%;max-height:90vh;overflow:auto">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <h3 id="modal-dm-titulo" style="margin:0">➕ Nova Mensagem de DM</h3>
          <button type="button" class="btn" onclick="fecharModalDm()" style="padding:4px 10px">✖</button>
        </div>
        ...
        <form id="form-dm" oninput="atualizarPreviewDm('${guild.id}')" onchange="atualizarPreviewDm('${guild.id}')">
          <input type="hidden" name="id" id="dm_id" value="">

          <div class="form-group">
            <label>Nome interno (só para te ajudar a identificar na lista)</label>
            <input type="text" name="name" id="dm_name" placeholder="Ex: DM de Boas-vindas" required>
          </div>

          <div class="form-group">
            <label class="toggle">
              <input type="checkbox" name="dm_embed" id="dm_embed" value="1" checked>
              <span>Usar Embed na mensagem de DM</span>
            </label>
          </div>

          <div class="form-group">
            <label>Mensagem fora da Embed (opcional)</label>
            <input type="text" name="dm_content" id="dm_content" placeholder="Bem-Vindo \${usermention} à **\${guildname}**!">
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:10px">
            ...
            <div>
              <div class="form-group">
                <label>Nome do Autor (linha pequena acima do título, opcional)</label>
                <input type="text" name="dm_author_name" id="dm_author_name" placeholder="Ex: ${CONFIG.BOT_NAME}">
              </div>
              <div class="form-group">
                <label>Ícone do Autor (URL, opcional)</label>
                <input type="text" name="dm_author_icon" id="dm_author_icon" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Título da Embed</label>
                <input type="text" name="dm_title" id="dm_title" placeholder="Ex: 🔥 Bem-vindo(a) a \${guildname}!">
              </div>
              <div class="form-group">
                <label>Link do Título (opcional)</label>
                <input type="text" name="dm_url" id="dm_url" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Descrição</label>
                <textarea name="dm_msg" id="dm_msg" rows="5">Bem-vindo(a) \${usermention} ao \${guildname}!</textarea>
              </div>
              <div class="form-group">
                <label>Cor da barra lateral</label>
                <input type="color" name="dm_color" id="dm_color" value="#5865F2" style="width:60px;height:38px;padding:2px;cursor:pointer">
              </div>
              <div class="form-group">
                <label>Imagem pequena (thumbnail, canto superior direito, opcional)</label>
                <input type="text" name="dm_thumbnail" id="dm_thumbnail" placeholder="https://exemplo.com/avatar.png">
              </div>
              <div class="form-group">
                <label>Imagem grande (banner, em baixo da embed, opcional)</label>
                <input type="text" name="dm_image" id="dm_image" placeholder="https://exemplo.com/imagem.png">
              </div>
              <div class="form-group">
                <label>Rodapé (footer, opcional)</label>
                <input type="text" name="dm_footer" id="dm_footer" placeholder="Ex: Membro nº \${membercount}">
              </div>
              <button type="button" class="btn btn-primary" onclick="guardarDmMessage('${guild.id}')">💾 Guardar</button>
            </div>

            <!-- COLUNA DIREITA: PREVIEW AO VIVO -->
            <div>
              <label style="display:block;margin-bottom:8px;color:var(--text2);font-size:0.85rem;font-weight:600">👁️ PREVIEW</label>
              <div id="dm-preview-box" style="background:#313338;border-radius:8px;padding:16px;min-height:120px;font-family:'gg sans',Whitney,Helvetica,Arial,sans-serif">
                <p style="color:#949ba4;font-size:0.85rem">A carregar preview...</p>
              </div>
            </div>
          </div>
    ...
    <!-- MODAL: CRIAR/EDITAR MENSAGEM DE SAÍDA -->
    <div id="modal-leave" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:1000;align-items:center;justify-content:center">
      <div class="card" style="max-width:960px;width:95%;max-height:90vh;overflow:auto">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <h3 id="modal-leave-titulo" style="margin:0">➕ Nova Mensagem de Saída</h3>
          <button type="button" class="btn" onclick="fecharModalLeave()" style="padding:4px 10px">✖</button>
        </div>
        ...
        <form id="form-leave" oninput="atualizarPreviewLeave('${guild.id}')" onchange="atualizarPreviewLeave('${guild.id}')">
          <input type="hidden" name="id" id="leave_id" value="">

          <div class="form-group">
            <label>Nome interno (só para te ajudar a identificar na lista)</label>
            <input type="text" name="name" id="leave_name" placeholder="Ex: Saída do Servidor" required>
          </div>

          <div class="form-group">
            <label>Canal da Mensagem de Saída</label>
            ${makeSelect('leave_channel', channels, null, 'Canal')}
          </div>

          <div class="form-group">
            <label class="toggle">
              <input type="checkbox" name="leave_embed" id="leave_embed" value="1" checked>
              <span>Usar Embed na mensagem de saída</span>
            </label>
          </div>

          <div class="form-group">
            <label>Mensagem fora da Embed (opcional)</label>
            <input type="text" name="leave_content" id="leave_content" placeholder="\${usermention} saiu de **\${guildname}**.">
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:10px">
            ...
            <div>
              <div class="form-group">
                <label>Nome do Autor (linha pequena acima do título, opcional)</label>
                <input type="text" name="leave_author_name" id="leave_author_name" placeholder="Ex: ${CONFIG.BOT_NAME}">
              </div>
              <div class="form-group">
                <label>Ícone do Autor (URL, opcional)</label>
                <input type="text" name="leave_author_icon" id="leave_author_icon" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Título da Embed</label>
                <input type="text" name="leave_title" id="leave_title" placeholder="Ex: 👋 Até já, \${username}!">
              </div>
              <div class="form-group">
                <label>Link do Título (opcional)</label>
                <input type="text" name="leave_url" id="leave_url" placeholder="https://...">
              </div>
              <div class="form-group">
                <label>Descrição</label>
                <textarea name="leave_msg" id="leave_msg" rows="5">\${username} saiu de \${guildname}.</textarea>
              </div>
              <div class="form-group">
                <label>Cor da barra lateral</label>
                <input type="color" name="leave_color" id="leave_color" value="#ED4245" style="width:60px;height:38px;padding:2px;cursor:pointer">
              </div>
              <div class="form-group">
                <label>Imagem pequena (thumbnail, canto superior direito, opcional)</label>
                <input type="text" name="leave_thumbnail" id="leave_thumbnail" placeholder="https://exemplo.com/avatar.png">
              </div>
              <div class="form-group">
                <label>Imagem grande (banner, em baixo da embed, opcional)</label>
                <input type="text" name="leave_image" id="leave_image" placeholder="https://exemplo.com/imagem.png">
              </div>
              <div class="form-group">
                <label>Rodapé (footer, opcional)</label>
                <input type="text" name="leave_footer" id="leave_footer" placeholder="Ex: Membro nº \${membercount}">
              </div>
              <button type="button" class="btn btn-primary" onclick="guardarLeaveMessage('${guild.id}')">💾 Guardar</button>
            </div>

            <!-- COLUNA DIREITA: PREVIEW AO VIVO -->
            <div>
              <label style="display:block;margin-bottom:8px;color:var(--text2);font-size:0.85rem;font-weight:600">👁️ PREVIEW</label>
              <div id="leave-preview-box" style="background:#313338;border-radius:8px;padding:16px;min-height:120px;font-family:'gg sans',Whitney,Helvetica,Arial,sans-serif">
                <p style="color:#949ba4;font-size:0.85rem">A carregar preview...</p>
              </div>
            </div>
          </div>
    ...
    <div id="antispam" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        <h2>🤖 AutoMod IA</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Usa uma IA para analisar mensagens e detetar insultos, discurso de ódio, ameaças ou conteúdo impróprio — mesmo disfarçado (ex: <code>merd4</code>, <code>m.e.r.d.a</code>). Só corre em mensagens que passam pelo filtro de palavras acima, para poupar chamadas à API.
        </p>
        ${!CONFIG.AI_API_KEY ? `
        <div style="background:rgba(237,66,69,0.12);border:1px solid var(--red,#ED4245);border-radius:8px;padding:10px 14px;margin-bottom:16px;font-size:0.85rem">
          ⚠️ O AutoMod IA ainda não foi configurado pelo dono do bot (falta a chave de API nas variáveis de ambiente). Ativar aqui não terá efeito até isso ser feito.
        </div>` : ''}
        <form id="form-ai-moderation">
          <div class="form-group">
            <label class="toggle">
              <input type="checkbox" name="enabled" value="1" ${aiModerationConfig?.enabled ? 'checked' : ''}>
              <span><strong>Ativar AutoMod IA</strong></span>
            </label>
          </div>
          <div class="grid-2">
            <div class="form-group">
              <label>Canal de Log</label>
              ${makeSelect('log_channel', channels, aiModerationConfig?.log_channel, 'Canal')}
            </div>
            <div class="form-group">
              <label>Advertências até Castigo Automático</label>
              <input type="number" name="max_warnings" value="${aiModerationConfig?.max_warnings || 3}" min="1" max="20">
            </div>
            <div class="form-group">
              <label>Duração do Timeout (minutos)</label>
              <input type="number" name="timeout_minutes" value="${aiModerationConfig?.timeout_minutes || 10}" min="1" max="40320">
            </div>
            <div class="form-group">
              <label>Sensibilidade Mínima para Agir</label>
              <select name="min_severity">
                <option value="LOW" ${aiModerationConfig?.min_severity === 'LOW' ? 'selected' : ''}>Baixa (mais rígido, mais falsos positivos)</option>
                <option value="MEDIUM" ${(!aiModerationConfig?.min_severity || aiModerationConfig?.min_severity === 'MEDIUM') ? 'selected' : ''}>Média (recomendado)</option>
                <option value="HIGH" ${aiModerationConfig?.min_severity === 'HIGH' ? 'selected' : ''}>Alta (só casos graves)</option>
              </select>
            </div>
            <div class="form-group">
              <label>Reset Automático de Advertências</label>
              <select name="warning_reset_days">
                <option value="0" ${(!aiModerationConfig?.warning_reset_days || aiModerationConfig?.warning_reset_days == 0) ? 'selected' : ''}>Nunca (advertências acumulam para sempre)</option>
                <option value="1" ${aiModerationConfig?.warning_reset_days == 1 ? 'selected' : ''}>1 dia sem nova infração</option>
                <option value="3" ${aiModerationConfig?.warning_reset_days == 3 ? 'selected' : ''}>3 dias sem nova infração</option>
                <option value="7" ${aiModerationConfig?.warning_reset_days == 7 ? 'selected' : ''}>7 dias sem nova infração</option>
                <option value="14" ${aiModerationConfig?.warning_reset_days == 14 ? 'selected' : ''}>14 dias sem nova infração</option>
                <option value="30" ${aiModerationConfig?.warning_reset_days == 30 ? 'selected' : ''}>30 dias sem nova infração</option>
                <option value="90" ${aiModerationConfig?.warning_reset_days == 90 ? 'selected' : ''}>90 dias sem nova infração</option>
              </select>
              <small style="opacity:.7">Se o utilizador ficar este tempo sem nova violação, a contagem de advertências volta a 0.</small>
            </div>
          </div>
          <button type="button" class="btn btn-primary" onclick="saveConfig('${guild.id}','ai-moderation-config','form-ai-moderation')">💾 Guardar AutoMod IA</button>
        </form>
      </div>
    ...
    <div id="suggestions_tab" class="section" style="display:none">
      ...
      <div class="card">
        <h2>⚙️ Configuração das Sugestões</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Define o canal onde as sugestões submetidas com <code>/sugerir</code> são publicadas, o canal de log (opcional) e o cargo a mencionar (opcional).
        </p>
        <form id="form-sugestao-config">
          <div class="grid-2">
            <div class="form-group">
            ...
            <div class="form-group">
              <label>Ativo</label>
              <select name="enabled"><option value="1">Sim</option><option value="0">Não</option></select>
            </div>
          </div>
          <div class="grid-2">
            <div class="form-group">
              <label>Canal onde as sugestões são publicadas</label>
          ...
          <button type="button" class="btn btn-primary" onclick="guardarSugestaoConfig('${guild.id}')">💾 Guardar</button>
        </form>
      </div>
    ...
    <!-- CALLS TEMPORÁRIAS -->
    <div id="tempvoice_tab" class="section" style="display:none">
      <div class="section-title"><span>🔊</span> Calls Temporárias</div>
      <div class="card">
        <div style="margin:0 0 16px;padding:12px;background:var(--bg3);border-radius:8px;font-size:0.85rem;color:var(--text2)">
          <strong>ℹ️ Como funciona:</strong> cria uma categoria com um canal "Criar Call" e um canal de texto "Interface". Ao entrar no canal "Criar Call", o membro recebe automaticamente a sua própria call de voz, que pode configurar através dos botões na Interface. A call é apagada assim que ficar vazia.
        </div>

        ${tvConfig?.enabled ? `
          <div class="form-group">
            <label>Estado</label>
            <div style="display:flex;align-items:center;gap:8px;color:var(--success,#57F287);font-weight:600">✅ Ativo · ${tvChannelsCount} call(s) ativa(s) neste momento</div>
          </div>
        ` : `
          <div class="form-group">
            <label>Estado</label>
            <div style="color:var(--text2)">⛔ Sistema ainda não configurado neste servidor.</div>
          </div>
        `}

        <form id="form-tempvoice">
          <div class="form-group">
            <label>Nome padrão das calls criadas</label>
            <input type="text" name="name_template" value="${tvConfig?.name_template ? tvConfig.name_template.replace(/"/g, '&quot;') : '🔊 Call de {user}'}" placeholder="Usa {user} para o nome de quem criou" maxlength="100">
          </div>
          <div class="form-group">
            <label>Limite padrão de utilizadores (0 = sem limite)</label>
            <input type="number" name="default_limit" value="${tvConfig?.default_limit ?? 0}" min="0" max="99">
          </div>
        </form>

        <div class="grid-2">
          <button type="button" class="btn btn-primary" onclick="saveTempVoiceConfig('${guild.id}', true)">✅ ${tvConfig?.enabled ? 'Guardar Alterações' : 'Ativar / Criar Sistema'}</button>
          ${tvConfig?.enabled ? `<button type="button" class="btn btn-danger" onclick="saveTempVoiceConfig('${guild.id}', false)">⛔ Remover (apaga a categoria e os canais)</button>` : ''}
        </div>
      </div>
    </div>
    ...
    <div id="infopanels_tab" class="section" style="display:none">
      ...
      <div class="card" id="infopanel-form-card">
        <h2 id="infopanel-form-titulo">➕ Criar Painel</h2>
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Cria um embed de informação (ex: "Bem-vindo ao Servidor") com título, imagens, dono, tempo de fundação, etc.
          Um painel novo fica em <strong>rascunho</strong> — só é enviado para o Discord quando clicares em <strong>🚀 Publicar</strong> na tabela abaixo, depois de teres adicionado todos os botões. Assim a mensagem já nasce completa, sem aparecer "(editado)".
          Se estiveres a <strong>editar</strong> um painel já publicado, guardar as alterações atualiza automaticamente a mensagem existente no Discord.
        </p>
        <form id="form-infopanel-criar">
          <input type="hidden" name="id" id="infopanel-edit-id" value="">
          <div class="grid-2">
            <div class="form-group">
          ...
          <button type="button" class="btn btn-primary" id="infopanel-form-btn" onclick="guardarInfoPanel('${guild.id}')">🪧 Criar Painel</button>
          <button type="button" class="btn btn-secondary" id="infopanel-form-cancelar" style="display:none" onclick="cancelarEdicaoInfoPanel()">✕ Cancelar edição</button>
        </form>
      </div>
      ...
      <div class="card" id="infopanel-buttons-card" style="display:none">
        ...
        <h3 id="infopanel-botao-form-titulo" style="margin-bottom:10px">➕ Adicionar Botão</h3>
        <form id="form-infopanel-botao-add">
          <input type="hidden" name="panel_id" id="infopanel-buttons-panel-id">
          <input type="hidden" id="infopanel-botao-edit-id" value="">
          <div class="grid-2">
            <div class="form-group">
          ...
          <button type="button" class="btn btn-primary" id="infopanel-botao-form-btn" onclick="guardarInfoPanelBotao('${guild.id}')">➕ Adicionar Botão</button>
          <button type="button" class="btn btn-secondary" id="infopanel-botao-form-cancelar" style="display:none" onclick="cancelarEdicaoInfoPanelBotao()">✕ Cancelar edição</button>
        </form>
      </div>
    </div>
  ...
  <script>
    ...
    // Menu lateral em modo "gaveta" no telemóvel (a sidebar normal fica
    // escondida por defeito em ecrãs pequenos e desliza para dentro/fora).
    ...
    function showSection(id, evt) {
      ...
      if (window.innerWidth <= 768) toggleMobileSidebar(false); // fecha o menu ao escolher uma secção no telemóvel
      if(id==='overview') loadOverviewData();
      if(id==='ratings') loadRatings();
      if(id==='suggestions_tab') { loadSuggestions(); loadSugestaoConfig(); }
    ...
    function restoreSection() {
      if (window.location.hash === '#tickets-editor') {
        showSection('tickets', null);
        return;
      }
      let saved = null;
    ...
    async function loadSugestaoConfig() {
      const form = document.getElementById('form-sugestao-config');
      if (!form) return;
      try {
        const r = await fetch('/api/'+GUILD_ID+'/sugestao-config');
        const cfg = await r.json();
        if (cfg) {
          form.elements['emoji'].value = cfg.emoji || '';
          form.elements['enabled'].value = cfg.enabled ? '1' : '0';
          form.elements['channel_id'].value = cfg.channel_id || '';
          form.elements['log_channel'].value = cfg.log_channel || '';
          form.elements['ping_role'].value = cfg.ping_role || '';
        }
      } catch (e) {}
    }

    async function guardarSugestaoConfig(guildId) {
      const form = document.getElementById('form-sugestao-config');
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      try {
        const r = await fetch('/api/'+guildId+'/sugestao-config', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body)
        });
        ...
        toast('❌ Erro de ligação ao guardar a configuração.', 'error');
      }
    }
          ...
          paineis.map(p => {
            ...
            return '<tr>' +
              ...
              '<td>' +
                ...
                '<button class="btn btn-secondary" style="padding:4px 10px;font-size:0.8rem" onclick="editarInfoPanel('+p.id+')" title="Editar título, descrição, imagens, cor, etc.">✏️ Editar</button> ' +
    ...
    async function guardarInfoPanel(guildId) {
      const form = document.getElementById('form-infopanel-criar');
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      const id = (document.getElementById('infopanel-edit-id').value || '').trim();
      const url = id ? '/api/'+guildId+'/infopanels/'+id : '/api/'+guildId+'/infopanels';
      ...
      try {
        ...
        const data = await r.json();
        toast(data.message || (data.ok ? 'Guardado!' : 'Erro.'), data.ok ? 'success' : 'error');
        if (data.ok) { cancelarEdicaoInfoPanel(); loadInfoPanels(); }
      } catch (e) {
        toast('❌ Erro de ligação ao guardar o painel.', 'error');
      }
    }

    async function editarInfoPanel(id) {
      try {
        const r = await fetch('/api/'+GUILD_ID+'/infopanels');
        const paineis = await r.json();
        const p = paineis.find(x => x.id === id);
        if (!p) { toast('❌ Painel não encontrado.', 'error'); return; }
        const form = document.getElementById('form-infopanel-criar');
        document.getElementById('infopanel-edit-id').value = p.id;
        form.querySelector('[name="name"]').value = p.name || '';
        form.querySelector('[name="title"]').value = p.title || '';
        form.querySelector('[name="description"]').value = p.description || '';
        form.querySelector('[name="banner_url"]').value = p.banner_url || '';
        form.querySelector('[name="thumbnail_url"]').value = p.thumbnail_url || '';
        form.querySelector('[name="owner_text"]').value = p.owner_text || '';
        form.querySelector('[name="founded_text"]').value = p.founded_text || '';
        form.querySelector('[name="footer_text"]').value = p.footer_text || '';
        form.querySelector('[name="color"]').value = p.color || '#5865F2';
        const selCanal = form.querySelector('[name="channel_id"]');
        if (selCanal) selCanal.value = p.channel_id || '';

        document.getElementById('infopanel-form-titulo').textContent = '✏️ Editar Painel: ' + p.name;
        document.getElementById('infopanel-form-btn').textContent = '💾 Guardar Alterações';
        document.getElementById('infopanel-form-cancelar').style.display = 'inline-block';
        document.getElementById('infopanel-form-card').scrollIntoView({ behavior: 'smooth' });
      } catch (e) {
        toast('❌ Erro de ligação ao carregar o painel.', 'error');
      }
    }

    function cancelarEdicaoInfoPanel() {
      const form = document.getElementById('form-infopanel-criar');
      form.reset();
      document.getElementById('infopanel-edit-id').value = '';
      document.getElementById('infopanel-form-titulo').textContent = '➕ Criar Painel';
      document.getElementById('infopanel-form-btn').textContent = '🪧 Criar Painel';
      document.getElementById('infopanel-form-cancelar').style.display = 'none';
    }
    ...
    async function loadInfoPanelBotoes(panelId) {
      try {
        ...
        window._infopanelBotoesCache = botoes;
        const html = botoes.length ? botoes.map(b =>
          '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">' +
            ...
            '<div style="display:flex;gap:6px">' +
              '<button class="btn btn-secondary" style="padding:4px 10px;font-size:0.8rem" onclick="editarInfoPanelBotao('+b.id+')" title="Editar botão">✏️</button>' +
    ...
    async function guardarInfoPanelBotao(guildId) {
      const form = document.getElementById('form-infopanel-botao-add');
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      const buttonId = (document.getElementById('infopanel-botao-edit-id').value || '').trim();
      const url = buttonId
        ? '/api/'+guildId+'/infopanels/'+currentInfoPanelId+'/buttons/'+buttonId
        : '/api/'+guildId+'/infopanels/'+currentInfoPanelId+'/buttons';
      ...
      try {
        ...
        toast('❌ Erro de ligação ao guardar o botão.', 'error');
      }
    }

    function editarInfoPanelBotao(buttonId) {
      const b = (window._infopanelBotoesCache || []).find(x => x.id === buttonId);
      if (!b) { toast('❌ Botão não encontrado.', 'error'); return; }
      const form = document.getElementById('form-infopanel-botao-add');
      document.getElementById('infopanel-botao-edit-id').value = b.id;
      form.querySelector('[name="label"]').value = b.label || '';
      form.querySelector('[name="emoji"]').value = b.emoji || '';
      form.querySelector('[name="style"]').value = b.style || 'Primary';
      form.querySelector('[name="response_text"]').value = b.response_text || '';
      form.querySelector('[name="response_title"]').value = b.response_title || '';
      form.querySelector('[name="response_color"]').value = b.response_color || '';
      form.querySelector('[name="response_image"]').value = b.response_image || '';
      form.querySelector('[name="response_thumbnail"]').value = b.response_thumbnail || '';

      document.getElementById('infopanel-botao-form-titulo').textContent = '✏️ Editar Botão: ' + b.label;
      document.getElementById('infopanel-botao-form-btn').textContent = '💾 Guardar Alterações';
      document.getElementById('infopanel-botao-form-cancelar').style.display = 'inline-block';
      form.scrollIntoView({ behavior: 'smooth' });
    }

    function cancelarEdicaoInfoPanelBotao() {
      const form = document.getElementById('form-infopanel-botao-add');
      form.reset();
      document.getElementById('infopanel-botao-edit-id').value = '';
      document.getElementById('infopanel-buttons-panel-id').value = currentInfoPanelId;
      document.getElementById('infopanel-botao-form-titulo').textContent = '➕ Adicionar Botão';
      document.getElementById('infopanel-botao-form-btn').textContent = '➕ Adicionar Botão';
      document.getElementById('infopanel-botao-form-cancelar').style.display = 'none';
    }
    ...
    // ============================
    // SUB-ABAS DE "MENSAGENS PERSONALIZADAS" (DM / Boas-vindas / Saída)
    // ============================
    const WELCOME_SUBTABS = ['dm', 'boasvindas', 'saida'];
    function showWelcomeSubtab(nome) {
      WELCOME_SUBTABS.forEach(t => {
        const painel = document.getElementById('subtab-' + t);
        const botao = document.getElementById('subtab-btn-' + t);
        if (painel) painel.style.display = (t === nome) ? '' : 'none';
        if (botao) botao.className = (t === nome) ? 'btn btn-primary' : 'btn';
      });
      try { localStorage.setItem('welcomeSubtab', nome); } catch (_) {}
      if (nome === 'dm') loadDmMessages(GUILD_ID);
      if (nome === 'boasvindas') loadWelcomeMessages(GUILD_ID);
      if (nome === 'saida') loadLeaveMessages(GUILD_ID);
    }

    // ============================
    // LISTA + MODAL DE MENSAGENS DE DM AO ENTRAR
    // ============================
    async function loadDmMessages(guildId) {
      const box = document.getElementById('dm-messages-list');
      if (!box) return;
      try {
        const r = await fetch('/api/'+guildId+'/dm-messages');
        const lista = await r.json();
        if (!Array.isArray(lista) || !lista.length) {
          box.innerHTML = '<p style="color:var(--text2)">Nenhuma mensagem de DM criada ainda. Clica em "➕ Criar nova" para começar.</p>';
          return;
        }
        box.innerHTML = lista.map(w => \`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border:1px solid var(--border);border-radius:8px;margin-bottom:10px;background:var(--bg2)">
            <div>
              ...
              <div style="color:var(--text2);font-size:0.8rem;margin-top:2px">Enviada por mensagem privada (DM)</div>
            </div>
            <div style="display:flex;gap:6px">
              \${!w.is_active ? \`<button type="button" class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick="ativarDmMessage('\${guildId}', \${w.id})" title="Tornar ativa">✅ Ativar</button>\` : ''}
              <button type="button" class="btn" style="padding:4px 10px;font-size:0.8rem" onclick="abrirModalDm('\${guildId}', \${w.id})">✏️ Editar</button>
              <button type="button" class="btn btn-danger" style="padding:4px 10px;font-size:0.8rem" onclick="apagarDmMessage('\${guildId}', \${w.id})">🗑️</button>
            </div>
          </div>
        \`).join('');
      } catch (e) {
        box.innerHTML = '<p style="color:#f23f42">Erro de ligação ao carregar as mensagens de DM.</p>';
      }
    }

    function limparFormDm() {
      const form = document.getElementById('form-dm');
      if (form) form.reset();
      document.getElementById('dm_id').value = '';
      document.getElementById('dm_embed').checked = true;
      document.getElementById('dm_color').value = '#5865F2';
      document.getElementById('dm_msg').value = 'Bem-vindo(a) \${usermention} ao \${guildname}!';
    }

    async function abrirModalDm(guildId, id) {
      limparFormDm();
      const modal = document.getElementById('modal-dm');
      const titulo = document.getElementById('modal-dm-titulo');

      if (id) {
        titulo.textContent = '✏️ Editar Mensagem de DM';
        try {
          const r = await fetch('/api/'+guildId+'/dm-messages/'+id);
          const w = await r.json();
          if (w && w.id) {
            document.getElementById('dm_id').value = w.id;
            document.getElementById('dm_name').value = w.name || '';
            document.getElementById('dm_content').value = w.dm_content || '';
            document.getElementById('dm_author_name').value = w.dm_author_name || '';
            document.getElementById('dm_author_icon').value = w.dm_author_icon || '';
            document.getElementById('dm_title').value = w.dm_title || '';
            document.getElementById('dm_url').value = w.dm_url || '';
            document.getElementById('dm_msg').value = w.dm_msg || '';
            document.getElementById('dm_color').value = w.dm_color || '#5865F2';
            document.getElementById('dm_image').value = w.dm_image || '';
            document.getElementById('dm_thumbnail').value = w.dm_thumbnail || '';
            document.getElementById('dm_footer').value = w.dm_footer || '';
            document.getElementById('dm_embed').checked = !!w.dm_embed;
          }
        } catch (e) {
          toast('❌ Erro ao carregar a mensagem de DM.', 'error');
        }
      } else {
        titulo.textContent = '➕ Nova Mensagem de DM';
      }

      modal.style.display = 'flex';
      setTimeout(() => enviarPreviewDm(guildId), 50);
    }
    ...
    async function guardarDmMessage(guildId) {
      const form = document.getElementById('form-dm');
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      body.dm_embed = document.getElementById('dm_embed').checked ? '1' : '0';

      const id = body.id;
      const url = id ? '/api/'+guildId+'/dm-messages/'+id : '/api/'+guildId+'/dm-messages';
      const method = id ? 'PUT' : 'POST';
    ...
    async function ativarDmMessage(guildId, id) {
      try {
        const r = await fetch('/api/'+guildId+'/dm-messages/'+id+'/activate', { method: 'POST' });
        const data = await r.json();
        toast(data.message || (data.ok ? 'Ativada!' : 'Erro.'), data.ok ? 'success' : 'error');
        if (data.ok) loadDmMessages(guildId);
      } catch (e) {
        toast('❌ Erro de ligação.', 'error');
      }
    }

    async function apagarDmMessage(guildId, id) {
      if (!confirm('Apagar esta mensagem de DM?')) return;
      try {
        const r = await fetch('/api/'+guildId+'/dm-messages/'+id, { method: 'DELETE' });
        const data = await r.json();
        toast(data.message || (data.ok ? 'Apagada!' : 'Erro.'), data.ok ? 'success' : 'error');
        if (data.ok) loadDmMessages(guildId);
      } catch (e) {
    ...
    let dmPreviewTimer = null;
    function atualizarPreviewDm(guildId) {
      clearTimeout(dmPreviewTimer);
      dmPreviewTimer = setTimeout(() => enviarPreviewDm(guildId), 250);
    }

    async function enviarPreviewDm(guildId) {
      const form = document.getElementById('form-dm');
      ...
      try {
        const r = await fetch('/api/'+guildId+'/dm-preview', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body)
        });
        const data = await r.json();
        if (!data.ok) { box.innerHTML = '<p style="color:#f23f42">'+ (data.message||'Erro ao gerar preview') +'</p>'; return; }
        // Reaproveita o renderizador de preview do welcome, mapeando os campos dm_* para welcome_*
        const bodyMapeado = {
          welcome_content: body.dm_content, welcome_author_name: body.dm_author_name,
          welcome_author_icon: body.dm_author_icon, welcome_title: body.dm_title,
          welcome_url: body.dm_url, welcome_msg: body.dm_msg, welcome_color: body.dm_color,
          welcome_image: body.dm_image, welcome_thumbnail: body.dm_thumbnail, welcome_footer: body.dm_footer
        };
        box.innerHTML = renderizarPreviewEmbed(bodyMapeado, data.embed, body.dm_embed === '1');
      } catch (e) {
        box.innerHTML = '<p style="color:#f23f42">Erro de ligação ao gerar preview.</p>';
      }
    }
    ...
    // ============================
    // LISTA + MODAL DE MENSAGENS DE SAÍDA
    // ============================
    async function loadLeaveMessages(guildId) {
      const box = document.getElementById('leave-messages-list');
      if (!box) return;
      try {
        const r = await fetch('/api/'+guildId+'/leave-messages');
        const lista = await r.json();
        if (!Array.isArray(lista) || !lista.length) {
          box.innerHTML = '<p style="color:var(--text2)">Nenhuma mensagem de saída criada ainda. Clica em "➕ Criar nova" para começar.</p>';
          return;
        }
        box.innerHTML = lista.map(w => \`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border:1px solid var(--border);border-radius:8px;margin-bottom:10px;background:var(--bg2)">
            ...
            <div style="display:flex;gap:6px">
              \${!w.is_active ? \`<button type="button" class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick="ativarLeaveMessage('\${guildId}', \${w.id})" title="Tornar ativa">✅ Ativar</button>\` : ''}
              <button type="button" class="btn" style="padding:4px 10px;font-size:0.8rem" onclick="abrirModalLeave('\${guildId}', \${w.id})">✏️ Editar</button>
              <button type="button" class="btn btn-danger" style="padding:4px 10px;font-size:0.8rem" onclick="apagarLeaveMessage('\${guildId}', \${w.id})">🗑️</button>
            </div>
          </div>
        \`).join('');
      } catch (e) {
        box.innerHTML = '<p style="color:#f23f42">Erro de ligação ao carregar as mensagens de saída.</p>';
      }
    }

    function limparFormLeave() {
      const form = document.getElementById('form-leave');
      if (form) form.reset();
      document.getElementById('leave_id').value = '';
      document.getElementById('leave_embed').checked = true;
      document.getElementById('leave_color').value = '#ED4245';
      document.getElementById('leave_msg').value = '\${username} saiu de \${guildname}.';
    }

    async function abrirModalLeave(guildId, id) {
      limparFormLeave();
      const modal = document.getElementById('modal-leave');
      const titulo = document.getElementById('modal-leave-titulo');

      if (id) {
        titulo.textContent = '✏️ Editar Mensagem de Saída';
        try {
          const r = await fetch('/api/'+guildId+'/leave-messages/'+id);
          const w = await r.json();
          if (w && w.id) {
            document.getElementById('leave_id').value = w.id;
            document.getElementById('leave_name').value = w.name || '';
            document.getElementById('leave_content').value = w.leave_content || '';
            document.getElementById('leave_author_name').value = w.leave_author_name || '';
            document.getElementById('leave_author_icon').value = w.leave_author_icon || '';
            document.getElementById('leave_title').value = w.leave_title || '';
            document.getElementById('leave_url').value = w.leave_url || '';
            document.getElementById('leave_msg').value = w.leave_msg || '';
            document.getElementById('leave_color').value = w.leave_color || '#ED4245';
            document.getElementById('leave_image').value = w.leave_image || '';
            document.getElementById('leave_thumbnail').value = w.leave_thumbnail || '';
            document.getElementById('leave_footer').value = w.leave_footer || '';
            document.getElementById('leave_embed').checked = !!w.leave_embed;
            const selCanal = document.querySelector('#form-leave [name="leave_channel"]');
            if (selCanal) selCanal.value = w.leave_channel || '';
          }
        } catch (e) {
          toast('❌ Erro ao carregar a mensagem de saída.', 'error');
        }
      } else {
        titulo.textContent = '➕ Nova Mensagem de Saída';
      }

      modal.style.display = 'flex';
      setTimeout(() => enviarPreviewLeave(guildId), 50);
    }

    function fecharModalLeave() {
      document.getElementById('modal-leave').style.display = 'none';
    }

    async function guardarLeaveMessage(guildId) {
      const form = document.getElementById('form-leave');
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      body.leave_embed = document.getElementById('leave_embed').checked ? '1' : '0';

      const id = body.id;
      const url = id ? '/api/'+guildId+'/leave-messages/'+id : '/api/'+guildId+'/leave-messages';
      const method = id ? 'PUT' : 'POST';
    ...
    async function ativarLeaveMessage(guildId, id) {
      try {
        const r = await fetch('/api/'+guildId+'/leave-messages/'+id+'/activate', { method: 'POST' });
        const data = await r.json();
        toast(data.message || (data.ok ? 'Ativada!' : 'Erro.'), data.ok ? 'success' : 'error');
        if (data.ok) loadLeaveMessages(guildId);
      } catch (e) {
        toast('❌ Erro de ligação.', 'error');
      }
    }

    async function apagarLeaveMessage(guildId, id) {
      if (!confirm('Apagar esta mensagem de saída?')) return;
      try {
        const r = await fetch('/api/'+guildId+'/leave-messages/'+id, { method: 'DELETE' });
        const data = await r.json();
        toast(data.message || (data.ok ? 'Apagada!' : 'Erro.'), data.ok ? 'success' : 'error');
        if (data.ok) loadLeaveMessages(guildId);
      } catch (e) {
    ...
    function atualizarPreviewLeave(guildId) {
      clearTimeout(leavePreviewTimer);
      leavePreviewTimer = setTimeout(() => enviarPreviewLeave(guildId), 250);
    }

    async function enviarPreviewLeave(guildId) {
      const form = document.getElementById('form-leave');
      const box = document.getElementById('leave-preview-box');
      if (!form || !box) return;
      const fd = new FormData(form);
      const body = Object.fromEntries(fd.entries());
      body.leave_embed = document.getElementById('leave_embed').checked ? '1' : '0';
      ...
      try {
        const r = await fetch('/api/'+guildId+'/leave-preview', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body)
        });
        ...
        if (!data.ok) { box.innerHTML = '<p style="color:#f23f42">'+ (data.message||'Erro ao gerar preview') +'</p>'; return; }
        const bodyMapeado = {
          welcome_content: body.leave_content, welcome_author_name: body.leave_author_name,
          welcome_author_icon: body.leave_author_icon, welcome_title: body.leave_title,
          welcome_url: body.leave_url, welcome_msg: body.leave_msg, welcome_color: body.leave_color,
          welcome_image: body.leave_image, welcome_thumbnail: body.leave_thumbnail, welcome_footer: body.leave_footer
        };
        box.innerHTML = renderizarPreviewEmbed(bodyMapeado, data.embed, body.leave_embed === '1');
      } catch (e) {
        box.innerHTML = '<p style="color:#f23f42">Erro de ligação ao gerar preview.</p>';
      }
    }

    // Carrega a sub-aba certa (última visitada, por defeito "Boas-vindas" para não
    // mudar o comportamento de quem já usava isto) assim que a secção é aberta
    ...
    if (typeof origShowSection === 'function') {
      window.showSection = function(name) {
        ...
        if (name === 'welcome') {
          let sub = 'boasvindas';
          try { sub = localStorage.getItem('welcomeSubtab') || 'boasvindas'; } catch (_) {}
          showWelcomeSubtab(sub);
        }
      };
    }
    ...
    restoreSection();
    if (window.location.hash === '#tickets-editor' && typeof tkOpenEditor === 'function') {
      tkOpenEditor();
    }
