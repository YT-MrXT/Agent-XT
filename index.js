...
function initDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS welcome_messages (
      ...
      welcome_footer     TEXT,
      welcome_fields     TEXT DEFAULT '[]',
      created_at         DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at         DATETIME DEFAULT CURRENT_TIMESTAMP
    );
  `);
  // Migração: garante a coluna welcome_fields (campos nome/valor, estilo Sapphire) em bases já existentes
  try {
    const wmCols = db.prepare("PRAGMA table_info(welcome_messages)").all();
    if (!wmCols.some(c => c.name === 'welcome_fields')) {
      db.exec("ALTER TABLE welcome_messages ADD COLUMN welcome_fields TEXT DEFAULT '[]'");
    }
  } catch (e) { console.error('❌ Erro na migração de welcome_fields:', e.message); }
  ...
  try {
    const guildsComWelcomeAntigo = db.prepare(`
    ...
    for (const gc of guildsComWelcomeAntigo) {
      const jaTem = db.prepare('SELECT id FROM welcome_messages WHERE guild_id = ?').get(gc.guild_id);
      if (jaTem) continue;
      insertWelcomeMsg.run(
        gc.guild_id, 'Boas-vindas', gc.welcome_channel || null, gc.autorole || null,
        gc.welcome_embed ?? 1, gc.welcome_content || null, gc.welcome_title || null,
        gc.welcome_url || null, gc.welcome_msg || null, gc.welcome_color || '#5865F2',
        gc.welcome_author_name || null, gc.welcome_author_icon || null,
        gc.welcome_image_pos || 'bottom', gc.welcome_image || null,
        gc.welcome_thumbnail || null, gc.welcome_footer || null,
      );
    }
  } catch (e) { console.error('❌ Erro na migração de welcome_messages:', e.message); }
...
function construirEmbedWelcome(config, member) {
  ...
  // Fields (campos nome/valor, com opção de lado-a-lado) — tal como no editor da Sapphire.
  const camposEmbed = normalizarWelcomeFields(config.welcome_fields);
  if (camposEmbed.length) {
    embed.addFields(camposEmbed.map(f => ({
      name: substituirVariaveisWelcome(f.name, member),
      value: substituirVariaveisWelcome(f.value, member),
      inline: !!f.inline,
    })));
  }
...
async function sendWelcome(member) {
  const config = db.prepare('SELECT * FROM welcome_messages WHERE guild_id = ? AND is_active = 1').get(member.guild.id);
...
// ============================
const WELCOME_MSG_CAMPOS = [
  'welcome_channel', 'autorole', 'welcome_embed', 'welcome_content', 'welcome_title',
  'welcome_url', 'welcome_msg', 'welcome_color', 'welcome_author_name',
  'welcome_author_icon', 'welcome_image_pos', 'welcome_image', 'welcome_thumbnail',
  'welcome_footer',
];

/** Normaliza os "fields" (campos nome/valor) da embed de boas-vindas vindos do formulário do dashboard.
 *  Aceita tanto um array já pronto como uma string JSON. Filtra campos vazios e limita a 25 (limite do Discord). */
function normalizarWelcomeFields(raw) {
  let lista = [];
  try {
  ...
  return lista
    .map(f => ({
      name: String(f?.name || '').trim().slice(0, 256),
      value: String(f?.value || '').trim().slice(0, 1024),
      inline: !!(f?.inline === true || f?.inline === 'true' || f?.inline === 1 || f?.inline === '1'),
    }))
    .filter(f => f.name && f.value)
    .slice(0, 25);
}

app.get('/api/:guildId/welcome-messages', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const guild = client.guilds.cache.get(guildId);
  const linhas = db.prepare('SELECT * FROM welcome_messages WHERE guild_id = ? ORDER BY created_at ASC').all(guildId);
...
app.get('/api/:guildId/welcome-messages/:id', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const linha = db.prepare('SELECT * FROM welcome_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de boas-vindas não encontrada.' });
  res.json(linha);
});

app.post('/api/:guildId/welcome-messages', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const nome = (req.body?.name || '').trim();
  if (!nome) return res.status(400).json({ ok: false, message: '❌ Dá um nome a esta mensagem de boas-vindas.' });

  const existente = db.prepare('SELECT id FROM welcome_messages WHERE guild_id = ? AND name = ?').get(guildId, nome);
  ...
  const camposEmbed = normalizarWelcomeFields(req.body?.welcome_fields);

  // A primeira mensagem de boas-vindas de um servidor nasce automaticamente ativa
  const totalExistentes = db.prepare('SELECT COUNT(*) AS c FROM welcome_messages WHERE guild_id = ?').get(guildId).c;
  ...
  const info = db.prepare(`
    INSERT INTO welcome_messages (
      ...
      welcome_content, welcome_title, welcome_url, welcome_msg, welcome_color,
      welcome_author_name, welcome_author_icon, welcome_image_pos, welcome_image,
      welcome_thumbnail, welcome_footer, welcome_fields
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    guildId, nome, ativa,
    req.body?.welcome_channel || null,
    req.body?.autorole || null,
    req.body?.welcome_embed === '1' || req.body?.welcome_embed === 1 ? 1 : 0,
    (req.body?.welcome_content || '').trim() || null,
    (req.body?.welcome_title || '').trim() || null,
    (req.body?.welcome_url || '').trim() || null,
    (req.body?.welcome_msg || '').trim() || null,
    cor,
    (req.body?.welcome_author_name || '').trim() || null,
    (req.body?.welcome_author_icon || '').trim() || null,
    posValida,
    (req.body?.welcome_image || '').trim() || null,
    (req.body?.welcome_thumbnail || '').trim() || null,
    (req.body?.welcome_footer || '').trim() || null,
    JSON.stringify(camposEmbed),
  );

  res.json({ ok: true, message: `✅ Mensagem "${nome}" criada!`, id: info.lastInsertRowid });
});

app.put('/api/:guildId/welcome-messages/:id', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const linha = db.prepare('SELECT * FROM welcome_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de boas-vindas não encontrada.' });
  ...
  const duplicado = db.prepare('SELECT id FROM welcome_messages WHERE guild_id = ? AND name = ? AND id != ?').get(guildId, nome, id);
  ...
  db.prepare(`
    UPDATE welcome_messages SET
      ...
      welcome_image = ?, welcome_thumbnail = ?, welcome_footer = ?, welcome_fields = ?,
...
app.delete('/api/:guildId/welcome-messages/:id', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const linha = db.prepare('SELECT * FROM welcome_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de boas-vindas não encontrada.' });

  db.prepare('DELETE FROM welcome_messages WHERE id = ? AND guild_id = ?').run(id, guildId);

  // Se a que foi apagada era a ativa, promove a mais antiga que sobrou (se houver)
  if (linha.is_active) {
    const proxima = db.prepare('SELECT id FROM welcome_messages WHERE guild_id = ? ORDER BY created_at ASC LIMIT 1').get(guildId);
    if (proxima) db.prepare('UPDATE welcome_messages SET is_active = 1 WHERE id = ?').run(proxima.id);
  }

  res.json({ ok: true, message: '✅ Mensagem de boas-vindas apagada.' });
});

// Marca uma mensagem de boas-vindas como a ATIVA (só uma pode estar ativa por servidor)
app.post('/api/:guildId/welcome-messages/:id/activate', requireGuildAdminApi, (req, res) => {
  const { guildId, id } = req.params;
  const linha = db.prepare('SELECT * FROM welcome_messages WHERE id = ? AND guild_id = ?').get(id, guildId);
  if (!linha) return res.status(404).json({ ok: false, message: 'Mensagem de boas-vindas não encontrada.' });

  db.prepare('UPDATE welcome_messages SET is_active = 0 WHERE guild_id = ?').run(guildId);
  db.prepare('UPDATE welcome_messages SET is_active = 1 WHERE id = ?').run(id);

  res.json({ ok: true, message: `✅ "${linha.name}" é agora a mensagem de boas-vindas ativa.` });
});
    ...
    <div class="card" style="padding:0;overflow:hidden">
      ...
      <div style="max-height:420px;overflow-y:auto">
        ${availableGuilds.length === 0
          ? `<div class="alert alert-error" style="margin:16px">❌ Não tens servidores em comum com o bot. <a href="${inviteUrl}" target="_blank">Adiciona o bot aqui</a></div>`
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
              ...
              <tbody>
                ${savedEmbeds.map(e => {
                  ...
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" data-embed="${escHtml(JSON.stringify({ id: e.id, name: e.name, image_pos: 'bottom', ...embedData }))}" onclick="abrirEditarEmbed(JSON.parse(this.getAttribute('data-embed')))" title="Editar esta embed">✏️</button>
    ...
    <div id="modal-welcome" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:1000;align-items:center;justify-content:center">
      <div class="card" style="max-width:960px;width:95%;max-height:90vh;overflow:auto">
        ...
        <form id="form-welcome" oninput="atualizarPreviewWelcome('${guild.id}')" onchange="atualizarPreviewWelcome('${guild.id}')">
          ...
          <input type="hidden" name="welcome_fields" id="welcome_fields" value="[]">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:10px">
            ...
            <div>
              ...
              <div class="form-group">
                ...
                <div id="welcome-fields-list" style="display:flex;flex-direction:column;gap:8px"></div>
                <button type="button" class="btn" style="margin-top:8px" onclick="adicionarWelcomeField('${guild.id}')">➕ Adicionar Campo</button>
              </div>
  ...
  <script>
    ...
    function renderizarPreviewEmbed(body, embed, usarEmbed) {
      ...
      if (embed && Array.isArray(embed.fields) && embed.fields.length) {
        html += '<div style="display:flex;flex-wrap:wrap;gap:8px 16px;margin-top:10px">';
        embed.fields.forEach(f => {
          html += '<div style="'+(f.inline ? 'flex:1;min-width:120px' : 'flex-basis:100%')+'">';
          html += '<div style="color:#f2f3f5;font-weight:700;font-size:0.85rem;margin-bottom:2px">'+esc(f.name)+'</div>';
          html += '<div style="color:#dbdee1;font-size:0.85rem;white-space:pre-wrap;line-height:1.4">'+esc(f.value)+'</div>';
          html += '</div>';
        });
        html += '</div>';
      }
    ...
    function limparFormWelcome() {
      ...
      renderizarWelcomeFields('${guild.id}', []);
    }

    // ---- Gestão dos "Fields" (campos nome/valor) da embed de boas-vindas, estilo Sapphire ----

    function lerWelcomeFields() {
      try { return JSON.parse(document.getElementById('welcome_fields').value || '[]'); }
      catch (e) { return []; }
    }

    function gravarWelcomeFields(lista) {
      document.getElementById('welcome_fields').value = JSON.stringify(lista);
    }

    function renderizarWelcomeFields(guildId, lista) {
      gravarWelcomeFields(lista);
      const box = document.getElementById('welcome-fields-list');
      if (!box) return;
      if (!lista.length) {
        box.innerHTML = '<p style="color:var(--text2);font-size:0.8rem;margin:0">Sem campos adicionados.</p>';
        return;
      }
      box.innerHTML = lista.map((f, i) => \`
        <div style="border:1px solid var(--border);border-radius:8px;padding:10px;display:flex;flex-direction:column;gap:6px">
          <div style="display:flex;gap:6px;align-items:center">
            <input type="text" placeholder="Nome do campo" value="\${(f.name||'').replace(/"/g,'&quot;')}" style="flex:1" onchange="atualizarWelcomeField('\${guildId}', \${i}, 'name', this.value)">
            <button type="button" class="btn btn-danger" style="padding:4px 8px" onclick="removerWelcomeField('\${guildId}', \${i})">🗑️</button>
          </div>
          <textarea placeholder="Valor do campo" rows="2" onchange="atualizarWelcomeField('\${guildId}', \${i}, 'value', this.value)">\${(f.value||'').replace(/</g,'&lt;')}</textarea>
          <label class="toggle" style="font-size:0.8rem">
            <input type="checkbox" \${f.inline ? 'checked' : ''} onchange="atualizarWelcomeField('\${guildId}', \${i}, 'inline', this.checked)">
            <span>Lado a lado (inline)</span>
          </label>
        </div>
      \`).join('');
    }

    function adicionarWelcomeField(guildId) {
      const lista = lerWelcomeFields();
      if (lista.length >= 25) { toast('❌ Máximo de 25 campos por embed (limite do Discord).', 'error'); return; }
      lista.push({ name: '', value: '', inline: false });
      renderizarWelcomeFields(guildId, lista);
    }

    function atualizarWelcomeField(guildId, index, chave, valor) {
      const lista = lerWelcomeFields();
      if (!lista[index]) return;
      lista[index][chave] = valor;
      gravarWelcomeFields(lista);
      atualizarPreviewWelcome(guildId);
    }

    function removerWelcomeField(guildId, index) {
      const lista = lerWelcomeFields();
      ...
      if (id) {
        ...
        try {
          ...
          if (w && w.id) {
            ...
            try { camposCarregados = JSON.parse(w.welcome_fields || '[]'); } catch (e) { camposCarregados = []; }
            renderizarWelcomeFields(guildId, Array.isArray(camposCarregados) ? camposCarregados : []);
