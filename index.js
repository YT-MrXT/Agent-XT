...
client.on(Events.MessageCreate, async message => {
  ...
  await verificarComandoEmbed(message);
});

/**
 * Verifica se a mensagem é um comando de embed configurado no dashboard
 * (ex: "+abrirservidor"). O prefixo é o configurado por servidor (guild_config.prefix,
 * por defeito CONFIG.PREFIX = "+"). O comando só dispara a embed guardada NESSE
 * servidor — cada servidor tem os seus próprios comandos, isolados por guild_id.
 */
async function verificarComandoEmbed(message) {
  try {
    const guildConfig = db.prepare('SELECT prefix FROM guild_config WHERE guild_id = ?').get(message.guild.id);
    const prefixo = (guildConfig?.prefix || CONFIG.PREFIX || '+').trim() || '+';

    if (!message.content.startsWith(prefixo)) return;

    const semPrefixo = message.content.slice(prefixo.length).trim();
    if (!semPrefixo) return;
    const comando = semPrefixo.split(/\s+/)[0].toLowerCase();
    if (!comando) return;

    const saved = db.prepare(
      'SELECT * FROM saved_embeds WHERE guild_id = ? AND trigger_command = ?'
    ).get(message.guild.id, comando);
    if (!saved) return; // não há nenhuma embed configurada com este comando neste servidor

    // Permissão: só quem pode gerir mensagens/servidor pode usar comandos de embed (evita spam por membros comuns)
    ...
    await message.channel.send({ content: data.content || undefined, embeds: [embed] });
    await message.delete().catch(() => {}); // limpa o comando do chat, se possível
  } catch (e) {
    console.error('❌ Erro ao processar comando de embed:', e.message);
  }
}
...
// ── Embeds: comando personalizado (ex: +abrirservidor) que envia a embed guardada ──
// O comando só funciona no servidor onde foi configurado (é gravado por guild_id).
app.post('/api/:guildId/embeds/comando', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  ...
  res.json({ ok: true, message: `✅ Comando "${comando}" configurado! Usa o prefixo do servidor + "${comando}" para publicar esta embed.` });
});

app.post('/api/:guildId/embeds/comando-remover', requireGuildAdminApi, (req, res) => {
  const { guildId } = req.params;
  const { id } = req.body;
    ...
    <div id="embeds_tab" class="section" style="display:none">
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div id="embeds-table">
          ${savedEmbeds.length ? `
            <table class="data-table">
              <thead><tr><th>Nome</th><th>Título</th><th>Comando (só neste servidor)</th><th>Enviar uma vez</th><th>Envio automático (intervalo)</th><th>Envio diário (horas fixas)</th><th></th></tr></thead>
              <tbody>
                ${savedEmbeds.map(e => {
                  let titulo = '—';
                  try { titulo = JSON.parse(e.data).title || '—'; } catch(_) {}
                  const prefixoAtual = (guildConfig?.prefix || '+');
                  ...
                  <tr>
                    ...
                    <td>
                      ${e.trigger_command
                        ? `<div style="margin-bottom:4px"><span style="color:var(--success, #3ba55c)">🟢 <code>${prefixoAtual}${e.trigger_command}</code></span></div>
                        ...
                        : `<input type="text" id="embed-cmd-${e.id}" placeholder="ex: abrirservidor" style="width:110px;display:inline-block;padding:4px 8px;font-size:0.8rem">
                           <button type="button" class="btn btn-primary" style="padding:4px 10px;font-size:0.8rem" onclick="configurarComandoEmbed('${guild.id}', ${e.id})" title="Define um comando (${prefixoAtual}comando) que só funciona neste servidor">💾</button>`
                      }
                    </td>
                    <td>
