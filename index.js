...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Cria um giveaway (sorteio)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('titulo').setDescription('Título do giveaway').setRequired(true))
    .addStringOption(o => o.setName('descricao').setDescription('Descrição do giveaway').setRequired(true))
    .addStringOption(o => o.setName('premio').setDescription('Prémio a sortear').setRequired(true))
    .addStringOption(o => o.setName('tempo').setDescription('Duração (ex: 10m, 2h, 1d)').setRequired(true))
    ...
    .addChannelOption(o => o.setName('canal').setDescription('Canal onde publicar (padrão: este canal)').setRequired(false)
      .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
    .addStringOption(o => o.setName('imagem').setDescription('URL da imagem do giveaway').setRequired(false))
    .addStringOption(o => o.setName('mensagem').setDescription('Mensagem enviada fora do embed (ex: @everyone)').setRequired(false)),

  new SlashCommandBuilder()
    .setName('giveaway-lista')
    .setDescription('Lista os giveaways ativos e os seus IDs')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  ...
  new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),

  new SlashCommandBuilder()
    .setName('giveaway-reroll')
    .setDescription('Sorteia novo(s) vencedor(es) de um giveaway já terminado')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),

  new SlashCommandBuilder()
    .setName('giveaway-terminar')
    .setDescription('Termina um giveaway imediatamente e sorteia o(s) vencedor(es)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do giveaway (vê com /giveaway-lista)').setRequired(true)),
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'giveaway-criar') {
    ...
    const tempoStr  = options.getString('tempo');
    const vencedores = options.getInteger('vencedores');
    const canal     = options.getChannel('canal') || interaction.channel;
    const imagem    = options.getString('imagem');
    const mensagem  = options.getString('mensagem');

    const tempoMs = parseDuration(tempoStr);
    if (!tempoMs) {
      return interaction.editReply({ content: '❌ Duração inválida. Usa o formato: `10m`, `2h`, `1d`, etc.' });
    }

    try {
      const giveaway = await criarGiveaway({
        guild: interaction.guild,
        canal,
        titulo, descricao, premio, tempoMs, vencedores,
        imagem, mensagemExtra: mensagem,
        criadoPor: user.id,
      });
      return interaction.editReply({ content: `✅ Giveaway criado em ${canal} com o ID \`${giveaway.id}\`! Termina <t:${Math.floor(giveaway.end_time / 1000)}:R>.` });
    } catch (err) {
      return interaction.editReply({ content: `❌ Erro ao criar o giveaway: ${err.message}` });
    }
  }

  if (commandName === 'giveaway-lista') {
    const ativos = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? AND ended = 0 ORDER BY end_time ASC').all(guild.id);

    if (!ativos.length) {
      return interaction.reply({ content: '📭 Não há giveaways ativos neste servidor.', ephemeral: true });
    }

    const embed = new EmbedBuilder()
      .setTitle('🎉 Giveaways Ativos')
      .setColor(CONFIG.COR_PRINCIPAL)
      .setDescription(ativos.map(g => {
        const numEntradas = db.prepare('SELECT COUNT(*) as c FROM giveaway_entries WHERE giveaway_id = ?').get(g.id).c;
        return `**ID \`${g.id}\`** — ${g.titulo}\n🎁 ${g.premio} · 🏆 ${g.vencedores} vencedor(es) · 👥 ${numEntradas} participante(s)\n📍 <#${g.channel_id}> · ⏰ Termina <t:${Math.floor(g.end_time / 1000)}:R>`;
      }).join('\n\n'))
      .setTimestamp();
  ...
  if (commandName === 'giveaway-cancelar') {
    const id = options.getInteger('id');
    const giveaway = db.prepare('SELECT * FROM giveaways WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!giveaway) {
      return interaction.reply({ content: '❌ Giveaway não encontrado neste servidor.', ephemeral: true });
    }
    if (giveaway.ended) {
      return interaction.reply({ content: '❌ Esse giveaway já terminou.', ephemeral: true });
    }

    db.prepare('DELETE FROM giveaways WHERE id = ?').run(giveaway.id);
    db.prepare('DELETE FROM giveaway_entries WHERE giveaway_id = ?').run(giveaway.id);

    const canal = guild.channels.cache.get(giveaway.channel_id);
    if (canal && giveaway.message_id) {
      try {
        const msg = await canal.messages.fetch(giveaway.message_id);
        const embedCancelado = new EmbedBuilder()
          .setTitle(`🎉 ${giveaway.titulo}`)
          .setDescription(`❌ **Este giveaway foi cancelado.**`)
          .setColor(CONFIG.COR_ERRO);
        await msg.edit({ embeds: [embedCancelado], components: [buildGiveawayRow(giveaway.id, true)] });
      } catch (_) {}
    }

    return interaction.reply({ content: `✅ Giveaway \`${id}\` cancelado.`, ephemeral: true });
  }
  ...
  if (commandName === 'giveaway-terminar') {
    ...
    await interaction.deferReply({ ephemeral: true });
    await terminarGiveaway(id);
    return interaction.editReply({ content: `✅ Giveaway \`${id}\` terminado e vencedor(es) sorteado(s).` });
  }
  ...
  if (commandName === 'giveaway-reroll') {
    ...
    if (!giveaway.ended) {
      return interaction.reply({ content: '❌ Esse giveaway ainda não terminou. Usa `/giveaway-terminar` primeiro.', ephemeral: true });
    }

    await interaction.deferReply({ ephemeral: true });
    const vencedoresIds = sortearVencedores(giveaway.id, giveaway.vencedores);
    const canal = guild.channels.cache.get(giveaway.channel_id);

    if (canal) {
      if (vencedoresIds.length) {
        await canal.send({ content: `🎉 Novo(s) vencedor(es) sorteado(s) para **${giveaway.premio}**: ${vencedoresIds.map(uid => `<@${uid}>`).join(', ')}!` }).catch(() => {});
      } else {
        await canal.send({ content: `😕 Não há participantes suficientes para novo sorteio de **${giveaway.titulo}**.` }).catch(() => {});
      }
      if (giveaway.message_id) {
        try {
          const msg = await canal.messages.fetch(giveaway.message_id);
          const embed = buildGiveawayEmbed(giveaway, { ended: true, vencedoresIds });
          await msg.edit({ embeds: [embed] });
        } catch (_) {}
      }
    }

    return interaction.editReply({ content: '✅ Reroll efetuado.' });
  }
  ...
  if (commandName === 'help') {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '🎉 Giveaways',
          value: '`/giveaway-criar` · Cria um giveaway (título, descrição, prémio, tempo, vencedores, imagem, mensagem)\n`/giveaway-lista` · Lista os giveaways ativos e os seus IDs\n`/giveaway-cancelar` · Cancela um giveaway ativo\n`/giveaway-reroll` · Sorteia novo(s) vencedor(es) de um giveaway terminado\n`/giveaway-terminar` · Termina um giveaway imediatamente',
          inline: false,
        },
        {
          name: 'ℹ️ Informação',
...
async function handleButton(interaction) {
  ...
  if (customId.startsWith('giveaway_enter_')) {
    const giveawayId = parseInt(customId.slice('giveaway_enter_'.length), 10);
    const giveaway = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(giveawayId);

    if (!giveaway || giveaway.ended || giveaway.end_time <= Date.now()) {
    ...
    if (jaParticipa) {
      ...
      return interaction.reply({ content: '❌ Saíste do giveaway. Clica novamente para participar.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: `🎉 Estás a participar no giveaway de **${giveaway.premio}**! Boa sorte!`, ephemeral: true });
  }
...
/** Constrói o embed de um giveaway (ativo ou terminado) */
function buildGiveawayEmbed(giveaway, { ended = false, vencedoresIds = [] } = {}) {
  const numEntradas = db.prepare('SELECT COUNT(*) as c FROM giveaway_entries WHERE giveaway_id = ?').get(giveaway.id).c;

  const embed = new EmbedBuilder()
    .setTitle(`🎉 ${giveaway.titulo}`)
    .setColor(ended ? CONFIG.COR_ERRO : CONFIG.COR_PRINCIPAL)
    .setTimestamp(giveaway.end_time);

  let desc = `${giveaway.descricao}\n\n`;
  desc += `🎁 **Prémio:** ${giveaway.premio}\n`;
  desc += `🏆 **Vencedores:** ${giveaway.vencedores}\n`;
  desc += `👥 **Participantes:** ${numEntradas}\n`;

  if (ended) {
    desc += vencedoresIds.length
      ? `\n🎊 **Vencedor(es):** ${vencedoresIds.map(id => `<@${id}>`).join(', ')}`
      : `\n😕 Ninguém participou, sem vencedores.`;
    desc += `\n\n**Giveaway terminado.**`;
  } else {
    desc += `⏰ **Termina:** <t:${Math.floor(giveaway.end_time / 1000)}:R>`;
  }

  embed.setDescription(desc);
  if (giveaway.imagem) embed.setImage(giveaway.imagem);
  embed.setFooter({ text: `ID do Giveaway: ${giveaway.id}` });
  return embed;
}

/** Linha de botão para participar num giveaway */
function buildGiveawayRow(giveawayId, disabled = false) {
...
/** Cria e publica um novo giveaway */
async function criarGiveaway({ guild, canal, titulo, descricao, premio, tempoMs, vencedores, imagem, mensagemExtra, criadoPor }) {
  const endTime = Date.now() + tempoMs;

  const info = db.prepare(`
    INSERT INTO giveaways (guild_id, channel_id, titulo, descricao, premio, imagem, mensagem_extra, vencedores, end_time, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(guild.id, canal.id, titulo, descricao, premio, imagem || null, mensagemExtra || null, vencedores, endTime, criadoPor);

  const giveaway = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(info.lastInsertRowid);
  const embed = buildGiveawayEmbed(giveaway);
  const row = buildGiveawayRow(giveaway.id);

  const payload = { embeds: [embed], components: [row] };
  if (mensagemExtra) payload.content = mensagemExtra;

  const msg = await canal.send(payload);
  db.prepare('UPDATE giveaways SET message_id = ? WHERE id = ?').run(msg.id, giveaway.id);

  return { ...giveaway, message_id: msg.id };
}

/** Escolhe aleatoriamente N vencedores de entre os participantes */
function sortearVencedores(giveawayId, quantidade) {
  ...
  const embaralhados = entradas.sort(() => Math.random() - 0.5);
  return embaralhados.slice(0, Math.min(quantidade, embaralhados.length));
}

/** Termina um giveaway: sorteia vencedor(es), edita a mensagem e anuncia o resultado */
async function terminarGiveaway(giveawayId, { silencioso = false } = {}) {
  const giveaway = db.prepare('SELECT * FROM giveaways WHERE id = ?').get(giveawayId);
  ...
  if (guild) {
    ...
    if (canal) {
      const embed = buildGiveawayEmbed(giveaway, { ended: true, vencedoresIds });
      const row = buildGiveawayRow(giveaway.id, true);
      ...
      if (!silencioso) {
        if (vencedoresIds.length) {
          await canal.send({
            content: `🎉 Parabéns ${vencedoresIds.map(id => `<@${id}>`).join(', ')}! Ganhaste(s) **${giveaway.premio}**!`,
          }).catch(() => {});
        } else {
          await canal.send({ content: `😕 O giveaway **${giveaway.titulo}** terminou sem participantes.` }).catch(() => {});
        }
      }
    }
  }

  return { giveaway, vencedoresIds };
}

/** Verifica giveaways ativos e termina os que já passaram do tempo */
async function verificarGiveaways() {
  const agora = Date.now();
  const ativos = db.prepare('SELECT * FROM giveaways WHERE ended = 0 AND end_time <= ?').all(agora);
  for (const g of ativos) {
    await terminarGiveaway(g.id).catch(err => console.error('❌ Erro ao terminar giveaway:', err.message));
  }
}
...
function iniciarCrons() {
  ...
  // Verifica giveaways ativos e termina os que já passaram do tempo (a cada minuto)
...
app.get('/dashboard/:guildId', requireAuth, requireGuildAdminPage, async (req, res) => {
  ...
  const giveaways = db.prepare('SELECT * FROM giveaways WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50').all(guildId).map(g => ({
    ...g,
    num_entradas: db.prepare('SELECT COUNT(*) as c FROM giveaway_entries WHERE giveaway_id = ?').get(g.id).c
  }));
...
const dashboardJS = `
  ...
  // ── Giveaways ──
  async function criarGiveawayDashboard(guildId) {
    const body = {
      canal: document.getElementById('gw_canal').value,
      titulo: document.getElementById('gw_titulo').value,
      descricao: document.getElementById('gw_descricao').value,
      premio: document.getElementById('gw_premio').value,
      tempo: document.getElementById('gw_tempo').value,
      vencedores: document.getElementById('gw_vencedores').value,
      imagem: document.getElementById('gw_imagem').value,
      mensagem: document.getElementById('gw_mensagem').value,
    };
    if (!body.canal || !body.titulo || !body.descricao || !body.premio || !body.tempo) {
      toast('❌ Preenche canal, título, descrição, prémio e duração.', 'error'); return;
    }
    try {
      const res = await fetch('/api/' + guildId + '/giveaways/criar', {
  ...
  async function cancelarGiveawayDashboard(guildId, id) {
    if (!confirm('Cancelar este giveaway?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaways/' + id + '/cancelar', { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : '❌ Erro', json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 800);
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  async function terminarGiveawayDashboard(guildId, id) {
    if (!confirm('Terminar este giveaway agora e sortear vencedor(es)?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaways/' + id + '/terminar', { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : '❌ Erro', json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 800);
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
  }
  async function rerollGiveawayDashboard(guildId, id) {
    if (!confirm('Sortear novo(s) vencedor(es)?')) return;
    try {
      const res = await fetch('/api/' + guildId + '/giveaways/' + id + '/reroll', { method: 'POST' });
      const json = await res.json();
      toast(json.ok ? json.message : '❌ Erro', json.ok ? 'success' : 'error');
      if (json.ok) setTimeout(() => location.reload(), 800);
    } catch(e) { toast('❌ Erro de ligação', 'error'); }
...
function renderGuildDashboard(user, guild, data) {
  const { ticketConfig, guildConfig, antispam, statsConfig, votacaoConfig, sugestaoConfig, reactionRoles, ticketTypes, savedEmbeds, perguntas, staffRanking, members, totalTickets, openTickets, totalWarns, totalSugs, channels, roles, categories, autoroleHumanos, autoroleBots, roleExclusivity, blacklist, immuneRoles, giveaways } = data;
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
    ...
    <div id="giveaways_tab" class="section" style="display:none">
      ...
      <div class="card">
        <h3>Criar Giveaway</h3>
        <form id="giveaway-form" onsubmit="return false;">
          <div class="grid-2">
            <div class="form-group">
              <label>Canal onde publicar</label>
              ${makeSelect('gw_canal', channels, '', 'Seleciona um canal')}
            </div>
            <div class="form-group">
              <label>Duração (ex: 10m, 2h, 1d)</label>
              <input type="text" name="gw_tempo" id="gw_tempo" placeholder="Ex: 1d">
            </div>
          </div>
          <div class="form-group">
            <label>Título</label>
            <input type="text" name="gw_titulo" id="gw_titulo" placeholder="Ex: Giveaway de Nitro">
          </div>
          <div class="form-group">
            <label>Descrição</label>
            <textarea name="gw_descricao" id="gw_descricao" rows="3" placeholder="Ex: Participa e boa sorte!"></textarea>
          </div>
          <div class="grid-2">
            <div class="form-group">
              <label>Prémio</label>
              <input type="text" name="gw_premio" id="gw_premio" placeholder="Ex: Discord Nitro 1 Mês">
            </div>
            <div class="form-group">
              <label>Número de vencedores</label>
              <input type="number" name="gw_vencedores" id="gw_vencedores" min="1" max="20" value="1">
            </div>
          </div>
          <div class="form-group">
            <label>URL da Imagem (opcional)</label>
            <input type="text" name="gw_imagem" id="gw_imagem" placeholder="https://...">
          </div>
          <div class="form-group">
            <label>Mensagem fora da embed (opcional, ex: @everyone)</label>
            <input type="text" name="gw_mensagem" id="gw_mensagem" placeholder="Ex: @everyone Novo giveaway!">
          </div>
          <button type="button" class="btn btn-primary" onclick="criarGiveawayDashboard('${guild.id}')">🎉 Criar e Publicar</button>
        </form>
      </div>
      ...
      <div class="card" style="margin-top:20px">
        <h3>Giveaways</h3>
        <table class="table">
          <thead><tr><th>ID</th><th>Título</th><th>Prémio</th><th>Canal</th><th>Vencedores</th><th>Participantes</th><th>Estado</th><th>Ações</th></tr></thead>
          <tbody>
            ${giveaways.length ? giveaways.map(g => `
              <tr>
                <td>${g.id}</td>
                <td>${escHtml(g.titulo)}</td>
                <td>${escHtml(g.premio)}</td>
                <td>#${(guild.channels.cache.get(g.channel_id)?.name) || g.channel_id}</td>
                <td>${g.vencedores}</td>
                <td>${g.num_entradas}</td>
                <td>${g.ended ? '⚪ Terminado' : (g.end_time <= Date.now() ? '🟡 A terminar...' : '🟢 Ativo')}</td>
                <td>
                  ${!g.ended ? `
                    <button class="btn btn-sm btn-secondary" onclick="terminarGiveawayDashboard('${guild.id}', ${g.id})">🏁 Terminar</button>
                    <button class="btn btn-sm btn-danger" onclick="cancelarGiveawayDashboard('${guild.id}', ${g.id})">🗑️ Cancelar</button>
                  ` : `
                    <button class="btn btn-sm btn-secondary" onclick="rerollGiveawayDashboard('${guild.id}', ${g.id})">🔁 Reroll</button>
                  `}
                </td>
              </tr>
            `).join('') : '<tr><td colspan="8" style="color:var(--text2)">Nenhum giveaway ainda.</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>
