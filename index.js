...
function embedGiveaway(gw, totalEntradas, encerrado, vencedoresTexto) {
  ...
  const embed = new EmbedBuilder()
    .setTitle(encerrado ? `🔴 ${tituloBase} — Encerrado` : `🟢 ${tituloBase}`)
    // Cor muda consoante o estado: verde (sucesso) enquanto ativo, vermelho (erro) quando termina
    .setColor(encerrado ? CONFIG.COR_ERRO : CONFIG.COR_SUCESSO)
    .setDescription(
      (gw.descricao ? `${gw.descricao}\n\n` : '') +
      `**Prémio:** ${gw.premio}\n` +
      `**Vencedores:** ${gw.vencedores}\n` +
      (encerrado
        ? `**Termina:** encerrado\n${vencedoresTexto ? `\n🏆 **Vencedor(es):** ${vencedoresTexto}` : '\n😔 Ninguém participou.'}`
        : `**Termina:** <t:${Math.floor(new Date(gw.ends_at + 'Z').getTime() / 1000)}:R>\n\nClica no botão abaixo para participares!`)
    )
    .addFields({ name: 'Participantes', value: `${totalEntradas}`, inline: true })
...
async function atualizarBotaoGiveaway(guild, gw, totalEntradas) {
  ...
  if (!msg) return;
  await msg.edit({ components: [botaoGiveaway(gw, totalEntradas, false)] }).catch(() => {});
}
...
async function encerrarGiveaway(guild, gw) {
  ...
  if (vencedores.length) {
    ...
    await canal.send({ content: `😔 O sorteio de **${gw.premio}** terminou sem participantes.` }).catch(() => {});
  }
