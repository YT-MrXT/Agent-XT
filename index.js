...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'sugerir') {
    const nomeTipoInput = options.getString('tipo');
    let tipo;
    if (nomeTipoInput && nomeTipoInput.trim()) {
      const nomeTipo = nomeTipoInput.trim();
      tipo = await db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? AND name = ? AND enabled = 1').get(guild.id, nomeTipo);
      if (!tipo) return interaction.reply({
        content: `❌ Tipo de sugestão **${nomeTipo}** não encontrado ou desativado. Usa \`/sugestao-tipo-lista\` para veres os tipos disponíveis.`,
        ephemeral: true
      });
    } else {
      // Não escolheu tipo: se o servidor só tiver 1 tipo ativo, usa-o automaticamente
      const tiposAtivos = await db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? AND enabled = 1 ORDER BY order_num, id').all(guild.id);
      if (tiposAtivos.length === 0) {
        return interaction.reply({
          content: '❌ Ainda não há nenhum tipo de sugestão configurado neste servidor. Um administrador tem de usar `/sugestao-tipo-criar` primeiro.',
          ephemeral: true
        });
        ...
        return interaction.reply({
          content: `❌ Este servidor tem mais do que um tipo de sugestão. Indica qual com a opção \`tipo\`. Usa \`/sugestao-tipo-lista\` para veres os tipos disponíveis.`,
          ephemeral: true
        });
      }
    }
