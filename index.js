...
async function enviarPergunta(guild, canal, texto, criadoPorId) {
  ...
  try {
    ...
    // Cria primeiro o tópico "vazio" associado ao canal não é possível sem uma
    // mensagem-mãe, por isso enviamos a mensagem sem o botão, criamos a thread,
    // e só depois enviamos o botão dentro da própria thread (evita "(editado)"
    // na mensagem original e evita a repetição da mesma frase na embed + botão).
    const msg = await canal.send({ embeds: [embed] });

    // Cria o tópico (thread) associado à mensagem para as pessoas responderem
    const nomeTopico = texto.length > 90 ? texto.slice(0, 87) + '...' : texto;
    const thread = await msg.startThread({
      name: nomeTopico || 'Pergunta à Comunidade',
      autoArchiveDuration: 10080, // 7 dias
      reason: 'Tópico de respostas para pergunta à comunidade',
    });

    // Botão que leva diretamente ao tópico, enviado como mensagem de boas-vindas
    // dentro do próprio tópico (mais limpo do que editar a mensagem original).
