...
1. HIGH — discurso de ódio (insultos por raça, etnia, orientação sexual, religião, deficiência, etc.), ameaças diretas ou veladas, assédio persistente, insultos diretos a uma pessoa específica ("és um/a [insulto]", "vai-te foder", chamar nomes ofensivos a alguém).

2. MEDIUM — qualquer palavrão ou calão vulgar/sexual explícito na mensagem, mesmo sem intenção agressiva clara ou usado como interjeição/brincadeira (ex.: "pila", "caralho", "foda-se", "cabrão", "puta", "merda" e equivalentes ou variações disfarçadas). O simples facto de a palavra aparecer na frase, em qualquer posição ou função gramatical, já conta como violação MEDIUM — não decidas que é "inofensivo" só porque o resto da frase é neutro ou positivo. Isto aplica-se IGUALMENTE se a palavra aparecer sozinha, sem mais nenhum texto à volta — uma mensagem que seja só "pila" é, na mesma, violação MEDIUM. Nunca dês benefício da dúvida a um significado alternativo inofensivo (ex.: "pila" como gíria antiga para dinheiro) — no contexto de um chat de Discord em português atual, assume sempre o sentido vulgar/calão.

Exemplos (aplica o mesmo padrão a palavras semelhantes, mesmo que não estejam nesta lista):
...
3. LOW — linguagem levemente informal/atrevida sem palavrão explícito (ex.: sarcasmo leve, indireta educada), que não se enquadra em 1 ou 2.

4. NENHUMA (violacao: false) — mensagens sem qualquer calão, insulto ou conteúdo impróprio.
