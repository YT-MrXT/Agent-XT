  ...
  if (commandName === 'blacklist-add') {
    ...
    if (membroEncontrado) {
      ...
      if (membroEncontrado.bannable) {
        await membroEncontrado.ban({ reason: `Blacklist: ${motivo}` }).catch(() => {});
        jaBanido = true;
...
app.get('/dashboard/:guildId', requireAuth, requireGuildAdminPage, async (req, res) => {
  ...
  res.send(renderGuildDashboard(req.session.user, guild, {
    ...
    autoroleHumanos, autoroleBots, roleExclusivity, blacklist
  }));
});
...
const dashboardJS = `
  ...
  async function saveAutoRole(guildId, target) {
    ...
    const roleIds = Array.from(container.querySelectorAll('input[type="checkbox"]:checked')).map(c => c.value);
    try {
...
function renderGuildDashboard(user, guild, data) {
  const { ticketConfig, guildConfig, antispam, statsConfig, votacaoConfig, sugestaoConfig, reactionRoles, ticketTypes, savedEmbeds, staffRanking, members, totalTickets, openTickets, totalWarns, totalSugs, channels, roles, categories, autoroleHumanos, autoroleBots, roleExclusivity, blacklist } = data;
...
A nossa equipa irá responder o mais brevemente possível!</textarea>
      ...
      <div class="grid-2">
        ...
        <div class="card">
          ...
          <form id="form-mod-blacklist">
            <div class="form-group"><label>Username</label><input type="text" name="username" placeholder="Ex: mrxt.wls"></div>
    ...
    <div id="cargos_tab" class="section" style="display:none">
      ...
      <div class="card">
        ...
        <p style="color:var(--text2);font-size:0.85rem;margin-bottom:16px">
          Cargos atribuídos automaticamente a novos membros humanos que entrarem no servidor. Marca as caixas dos cargos que queres atribuir.
        </p>
        <div class="form-group">
          ${makeCheckboxGroup('autorole_human_roles', roles, autoroleHumanos)}
        </div>
      ...
      <div class="card" style="margin-top:20px">
        ...
        <div class="form-group">
          ${makeCheckboxGroup('autorole_bot_roles', roles, autoroleBots)}
        </div>
