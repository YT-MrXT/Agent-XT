...
function initDatabase() {
  ...
  `);
  // Migração: garante a coluna type_id em bases de dados já existentes
  try {
    const sugCols = db.prepare("PRAGMA table_info(suggestions)").all();
    if (!sugCols.some(c => c.name === 'type_id')) db.exec('ALTER TABLE suggestions ADD COLUMN type_id INTEGER');
  } catch (e) { console.error('❌ Erro na migração de suggestions (type_id):', e.message); }

  // Tipos de sugestão (ex: "Sugestão", "Sugestão de Construção"), configuráveis pelo dashboard
  db.exec(`
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('tipo').setDescription('Tipo de sugestão').setRequired(false).setAutocomplete(true)),

  new SlashCommandBuilder()
    .setName('sugestao-tipo')
    .setDescription('Adiciona um tipo de sugestão (ex: Sugestão, Sugestão de Construção)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(o => o.setName('nome').setDescription('Nome do tipo').setRequired(true))
    .addStringOption(o => o.setName('emoji').setDescription('Emoji').setRequired(false))
    .addChannelOption(o => o.setName('canal').setDescription('Canal específico para este tipo (padrão: canal geral de sugestões)').setRequired(false))
    .addChannelOption(o => o.setName('log').setDescription('Canal de log específico para este tipo').setRequired(false))
    .addRoleOption(o => o.setName('ping').setDescription('Cargo a mencionar para este tipo').setRequired(false)),

  new SlashCommandBuilder()
    .setName('sugestao-tipos-lista')
    .setDescription('Lista os tipos de sugestão configurados')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

  new SlashCommandBuilder()
    .setName('sugestao-tipo-remover')
    .setDescription('Remove um tipo de sugestão pelo ID')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(o => o.setName('id').setDescription('ID do tipo de sugestão (vê com /sugestao-tipos-lista)').setRequired(true)),
...
async function handleAutocomplete(interaction) {
  ...
  if (commandName === 'sugerir') {
    ...
    const filtrados = tipos
      ...
      .slice(0, 25)
      .map(t => ({ name: `${t.emoji || '💡'} ${t.label}`, value: String(t.id) }));
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'sugerir') {
    ...
    const tipoId   = options.getString('tipo');
    const tipo     = tipoId ? db.prepare('SELECT * FROM suggestion_types WHERE id = ? AND guild_id = ?').get(tipoId, guild.id) : null;

    // Usa o canal/log/ping específico do tipo, se definido; senão cai para a config geral
    const canalId = tipo?.channel_id || config.channel_id;
    ...
    `);
    const info = stmt.run(guild.id, canal.id, user.id, conteudo, guildSeq, tipo?.id || null);
    const sugId = info.lastInsertRowid;

    const tituloTipo = tipo ? `${tipo.emoji || '💡'} ${tipo.label} #${guildSeq}` : `💡 Sugestão #${guildSeq}`;
    ...
    const pingRoleId = tipo?.ping_role || config.ping_role;
  ...
  if (commandName === 'sugestao-tipo') {
    ...
    const tipos = db.prepare('SELECT COUNT(*) as c FROM suggestion_types WHERE guild_id = ?').get(guild.id);
    if (tipos.c >= 25) return interaction.reply({ content: '❌ Já tens 25 tipos de sugestão (limite do menu).', ephemeral: true });

    db.prepare(`
      INSERT INTO suggestion_types (guild_id, label, emoji, channel_id, log_channel, ping_role, order_num)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(guild.id, nome, emoji, canal?.id || null, log?.id || null, ping?.id || null, tipos.c);

    return interaction.reply({ content: `✅ Tipo de sugestão **${emoji} ${nome}** adicionado! Já vai aparecer nas opções de \`/sugerir\`.`, ephemeral: true });
  }

  if (commandName === 'sugestao-tipos-lista') {
    const tipos = db.prepare('SELECT * FROM suggestion_types WHERE guild_id = ? ORDER BY order_num, id').all(guild.id);
    if (!tipos.length) return interaction.reply({ content: '❌ Não há tipos de sugestão configurados. Todas as sugestões usam o canal geral.', ephemeral: true });

    const embed = embedPadrao(
      '📋 Tipos de Sugestão',
      tipos.map((t, i) => `**${i+1}.** ${t.emoji || '💡'} **${t.label}** (ID: ${t.id})\n↳ Canal: ${t.channel_id ? `<#${t.channel_id}>` : 'Canal geral'}`).join('\n\n')
    );
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }

  if (commandName === 'sugestao-tipo-remover') {
    const id = options.getInteger('id');

    const tipo = db.prepare('SELECT * FROM suggestion_types WHERE id = ? AND guild_id = ?').get(id, guild.id);
    if (!tipo) return interaction.reply({ content: `❌ Não existe nenhum tipo de sugestão com o ID **${id}** neste servidor. Usa \`/sugestao-tipos-lista\` para ver os IDs corretos.`, ephemeral: true });

    db.prepare('DELETE FROM suggestion_types WHERE id = ? AND guild_id = ?').run(id, guild.id);

    return interaction.reply({ content: `✅ Tipo de sugestão **${tipo.emoji || '💡'} ${tipo.label}** (ID: ${id}) foi removido!`, ephemeral: true });
  }
